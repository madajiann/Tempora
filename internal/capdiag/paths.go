...
//   - under Tempora home → <tempora-home>/...
//   - under OS home → ~/...
//   - elsewhere → <external>/basename (no full external path, no username)
func displayPath(p, workspace, home, temporaHome string) string {
	...
	if rh := strings.TrimSpace(temporaHome); rh != "" {
		...
		if rel, err := filepath.Rel(rh, clean); err == nil && rel != ".." && !strings.HasPrefix(rel, ".."+string(os.PathSeparator)) {
			return "<tempora-home>/" + filepath.ToSlash(rel)
		}
	}
	if home == "" {
