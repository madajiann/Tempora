...
func TestHookRuntimeIssueIsActionableAndScoped(t *testing.T) {
	entry := hook.Entry{
		...
		Source: `C:\Users\Test\AppData\Roaming\tempora\plugins\superpowers\.codex-plugin\plugin.json`,
	}
	issue, ok := hookRuntimeIssue(entry, errors.New("hook requires a POSIX shell on Windows, but no usable Git Bash was found"), func(string) string {
		return "<tempora-home>/plugins/superpowers/.codex-plugin/plugin.json"
	})
	if !ok {
	...
	if issue.Source != "<tempora-home>/plugins/superpowers/.codex-plugin/plugin.json" {
