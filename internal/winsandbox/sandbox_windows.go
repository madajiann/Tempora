...
func runWindowsRestrictedSandboxed(spec Spec, argv []string, opts RunOptions) (int, error) {
	...
	// it retain whole-run coordination and crash-residue cleanup. Workspace
	// capability grants are standing ACEs and are materialized under short
...
func windowsAppContainerName(spec Spec) string {
	...
	b.WriteByte(0)
	b.WriteString("ro")
	for _, root := range normalizedWindowsRoots(spec.ReadableRoots) {
		b.WriteByte(0)
		b.WriteString("read=")
...
// WRITE_RESTRICTED launches intentionally omit CREATE_NO_WINDOW. PowerShell,
// Node, and CNG initialization under a restricted token depend on normal
// console inheritance semantics. The Tempora helper itself is already hidden
// by internal/proc, so this does not create a visible console window.
func windowsRestrictedProcessCreationFlags() uint32 {
	return uint32(windows.CREATE_UNICODE_ENVIRONMENT |
		windows.EXTENDED_STARTUPINFO_PRESENT |
		windows.CREATE_SUSPENDED)
}
...
func grantAppContainerFilesystem(residueRun *windowsResidueRun, sid *windows.SID, spec Spec, extraWritableRoots ...string) (func(), error) {
	...
	var cleanup []func()
	readableRoots := normalizedWindowsRoots(spec.ReadableRoots)
	writableRoots := windowsWritableRoots(spec, extraWritableRoots...)
	if !spec.Writable && len(readableRoots) == 0 {
		readableRoots = append([]string(nil), writableRoots...)
		writableRoots = nil
	}
	grantRoots := func(roots []string, writable bool) error {
		for _, root := range roots {
			restore, _, err := snapshotPathSecurity(root, writable)
			if err != nil {
				return err
			}
	...
	if err := grantRoots(readableRoots, false); err != nil {
		runCleanup(cleanup)()
		return func() {}, err
	}
	if err := grantRoots(writableRoots, true); err != nil {
		runCleanup(cleanup)()
		return func() {}, err
	}
	for _, root := range normalizedWindowsRoots(spec.ForbidReadRoots) {
...
func hiddenWindowsSystemCommandContext(ctx context.Context, name string, args ...string) *exec.Cmd {
	cmd := proc.CommandContext(ctx, systemRootTool(name), args...)
	proc.HideWindow(cmd)
	return cmd
}

func hiddenWindowsSystemCommand(name string, args ...string) *exec.Cmd {
	cmd := proc.Command(systemRootTool(name), args...)
	proc.HideWindow(cmd)
	return cmd
}
...
func windowsSandboxTempRoot(spec Spec) (string, func(), error) {
	if configured := strings.TrimSpace(spec.TempDir); configured != "" {
		canonical, err := canonicalWindowsDirectory(configured)
		if err != nil {
			return "", nil, fmt.Errorf("use session sandbox temp %q: %w", configured, err)
		}
