...
Tempora's bundled native Windows process sandbox helpers.
...
For a workspace-write shell, Tempora derives a capability SID from each
...
result, err := winsandbox.Run(winsandbox.Spec{
    ...
    ProtectedWriteRoots: []string{temporaState},
...
`Network: false` therefore fails closed. Tempora reports Windows enforcement as
