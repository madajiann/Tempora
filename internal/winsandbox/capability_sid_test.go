...
func TestCapabilitySIDIsDeterministicAndValid(t *testing.T) {
	first := deriveCapabilitySID(capabilityWorkspace, `c:\work\tempora`)
	second := deriveCapabilitySID(capabilityWorkspace, `c:\work\tempora`)
	if first != second {
...
func TestCapabilitySIDSeparatesPurposeAndPath(t *testing.T) {
	workspace := deriveCapabilitySID(capabilityWorkspace, `c:\work\tempora`)
	temp := deriveCapabilitySID(capabilitySessionTemp, `c:\work\tempora`)
...
func TestCapabilitySIDSubauthoritiesStayInThirtyBitRange(t *testing.T) {
	first, second := capabilitySubauthorities(capabilityWorkspace, `c:\work\tempora`)
