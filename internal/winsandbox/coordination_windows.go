...
// Tempora launches every sandboxed command as its own helper process, so two
// commands preparing the same directory are separate OS processes. Standing
// capability ACEs use this lock only while the exact entry is materialized and
// its directory identity is revalidated. Temporary forbid-read ACEs and the
// legacy AppContainer lane still restore descriptor snapshots, so they keep the
// lock for their whole mutation window. A shared path has no per-process ACL
// view; serializing those mutations prevents one helper's cleanup from undoing
// another helper's boundary.
//
...
// in opposite orders.
//
// Trade-off: a long-running command holds a root lock only when it has a
// temporary forbid-read or legacy AppContainer mutation on that root. Ordinary
// workspace capability setup is short and commands sharing the workspace run
// concurrently afterward. Lock waits remain bounded and identify their holder.
...
func windowsRootLockNames(roots []string) []string {
	...
	for _, root := range roots {
		for _, key := range windowsRootLockDomains(root) {
			if seen[key] {
				continue
			}
			seen[key] = true
...
// windowsRootLockDomains includes each directory and its non-volume ancestors.
// Consequently ACL mutations for a parent and child path share at least one
// mutex even when callers spell different roots. This is deliberately
// conservative: short standing-capability materialization may serialize with a
// sibling under a common ancestor, while whole-run locks remain limited to the
// comparatively rare forbid-read paths.
func windowsRootLockDomains(root string) []string {
	root = strings.ToLower(filepath.Clean(strings.TrimSpace(root)))
	if root == "" || root == "." {
		return nil
	}
	volume := strings.ToLower(filepath.Clean(filepath.VolumeName(root) + string(filepath.Separator)))
	var domains []string
	for current := root; current != "" && current != "."; current = filepath.Dir(current) {
		if current == volume || filepath.Dir(current) == current {
			break
		}
