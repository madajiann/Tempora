...
func CreateOrRefreshPairingRequest(msg InboundMessage, cfg PairingConfig) (PairingRequest, bool, error) {
	...
	if path == "" {
		return PairingRequest{}, false, errors.New("tempora user state directory is unavailable")
	}
	pairingMu.Lock()
...
func ListPairingRequests() ([]PairingRequest, error) {
	path := PairingStorePath()
	if path == "" {
		return nil, errors.New("tempora user state directory is unavailable")
	}
	pairingMu.Lock()
	defer pairingMu.Unlock()
	store, err := loadPairingFile(path)
...
func ApprovePairingCode(code string) (PairingRequest, error) {
	...
	if userPath == "" {
		return PairingRequest{}, errors.New("tempora user config path is unavailable")
	}
...
func removePairingCode(code string) (PairingRequest, error) {
	...
	path := PairingStorePath()
	if path == "" {
		return PairingRequest{}, errors.New("tempora user state directory is unavailable")
	}
	pairingMu.Lock()
	defer pairingMu.Unlock()
	store, err := loadPairingFile(path)
