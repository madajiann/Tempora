...
func TestAttachmentParentSymlinkCannotEscape(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".tempora"), 0700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(outside, "secret.png"), opaquePNG(t, 8, 8), 0600); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink(outside, filepath.Join(root, ".tempora", "attachments")); err != nil {
		t.Fatal(err)
	}
	_, err := testService(t).PrepareBatch(t.Context(), []Source{{WorkspaceRoot: root, Path: ".tempora/attachments/secret.png", Confine: ".tempora/attachments"}})
	if err == nil {
