...
const unsupportedPortableLocationMessage = "Tempora portable cannot run from a network or virtual shared folder.\n" +
	"Copy the entire extracted folder to a local Windows drive, then start Tempora.exe, or use the installer.\n\n" +
	"Tempora 便携版无法从网络或虚拟机共享目录启动。\n" +
	"请将整个解压目录复制到 Windows 本地磁盘后运行 Tempora.exe，或使用安装器。"
