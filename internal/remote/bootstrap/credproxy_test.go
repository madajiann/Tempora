...
func credProxyOpts(baseURL string) *CredentialProxyOptions {
	return &CredentialProxyOptions{
		...
		Provider: "tempora-desktop-proxy",
...
func TestEnsureCredentialProviderAppendsAndIsIdempotent(t *testing.T) {
	...
	first, rerr := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
	if rerr != nil {
		t.Fatal(rerr)
	}
	for _, want := range []string{
		`[[providers]]`,
		`name = "tempora-desktop-proxy"`,
		`base_url = "http://127.0.0.1:18999"`,
		`api_key_env = "TEMPORA_PROXY_TOKEN"`,
		`model = "deepseek-v4-flash"`,
	} {
	...
	second, _ := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
	...
	third, _ := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
	...
	other.Provider, other.TokenEnv, other.Token = "tempora-desktop-proxy-other", "TEMPORA_PROXY_TOKEN_OTHER", "other-token"
	if _, err := ensureCredentialProvider(ctx, fs, root, other); err != nil {
		t.Fatalf("second workspace: %v", err)
	}
	cfg, _ := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
	env, _ := os.ReadFile(filepath.Join(root, ".tempora", ".env"))
	if !strings.Contains(string(cfg), `name = "tempora-desktop-proxy-other"`) || !strings.Contains(string(env), "TEMPORA_PROXY_TOKEN=virtual-token-123") || !strings.Contains(string(env), "TEMPORA_PROXY_TOKEN_OTHER=other-token") {
...
func TestEnsureCredentialProviderPreservesUserConfig(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, ".tempora", "config.toml"), []byte(existing), 0o600); err != nil {
		t.Fatal(err)
	}
	conn := newFakeConn(t, root, func(string) (remote.ExecResult, error) { return ok("") })
	fs, err := conn.SFTP()
	...
	got, _ := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
...
func TestLaunchCommandCredentialInjection(t *testing.T) {
	...
	cmd := LaunchCommand("/usr/bin/tempora", "/ws", paths, &CredentialProxyOptions{
		BaseURL: "http://127.0.0.1:18999", Token: "to'ken $x", Provider: "tempora-desktop-proxy", Model: "m",
	}, nil)
	for _, want := range []string{
		`--model 'tempora-desktop-proxy'`,
		`nohup '/usr/bin/tempora' serve`,
	} {
...
func TestEnsureCredentialProviderMaterializesBuiltinDefault(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, ".tempora", "config.toml"), []byte(cfg), 0o600); err != nil {
		t.Fatal(err)
	}
	conn := newFakeConn(t, root, func(string) (remote.ExecResult, error) { return ok("") })
	fs, err := conn.SFTP()
	...
	first, _ := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
	got := string(first)
	for _, want := range []string{
		`default_model = "deepseek-flash"   # user's choice stays`,
		`name = "deepseek-flash"`,
	...
	if strings.Index(got, `name = "deepseek-flash"`) > strings.Index(got, `name = "tempora-desktop-proxy"`) {
...
func TestEnsureCredentialProviderRewritesKindDrift(t *testing.T) {
	...
	installed, _ := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
	...
	rewritten, _ := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
	...
	again, _ := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
...
func TestEnsureCredentialProviderPersistsLateBuiltinMaterialization(t *testing.T) {
	skipOnWindows(t)
	root := t.TempDir()
	configDir := filepath.Join(root, ".tempora")
...
func TestEnsureCredentialProviderRewritesModelDrift(t *testing.T) {
	...
	got, err := os.ReadFile(filepath.Join(root, ".tempora", "config.toml"))
	if err != nil {
		t.Fatal(err)
	}
