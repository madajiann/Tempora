...
// known_hosts files (read-only) and a Tempora-managed file (read-write, TOFU).
type HostKeyPolicy struct {
	...
	// ManagedPath is the Tempora-managed known_hosts file that accepted TOFU
...
func (hostKeyLookupProbe) Type() string    { return "tempora-host-key-lookup-probe" }
func (hostKeyLookupProbe) Marshal() []byte { return []byte("tempora-host-key-lookup-probe") }
