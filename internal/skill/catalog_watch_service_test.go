...
func TestHostWatchServiceSharedAcrossStores(t *testing.T) {
	root := t.TempDir()
	skillsDir := filepath.Join(root, ".tempora", "skills")
	...
	storeA := New(Options{HomeDir: t.TempDir(), TemporaHomeDir: t.TempDir(), ProjectRoot: root, Stderr: io.Discard, Watch: true, WatchService: svc})
	storeB := New(Options{HomeDir: t.TempDir(), TemporaHomeDir: t.TempDir(), ProjectRoot: root, Stderr: io.Discard, Watch: true, WatchService: svc})
	defer storeA.Close()
	defer storeB.Close()
...
func TestHostWatchServiceIgnoresSkippedBodies(t *testing.T) {
	...
	store := New(Options{HomeDir: t.TempDir(), TemporaHomeDir: t.TempDir(), ProjectRoot: root, Stderr: io.Discard, Watch: true, WatchService: svc})
	defer store.Close()
