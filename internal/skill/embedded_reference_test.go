...
func TestEmbeddedGuideReferencesAreReadableOnDemand(t *testing.T) {
	...
	root, err := loader.Execute(context.Background(), json.RawMessage(`{"name":"tempora-guide"}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, link := range links {
		args, _ := json.Marshal(map[string]string{"name": "tempora-guide", "reference": link[1]})
	...
	guide, _ := store.Read("tempora-guide")
...
func TestEmbeddedReferenceRejectsTraversalAndNonReferenceFiles(t *testing.T) {
	...
	for _, ref := range []string{
		...
		t.Run(ref, func(t *testing.T) {
			args, _ := json.Marshal(map[string]string{"name": "tempora-guide", "reference": ref})
			if out, err := loader.Execute(context.Background(), args); err == nil || out != "" {
				t.Fatalf("reference %q allowed: %q, %v", ref, out, err)
			}
		})
	}
	if _, err := loader.Execute(context.Background(), json.RawMessage(`{"name":"tempora-guide","reference":"references/skills.md","arguments":"task"}`)); err == nil {
...
func TestEmbeddedReferenceHonorsOverrideAndDisabledSkill(t *testing.T) {
	for _, disabled := range []bool{false, true} {
		t.Run(map[bool]string{false: "override", true: "disabled"}[disabled], func(t *testing.T) {
			...
			if disabled {
				opts.DisabledNames = []string{"tempora-guide"}
			}
			store := skill.New(opts)
			if !disabled {
				_, err := store.CreateWithContent("tempora-guide", skill.ScopeProject,
			...
			out, err := loader.Execute(context.Background(), json.RawMessage(`{"name":"tempora-guide","reference":"references/skills.md"}`))
			if err == nil || out != "" {
				t.Fatal("embedded page bypassed the winning/disabled skill")
			}
			if !disabled {
				out, err = loader.Execute(context.Background(), json.RawMessage(`{"name":"tempora-guide"}`))
