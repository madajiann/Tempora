---
name: tempora-guide
description: "Diagnose missing or misconfigured Tempora skills, commands, hooks, MCP servers, plugins, and instructions."
runAs: inline
---

# Tempora capability diagnostics

For a loading or configuration problem, start with the relevant section of
`tempora doctor capabilities --json`. This static report starts no MCP
...
Read a page using `read_skill` with `name="tempora-guide"` and
`reference="references/skills.md"` (substitute the page path). If the tool is
not directly exposed, use:

```json
{"action":"call","capability_id":"tool:read_skill","arguments":{"name":"tempora-guide","reference":"references/skills.md"}}
```
...
Use `tempora doctor capabilities --live --timeout 5s --json` only when starting
