...
func TestMouseCaptureDefaultsOffOverSSH(t *testing.T) {
	for _, k := range []string{"SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "TEMPORA_DISABLE_MOUSE"} {
		t.Setenv(k, "")
	}
	...
	t.Setenv("TEMPORA_DISABLE_MOUSE", "0")
	if mouseCaptureOffByDefault() {
		t.Fatal("TEMPORA_DISABLE_MOUSE=0 must force capture on even over SSH")
	}
	t.Setenv("TEMPORA_DISABLE_MOUSE", "1")
	t.Setenv("SSH_TTY", "")
	if !mouseCaptureOffByDefault() {
		t.Fatal("TEMPORA_DISABLE_MOUSE=1 keeps capture off locally")
	}
}
