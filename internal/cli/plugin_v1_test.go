...
// the test Tempora home and registers it in plugin state.
func installV2RuntimePlugin(t *testing.T, home string) string {
  ...
  "runtime": {
	...
	writePluginTestFile(t, filepath.Join(root, "themes", "neon.tempora-theme"), "theme bytes")
	writePluginTestFile(t, filepath.Join(root, "bin", "example"), "#!/bin/sh\n")
	if err := pluginpkg.Upsert(home, pluginpkg.InstalledPlugin{
		Name: "example", Root: "plugins/example", Version: "1.0.0", ManifestKind: "tempora", Enabled: true,
	}); err != nil {
		t.Fatal(err)
	}
	return root
...
func TestPluginDoctorReportsMissingV2PathsAsWarnings(t *testing.T) {
  ...
  "contributes": {
	...
	if err := pluginpkg.Upsert(home, pluginpkg.InstalledPlugin{
		Name: "gap", Root: "plugins/gap", ManifestKind: "tempora", Enabled: true,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(out, `theme glob "themes/*.tempora-theme" matched no files`) {
