...
func (p *mcpManager) renderDetail(width int) string {
	...
	if v.BuiltIn {
		...
		writeMCPDetailField(&b, "Config location", "current project tempora.toml")
	} else {
...
func (p *mcpManager) renderConfirmRemove(width int) string {
	...
	b.WriteString(viewMeta("This removes it from Tempora config. It cannot be undone from this panel.") + "\n\n")
