...
func pluginUsage() {
  ...
  tempora plugin install <source> [--yes] [--dry-run] [--link] [--replace]
  tempora plugin list
  tempora plugin show <name>
  tempora plugin enable <name>
  tempora plugin disable <name>
  tempora plugin remove <name>
  tempora plugin doctor <name>
  tempora plugin migrate <name> --to-v2`)
}
...
func pluginShowCommand(args []string) int {
	...
	root := pluginpkg.ResolveRoot(config.TemporaHomeDir(), p.Root)
...
func pluginDoctorCommand(args []string) int {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "missing apiVersion") {
			fmt.Fprintf(os.Stderr, "remediation: tempora plugin migrate %s --to-v2\n", args[0])
		}
		return 1
	}
...
// something runnable. ${TEMPORA_PLUGIN_ROOT} expands to the installed root;
...
func pluginSetEnabledCommand(args []string, enabled bool) int {
	...
	if err := pluginpkg.SetEnabled(config.TemporaHomeDir(), args[0], enabled); err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
