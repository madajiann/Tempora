...
func TestGitStatusRenderWithinCompactsRepoBeforeBranch(t *testing.T) {
	status := gitStatus{Repo: "VeryLongDeepSeekTemporaWorkspace", Branch: "codex/cli-tui-status-row"}

	full := ansi.Strip(status.RenderWithin(80, statusAutoColor))
	if full != "VeryLongDeepSeekTemporaWorkspace@codex/cli-tui-status-row" {
...
func TestGitStatusRenderWithinKeepsDirtySuffix(t *testing.T) {
	status := gitStatus{
		Repo:      "VeryLongDeepSeekTemporaWorkspace",
...
func TestLoadGitStatus(t *testing.T) {
	...
	runGitForTest(t, root, "config", "user.email", "tempora@example.invalid")
	runGitForTest(t, root, "config", "user.name", "Tempora Test")
