...
func TestRenderMemoryGroupsDocsAndStore(t *testing.T) {
	...
	got := renderMemory(width, &memory.Set{
		Docs:  []memory.Source{{Path: "/Users/me/project/TEMPORA.md", Scope: memory.ScopeProject}},
		Store: store,
		Index: store.Index(),
	})
	for _, want := range []string{
		"memory", "instructions", "precedence=1", "scope=project", "TEMPORA.md",
...
func TestRenderHooksUsesSharedVisualLanguage(t *testing.T) {
	...
	for _, want := range []string{"hooks (1 active)", "PreToolUse", "project", "…", ".tempora/settings.json"} {
		if !strings.Contains(got, want) {
...
func TestRenderHelpDocsShowsOnlyRuntimeWinner(t *testing.T) {
	...
	if !strings.Contains(got, "/tempora:docs") {
...
func TestRenderHelpDocsUsesQualifiedFallbackForHiddenAlias(t *testing.T) {
	...
	for _, want := range []string{"/tempora:docs", "/manuals:docs"} {
		if !strings.Contains(got, want) {
...
func TestRenderSkillPathsStaysWithinWidth(t *testing.T) {
	...
	got := renderSkillPaths(width, []skill.Root{{
		Dir:      "/Users/me/projects/really/deep/path/to/.tempora/skills",
