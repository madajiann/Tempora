...
func startTUIDiagnostics(temporaHome string) *tuiDiagnostics {
	d := &tuiDiagnostics{
	...
	if logDir := tuiDiagnosticLogDir(temporaHome); logDir != "" {
...
func tuiDiagnosticLogDir(temporaHome string) string {
	if strings.TrimSpace(temporaHome) == "" {
		return ""
	}
	return filepath.Join(temporaHome, "logs")
}
...
func (w *boundedDiagnosticWriter) Write(p []byte) (int, error) {
	...
	if n < total && !w.truncated {
		w.truncated = true
		_, _ = io.WriteString(w.dst, "\ntempora: CLI TUI diagnostic log limit reached; further diagnostics omitted\n")
		w.remaining = 0
	}
	return total, nil
}
