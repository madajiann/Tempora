...
func TestCLICompletionListsRootAndNestedCommands(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 1, []string{"tempora", "co"}, values)
	if want := []string{"config", "completion"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("root completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 2, []string{"tempora", "mcp", "b"}, values)
	if want := []string{"browse"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("mcp subcommand completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "remote", "serve", "st"}, values)
...
func TestCLICompletionListsCommandFlags(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"tempora", "run", "--m"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 1, []string{"tempora", "--d"}, values)
	if !containsCompletionValue(got, "--dir") {
		t.Errorf("root flag completion missing %q: %v", "--dir", got)
	}
	got = cliCompletionCandidatesWithValues(root, 1, []string{"tempora", "--p"}, values)
	if !containsCompletionValue(got, "--permission-mode") {
		t.Errorf("root flag completion missing %q: %v", "--permission-mode", got)
	}

	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "mcp", "add", "--h"}, values)
...
func TestCLICompletionUsesConfiguredModelsAndSessionIDs(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "run", "--model", "deep"}, values)
	if want := []string{"deepseek/deepseek-chat"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("model completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 1, []string{"tempora", "--model=mi"}, values)
	if want := []string{"--model=mimo/mimo-v2"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("inline model completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 2, []string{"tempora", "--resume", "b"}, values)
...
func TestCompletionCommandPrintsShellScripts(t *testing.T) {
	...
	for _, shell := range []string{"bash", "zsh", "fish"} {
		t.Run(shell, func(t *testing.T) {
			...
			if !strings.Contains(out, "tempora completion __complete") {
				t.Fatalf("completion %s script does not route to the shared registry:\n%s", shell, out)
			}
			if shell == "fish" && strings.Contains(out, "complete -c tempora -f ") {
...
func TestCLICompletionTaskNestedAndRunAblate(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"tempora", "task", "st"}, values)
	for _, want := range []string{"status", "stop"} {
		if !containsCompletionValue(got, want) {
			t.Fatalf("task prefix st missing %q: %v", want, got)
		}
	}
	got = cliCompletionCandidatesWithValues(root, 2, []string{"tempora", "task", "mon"}, values)
	if !containsCompletionValue(got, "monitor") {
		t.Fatalf("task monitor missing: %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "task", "monitor", "st"}, values)
	if !containsCompletionValue(got, "status") || !containsCompletionValue(got, "stop") {
		t.Fatalf("task monitor st = %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 2, []string{"tempora", "run", "--a"}, values)
...
func TestCLICompletionOptionalResumeThenFlag(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"tempora", "--resume", "--m"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 1, []string{"tempora", "--resume=a"}, values)
...
func TestCLICompletionRunServeResumeRequiresValue(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "run", "--resume", "--m"}, values)
	if containsCompletionValue(got, "--model") {
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "run", "--resume", "b"}, values)
	if want := []string{"beta-session"}; !reflect.DeepEqual(got, want) {
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "serve", "--resume", "a"}, values)
	if len(got) != 0 {
	...
	got = cliCompletionCandidatesWithValues(root, 2, []string{"tempora", "run", "--resume=b"}, values)
...
func TestCLICompletionTaskPerOperationFlags(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "task", "status", "--p"}, values)
	if containsCompletionValue(got, "--project-root") {
		t.Fatalf("task status must not offer --project-root: %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "task", "status", "--j"}, values)
	if !containsCompletionValue(got, "--json") {
		t.Fatalf("task status missing --json: %v", got)
	}

	// events has --jsonl/--after/--follow.
	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "task", "events", "--"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "task", "stop", "--"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "task", "list", "--p"}, values)
	if !containsCompletionValue(got, "--project-root") {
		t.Fatalf("task list missing --project-root: %v", got)
	}

	// tmux attach has --session.
	got = cliCompletionCandidatesWithValues(root, 4, []string{"tempora", "task", "tmux", "attach", "--s"}, values)
...
func TestCLICompletionPathFlagReturnsEmptyForShellFallback(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"tempora", "run", "--dir", "do"}, values)
	if len(got) != 0 {
