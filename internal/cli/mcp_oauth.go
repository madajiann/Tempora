...
func mcpAuthCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: tempora mcp auth <name>")
		return 2
	}
	...
	specs := boot.PluginSpecsForRootWithOptions([]config.PluginEntry{entry}, workspace, boot.PluginSpecOptions{
		...
		StateHome: config.TemporaHomeDir(), Network: true, OAuthHTTPClient: client,
	})
	if len(specs) != 1 {
	...
	fmt.Printf("authorized MCP server %q — run `tempora mcp retry %s` or reconnect it in the current session\n", name, name)
	return 0
}
