...
func TestIngestSeparatesReasoningFromAnswer(t *testing.T) {
	...
	if plain := ansi.Strip(m.transcript[2]); !strings.HasPrefix(plain, "  ◆ Tempora\n\n  Hello answer") {
...
func TestAssistantAnswerWithoutReasoningHasNoLeadingSpacer(t *testing.T) {
	...
	if plain := ansi.Strip(m.transcript[0]); !strings.HasPrefix(plain, "  ◆ Tempora\n\n  Direct answer") {
...
// an older CLI must suppress a future tempora.subagent.* channel instead of
