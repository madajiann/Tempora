...
func TestRenderMCPManagerDetailCompactsConfigPath(t *testing.T) {
	...
	if strings.Contains(got, "Application Support/tempora/config.toml") {
...
func TestMCPEditConfigLaunchEditorRejectsUnterminatedQuote(t *testing.T) {
	...
	_, err := mcpEditConfigLaunchCommand("/tmp/tempora.toml", func(string) (string, error) {
		t.Fatal("lookPath should not be called when VISUAL is set")
		return "", errors.New("unexpected lookup")
	})
	if err == nil {
...
func TestMCPEditConfigLaunchEditorExpandsEnvVar(t *testing.T) {
	t.Setenv("TEMPORA_TEST_EDITOR_BIN", "/opt/custom/bin/myed")
	t.Setenv("VISUAL", "$TEMPORA_TEST_EDITOR_BIN --flag")
	t.Setenv("EDITOR", "")
...
func TestMCPEditConfigLaunchEditorExpandsTilde(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			t.Setenv("EDITOR", "")
			launch, err := mcpEditConfigLaunchCommand("/tmp/tempora.toml", func(string) (string, error) {
				t.Fatal("lookPath should not be called when VISUAL is set")
				return "", errors.New("unexpected lookup")
			})
			if err != nil {
...
func TestMCPEditConfigLaunchFallsBackToTerminalEditor(t *testing.T) {
	...
	launch, err := mcpEditConfigLaunchCommand("/tmp/tempora.toml", func(name string) (string, error) {
		if name == "vim" {
	...
	if len(launch.cmd.Args) != 2 || launch.cmd.Args[0] != "/usr/bin/vim" || launch.cmd.Args[1] != "/tmp/tempora.toml" {
...
func TestApplyMCPModeDropsLegacyTier(t *testing.T) {
	...
	if err := cfg.SaveTo("tempora.toml"); err != nil {
	...
	m.mcp = &mcpManager{
		stage: mcpStageMode,
		name:  "github",
		snapshot: mcpSnapshot{configPath: "tempora.toml", servers: []mcpServerView{{
	...
	raw, err := os.ReadFile("tempora.toml")
	if err != nil {
...
func TestApplyMCPModeRecordsPluginConnectFailure(t *testing.T) {
	...
	t.Setenv("PATH", "")
	cfg := config.Default()
	cfg.Plugins = []config.PluginEntry{{Name: "broken", Command: "definitely-missing-tempora-mcp", Tier: "background"}}
	if err := cfg.SaveTo("tempora.toml"); err != nil {
		t.Fatalf("save config: %v", err)
	}
