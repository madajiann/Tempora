...
func TestBashPowerShellRunsNativeCommand(t *testing.T) {
	...
	out, err := runPS(t, "Write-Output tempora-ok")
	if err != nil {
		t.Fatalf("powershell command failed: %v (out=%q)", err, out)
	}
	if !strings.Contains(out, "tempora-ok") {
		t.Fatalf("output = %q, want it to contain tempora-ok", out)
	}
}
...
func assertPowerShellDetailedContract(t *testing.T, psPath string) {
	...
	work := filepath.Join(t.TempDir(), "中文目录-tempora")
