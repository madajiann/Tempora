...
// warning to command output when the command references Tempora's own session
...
// ConfineWebFetch returns the web_fetch built-in bound to Tempora proxy
...
// guard additionally rejects writes into Tempora's own session stores even
// when the roots would allow them (see SessionDataGuard). managed names the
// Tempora-owned config files writable outside the roots after a fresh human
...
// user enabled [secrets] protect_sensitive_files, matches Tempora's built-in
...
func confine(roots []string, target string) error {
	...
	return fmt.Errorf("path %q is outside the writable roots (writes are confined to %s); "+
		"write inside the workspace or a configured allow_write root, or widen [sandbox] workspace_root / allow_write in tempora.toml",
...
// it targets Tempora's own session stores. A target outside every root that
// matches a Tempora-managed config file (see ManagedConfigPaths) may proceed
