...
func TestBashMergesLoginShellPath(t *testing.T) {
	...
	probe := filepath.Join(bin, "tempora-path-probe")
	...
	args, _ := json.Marshal(map[string]string{"command": "tempora-path-probe"})
...
func TestBashCommandEnvFiltersSensitiveKeysWhenEnabled(t *testing.T) {
	...
	t.Setenv("TEMPORA_TEST_VISIBLE", "ok")
	...
	if !strings.Contains(env, "TEMPORA_TEST_VISIBLE=ok") {
...
func TestRunShellPATHCommandFiltersEnvWhenEnabled(t *testing.T) {
	...
	out := runShellPATHCommand(context.Background(), "/bin/sh", []string{"-c", `printf 'tok=%s' "${TEMPORA_TEST_SECRET_TOKEN:-none}"`})
