...
// SessionDataGuard rejects agent writes into Tempora's own session stores:
...
// NewSessionDataGuard builds a guard for the given Tempora state root
...
func (g SessionDataGuard) Check(target string) error {
	...
	if g.deniesSecurity(abs) {
		return fmt.Errorf("path %q is a Tempora security boundary file (%s holds the global hooks; hooks execute arbitrary shell commands on every future session). Agents may not modify it. "+
			"Ask the user to edit it themselves, or to add the directory to [sandbox] allow_write in tempora.toml if raw access is truly intended",
			target, g.stateRoot)
	}
	if !g.denies(abs) {
		return nil
	}
	return fmt.Errorf("path %q is inside Tempora's own session/state data (%s); the app is the only writer of these files, and edits from a chat race its saves — that surfaces as repeated save-conflict copies. "+
		"Do not modify session or runtime-state files directly; report the underlying problem instead. If raw access is truly intended, add the directory to [sandbox] allow_write in tempora.toml",
		target, g.stateRoot)
}
...
// volumes ~/.tempora/SESSIONS reaches the very same files (the same shape as
...
func (g SessionDataGuard) CommandHint(workDir, command string) string {
	...
	warn := fmt.Sprintf("WARNING: this command referenced Tempora's own session/state data under %s. "+
