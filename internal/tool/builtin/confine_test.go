...
func TestManagedConfigWriteFailsClosedWithoutApprover(t *testing.T) {
	...
	cfg := config.Default()
	managed := NewManagedConfigPaths(config.TemporaManagedConfigPaths())
...
func TestManagedConfigWriteGatedOnApprover(t *testing.T) {
	...
	decline := &stubConfigWriteApprover{allow: false, reason: "the user declined this Tempora config write"}
	...
	// Even with an always-allowing approver, non-config files in the Tempora
	...
	for _, target := range []string{
		...
		filepath.Join(home, ".tempora", ".env"),
		filepath.Join(home, ".tempora", "settings.json"),
		filepath.Join(home, ".tempora", "skills", "evil", "SKILL.md"),
	} {
...
func TestBashSandboxConfinement(t *testing.T) {
	...
	work, err := os.MkdirTemp(home, ".tempora-bashsb-*")
	if err != nil {
	...
	outPath := filepath.Join(home, ".tempora-bashsb-escape.txt")
