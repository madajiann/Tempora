...
// ManagedConfigPaths is the set of Tempora-owned configuration FILES a file
...
func (m ManagedConfigPaths) approve(ctx context.Context, target string) error {
	...
	if !ok {
		return fmt.Errorf("path %q is a Tempora-managed config file outside the writable roots; writing it requires interactive user approval, which this session cannot provide. "+
			"Ask the user to retry in an interactive session, or to add the directory to [sandbox] allow_write in tempora.toml", target)
	}
	...
	if !allow {
		if strings.TrimSpace(reason) == "" {
			reason = "the user declined this Tempora config write — do not retry it; ask how they would like to proceed."
		}
