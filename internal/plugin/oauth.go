...
// mcpOAuthState is Tempora-owned authorization state for one MCP server. It
...
// ClearHTTPMCPOAuth removes Tempora-owned OAuth client and token state for one
...
// ReconcileHTTPMCPOAuthAfterRemoval removes Tempora-owned OAuth state after an
...
func oauthCallbackHandler(expectedState string, result chan<- oauthCallbackResult) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if callback.Err != nil {
			http.Error(w, "Tempora could not complete MCP authorization. You can close this window.", http.StatusBadRequest)
		} else {
			w.Header().Set("Content-Type", "text/html; charset=utf-8")
			_, _ = io.WriteString(w, "<!doctype html><title>Tempora MCP authorized</title><p>Authorization completed. You can close this window and return to Tempora.</p>")
		}
		select {
