...
// has invalid 'parameters' schema" back to the stable Tempora tool identity.
...
func AnnotateToolSchemaError(err error, tools []ToolSchema) error {
	...
	tool := tools[index]
	context := fmt.Sprintf("Provider tool %d maps to Tempora tool %q.", index, tool.Name)
	if server, rawName, ok := splitMCPToolName(tool.Name); ok {
		context = fmt.Sprintf("Provider tool %d maps to Tempora tool %q (MCP server %q, tool %q).", index, tool.Name, server, rawName)
	}
	annotated := *apiErr
