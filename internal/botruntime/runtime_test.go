...
func TestConnectionChannelConfigsCarrySessionMappingsOnlyPerConnection(t *testing.T) {
	connections := []config.BotConnectionConfig{
		{
			...
			SessionMappings: []config.BotConnectionSessionMapping{{
				RemoteID:      "wx-group-1",
				SessionID:     "path:/tmp/tempora-session.jsonl",
	...
	if got := mappings[0]; got.RemoteID != "wx-group-1" || got.SessionID != "path:/tmp/tempora-session.jsonl" || got.ChatType != string(bot.ChatGroup) || got.UserID != "wx-user-1" || got.WorkspaceRoot != "/mapped" || got.UpdatedAt == "" {
