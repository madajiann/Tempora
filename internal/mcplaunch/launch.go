...
// Scope remains in the JSON solely so previous Tempora versions can read new
...
// ForWorkspace returns the process-shared manager for one Tempora home and
// workspace so sibling tabs observe one launch authorization state.
func ForWorkspace(temporaHome, workspace string) *Manager {
	path := StatePath(temporaHome)
