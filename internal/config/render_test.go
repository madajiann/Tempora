...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Bot.Feishu.OutboundMediaRoots = []string{"/tmp/tempora-media", "/srv/shots"}
	orig.Bot.Routes = []BotRouteConfig{{
		...
		WorkspaceRoot:    "/tmp/tempora-route",
	}}
	...
	if len(got.Bot.Connections) != 1 || got.Bot.Connections[0].Model != "deepseek-pro" || got.Bot.Connections[0].WorkspaceRoot != "/tmp/tempora-bot" {
	...
	if len(got.Bot.Feishu.OutboundMediaRoots) != 2 || got.Bot.Feishu.OutboundMediaRoots[0] != "/tmp/tempora-media" {
		t.Errorf("feishu outbound_media_roots not preserved: %+v", got.Bot.Feishu.OutboundMediaRoots)
	}
	if len(got.Bot.Routes) != 1 || got.Bot.Routes[0].WorkspaceRoot != "/tmp/tempora-route" || got.Bot.Routes[0].ChatID != "oc_group" {
	...
	if len(got.Bot.Connections[0].SessionMappings) != 1 || got.Bot.Connections[0].SessionMappings[0].Scope != "project" || got.Bot.Connections[0].SessionMappings[0].WorkspaceRoot != "/tmp/tempora-bot" {
...
func TestIsolatedHomeDirReturnsCleanPath(t *testing.T) {
	raw := filepath.Join(t.TempDir(), "isolated-tempora")
...
func TestLegacyOSSupportDirEmptyWhenIsolated(t *testing.T) {
	isolateUserConfigHome(t)
	t.Setenv("TEMPORA_HOME", filepath.Join(t.TempDir(), "isolated-home"))
