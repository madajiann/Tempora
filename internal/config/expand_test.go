...
func TestExpandVars(t *testing.T) {
	t.Setenv("TEMPORA_TEST_TOKEN", "sk-123")
	t.Setenv("TEMPORA_TEST_EMPTY", "")

	cases := []struct{ in, want string }{
		{"Bearer ${TEMPORA_TEST_TOKEN}", "Bearer sk-123"},
		{"${TEMPORA_TEST_MISSING}", ""},                                  // unset, no default → empty
		{"${TEMPORA_TEST_MISSING:-fallback}", "fallback"},                // unset → default
		{"${TEMPORA_TEST_EMPTY:-fallback}", "fallback"},                  // set-but-empty → default
		{"${TEMPORA_TEST_TOKEN:-fallback}", "sk-123"},                    // set → value, default ignored
		{"no vars here", "no vars here"},                                 // untouched
		{"a${TEMPORA_TEST_TOKEN}b${TEMPORA_TEST_MISSING}c", "ask-123bc"}, // multiple refs
	}
	for _, c := range cases {
...
func TestExpandedPlugin(t *testing.T) {
	t.Setenv("TEMPORA_TEST_KEY", "secret")
	e := PluginEntry{
		Name:    "x",
		Type:    "http",
		URL:     "https://api/${TEMPORA_TEST_MISSING:-v1}",
		Args:    []string{"--token", "${TEMPORA_TEST_KEY}"},
		Env:     map[string]string{"K": "${TEMPORA_TEST_KEY}"},
		Headers: map[string]string{"Authorization": "Bearer ${TEMPORA_TEST_KEY}"},
	}
	...
	if e.Headers["Authorization"] != "Bearer ${TEMPORA_TEST_KEY}" {
...
func TestForbidReadRootsForRootResolvesRelativePathsAndScopedEnv(t *testing.T) {
	root := t.TempDir()
	cfg := Default()
	cfg.setExpansionEnv(map[string]string{"TEMPORA_TEST_SECRET_DIR": "from-dotenv"})
	cfg.Sandbox.ForbidRead = []string{
		"relative-secret",
		"${TEMPORA_TEST_SECRET_DIR}",
