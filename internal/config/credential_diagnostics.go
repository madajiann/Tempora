...
func DiagnoseCredentials(opts CredentialDiagnosticOptions) (CredentialDiagnosticReport, error) {
	home, path := TemporaHomeDir(), UserCredentialsPath()
	...
	if strings.TrimSpace(home) == "" || strings.TrimSpace(path) == "" {
		report.add("location", "failed", path, "Tempora home could not be resolved")
		return report, nil
	}
	homeAbs, homeErr := filepath.Abs(home)
	pathAbs, pathErr := filepath.Abs(path)
	if homeErr != nil || pathErr != nil || !pathWithinRoot(homeAbs, pathAbs) {
		report.add("location", "failed", path, "credential path is outside Tempora home")
		return report, nil
	}
	report.add("location", "passed", path, "credential path is inside Tempora home")
	...
	if opts.Repair {
		if repairPathErr != nil {
			...
			report.add("repair", "failed", path, "repair requires a current-user-owned regular file inside Tempora home")
		} else if opts.DryRun {
...
func probeCredentialDirectory(report *CredentialDiagnosticReport, path string) {
	...
	tmp, err := os.CreateTemp(dir, ".tempora-credential-probe-*")
	if err != nil {
		report.add("directory_replace_probe", "failed", dir, classifyCredentialAccessError(err))
		return
	}
...
func validateCredentialRepairPath(home, path string, expected os.FileInfo) error {
	...
	if !pathWithinRoot(home, path) {
		return fmt.Errorf("credential path is outside Tempora home")
	}
