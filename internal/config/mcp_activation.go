...
// MCPActivationFile is the on-disk shape of $TEMPORA_HOME/mcp-activation.json.
...
// MCPActivationPath returns the durable activation file under Tempora home.
func MCPActivationPath(temporaHome string) string {
	return filepath.Join(strings.TrimSpace(temporaHome), mcpActivationFilename)
}

// NewMCPActivationStore opens the activation store for temporaHome.
func NewMCPActivationStore(temporaHome string) *MCPActivationStore {
	return &MCPActivationStore{path: MCPActivationPath(temporaHome)}
}

// DefaultMCPActivationStore uses the process Tempora home.
func DefaultMCPActivationStore() *MCPActivationStore {
	return NewMCPActivationStore(TemporaHomeDir())
}
