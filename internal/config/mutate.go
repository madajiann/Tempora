...
// other Tempora processes cannot save stale snapshots over one another.
...
// with both other goroutines and other Tempora processes. The cross-process
...
// Tempora credential environment.
...
// two Tempora processes editing overlapping config-source sets from deadlocking.
func lockConfigFilesEdits(paths ...string) (func(), error) {
	userEditMu.Lock()
...
func configEditLockRegistryDir() (string, error) {
	...
	if runtime.GOOS != "windows" {
		...
		return filepath.Join(string(filepath.Separator), "tmp", fmt.Sprintf("tempora-config-locks-%x", digest[:8])), nil
	}
	home := strings.TrimSpace(current.HomeDir)
	if home == "" {
		return "", fmt.Errorf("lock config edits: OS user home unavailable")
	}
	return filepath.Join(filepath.Clean(home), ".tempora", "locks", fmt.Sprintf("config-edits-%x", digest[:8])), nil
}
