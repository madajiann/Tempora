...
func TestLoadCCSwitchLegacyConfigPrefersTemporaFlag(t *testing.T) {
	dir := t.TempDir()
		...
		"mcp": {
			"servers": {
				...
				"tempora-off": {
					"name": "tempora-off",
					"server": {"command": "node", "args": ["off.js"]},
					"apps": {"codex": true, "tempora": false}
				},
				"tempora-on": {
					"name": "tempora-on",
					"server": {"command": "node", "args": ["on.js"]},
					"apps": {"codex": false, "tempora": true}
				}
			}
		}
	}`
	...
	if len(got) != 2 || got[0].Name != "legacy" || got[1].Name != "tempora-on" {
		t.Fatalf("entries = %+v, want legacy fallback and explicit Tempora enablement", got)
	}
}

func TestLoadCCSwitchMCPDBPrefersTemporaColumn(t *testing.T) {
	...
	setup := `CREATE TABLE mcp_servers (
		...
		enabled_tempora BOOLEAN NOT NULL DEFAULT 0
	);
	INSERT INTO mcp_servers VALUES ('codex-only', 'codex-only', '{"command":"node","args":["codex.js"]}', 1, 0);
	INSERT INTO mcp_servers VALUES ('tempora-only', 'tempora-only', '{"command":"node","args":["tempora.js"]}', 0, 1);`
	...
	if len(got) != 1 || got[0].Name != "tempora-only" {
		t.Fatalf("entries = %+v, want only explicit Tempora enablement", got)
	}
}

func TestLoadCCSwitchMCPDBFallsBackToCodexWithoutTemporaColumn(t *testing.T) {
	if _, err := exec.LookPath("sqlite3"); err != nil {
		t.Skip("sqlite3 not available")
	}
	dbPath := filepath.Join(t.TempDir(), "cc-switch.db")
