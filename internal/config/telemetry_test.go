...
func TestCLITelemetryIsUserGlobalAndSafeModeForcesOff(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "tempora.toml"), []byte("[telemetry]\ncli_metrics = \"off\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	// v1.20+ ignores TEMPORA_SAFE_MODE: user telemetry preference is preserved.
	...
	if got := still.CLITelemetryMode(); got != "on" {
		t.Fatalf("telemetry under TEMPORA_SAFE_MODE = %q, want on", got)
	}
}
