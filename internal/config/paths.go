...
func userConfigDir() string {
	return temporaHomeDir()
}

func temporaHomeDir() string {
	if dir := cleanEnvDir("TEMPORA_HOME"); dir != "" {
		return dir
	}
...
func legacyOSSupportDir() string {
	...
	if current := temporaHomeDir(); current != "" && samePath(path, current) {
		return ""
	}
	return path
}
...
func userCacheDir() string {
	if dir := cleanEnvDir("TEMPORA_CACHE_HOME"); dir != "" {
		return dir
	}
	if dir := cleanEnvDir("TEMPORA_HOME"); dir != "" {
...
func TemporaManagedConfigPaths() []string {
	var out []string
...
// unavailable.
func TemporaHomeDir() string { return temporaHomeDir() }
...
func WorkspaceLeaseDir() string {
	// Deliberately ignore TEMPORA_HOME/TEMPORA_CACHE_HOME here. Two app
	...
	return filepath.Join(dir, "tempora", "workspace-leases")
}
...
func RepairMutationLockDir() string {
	...
	return filepath.Join(dir, "tempora", "repair-mutation-locks")
}
...
func DeliveryWorktreeDir() string {
	...
	if runtimeGOOS == "windows" {
		if dir := osUserCacheDir(); dir != "" {
			return filepath.Join(dir, "tempora", "worktrees")
		}
		if home, err := osUserHomeDir(); err == nil && home != "" {
			return filepath.Join(home, "AppData", "Local", "tempora", "worktrees")
		}
		return ""
	}
	dir := userSupportDir()
...
// .tempora/settings.json (see internal/hook).
...
// ~/.tempora/commands), the Tempora home commands dir, the legacy OS
// app-support dir if different, then the project's
// convention dirs (.claude/commands … .tempora/commands). Scanning the .claude /
