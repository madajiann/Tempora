...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. Tempora reads
...
// stdio server; type/url/headers describe a remote one. Tempora also accepts
// startup and call timeout fields as Tempora policy extensions.
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.tempora/config.json.
func legacyConfigPath() string {
	...
	return filepath.Join(home, ".tempora", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.tempora/config.json and returns its enabled
...
// already declare. tempora.toml's [[plugins]] win on a name collision: it is the
// Tempora-specific, more explicit of the two, so it overrides the shared,
...
func (c *Config) mergeMCPJSON(entries []PluginEntry) {
	...
	for _, e := range entries {
		if i, exists := index[e.Name]; exists {
			...
			// Within one project, tempora.toml remains more specific than the
...
func removeMCPJSONApprovalModes(server map[string]json.RawMessage) error {
	...
	// Remove Tempora's retired approval_mode while preserving tool fields owned
