...
// the state home is TEMPORA_STATE_HOME when set and TEMPORA_HOME otherwise.
...
func TestStatsDirResolution(t *testing.T) {
	...
	if got := StatsDir(); got != filepath.Join(home, "stats") {
		t.Fatalf("TEMPORA_HOME stats dir: want %q, got %q", filepath.Join(home, "stats"), got)
	}
	t.Setenv("TEMPORA_STATE_HOME", state)
	if got := StatsDir(); got != filepath.Join(state, "stats") {
		t.Fatalf("TEMPORA_STATE_HOME stats dir: want %q, got %q", filepath.Join(state, "stats"), got)
	}
}
