...
// legacyConfig is the subset of the v0.x (~/.tempora/config.json) schema this
// import carries forward. Fields absent here are dropped on purpose: desktop tab
// state is frontend-owned, and skills already live in the shared ~/.tempora/skills
...
func (r *MigrationResult) Notice() string {
	...
	if r.KeyToEnv {
		b.WriteString("; API key saved to tempora's credentials store")
	}
...
// checks v1-era TOML first, then v0.5/v0.x ~/.tempora/config.json, and never
...
func MigrateLegacyIfNeededForRoot(root string) (*MigrationResult, error) {
	...
	src := filepath.Join(home, ".tempora", "config.json")
	...
	if qqSecret := strings.TrimSpace(legacy.QQ.AppSecret); qqSecret != "" {
		...
		res.Warnings = append(res.Warnings, "your previous QQ Bot App Secret was saved to tempora's credentials store")
	}
...
func migrateMCPToUserConfig(projectRoots []string) (*MCPGlobalMigrationResult, error) {
	...
	for _, root := range normalizedMCPMigrationRoots(projectRoots) {
		addEntries(loadPluginEntriesFromTOML(filepath.Join(root, "tempora.toml")))
...
func legacyTOMLPaths(dest, home string) []string {
	...
	for _, legacy := range legacyXDGConfigPaths() {
		add(legacy)
		add(filepath.Join(filepath.Dir(legacy), "tempora.toml"))
	}
	add(filepath.Join(filepath.Dir(dest), "tempora.toml"))
	if home != "" {
		add(filepath.Join(home, ".tempora", "tempora.toml"))
	}
	return paths
}
...
// writeCredentialsEnv merges lines into Tempora's global .env
// and pins them into the current process env so the just-built session resolves
// the key without a restart. Falls back to ~/.env only when Tempora home can't
