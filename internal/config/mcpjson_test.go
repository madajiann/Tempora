...
func TestMCPJSONUpdateRemovesRetiredApprovalFieldsAndPreservesUnknownFields(t *testing.T) {
  ...
  "mcpServers": {
	...
	if _, ok := tools["remove_keep"]["approval_mode"]; ok {
		t.Fatal("removed Tempora approval mode survived")
	}
...
command = "local-bin"
`
	if err := os.WriteFile("tempora.toml", []byte(toml), 0o644); err != nil {
		t.Fatal(err)
	}
	mcp := `{ "mcpServers": {
  ...
  "extra":  { "command": "extra-bin", "auto_start": false }
	...
	if byName["shared"].Command != "local-bin" || byName["shared"].URL != "" {
		t.Errorf("tempora.toml should win the collision, got %+v", byName["shared"])
	}
...
func TestLoadMergesPluginsAcrossTOMLSources(t *testing.T) {
	...
	if err := os.WriteFile("tempora.toml", []byte("[[plugins]]\nname = \"projectmcp\"\ncommand = \"project-bin\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !names["globalmcp"] || !names["projectmcp"] {
		t.Fatalf("a project tempora.toml [[plugins]] dropped the global config's server; got %+v", cfg.Plugins)
	}
...
func TestLoadProjectMCPPriorityIsTemporaThenMCPJSONThenGlobal(t *testing.T) {
...
`), 0o644); err != nil {
	...
	entry, ok = pluginEntryByName(cfg.Plugins, "shared")
	if !ok || entry.Command != "project-tempora-mcp" || entry.Source != MCPSourceProjectConfig {
		t.Fatalf("tempora.toml + .mcp.json + global effective entry = %+v, want project tempora.toml", entry)
	}
}
...
func TestMergeMCPJSONPrecedence(t *testing.T) {
	// tempora.toml already declares "shared" (stdio); .mcp.json offers a colliding
	// "shared" (http) plus a fresh "extra". tempora.toml must win on the collision;
	// "extra" gets appended.
	...
	if cfg.Plugins[0].Name != "shared" || cfg.Plugins[0].Command != "local-bin" || cfg.Plugins[0].URL != "" {
		t.Errorf("collision not won by tempora.toml: %+v", cfg.Plugins[0])
	}
...
name = "dida"
type = "http"
url = "https://tempora.example/mcp?access_token=toml"
[plugins.headers]
...
`), 0o644); err != nil {
  ...
  "dida": {
	...
	if source != "tempora.toml" {
		t.Fatalf("source = %q, want tempora.toml", source)
	}
	if updated.URL != "https://tempora.example/mcp" {
		t.Fatalf("updated URL = %q", updated.URL)
	}

	projectRaw, err := os.ReadFile("tempora.toml")
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(projectRaw), "access_token=toml") || strings.Contains(string(projectRaw), "Authorization") {
		t.Fatalf("tempora.toml auth material should be removed:\n%s", projectRaw)
	}
...
`, token)
	...
	if want := filepath.Join(rootA, "tempora.toml"); !samePath(source, want) {
		t.Fatalf("source = %q, want %q", source, want)
	}
	rootBRaw, err := os.ReadFile(filepath.Join(rootB, "tempora.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemovePluginFromSourcesForRootRemovesEveryWritableDeclaration(t *testing.T) {
	...
	for _, path := range []string{userConfig, filepath.Join(root, "tempora.toml")} {
		if err := os.WriteFile(path, []byte(`
[[plugins]]
name = "duplicate"
...
func TestClearPluginAuthenticationHonorsMCPJSONFileLock(t *testing.T) {
	root := t.TempDir()
	t.Setenv("TEMPORA_HOME", filepath.Join(root, "home"))
	mcpPath := filepath.Join(root, mcpJSONFile)
	if err := os.WriteFile(mcpPath, []byte(`{
  "mcpServers": {
    "remote": {
...
func TestRemovePluginFromSourcesRejectsBrokenConfigSymlink(t *testing.T) {
	root := t.TempDir()
	t.Setenv("TEMPORA_HOME", filepath.Join(root, "home"))
	link := filepath.Join(root, "tempora.toml")
