...
// RemotePasswordCredentialEnvName returns the Tempora-owned credential slot
...
// RemotePassphraseCredentialEnvName returns the Tempora-owned credential slot
...
func remoteCredentialEnvName(hostID, kind string) string {
	...
	return fmt.Sprintf("TEMPORA_REMOTE_%X_%s", sum[:8], kind)
}

// IsGeneratedRemoteCredential reports whether key is a Tempora-owned slot
...
// EditUserConfigWithCredentials updates config and its Tempora-owned secret
