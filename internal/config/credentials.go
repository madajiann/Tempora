...
const (
	...
	credentialsKeyringService = "tempora"
	credentialClearedPrefix   = "# tempora-cleared "
)
...
// userCredentialEditMu serializes Tempora-owned credential-store writes.
...
// ResolveGlobalFirst resolves key from Tempora's global .env only. Repeated
...
func credentialsStoreMode() string {
	if mode := strings.TrimSpace(os.Getenv("TEMPORA_CREDENTIALS_STORE")); mode != "" {
...
// be loaded from Tempora's global credential store. This includes configured
...
func loadCredentialStoreForRoot(root string) {
	...
	if p := UserCredentialsPath(); p != "" {
		loadDotEnvFileAs(p, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "Tempora credentials (.env)"})
	}
}

// StoreCredentialLines stores KEY=value assignments in Tempora's global .env
...
// Tempora process from overwriting a credential saved by another process.
...
// IsValidCredentialKey reports whether key can be stored in Tempora's dotenv
...
// transactions in this process and across Tempora processes. When both the
...
// Tempora credential store. Callers performing compare-and-apply must hold
...
func pinCredentialAssignments(assignments map[string]string) {
	for key, value := range assignments {
		...
		recordCredentialSource(key, value, CredentialSource{Kind: CredentialSourceCredentials, Path: UserCredentialsPath(), Label: "Tempora credentials (.env)"})
	}
}
...
func credentialSourceLabel(source CredentialSource) string {
	...
	case CredentialSourceCredentials:
		return "Tempora credentials"
	case CredentialSourceHomeEnv:
		return "home .env"
	case CredentialSourceLegacy:
		return "legacy Tempora credentials"
...
func storedCredentialValue(key string) (string, CredentialSource, bool) {
	if p := UserCredentialsPath(); p != "" {
		if value, ok := envFileValue(p, key); ok && value != "" {
			return value, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "Tempora credentials (.env)"}, true
		}
	}
	return "", CredentialSource{}, false
}
