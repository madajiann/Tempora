...
func TestServeIndexPresentsRecoveryPauseAsNotice(t *testing.T) {
	...
	for _, want := range []string{
		...
		"'recovery_paused': 'Automatic retries paused. Tempora stopped repeated attempts and kept completed work. Send “Continue” to start a fresh attempt, or add instructions to change direction.'",
		"'recovery_paused': '已暂停自动重试。Tempora 已停止重复尝试，并保留已完成的工作。发送“继续”即可开始新一轮，也可以补充要求来调整方向。'",
	} {
		if !strings.Contains(html, want) {
