...
func lockPendingUpdateStrict() (func(), error) {
	...
	if path == "" {
		return nil, fmt.Errorf("pending update: Tempora state directory is unavailable")
	}
...
func PrepareFileUpdate(fromVersion, toVersion, targetPath string, siblingPaths ...string) (*UpdateTransaction, error) {
	...
	root := config.MemoryUserDir()
	if root == "" {
		return nil, fmt.Errorf("prepare update: Tempora state directory is unavailable")
	}
...
func PrepareAppBundleUpdateHandoff(fromVersion, toVersion, appPath, backupPath, stagedAppPath, stagingPath string, ownerPID int) (*UpdateTransaction, error) {
	...
	// An uncooperative writer is not covered by Tempora's mutation lock. Recheck
...
func newAppBundleUpdateTransaction(fromVersion, toVersion, appPath, backupPath string) (*UpdateTransaction, error) {
	...
	if !strings.HasSuffix(strings.ToLower(tx.TargetPath), ".app") ||
		tx.BackupPath != tx.TargetPath+".tempora-update-backup" {
		return nil, fmt.Errorf("prepare update: invalid macOS bundle paths")
	}
	return tx, nil
}
...
func PublishClaimedFileUpdateMemberExact(
	...
	if !member.MissingBefore {
		...
		retained = member.TargetPath + ".tempora-update-aside-" + transactionID[:16]
...
func stageFileUpdateContent(targetPath string, content []byte, mode os.FileMode) (string, string, string, error) {
	tmp, err := os.CreateTemp(filepath.Dir(targetPath), "."+filepath.Base(targetPath)+".tempora-update-stage-*")
	if err != nil {
		return "", "", "", err
	}
	path := tmp.Name()
...
func quarantineExistingAppBundleUpdateBackup(tx *UpdateTransaction) (string, string, error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("resolve current Tempora executable: %w", err)
	}
	...
	if err != nil || rel == "." || rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) {
		return "", "", fmt.Errorf("existing handoff backup is outside the current Tempora installation")
	}
...
// a path merely because its name resembles a Tempora quarantine.
...
func writePendingUpdate(tx *UpdateTransaction, createOnly bool) error {
	...
	path := PendingUpdatePath()
	if path == "" {
		return fmt.Errorf("pending update: Tempora state directory is unavailable")
	}
...
func classifyPendingUpdate() (pendingUpdateDisposition, *UpdateTransaction, error) {
	path := PendingUpdatePath()
	if path == "" {
		return pendingUpdateNone, nil, fmt.Errorf("Tempora state directory is unavailable")
	}
...
// recovery needs regardless of where Tempora is installed: which release it
...
func quarantinePendingUpdate(reason string) (string, error) {
	path := PendingUpdatePath()
	if path == "" {
		return "", fmt.Errorf("Tempora state directory is unavailable")
	}
...
func rollbackPendingUpdateMatchingLocked(
	...
	// TEMPORA_HOME profiles cannot quarantine or restore the same binaries
	...
	case "app-bundle":
		...
		if _, statErr := os.Lstat(tx.TargetPath); statErr == nil {
			...
			var retainErr error
			failed, retainErr = retainUpdateRollbackNode(tx.TargetPath, "tempora-failed")
			if retainErr != nil {
...
func restoreReleaseUnit(
	...
	for i, f := range files {
		aside := f.TargetPath + ".tempora-rollback-aside"
...
func allowedUpdateTargetBase(base string, primary bool) bool {
	...
	case "tempora-desktop", "tempora-desktop.exe":
		return primary
	case "tempora.exe":
		return !primary
	case "tempora", "tempora-guard", "tempora-guard.exe", "tempora-launcher.exe", "tempora-update-helper.exe", "tempora-cli.exe":
		return !primary
	default:
		return false
	}
...
func validateUpdateTransactionForLauncher(tx *UpdateTransaction, launcher string) error {
	...
	case "file":
		if !allowedUpdateTargetBase(filepath.Base(tx.TargetPath), true) {
			return fmt.Errorf("pending update target is not a Tempora executable")
		}
	...
	case "app-bundle":
		if !strings.HasSuffix(strings.ToLower(tx.TargetPath), ".app") || tx.BackupPath != tx.TargetPath+".tempora-update-backup" {
...
func validateAppBundleHandoffMetadata(tx *UpdateTransaction) error {
	...
	if !strings.HasPrefix(stagingBase, "tempora-mac-update-") {
...
func validateOrphanedAppBundleBackupMetadata(tx *UpdateTransaction) error {
	...
	prefix := filepath.Base(tx.BackupPath) + ".tempora-orphaned-"
