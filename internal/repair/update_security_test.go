...
func TestPendingUpdateRejectsUnexpectedReleaseFile(t *testing.T) {
	...
	bad := []UpdateTransactionFile{
		...
		{TargetPath: filepath.Join(t.TempDir(), "tempora-guard"), BackupPath: backup, SHA256: hash},
		{TargetPath: filepath.Join(dir, "tempora-guard"), BackupPath: filepath.Join(t.TempDir(), "loose.previous"), SHA256: hash},
