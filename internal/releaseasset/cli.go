...
const (
	cliReleaseBase       = "https://github.com/tempora-dev/Tempora/releases/download"
...
func downloadCLIFromBase(ctx context.Context, client *http.Client, base, version, goos, goarch string, official bool) ([]byte, error) {
	...
	assetName := fmt.Sprintf("tempora-%s-%s.tar.gz", goos, goarch)
...
func extractCLI(archive []byte) ([]byte, error) {
	...
	for {
		...
		if path.Base(path.Clean(header.Name)) != "tempora" {
			continue
		}
