...
// Run with:
//
//	TEMPORA_LIVE_CHROME=1 go test ./internal/browser/cdp \
//	  -run '^TestLiveChrome$' -v -count=1 -timeout=3m
func TestLiveChrome(t *testing.T) {
	if os.Getenv("TEMPORA_LIVE_CHROME") != "1" {
		t.Skip("set TEMPORA_LIVE_CHROME=1 to run the real Chrome end-to-end test")
	}
	...
	act(t, ctx, exec, browser.ActRequest{
		...
		Action: browser.ActionType, Ref: queryRef, Text: "tempora",
	})
	act(t, ctx, exec, browser.ActRequest{
	...
	if !strings.Contains(after.Tree, "submitted:tempora:two") {
