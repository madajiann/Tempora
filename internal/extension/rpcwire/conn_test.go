...
func TestStructuredHandlerError(t *testing.T) {
	...
	conn.Handle("fail", func(context.Context, json.RawMessage) (any, error) {
		return nil, &RPCError{Code: -32000, Message: "controlled", Data: map[string]any{"temporaCode": "HOST_BUSY", "retryable": true}}
	})
	...
	if data["temporaCode"] != "HOST_BUSY" || data["retryable"] != true {
...
func TestRequestReturnsStructuredPeerError(t *testing.T) {
	...
	server.Handle("fail", func(context.Context, json.RawMessage) (any, error) {
		return nil, &RPCError{Code: -32000, Message: "busy", Data: map[string]any{"temporaCode": "HOST_BUSY"}}
	})
	ctx := t.Context()
