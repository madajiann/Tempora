...
func TestDependencyGraphExactAndRange(t *testing.T) {
	g, err := BuildDependencyGraph([]ComponentDescriptor{
		{ID: "host", Provides: []extensioncontract.Capability{cap("tempora", "provider", "deepseek/v4", "1.2.0", "sha256:p")}},
		{ID: "plug", Requires: []extensioncontract.Requirement{req("tempora", "provider", "deepseek/v4", ">=1.0.0", false)},
...
func TestDependencyGraphSchemaMismatch(t *testing.T) {
	_, err := BuildDependencyGraph([]ComponentDescriptor{
		{ID: "host", Provides: []extensioncontract.Capability{cap("tempora", "provider", "p", "1.0.0", "sha256:a")}},
		{ID: "plug", Requires: []extensioncontract.Requirement{{
			Capability: extensioncontract.Capability{
				Key:        extensioncontract.CapabilityKey{Namespace: "tempora", Kind: "provider", ID: "p"},
				SchemaHash: "sha256:b",
			},
			VersionRange: ">=1.0.0",
		}}},
...
func TestDependencyGraphOptionalMissing(t *testing.T) {
	g, err := BuildDependencyGraph([]ComponentDescriptor{
		{ID: "plug", Requires: []extensioncontract.Requirement{req("tempora", "provider", "missing", ">=1.0.0", true)}},
	})
	if err != nil {
		t.Fatal(err)
	}
