...
func TestFileStore_RoundTrip(t *testing.T) {
	...
	taskDir := filepath.Join(dir, ".tempora", "tasks", "task-1")
...
func TestFileStore_ListEvents_RoundTrip(t *testing.T) {
	...
	taskDir := filepath.Join(dir, ".tempora", "tasks", "t1")
	if err := os.MkdirAll(taskDir, 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestFileStore_RejectsSymlinkTaskDirectory(t *testing.T) {
	project := t.TempDir()
	outside := t.TempDir()
	root := filepath.Join(project, ".tempora", "tasks")
	...
	if err := NewFileStore(".tempora/tasks").SaveTask(context.Background(), project, snap); err == nil {
...
func TestFileStore_RejectsSymlinkStoreParent(t *testing.T) {
	project := t.TempDir()
	outside := t.TempDir()
	if err := os.Symlink(outside, filepath.Join(project, ".tempora")); err != nil {
		t.Skipf("symlink unavailable: %v", err)
	}
...
func TestFileStore_DefaultProjectRejectsSymlinkStoreParent(t *testing.T) {
	...
	if err := os.Symlink(outside, ".tempora"); err != nil {
		t.Skipf("symlink unavailable: %v", err)
	}

	snap := TaskSnapshot{SchemaVersion: 1, TaskID: "t1", SessionID: "s", Version: 1, State: TaskStateRunning, CreatedAt: time.Now(), UpdatedAt: time.Now()}
	if err := NewFileStore(".tempora/tasks").SaveTask(context.Background(), "", snap); err == nil {
...
func TestFileStore_RejectsSymlinkSnapshotAndEvents(t *testing.T) {
	project := t.TempDir()
	outside := t.TempDir()
	root := filepath.Join(project, ".tempora", "tasks", "t1")
	if err := os.MkdirAll(root, 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestFileStore_WritablePathsUsePrivateModes(t *testing.T) {
	...
	checks := map[string]os.FileMode{
		filepath.Join(project, ".tempora", "tasks"):                        0o700,
		filepath.Join(project, ".tempora", "tasks", "t1"):                  0o700,
		filepath.Join(project, ".tempora", "tasks", "t1", "snapshot.json"): 0o600,
		filepath.Join(project, ".tempora", "tasks", "t1", "events.jsonl"):  0o600,
		filepath.Join(project, ".tempora", "tasks", "t1", "task.lock"):     0o600,
	}
...
func TestFileStore_SaveTask_ConcurrentCAS(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().Truncate(time.Second)
	seed := NewFileStore(".tempora/tasks")
	v1 := TaskSnapshot{
		SchemaVersion: 1, TaskID: "t1", SessionID: "s1",
		State: TaskStateRunning, Version: 1, CreatedAt: now, UpdatedAt: now,
	}
	...
	write := func(v uint64) error {
		...
		return NewFileStore(".tempora/tasks").SaveTask(ctx, dir, snap)
	}
