...
func TestPowerShellPersistentLive(t *testing.T) {
	path := os.Getenv("TEMPORA_TEST_PWSH")
	if path == "" {
	...
	if got := run("[Console]::Write('TEMPORA_END_fake:0'); [Console]::Error.Write('stderr')"); got.Err != nil || !strings.Contains(got.Output, "TEMPORA_END_fake:0") || !strings.Contains(got.Output, "stderr") {
