...
func initRepo(t *testing.T) string {
	...
	git := func(args ...string) string {
		...
		cmd.Env = append(os.Environ(),
			...
			"GIT_AUTHOR_NAME=Tempora Test", "GIT_AUTHOR_EMAIL=tempora@example.invalid",
			"GIT_COMMITTER_NAME=Tempora Test", "GIT_COMMITTER_EMAIL=tempora@example.invalid")
...
func TestCreateManagedWorktreeFromRepositoryFolder(t *testing.T) {
	...
	if !strings.HasPrefix(result.Branch, "tempora/delivery-") {
	...
	if _, err := os.Stat(filepath.Join(result.WorktreeRoot, ".tempora-source")); !os.IsNotExist(err) {
		t.Fatalf("worktree must not contain a Tempora marker, stat err = %v", err)
	}
...
func TestCreatePreservesSelectedRepositorySubdirectory(t *testing.T) {
	...
	cmd = exec.Command("git", "-C", repo, "-c", "user.name=Tempora Test", "-c", "user.email=tempora@example.invalid", "commit", "-m", "subdir")
	if out, err := cmd.CombinedOutput(); err != nil {
...
func TestInspectRejectsUncommittedSelectedSubdirectoryWithoutGitMutation(t *testing.T) {
	...
	before, _, err := runGit(context.Background(), repo, "for-each-ref", "--format=%(refname)", "refs/heads/tempora/delivery-")
	if err != nil {
		t.Fatal(err)
	}
	...
	after, _, err := runGit(context.Background(), repo, "for-each-ref", "--format=%(refname)", "refs/heads/tempora/delivery-")
	if err != nil {
		t.Fatal(err)
	}
	if after != before {
...
func TestRollbackCreatePreservesIgnoredContent(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			git = exec.Command("git", "-C", repo, "-c", "user.name=Tempora Test", "-c", "user.email=tempora@example.invalid", "commit", "-m", "ignore generated files")
			if out, err := git.CombinedOutput(); err != nil {
...
func TestCreateSupportsPathsWithSpaces(t *testing.T) {
	...
	git := func(args ...string) {
		cmd := exec.Command("git", append([]string{"-C", repo}, args...)...)
		cmd.Env = append(os.Environ(), "GIT_AUTHOR_NAME=Tempora Test", "GIT_AUTHOR_EMAIL=tempora@example.invalid", "GIT_COMMITTER_NAME=Tempora Test", "GIT_COMMITTER_EMAIL=tempora@example.invalid")
		if out, err := cmd.CombinedOutput(); err != nil {
