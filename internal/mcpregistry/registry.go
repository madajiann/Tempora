...
// Entry is one registry server reduced to the configuration Tempora can
...
func (c *Client) fetch(ctx context.Context, query string, limit int) ([]Entry, error) {
	...
	req.Header.Set("User-Agent", "tempora-mcp-registry/dev")
	client := c.HTTP
	if client == nil {
...
type cacheFile struct {
	...
	// Tempora versions. New writes timestamp each query independently so a
