...
func message(cfg config.NotificationsConfig, e event.Event) (Message, bool) {
	...
	case event.TurnDone:
		if cfg.TurnDone {
			if e.Err != nil {
				return Message{Title: "Tempora", Body: "Turn failed"}, true
			}
			return Message{Title: "Tempora", Body: "Turn finished"}, true
		}
	case event.ApprovalRequest:
		if cfg.ApprovalRequest {
			return Message{Title: "Tempora", Body: "Approval needed"}, true
		}
	case event.AskRequest:
		if cfg.AskRequest {
			return Message{Title: "Tempora", Body: "Question needs your answer"}, true
		}
	}
