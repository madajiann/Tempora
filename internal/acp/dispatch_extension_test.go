...
func TestUpdateSinkExtensionSupportedClientGetsMetaAndText(t *testing.T) {
	...
	vendor, _ := meta["tempora.io"].(map[string]any)
	surface, _ := vendor["extensionSurface"].(map[string]any)
	if surface == nil {
		t.Fatalf("structured update missing _meta.tempora.io.extensionSurface: %v", u)
	}
...
func TestInitializeAdvertisesExtensionSurface(t *testing.T) {
	...
	vendor, ok := ir.AgentCapabilities.Meta["tempora.io"].(TemporaExtensionCapabilities)
	if !ok {
		t.Fatalf("_meta[tempora.io] = %T", ir.AgentCapabilities.Meta["tempora.io"])
	}
	if vendor.ExtensionSurface == nil || !vendor.ExtensionSurface.Supported ||
		vendor.ExtensionSurface.SchemaVersion != temporaExtensionSurfaceSchemaVersion {
	...
	got := decoded.AgentCapabilities.Meta["tempora.io"].ExtensionSurface
	if got == nil || !got.Supported || got.SchemaVersion != temporaExtensionSurfaceSchemaVersion {
...
func TestClientExtensionSurfaceSupportedParsing(t *testing.T) {
	tests := []struct {
		...
		{"capability absent", map[string]any{"tempora.io": map[string]any{}}, false},
		{"supported", map[string]any{"tempora.io": map[string]any{
			"extensionSurface": map[string]any{"supported": true, "schemaVersion": 1},
		}}, true},
		{"explicit false", map[string]any{"tempora.io": map[string]any{
			"extensionSurface": map[string]any{"supported": false},
		}}, false},
		{"malformed vendor", map[string]any{"tempora.io": "nope"}, false},
		{"malformed capability", map[string]any{"tempora.io": map[string]any{
			"extensionSurface": "nope",
		}}, false},
		{"malformed flag", map[string]any{"tempora.io": map[string]any{
