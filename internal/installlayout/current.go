// Package installlayout implements the Tempora v1.20+ versioned install layout:
// InstallRoot/{current.json, tempora-launcher, versions/<version>/...}.
//
...
func DesktopBinaryName() string {
	if runtime.GOOS == "windows" {
		return "tempora-desktop.exe"
	}
	return "tempora-desktop"
}
...
func UpdateHelperBinaryName() string {
	if runtime.GOOS == "windows" {
		return "tempora-update-helper.exe"
	}
	return "tempora-update-helper"
}
...
func LauncherBinaryName() string {
	if runtime.GOOS == "windows" {
		return "tempora-launcher.exe"
	}
	return "tempora-launcher"
}

// PortableAliasName is the Windows portable entry (Tempora.exe).
