	...
	for _, legacy := range []bool{false, true} {
		for _, canonical := range []bool{false, true} {
			t.Run(map[bool]string{false: "fresh", true: "legacy"}[legacy]+map[bool]string{false: "", true: "-canonical"}[canonical], func(t *testing.T) {
				root := filepath.Join(t.TempDir(), "Tempora 测试")
				if err := os.Mkdir(root, 0o755); err != nil {
					t.Fatal(err)
				}
				if legacy {
					writeTempMember(t, root, "tempora-launcher.exe", "old")
				}
				if canonical {
					writeTempMember(t, root, "Tempora.exe", "old")
				}
				for i, version := range []string{"v1.38.9", "v1.38.9", "v1.39.0"} {
					...
					assertEntryBytes(t, root, "Tempora.exe", body+"-gui")
					assertEntryBytes(t, root, "tempora-cli.exe", body+"-cli")
					if legacy {
						assertEntryBytes(t, root, "tempora-launcher.exe", body+"-gui")
					} else if _, err := os.Lstat(filepath.Join(root, "tempora-launcher.exe")); !os.IsNotExist(err) {
...
func TestWindowsEntriesInspectLegacyAfterLockAcquisition(t *testing.T) {
	...
	err := activateVersion(req, func(ctx context.Context, path string) (func(), error) {
		writeTempMember(t, root, "tempora-launcher.exe", "previous-writer")
		return filelock.Acquire(ctx, path)
	})
	if err != nil {
		t.Fatal(err)
	}
	assertEntryBytes(t, root, "tempora-launcher.exe", "new-gui")
}
...
func TestConcurrentOldAndNewActivationPreservesCommittedLegacyEntry(t *testing.T) {
	...
	first.RootMembers = []Member{
		{Name: "Tempora.exe", Path: first.WindowsRootEntries.LauncherPath},
		{Name: "tempora-launcher.exe", Path: first.WindowsRootEntries.LauncherPath},
		{Name: "tempora-cli.exe", Path: first.WindowsRootEntries.CLIEntryPath},
	}
	first.RequiredRootNames = []string{"Tempora.exe", "tempora-launcher.exe", "tempora-cli.exe"}
	...
	assertEntryBytes(t, root, "Tempora.exe", "new-gui")
	assertEntryBytes(t, root, "tempora-launcher.exe", "new-gui")
	assertEntryBytes(t, root, "tempora-cli.exe", "new-cli")
	ptr, err := ReadCurrent(root)
...
func TestWindowsEntriesRollbackAllSelectedEntries(t *testing.T) {
	for _, failure := range []string{"process", "pointer", "root"} {
		t.Run(failure, func(t *testing.T) {
			root := t.TempDir()
			writeTempMember(t, root, "tempora-launcher.exe", "original")
			...
			case "root":
				if err := os.Remove(filepath.Join(root, "tempora-cli.exe")); err != nil {
					t.Fatal(err)
				}
				if err := os.Mkdir(filepath.Join(root, "tempora-cli.exe"), 0o755); err != nil {
					t.Fatal(err)
				}
			}
			...
			assertEntryBytes(t, root, "Tempora.exe", "old-gui")
			assertEntryBytes(t, root, "tempora-launcher.exe", "old-gui")
			if failure != "root" {
				assertEntryBytes(t, root, "tempora-cli.exe", "old-cli")
			}
			if failure != "pointer" {
				ptr, err := ReadCurrent(root)
...
func TestWindowsEntriesRejectInvalidLegacyAndMixedInputs(t *testing.T) {
	for _, kind := range []string{"directory", "symlink", "mixed"} {
		t.Run(kind, func(t *testing.T) {
			root := t.TempDir()
			req := windowsEntryRequest(t, root, "v1.39.0", "new")
			legacy := filepath.Join(root, "tempora-launcher.exe")
			switch kind {
			case "directory":
			...
			if _, err := os.Lstat(filepath.Join(root, "Tempora.exe")); !os.IsNotExist(err) {
