...
func windowsRootMembers(root string, sources WindowsRootEntrySources) ([]Member, []string, error) {
	members := []Member{
		{Name: "Tempora.exe", Path: sources.LauncherPath},
		{Name: "tempora-cli.exe", Path: sources.CLIEntryPath},
	}
	names := []string{"Tempora.exe", "tempora-cli.exe"}
	const legacy = "tempora-launcher.exe"
