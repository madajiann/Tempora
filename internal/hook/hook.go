...
// (.tempora/settings.json, only when the project is trusted) and a global
// (<Tempora home>/settings.json) file. A hook's exit
...
// it only contributes guidance via stdout.) This governs native Tempora hooks;
...
// aborts the action even though PermissionRequest is not one of Tempora's own
...
// launcher. The zero value is the legacy Tempora settings behavior, where a
...
type HookConfig struct {
	...
	// Tempora settings retain their legacy shell-command behavior.
	...
	// imported Claude hooks. Native Tempora settings keep their old behavior.
	Async         bool   `json:"-"`
...
const (
	SettingsDirname  = ".tempora"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is <Tempora home>/settings.json (homeDir overrides ~ for
// tests and legacy callers).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(temporaHome(homeDir), SettingsFilename)
}

// ProjectSettingsPath is <root>/.tempora/settings.json.
...
type LoadOptions struct {
	...
	// derived global path is <HomeDir>/.tempora unless TemporaHomeDir is set.
	HomeDir string
	// TemporaHomeDir is the exact current Tempora home (settings.json lives
	// directly under it). When set, it takes precedence over HomeDir for global
	// settings and plugin hooks so Windows %APPDATA%/tempora and TEMPORA_HOME
...
func Load(opts LoadOptions) []ResolvedHook {
	...
	temporaHomeDir := temporaHomeForOptions(opts)
	appendPluginHooks(&out, temporaHomeDir, opts.ProjectRoot)
	g := filepath.Join(temporaHomeDir, SettingsFilename)
	if temporaHomeDir == "" {
...
func appendPluginHooks(out *[]ResolvedHook, temporaHomeDir, projectRoot string) {
	if strings.TrimSpace(temporaHomeDir) == "" {
		return
	}
	installed, _ := pluginpkg.LoadInstalled(temporaHomeDir)
	for _, item := range installed {
		...
		for _, eventName := range events {
			...
			for _, h := range pkg.Manifest.Hooks[eventName] {
				...
				env["TEMPORA_PLUGIN_ROOT"] = pkg.Root
				env["TEMPORA_PLUGIN_NAME"] = item.Installed.Name
				env["TEMPORA_HOME"] = temporaHomeDir
				env["TEMPORA_WORKSPACE_ROOT"] = projectRoot
				env["CLAUDE_PROJECT_DIR"] = projectRoot
				env["CLAUDE_PLUGIN_ROOT"] = pkg.Root
				if item.Installed.Version != "" {
					env["TEMPORA_PLUGIN_VERSION"] = item.Installed.Version
				}
...
var pluginRootTokens = [...]struct {
	...
	{value: "%CLAUDE_PLUGIN_ROOT%"},
	{value: "${TEMPORA_PLUGIN_ROOT}"},
	{value: "$TEMPORA_PLUGIN_ROOT", needsBoundary: true},
	{value: "%TEMPORA_PLUGIN_ROOT%"},
}
...
// claudeAgentSpawningTools are every Tempora tool that spawns a subagent and
...
// claudeToolNames maps Tempora's own tool names to the *current* Claude Code
...
// imported hook's matcher may have been authored against for a Tempora
...
// tried against for a Tempora tool call.
...
// hook's tool_name payload field should see for a Tempora tool call.
// Tempora-only tools (wait, code_index, move_file, ...) have no Claude
...
// claudeToolInputKeyRenames maps, per Tempora tool name, JSON keys in its
// tool-call arguments that must be renamed to Claude's own tool_input field
// name — Tempora's file tools use "path", Claude's use "file_path" — so a
// hook script reading e.g. ".tool_input.file_path" sees the value instead of
// failing open on an empty field. Only tools whose Tempora schema differs
...
// opaque cell_id, which Tempora also accepts — so it passes through as an
...
// Read/Write/Edit/NotebookEdit). Tempora's file tools accept relative paths
...
func claudeFacingToolInput(toolName string, args json.RawMessage, cwd string) json.RawMessage {
	...
	if toolName == "wait" {
		...
		// An unbounded Tempora wait omits TaskOutput's optional timeout
...
// fillClaudeAskDefaults supplies fields Claude requires but Tempora treats as
// optional. Empty option descriptions are honest (Tempora has no explanation
...
// Tempora task content when the caller omitted it.
...
// carries a JSON deny decision (see HookOutput.Deny). Tempora must honor it
...
// claudeToolResponse adapts a Tempora tool result to the tool_response a
...
// and the check silently passes) — while Tempora's bash returns one combined
...
// Legacy settings retain Tempora's historical shell behavior and repairs.
...
func powerShellCommand(ctx context.Context, path, command string) *exec.Cmd {
	...
	// code page into Tempora's stdout/stderr text contract.
...
func temporaHome(override string) string {
	if override != "" {
		return filepath.Join(override, SettingsDirname)
	}
	if dir := config.TemporaHomeDir(); dir != "" {
		return dir
	}
...
func temporaHomeForOptions(opts LoadOptions) string {
	if dir := strings.TrimSpace(opts.TemporaHomeDir); dir != "" {
		return filepath.Clean(dir)
	}
	return temporaHome(opts.HomeDir)
}

func legacyGlobalSettingsPath(homeDir string) string {
	dir := legacyTemporaHome(homeDir)
	if dir == "" {
		return ""
	}
...
func legacyTemporaHome(override string) string {
	if override != "" {
		return ""
	}
	...
	if sameCleanPath(legacy, temporaHome("")) {
		return ""
	}
	return legacy
}
