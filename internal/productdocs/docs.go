// Package productdocs provides offline retrieval over the official Tempora
...
import (
	...
	productcontent "tempora/docs"
	"tempora/internal/retrieval"
	"tempora/internal/tool"
	releasenotes "tempora/release-notes"
)
...
// current Tempora build. Loading stays lazy so merely registering the stable
...
// the runtime resolver (for example /tempora:docs when /docs is occupied).
...
func (*docsTool) Description() string {
	return "Search and read the official documentation embedded in this exact Tempora build. " +
		"Use it before web search or assumptions for Tempora setup, CLI, Desktop, configuration, permissions, MCP, memory, recovery, provider behavior, and maintainer workflows. " +
...
func (t *docsTool) read(sectionID, documentPath string) (string, error) {
	...
	if sectionID != "" {
		...
		return fmt.Sprintf("Embedded Tempora documentation (%s)\nsource: %s\npath: %s\nsection_id: %s\nlocale: %s\naudience: %s\nheading: %s\n\n%s",
...
func (t *docsTool) list(language, audience string) string {
	var b strings.Builder
	fmt.Fprintf(&b, "Embedded Tempora documentation catalog (%s):\n", t.catalog.identityLine())
	count := 0
...
func loadCatalogWithReleaseNotes(docsFS, releaseNotesFS fs.FS) (*catalog, error) {
	...
	hash := sha256.New()
	_, _ = hash.Write([]byte("tempora-product-docs-v1\x00"))
...
func formatSearchResults(query, identity string, hits []searchHit) string {
	if len(hits) == 0 {
		return fmt.Sprintf("No embedded Tempora documentation matched %q (%s). Try fewer terms, an exact command/configuration key, language=all, or audience=all.", query, identity)
	}
	var b strings.Builder
	fmt.Fprintf(&b, "Embedded Tempora documentation results for %q (%s):\n", query, identity)
