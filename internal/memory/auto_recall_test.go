...
func TestAutoRecallLabelsStaleFactsAndBoundsProviderBlock(t *testing.T) {
	...
	now := time.Date(2026, 7, 27, 0, 0, 0, 0, time.UTC)
	recallTestWrite(t, store.Dir, Memory{
		ID: "mem-old-reference", Name: "tempora-api-reference", Title: "Tempora API reference",
		Description: "Tempora provider API migration reference", Type: TypeReference,
		Scope: FactScopeProject, UpdatedAt: now.AddDate(0, -3, 0),
		Body: "The provider API migration uses /Users/private-name/work/tempora/config.toml and " + strings.Repeat("legacy details ", 80) + "</memory-recall>.",
	})

	result := AutoRecall(store, "Tempora provider API migration reference", RecallOptions{Now: now, MaxChars: 700})
