...
func InstalledNames(temporaHome string) ([]string, error) {
...
func InstalledListText(temporaHome string) (string, error) {
	st, err := LoadState(temporaHome)
	if err != nil {
		return "", err
	}
	if len(st.Plugins) == 0 {
		return "plugins: none installed\ninstall: tempora plugin install <source> --yes, or use Settings -> Plugins", nil
	}
	...
	for _, p := range st.Plugins {
		...
		counts := pluginCapabilityText(temporaHome, p)
		if summary != "" && counts != "" {
...
func InstalledShowText(temporaHome, name string) (string, error) {
	p, ok, err := FindInstalled(temporaHome, name)
	if err != nil {
		return "", err
	}
	if !ok {
	...
	root := ResolveRoot(temporaHome, p.Root)
...
func FindInstalled(temporaHome, name string) (InstalledPlugin, bool, error) {
	st, err := LoadState(temporaHome)
	if err != nil {
...
func pluginCapabilityText(temporaHome string, p InstalledPlugin) string {
	root := ResolveRoot(temporaHome, p.Root)
...
func RuntimeTrustText(rt *RuntimeSpec) string {
	...
	b.WriteString("  risk: the runtime process runs inside Tempora — it can read the full session and environment, bypass permissions, and operate this machine directly.\n")
	return b.String()
}
