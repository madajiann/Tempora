...
func NewTool(opts Options) tool.Tool {
	...
	} else if dir := config.TemporaHomeDir(); dir != "" {
		temporaHome = dir
	} else if home != "" {
		temporaHome = filepath.Join(home, ".tempora")
	}
...
func (t *installSourceTool) Execute(ctx context.Context, raw json.RawMessage) (string, error) {
	...
	if len(actions) == 0 {
		out := response{
			...
			PlanID:   planID,
			Warnings: warnings,
			Next:     "No installable Tempora skill, MCP server, or plugin package was detected. Ask the user for a direct SKILL.md, skill root, .mcp.json, plugin manifest, MCP endpoint, or package name.",
		}
...
func (t *installSourceTool) uninstallActionsForScope(name, scope string) []action {
	...
	if scope == "global" || scope == "" {
		if st, err := pluginpkg.LoadState(t.temporaHome); err == nil {
			for _, p := range st.Plugins {
				if p.Name != name {
					continue
				}
				root := pluginpkg.ResolveRoot(t.temporaHome, p.Root)
				actions = append(actions, action{
					Kind:         "plugin",
...
func (t *installSourceTool) resolveSkillPath(name, scope string) (string, bool) {
	...
	if scope == "global" {
		...
		root = filepath.Join(t.temporaHome, skill.SkillsDirname)
	} else {
		root = filepath.Join(t.root, ".tempora", skill.SkillsDirname)
	}
...
func (t *installSourceTool) configPath(scope string) string {
	...
	return filepath.Join(t.root, "tempora.toml")
}
