...
func TestConvertWindowsSandboxSpec(t *testing.T) {
	...
	protected := `C:\Users\agent\AppData\Roaming\Tempora`
	spec := Spec{
		Mode:                "enforce",
	...
	if !got.Writable || !got.Network || got.TempPrefix != "tempora-sandbox-" {
	...
	if spec.WriteRoots[0] == got.WritableRoots[0] || spec.ForbidReadRoots[0] == got.ForbidReadRoots[0] {
		t.Fatal("converted spec should not alias Tempora slices")
	}
}
