...
func UnavailableRemediation() string {
	...
	case "windows":
		return "Windows has no OS-level shell sandbox. Permission presets are enforced by Tempora file tools and shell commands run as the current OS user; select Full access only when ordinary approval prompts should also be skipped."
	default:
...
func BackendUnavailableReason() string {
	...
	case "windows":
		return "Tempora does not ship an OS-level sandbox on Windows"
	default:
