...
// Command wraps the shell invocation in Tempora's hidden Windows sandbox
...
// Available reports whether Tempora can reach its bundled Windows sandbox
...
func RunWindowsSandboxHelper(args []string, stdin *os.File, stdout *os.File, stderr *os.File) int {
	if len(args) < 3 || args[1] != "--" {
		fmt.Fprintln(stderr, "usage: tempora "+WindowsHelperCommand+" <payload> -- <command> [args...]")
		return 2
	}
