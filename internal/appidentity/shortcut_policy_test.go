...
func TestCanonicalShortcutMigratesLegacyButPreservesCustomProperties(t *testing.T) {
	root := t.TempDir()
	canonical := filepath.Join(root, "Tempora.exe")
	...
	for _, tc := range []struct {
		...
		{"foreign identity", legacy, legacy, "io.tempora.studio", shortcutRepair{}},
	} {
	...
	oldTarget := filepath.Join(root, "versions", "v1.38.9", "app", "Tempora.exe")
	for _, tc := range []struct {
		dir  string
		want bool
	}{
