...
func TestDetectRefsOnlyKeepsMissingImageAttachments(t *testing.T) {
	...
	refs := c.detectRefs("inspect @.tempora/attachments/missing.png and @.tempora/attachments/missing.pdf")
	if len(refs) != 1 || refs[0].kind != refImage || refs[0].path != ".tempora/attachments/missing.png" {
...
func TestSubmitIdentifiedRejectsMissingExplicitImageBeforeRunner(t *testing.T) {
	...
	_, err := c.SubmitIdentified(SubmissionRequest{
		Input: "inspect @.tempora/attachments/missing.png", Display: "inspect image",
	})
...
func TestDirectAndEditedSubmissionsRejectMissingExplicitImageBeforeRunner(t *testing.T) {
	const input = "inspect @.tempora/attachments/missing.png"
...
func TestSubmitIdentifiedRejectsAllImagesWhenOneIsMissing(t *testing.T) {
	...
	runner := &recordingSessionRunner{session: agent.NewSession("sys")}
	c := newOwnedTestController(t, Options{WorkspaceRoot: workspace, Runner: runner})
	_, err = c.SubmitIdentified(SubmissionRequest{Input: "inspect @" + filepath.ToSlash(valid) + " @.tempora/attachments/missing.png"})
	var failures ImageReferenceFailures
...
func TestSubmitIdentifiedClassifiesExplicitImageFailuresBeforeRunner(t *testing.T) {
	workspace := t.TempDir()
	attachments := filepath.Join(workspace, ".tempora", "attachments")
	...
	if err := os.WriteFile(filepath.Join(workspace, ".tempora", "outside.png"), mustBase64(t, tinyPNG), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	symlinkAvailable := os.Symlink(filepath.Join(workspace, ".tempora", "outside.png"), linkPath) == nil
	...
	cases := []struct {
		...
		{name: "corrupt", ref: ".tempora/attachments/corrupt.png", code: ImageReferenceUnsupported},
		{name: "traversal", ref: ".tempora/attachments/../outside.png", code: ImageReferenceUnsafe},
		{name: "too large", ref: ".tempora/attachments/too-large.png", code: ImageReferenceTooLarge},
	}
	if symlinkAvailable {
		cases = append(cases, struct {
			...
			ref  string
			code ImageReferenceFailureCode
		}{name: "symlink", ref: ".tempora/attachments/link.png", code: ImageReferenceUnsafe})
	}
...
func TestSubmitIdentifiedRejectsInvocationImageBeforePreparingInvocation(t *testing.T) {
	...
	c := newOwnedTestController(t, Options{WorkspaceRoot: workspace, Runner: runner})
	_, err := c.SubmitIdentified(SubmissionRequest{
		Input:       "inspect @.tempora/attachments/missing.png",
...
func TestEnqueueInboxRejectsMissingExplicitImageWithoutDurableItem(t *testing.T) {
	...
	_, err := c.EnqueueInbox(InboxRequest{Submit: "inspect @.tempora/attachments/missing.png"})
	var failures ImageReferenceFailures
	if !errors.As(err, &failures) || len(failures) != 1 || failures[0].Code != ImageReferenceMissing {
...
func TestResolveRefsUnreadableImageDoesNotClaimAttached(t *testing.T) {
	...
	t.Chdir(dir)
	writeVisionTestConfig(t, dir)
	const imagePath = ".tempora/attachments/empty.png"
...
func TestFreezeInboxReferencesResolvesLargeImageOnce(t *testing.T) {
	...
	imagePath := filepath.Join(workspace, ".tempora", "attachments", "large.png")
	if err := os.MkdirAll(filepath.Dir(imagePath), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	env := sessioninbox.PromptEnvelope{SubmitText: "look", ExplicitRefs: []string{filepath.ToSlash(filepath.Join(".tempora", "attachments", "large.png"))}}
