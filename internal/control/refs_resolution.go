...
func isAttachmentRef(token string) bool {
	return strings.HasPrefix(filepath.ToSlash(token), ".tempora/attachments/")
}
...
func statAttachmentImage(root, path string) error {
	...
	confine := filepath.Join(absRoot, ".tempora", "attachments")
	rel, err := filepath.Rel(confine, joined)
	if err != nil || strings.HasPrefix(rel, "..") {
		return fmt.Errorf("image path is outside .tempora/attachments")
	}
...
func (c *Controller) explicitImageSources(line string) []attachment.Source {
	...
	for _, token := range parseRefTokens(line) {
		...
		seen[key] = true
		sources = append(sources, attachment.Source{DisplayName: filepath.Base(filepath.FromSlash(token)), Path: token, WorkspaceRoot: c.workspaceRoot, Confine: ".tempora/attachments"})
	}
	return sources
}
...
func imageReferenceFailure(path string, err error) ImageReferenceFailure {
	...
	case errors.Is(err, os.ErrPermission):
		code = ImageReferenceUnreadable
	case strings.Contains(message, "outside .tempora/attachments") || strings.Contains(message, "symlink") || strings.Contains(message, "must be relative"):
