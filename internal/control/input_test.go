...
func TestSkillsReflectStoreChangesAfterControllerBuild(t *testing.T) {
	...
	writeControlSkill(t, project, ".tempora/skills/hot/SKILL.md", "---\nname: hot\ndescription: Hot install\n---\nHot body")
...
func TestSubmitDocsShowsLocalOverviewAndGroundsModelTurn(t *testing.T) {
	...
	c.Submit("/docs v0.1.0 升级说明")
	waitForTurnDone(t, events)
	if len(runner.inputs) != 1 {
		t.Fatalf("/docs query model turns = %d, inputs=%q", len(runner.inputs), runner.inputs)
	}
	// Fork note: the embedded docs corpus is the Tempora fork's own (v0.1.0
	// release notes + rewritten guides), so the grounded query targets those
	// rather than the upstream v1.19.5 changelog.
	for _, want := range []string{"v0.1.0 升级说明", "changelog/v0.1.0.zh-CN.md", "embedded_docs_search_results"} {
...
func TestSubmitQualifiedTemporaDocsPreservesExistingCommandAndUsesNextFallback(t *testing.T) {
	runner := &fakeTurnRunner{}
	events := make(chan event.Event, 8)
	c := newOwnedTestController(t, Options{
		Runner: runner,
		Commands: []command.Command{
			...
			{Name: TemporaDocsSlashName, Body: "must not shadow built-in docs: $ARGUMENTS"},
		},
		Sink: event.FuncSink(func(e event.Event) {
			events <- e
		}),
	...
	c.Submit("/tempora:docs existing workflow")
	waitForTurnDone(t, events)
	if len(runner.inputs) != 1 {
	...
	c.Submit("/tempora:builtin:docs 1.19.5 update notes")
	waitForTurnDone(t, events)
	if len(runner.inputs) != 2 {
