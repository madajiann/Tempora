...
func TestSaveImageDataURL(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".tempora/attachments/clipboard-") || !strings.HasSuffix(got, ".png") {
...
func TestSaveAttachmentFile(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".tempora/attachments/clipboard-") || !strings.HasSuffix(got, ".pdf") {
...
func TestImageDataURLRejectsOutsideAttachmentDir(t *testing.T) {
	...
	if _, err := ImageDataURL("../.tempora/attachments/x.png"); err == nil {
...
func TestImageDataURLInRootIgnoresProcessWorkingDirectory(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(processDir, ".tempora")); !os.IsNotExist(err) {
...
func TestAttachmentRootAPIsRejectManagedDirectorySymlinks(t *testing.T) {
	for _, component := range []string{".tempora", "attachments"} {
		t.Run(component, func(t *testing.T) {
			workspace := t.TempDir()
			outside := t.TempDir()
			var link string
			if component == ".tempora" {
				if err := os.MkdirAll(filepath.Join(outside, "attachments"), 0o755); err != nil {
					t.Fatal(err)
				}
				link = filepath.Join(workspace, ".tempora")
			} else {
				if err := os.MkdirAll(filepath.Join(workspace, ".tempora"), 0o755); err != nil {
					t.Fatal(err)
				}
				link = filepath.Join(workspace, ".tempora", "attachments")
			}
			if err := os.Symlink(outside, link); err != nil {
				t.Skipf("symlink unsupported: %v", err)
			}
			if _, err := ImageDataURLInRoot(workspace, ".tempora/attachments/image.png"); err == nil || !strings.Contains(err.Error(), "symlink") {
...
func TestImageDataURLInRootKeepsSameRelativeNameIsolated(t *testing.T) {
	...
	rel := filepath.Join(".tempora", "attachments", "same.png")
...
func TestImageDataURLInRootDoesNotCreateMissingAttachmentDirectory(t *testing.T) {
	workspace := t.TempDir()
	_, err := ImageDataURLInRoot(workspace, ".tempora/attachments/missing.png")
	if !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("error = %v, want not exist", err)
	}
	if _, err := os.Stat(filepath.Join(workspace, ".tempora")); !os.IsNotExist(err) {
...
func TestImageDataURLRejectsSymlinkFile(t *testing.T) {
	...
	link := filepath.Join(".tempora", "attachments", "link.png")
...
func TestImageDataURLRejectsSymlinkAttachmentDir(t *testing.T) {
	t.Chdir(t.TempDir())
	if err := os.Mkdir(".tempora", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir("elsewhere", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("../elsewhere", filepath.Join(".tempora", "attachments")); err != nil {
		t.Skipf("symlink unsupported: %v", err)
	}
	if _, err := ImageDataURL(".tempora/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkSubdirectory(t *testing.T) {
	...
	link := filepath.Join(".tempora", "attachments", "link")
...
func TestValidateAttachmentInRootRejectsMissingAndSymlink(t *testing.T) {
	...
	link := filepath.Join(root, ".tempora", "attachments", "link.txt")
	if err := os.Symlink(outside, link); err != nil {
		t.Skipf("symlink unsupported: %v", err)
	}
	if err := ValidateAttachmentInRoot(root, filepath.ToSlash(filepath.Join(".tempora", "attachments", "link.txt"))); err == nil {
