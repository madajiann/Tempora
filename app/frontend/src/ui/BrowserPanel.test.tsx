...
describe("BrowserPanel", () => {
  it("opens a typed address when Enter is pressed", async () => {
    ...
    expect(screen.getByTitle("Tempora 内置 Browser").getAttribute("src")).toBe("https://example.com/docs");
  });
  ...
  it("keeps invalid addresses in the field and explains the error", async () => {
    ...
    expect(screen.getByTitle("Tempora 内置 Browser").hasAttribute("src")).toBe(false);
  });
});
