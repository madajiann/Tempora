...
func TestDialogDefaultDirectoryUsesFileParent(t *testing.T) {
	dir := t.TempDir()
	file := filepath.Join(dir, "tempora.toml")
...
func TestReadFileMediaPreview(t *testing.T) {
	...
	if !strings.HasPrefix(image.URL, "/__tempora_workspace_media/") || !strings.HasSuffix(image.URL, "/shot.PNG") {
	...
	if !strings.HasPrefix(pdf.URL, "/__tempora_workspace_media/") || !strings.HasSuffix(pdf.URL, "/report.pdf") {
...
func TestMediaTokenHandlerBadToken(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/__tempora_workspace_media/deadbeef/fake.png", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
