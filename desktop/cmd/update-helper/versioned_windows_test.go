...
func TestActivateVersionedWindowsFromStaging(t *testing.T) {
	...
	for _, name := range []string{"Tempora.exe", "tempora-cli.exe"} {
		if _, err := os.Stat(filepath.Join(installDir, name)); err != nil {
			t.Fatalf("root entry %s: %v", name, err)
		}
	}
	if _, err := os.Lstat(filepath.Join(installDir, "tempora-launcher.exe")); !os.IsNotExist(err) {
	...
	if prefer := preferRelaunchPath("", installDir); filepath.Base(prefer) != "Tempora.exe" {
...
func TestVersionedWindowsUpgradePreservesExistingLegacyEntry(t *testing.T) {
	acceptWindowsPayloadManifestForTest(t)
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "tempora-launcher.exe"), []byte("old"), 0o700); err != nil {
		t.Fatal(err)
	}
	for _, version := range []string{"v1.38.9", "v1.39.0"} {
		...
		for _, name := range []string{"Tempora.exe", "tempora-launcher.exe"} {
			body, err := os.ReadFile(filepath.Join(root, name))
			if err != nil || string(body) != version+":tempora-launcher.exe" {
...
func TestPreferRelaunchPathIgnoresStaleVersionedDesktop(t *testing.T) {
	...
	if filepath.Base(got) != "Tempora.exe" {
