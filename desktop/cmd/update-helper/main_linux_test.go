...
func baseFakeDeps(p *installProbe) installDeps {
	return installDeps{
		...
		mkTempDir: func() (string, error) {
			return os.MkdirTemp("", "tempora-helper-test-*")
		},
...
func writeInstallInputs(t *testing.T) (pkg, sig string) {
	t.Helper()
	dir := t.TempDir()
	pkg = filepath.Join(dir, "Tempora.deb")
	sig = filepath.Join(dir, "Tempora.deb.minisig")
...
func TestRunInstallCleansTempDirOnFailure(t *testing.T) {
	...
	d.mkTempDir = func() (string, error) {
		dir, err := os.MkdirTemp("", "tempora-helper-test-*")
		created = dir
		return dir, err
	}
