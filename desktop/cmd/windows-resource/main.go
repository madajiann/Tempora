// Command windows-resource stamps Tempora branding and metadata into a Windows
...
func stampExecutable(opts resourceOptions) error {
	...
	rs.SetManifest(winres.AppManifest{
		Identity: winres.AssemblyIdentity{
			Name:    "Tempora." + opts.internalName,
	...
	for key, value := range map[string]string{
		...
		version.ProductName:      "Tempora",
		version.ProductVersion:   opts.numericVersion,
		version.Comments:         "Tempora desktop support component.",
	} {
...
func replaceFile(path string, data []byte, mode os.FileMode) error {
	temp, err := os.CreateTemp(filepath.Dir(path), ".tempora-resource-*.exe")
	if err != nil {
		return err
	}
	tempName := temp.Name()
