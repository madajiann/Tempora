...
func TestStampExecutableSupportsWindowsReleaseArchitectures(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.arch, func(t *testing.T) {
			...
			executable := filepath.Join(dir, "tempora-launcher.exe")
			...
			opts := resourceOptions{
				...
				fileDescription: "Tempora Launcher",
				internalName:    "tempora-launcher",
				originalName:    "tempora-launcher.exe",
			}
			...
			if got := (*info.Table()[version.LangDefault])[version.OriginalFilename]; got != "tempora-launcher.exe" {
