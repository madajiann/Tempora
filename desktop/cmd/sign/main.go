...
var updaterArtifacts = map[string]struct {
	...
	"Tempora-darwin-arm64.zip":            {key: "darwin-arm64", kind: artifactPortable},
	"Tempora-darwin-amd64.zip":            {key: "darwin-amd64", kind: artifactPortable},
	"Tempora-windows-amd64-installer.exe": {key: "windows-amd64", kind: artifactPortable},
	"Tempora-windows-arm64-installer.exe": {key: "windows-arm64", kind: artifactPortable},
	"Tempora-linux-amd64.tar.gz":          {key: "linux-amd64", kind: artifactPortable},
	"Tempora-linux-amd64.deb":             {key: "linux-amd64", kind: artifactNative},
}
...
// (tempora.key) and the public key (tempora.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "tempora.key")
	pubPath := filepath.Join(dir, "tempora.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Tempora desktop release")
		out := f + ".minisig"
