...
func TestMemorySuggestionsForTabUsesSelectedTab(t *testing.T) {
	...
	wantSkillPath := filepath.Join(selectedCwd, ".tempora", "skills", "selected-workflow", "SKILL.md")
	if skillPath != wantSkillPath {
		t.Fatalf("skill path = %q, want %q", skillPath, wantSkillPath)
	}
	if _, err := os.Stat(filepath.Join(activeCwd, ".tempora", "skills", "selected-workflow", "SKILL.md")); !os.IsNotExist(err) {
...
func TestMemorySuggestionsAcceptSkillCandidate(t *testing.T) {
	...
	for _, item := range view.Skills {
		if item.Name == "tempora-pr-followup" {
			candidate = item
			break
		}
	}
	if candidate.Name == "" {
		t.Fatalf("MemorySuggestions() skills = %+v, want tempora-pr-followup", view.Skills)
	}
	path, err := app.AcceptSkillSuggestion(candidate)
	if err != nil {
		t.Fatalf("AcceptSkillSuggestion: %v", err)
	}
	wantSuffix := filepath.Join(".tempora", "skills", "tempora-pr-followup", "SKILL.md")
	...
	if !strings.Contains(string(body), "Review or update a Tempora GitHub PR") {
