...
func TestAuditDurableReceiptAcrossProcessRestart(t *testing.T) {
	...
	cmd.Env = append(os.Environ(), "TEMPORA_AUDIT_REQUEST="+path, "TEMPORA_AUDIT_HOME="+config.TemporaHomeDir())
...
func TestAuditDurableReceiptChild(t *testing.T) {
	path := os.Getenv("TEMPORA_AUDIT_REQUEST")
	if path == "" {
		t.Skip("child process only")
	}
	t.Setenv("TEMPORA_HOME", os.Getenv("TEMPORA_AUDIT_HOME"))
