...
func recoverExportPublications(parent string) error {
	...
	for _, entry := range entries {
		if !entry.IsDir() || !strings.HasPrefix(entry.Name(), ".tempora-export-batch-") {
			continue
		}
...
func publishJournaledImages(ctx context.Context, source string, targets []string) (err error) {
	...
	release, err := filelock.Acquire(ctx, filepath.Join(parent, ".tempora-export-publication.lock"))
	if err != nil {
		return err
	}
	defer release()
	...
	dir, err := os.MkdirTemp(parent, ".tempora-export-batch-")
	if err != nil {
		return err
	}
