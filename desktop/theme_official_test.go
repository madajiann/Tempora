...
func TestOfficialThemesAllValid(t *testing.T) {
	...
	for _, ot := range themes {
		...
		if m.Author != "Tempora Contributors" || m.License != "MIT" {
...
func TestOfficialReservedIDsRefused(t *testing.T) {
	...
	for _, id := range []string{"graphite", "aurora", officialID} {
		...
		if _, err := app.ExportThemePack(id, filepath.Join(t.TempDir(), "out.tempora-theme")); err == nil {
...
func importThemePackZIPBytesForID(id string) (*ThemePackManifest, string, error) {
	...
	tmp := filepath.Join(os.TempDir(), "hijack-"+id+".tempora-theme")
...
// v1.20+: TEMPORA_SAFE_MODE no longer restricts official theme packs.
