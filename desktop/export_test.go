...
func TestSaveExportImageFilesMatchesNormalExportPermissions(t *testing.T) {
	...
	if matches, err := filepath.Glob(filepath.Join(dir, ".tempora-export-*")); err != nil || len(matches) != 0 {
...
func TestSaveExportImageFilesDecodesAllPartsBeforeWriting(t *testing.T) {
	...
	if matches, globErr := filepath.Glob(filepath.Join(dir, ".tempora-export-*")); globErr != nil || len(matches) != 0 {
