...
func writeMacBundleFixture(t *testing.T, root, identifier string) string {
	t.Helper()
	contents := filepath.Join(root, "Tempora.app", "Contents")
...
func TestMacAppBundleForExecutableResolvesCompatibilitySymlink(t *testing.T) {
	...
	target := filepath.Join(contents, "Resources", "service", "tempora-desktop")
	if err := os.WriteFile(target, []byte("service"), 0o755); err != nil {
		t.Fatal(err)
	}
	link := filepath.Join(contents, "MacOS", "tempora-desktop")
	if err := os.MkdirAll(filepath.Dir(link), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink(filepath.Join("..", "Resources", "service", "tempora-desktop"), link); err != nil {
		t.Fatal(err)
	}
...
func TestMacAppBundleForExecutableRejectsInvalidInputs(t *testing.T) {
	outside := filepath.Join(t.TempDir(), "tempora-desktop")
	...
	malformedExe := filepath.Join(malformedContents, "Resources", "service", "tempora-desktop")
