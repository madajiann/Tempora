...
export function makeMockAttachmentBindings(): AttachmentBindings & Partial<AppBindings> & ThisType<AppBindings> {
  return {
    ...
    async SavePastedImage(dataURL: string) {
      const path = `.tempora/attachments/mock-${mockImages.size + 1}.png`;
    ...
    async SaveClipboardImage() {
      const path = `.tempora/attachments/mock-clipboard-${mockImages.size + 1}.png`;
    ...
    async AttachDropped(path: string) {
      ...
      if (!/\.\w{1,6}$/i.test(name)) {
        ...
        return { kind: "workspace", path: `__tempora_external_folder/mock/${tokenName}`, isDir: true, displayPath: path };
      }
