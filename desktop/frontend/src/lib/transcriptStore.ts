...
export class TranscriptStore {
  ...
  private reclaimPage(session: SessionTranscript, end: "oldest" | "newest"): string[] | undefined {
    ...
    if (end === "oldest") {
      ...
      session.nextCursor = session.pages[0]?.olderCursor || (anchor ? `tempora:message:${encodeURIComponent(anchor.message.messageId!)}:${session.latestSequence ?? session.revision}:${encodeURIComponent(session.digest)}:older` : page.olderCursor);
      ...
      session.newerCursor = session.pages[session.pages.length - 1]?.newerCursor || (anchor ? `tempora:message:${encodeURIComponent(anchor.message.messageId!)}:${session.latestSequence ?? session.revision}:${encodeURIComponent(session.digest)}:newer` : page.newerCursor);
    ...
    try {
      ...
      if (!session.hasNewer && (session.latestSequence ?? 0) > slice.revision && pageEntries.length) {
        ...
        if (last.message.messageId) {
          session.newerCursor = `tempora:message:${encodeURIComponent(last.message.messageId)}:${session.latestSequence}:${encodeURIComponent(session.digest)}:newer`;
          session.hasNewer = true;
        }
      }
