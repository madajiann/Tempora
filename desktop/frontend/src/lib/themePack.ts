...
const PACK_STYLE_ID = "tempora-theme-pack-overlay";
const TOKEN_KEYS = [
  "bg",
  "bgSoft",
  "bgElev",
...
// that the desktop service serves through /__tempora_theme_asset/. Only
// exact, internally
...
export function isSafeBackgroundURL(url: string): boolean {
  const u = url.trim();
  if (!u) return false;
  if (u.startsWith("/__tempora_theme_asset/")) return true;
