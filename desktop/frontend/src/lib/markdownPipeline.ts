...
import { temporaRemarkPlugins } from "../components/markdownRemarkPlugins";
import { temporaRehypePlugins } from "../components/rehypeTemporaKatex";
import {
...
function parseNormalizedMarkdownToHast(normalized: string): HastRoot {
  const processor = unified()
    .use(remarkParse)
    .use(temporaRemarkPlugins)
    .use(remarkRehype, { allowDangerousHtml: true })
    .use(temporaRehypePlugins);
...
const VIRTUAL_TABLE_TAG = "tempora-virtual-table";
        ...
        if (value.startsWith(markerPrefix)) {
          ...
          if (Number.isInteger(tableIndex) && tableIndex >= 0 && tableIndex < tableCount) {
            node.children[index] = {
              ...
              properties: { dataTemporaTableIndex: tableIndex },
              children: [],
            };
            continue;
          }
...
export function parseMarkdownToBlocks(text: string): MarkdownBlock[] {
  ...
  return fingerprintBlocks(sliceHastBlocks(tree).map((block) => {
    ...
    const index = Number(placeholder.properties.dataTemporaTableIndex);
    return {
      ...block,
