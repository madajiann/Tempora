  ...
  return installDesktopHostStub({
    ...
    ModelsForTab: async () => [],
		...
		StageImageForTarget: async (token: string, _operationID: string, displayName: string, mime: string, dataURL: string) => ({
			draftId: "",
			path: legacySave ? await legacySave(token, dataURL) : ".tempora/attachments/mock.png",
			displayName,
			mime,
			width: 1,
			height: 1,
...

{
  ...
  installBridgeApp({
    SavePastedImageForTarget: async () => {
      saveCalls += 1;
      return ".tempora/attachments/mock.png";
    },
...

{
  ...
  installBridgeApp({
    SavePastedImageForTarget: async () => ".tempora/attachments/mock.png",
    AttachmentDataURLForTarget: async () => "data:image/png;base64,iVBORw0KGgo=",
  });
  ...
  ok(sent[0]?.submit?.includes("@.tempora/attachments/mock.png") === true, "submitted text retains the local image attachment ref");
...

{
  ...
  const { root, paint } = renderUserMessage("check @[photo.png](.tempora/attachments/mock.png)");
  await paint();
