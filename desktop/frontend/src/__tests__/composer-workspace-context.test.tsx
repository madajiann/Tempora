...
installBridgeApp({
  GetProjectTreeSnapshot: async () => {
    ...
    return {
      ...
      projects: [
        { key: "project:/repo", kind: "project", label: "Tempora", root: "/repo" },
...
assert(document.querySelector('button[aria-label="Project: Tempora"]'));
assert(document.querySelector('button[aria-label="Current Git branch: feature/current"]'));

await click(document.querySelector('button[aria-label="Project: Tempora"]'));
...
assert(projectItems.some((item) => item.textContent?.includes("Tempora")));
assert(projectItems.some((item) => item.textContent?.includes("Other")));
await click(projectItems.find((item) => item.textContent?.includes("Tempora")) ?? null);
