...

{
  ...
  const { root } = await renderApproval({
    approval: {
      ...
      reason: "This bash command is not in Tempora's built-in read-only set. Confirm only if this exact prefix is read-only for planning and research. Auto/YOLO approval cannot answer this trust prompt.",
    },
  });
  ...
  ok(text.includes("不在 Tempora 内置只读集合中"), "plan-mode read-only command approval localizes reason in Chinese UI");
