...
try {
  ...
  const attachmentTask = releaseAttachment.promise.then(() => {
    owner.updateContentFor(draftB.draft.id, draftB.generation, {
      ...edited,
      text: "run B",
      attachments: [{ path: ".tempora/attachments/late.txt", displayName: "late.txt" }],
    });
  });
  ...
  assert.ok(window.__temporaFlushSessionDraft, "draft owner installs the Electron exit flush hook");
  act(() => { exitFlush = window.__temporaFlushSessionDraft!().then(() => { exitFlushFinished = true; }); });
  ...
  assert.equal(JSON.parse(drafts.get("draft-b")!.contentJson).attachments[0]?.path, ".tempora/attachments/late.txt",
    "attachment completion during exit is saved before shutdown continues");

  assert.ok(window.__temporaResumeSessionDraftEditing, "draft owner installs the exit-veto recovery hook");
  act(() => { window.__temporaResumeSessionDraftEditing!(); });
  ...
  assert.equal(window.__temporaFlushSessionDraft, undefined, "unmount removes the renderer exit hook");
  assert.equal(window.__temporaResumeSessionDraftEditing, undefined, "unmount removes the exit-veto recovery hook");
