...
try {
  ...
  const attachmentTask = attachmentFinished.promise.then(() => owner.updateContentFor(sourceDraftId, sourceGeneration, {
    ...
    attachments: [{ path: ".tempora/attachments/background.txt", name: "background.txt", mime: "text/plain" }],
  }));
  ...
  assert.equal(JSON.parse(drafts.get("draft-a")!.contentJson).attachments[0]?.path, ".tempora/attachments/background.txt",
  ...
  await act(async () => {
    window.dispatchEvent(new dom.window.Event("tempora:model-catalog-changed"));
  ...
  act(() => window.dispatchEvent(new dom.window.Event("tempora:model-catalog-changed")));
