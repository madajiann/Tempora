...
declare global {
  interface Window {
    __temporaFlushSessionDraft?: () => Promise<void>;
    __temporaResumeSessionDraftEditing?: () => void;
  }
}
...
export function useSessionDraftSurface(options: DraftSurfaceOptions) {
  ...
  const submitFrom = useCallback(async (
    ...
    const pending = (async () => {
      ...
      try { operation = await app.BeginDraftSubmission(request); }
        ...
        if (String(error).includes("draft submission not admitted:") || String(error).includes("tempora_error:")) throw error;
        ...
        for (;;) {
          ...
          if (state.operation && (!state.operation.requestId || state.operation.requestId === capture.preparationId)) {
            ...
            try { operation = await app.BeginDraftSubmission(request); }
              ...
              if (String(retryError).includes("draft submission not admitted:") || String(retryError).includes("tempora_error:")) throw retryError;
              source.error = "Verifying whether the submission was received. Reconnecting…";
              publish(draftId);
              await new Promise(resolve => window.setTimeout(resolve, 2000));
              continue;
  ...
  useEffect(() => {
    ...
    window.__temporaFlushSessionDraft = flushAll;
    window.__temporaResumeSessionDraftEditing = resumeEditing;
    return () => {
      if (window.__temporaFlushSessionDraft === flushAll) delete window.__temporaFlushSessionDraft;
      if (window.__temporaResumeSessionDraftEditing === resumeEditing) delete window.__temporaResumeSessionDraftEditing;
    };
  }, [flushDraft]);
