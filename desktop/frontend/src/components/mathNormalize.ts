...
const DM = "__TEMPORA_MATH_DISPLAY__";
const IM = "__TEMPORA_MATH_INLINE__";
const LB = "__TEMPORA_LATEX_LINEBREAK__";
const ED_BASE = "TEMPORAESCAPEDDOLLAR";
const INLINE_SOURCE_PREFIX = "\\temporaInternalSourceV1{";
const INLINE_RENDER_SOURCE_PREFIX = "\\temporaInternalRenderV1";
const INLINE_RENDER_SOURCE_RE = /\\temporaInternalRenderV1\{([^}]*)\}\{([^}]*)\}/g;
...
function unusedPlaceholderPrefix(s: string): string {
  let prefix = "__TEMPORA_PROTECTED_CODE__";
  let n = 0;
  while (s.includes(prefix)) {
    n += 1;
    prefix = `__TEMPORA_PROTECTED_CODE_${n}__`;
  }
  return prefix;
}
