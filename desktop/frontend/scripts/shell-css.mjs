// --tempora-draggable marks OS drag regions. Electron rewrites chrome
...
const DRAG_VALUE = /--tempora-draggable(\s*:\s*)(drag|no-drag)/g;
...
export function shellFromEnv(env = process.env) {
  const shell = (env.TEMPORA_SHELL ?? "").trim().toLowerCase();
  if (shell === "") return "browser";
  if (shell === "electron") return "electron";
  throw new Error(`TEMPORA_SHELL must be "electron" or unset, got ${JSON.stringify(shell)}`);
}
