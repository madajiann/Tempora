...
const evidence = process.env.TEMPORA_HANDOFF_EVIDENCE ?? path.join(tmpdir(), "tempora-handoff-evidence");
await mkdir(evidence, { recursive: true });
const server = await createServer({ root, server: { host: "127.0.0.1", port: 0 } });
await server.listen();
const url = `http://127.0.0.1:${server.httpServer.address().port}/bench/submission-handoff.html`;
console.log(url);
const host = await mkdtemp(path.join(tmpdir(), "tempora-handoff-host-"));
let browser, app;
...
try {
  ...
  if (electron) {
    ...
    app = await _electron.launch({ executablePath: createRequire(path.join(root, "../electron/package.json"))("electron"), args: [path.join(host, "main.cjs")], env: { ...process.env, TEMPORA_LAYOUT_URL: url } });
  ...
  await page.addInitScript(() => { window.handoffWrites = []; window.__TEMPORA_TRANSCRIPT_SCROLL_WRITE__ = write => window.handoffWrites.push(write); });
  await page.goto(url);
