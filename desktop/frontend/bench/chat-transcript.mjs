...
const outDir = await mkdtemp(path.join(tmpdir(), "tempora-chat-build-"));
const evidence = process.env.TEMPORA_CHAT_EVIDENCE ?? process.env.TEMPORA_LAYOUT_ARTIFACTS ?? path.join(tmpdir(), "tempora-chat-evidence");
...
try {
  for (const [name, engine] of Object.entries(process.env.CHAT_BROWSER === "electron" ? { electron: _electron } : process.env.CHAT_BROWSER === "webkit" ? { webkit } : process.env.CHAT_BROWSER === "chromium" ? { chromium } : { chromium, webkit })) {
    ...
    if (name === "electron") {
      ...
      electronApp = await engine.launch({ executablePath: createRequire(path.join(root, "../electron/package.json"))("electron"), args: [main], env: { ...process.env, TEMPORA_LAYOUT_URL: url } });
    }
    const nativeThumb = process.env.TEMPORA_TRANSCRIPT_NATIVE_THUMB === "1";
