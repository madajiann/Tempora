...
const electronHome = electronEngine ? await mkdtemp(path.join(tmpdir(), "tempora-popover-electron-")) : "";
...
try {
  ...
  await page.addInitScript(() => localStorage.setItem("tempora-lang", "en"));
  await page.goto(url);
  await page.locator("#composer-input").waitFor({ state: "visible" });
  await page.evaluate(() => {
    window.__temporaPopoverCornerSamples = [];
    const sample = () => {
      ...
      if (menu instanceof HTMLElement && getComputedStyle(menu).visibility === "visible") {
        ...
        if (rect.left <= 8.5 && rect.top <= 8.5) window.__temporaPopoverCornerSamples.push({ left: rect.left, top: rect.top });
      }
      window.__temporaPopoverFrame = requestAnimationFrame(sample);
    };
    sample();
  });
  ...
  assert.deepEqual(await page.evaluate(() => window.__temporaPopoverCornerSamples), [], "model menu never flashes in the top-left corner");
  ...
  await page.evaluate(() => cancelAnimationFrame(window.__temporaPopoverFrame));
