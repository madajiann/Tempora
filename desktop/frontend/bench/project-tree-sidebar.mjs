...
const port = Number(process.env.TEMPORA_SIDEBAR_BROWSER_PORT ?? 4679);
...
try {
  ...
  await page.getByRole('button', { name: 'Show more in tempora', exact: true }).click();
  await page.waitForFunction(() => document.querySelectorAll('.project-tree__topic-main').length === 9);
  assert.equal(await page.getByRole('button', { name: 'Show more in tempora', exact: true }).count(), 0, 'the disclosure disappears at the final page');
  ...
  assert.equal(await page.getByRole('button', { name: 'Show more in tempora', exact: true }).count(), 1, 'reopening a project restores the compact window over loaded rows');
  await page.getByRole('button', { name: 'Show more in tempora', exact: true }).click();
  await page.waitForFunction(() => document.querySelectorAll('.project-tree__topic-main').length === 9);
  ...
  await page.waitForFunction(() => document.querySelectorAll('.project-tree__topic-main').length === 5);
  assert.equal(await page.getByRole('button', { name: 'Show more in tempora', exact: true }).count(), 1, 'restoring after collapse all resets the compact window over loaded rows');
  await page.getByRole('button', { name: 'Show more in tempora', exact: true }).click();
  await page.waitForFunction(() => document.querySelectorAll('.project-tree__topic-main').length === 9);
