...
try {
  ...
  for (let index = 0; index < 20; index += 1) {
    ...
    const previousIntent = await page.evaluate(() => window.__temporaPerf?.stats().navigation?.intent ?? -1);
    ...
    await page.waitForFunction((intent) => {
      const navigation = window.__temporaPerf?.stats().navigation;
      return navigation?.intent !== intent && navigation?.clickToFirstPaintMs !== undefined;
    }, previousIntent);
    switchSamples.push(await page.evaluate(() => window.__temporaPerf.stats().navigation.clickToFirstPaintMs));
  }
