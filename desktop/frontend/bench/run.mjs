...
//
// Usage:
//   pnpm build            # once (the bench reuses dist/; TEMPORA_BENCH_BUILD=1 forces a rebuild)
//   pnpm test:bench
//
// Gates (env-overridable, defaults = plan values):
//   TEMPORA_BENCH_FIRST_PAINT_P95_MS   (100)
//   TEMPORA_BENCH_INTERACTIVE_P95_MS   (300)
//   TEMPORA_BENCH_INP_P95_MS           (200)
//   TEMPORA_BENCH_LONGTASK_P95_MS      (50)
//   TEMPORA_BENCH_LONGTASK_MAX_MS      (500)
//   TEMPORA_BENCH_MARKDOWN_PARSE_MAX_MS (3000)
//   TEMPORA_BENCH_IDLE_CPU_PERCENT     (3)
//   TEMPORA_BENCH_HEAP_GROWTH_MIB      (20)
//   TEMPORA_BENCH_SWITCHES             (100)
//   TEMPORA_BENCH_COLD_RUNS            (5)
//   TEMPORA_BENCH_WARMUP               (6)
//   TEMPORA_BENCH_PORT                 (4617)
//
...
const GATES = {
  firstPaintP95Ms: numEnv("TEMPORA_BENCH_FIRST_PAINT_P95_MS", 100),
  interactiveP95Ms: numEnv("TEMPORA_BENCH_INTERACTIVE_P95_MS", 300),
  inpP95Ms: numEnv("TEMPORA_BENCH_INP_P95_MS", 200),
  longTaskP95Ms: numEnv("TEMPORA_BENCH_LONGTASK_P95_MS", 50),
  longTaskMaxMs: numEnv("TEMPORA_BENCH_LONGTASK_MAX_MS", 500),
  markdownParseMaxMs: numEnv("TEMPORA_BENCH_MARKDOWN_PARSE_MAX_MS", 3000),
  idleCpuPercent: numEnv("TEMPORA_BENCH_IDLE_CPU_PERCENT", 3),
  heapGrowthMiB: numEnv("TEMPORA_BENCH_HEAP_GROWTH_MIB", 20),
};
const SWITCHES = Math.round(numEnv("TEMPORA_BENCH_SWITCHES", 100));
const COLD_RUNS = Math.round(numEnv("TEMPORA_BENCH_COLD_RUNS", 5));
const WARMUP_SWITCHES = Math.round(numEnv("TEMPORA_BENCH_WARMUP", 6));
const PORT = Math.round(numEnv("TEMPORA_BENCH_PORT", 4617));

// Cache budgets mirrored from transcriptStore.ts defaults (asserted via the
// __temporaPerf debug hook, which reports the store's own numbers).
...
async function ensureBuild() {
  ...
  const force = process.env.TEMPORA_BENCH_BUILD === "1";
...
async function settleMarkdownWorker(page, timeoutMs = 10_000) {
  await page
    .waitForFunction(() => (window.__temporaPerf?.stats()?.markdownWorker?.pending ?? 0) === 0, undefined, {
      timeout: timeoutMs,
      polling: 100,
    })
    .catch(() => {});
...
async function main() {
  ...
  try {
    ...
    await page.goto(PAGE_URL, { waitUntil: "domcontentloaded" });
    await page.waitForFunction(INTERACTIVE_FN, undefined, { timeout: 30_000, polling: "raf" });
    await page.waitForFunction(() => Boolean(window.__temporaPerf), { timeout: 10_000 });
    ...
    const baselineStats = await page.evaluate(() => window.__temporaPerf.stats());
    await page.evaluate(() => {
      window.__benchMetrics.longTasks.length = 0;
      window.__benchMetrics.events.length = 0;
      window.__temporaPerf.reset();
    });
    ...
    const { finalStats, activations, benchMetrics } = await page.evaluate(() => ({
      finalStats: window.__temporaPerf.stats(),
      activations: window.__temporaPerf.activations(),
      benchMetrics: {
