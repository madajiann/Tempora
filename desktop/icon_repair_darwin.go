...
func repairMacDesktopAliases(desktopDir, currentApp string) error {
	...
	for _, entry := range entries {
		...
		allowBroken := temporaExactAliasName(entry.Name())
...
func temporaExactAliasName(name string) bool {
	name = strings.TrimSpace(name)
	return strings.EqualFold(name, "Tempora") || strings.EqualFold(name, "Tempora.app")
}
...
func repairMacAlias(aliasPath, currentApp string, allowBroken bool) (bool, error) {
	...
	result := C.tempora_repair_alias(aliasCString, appCString, allow, &nativeErr)
	if nativeErr != nil {
...
func writeMacAlias(targetPath, aliasPath string) error {
	...
	defer C.free(unsafe.Pointer(aliasCString))
	var nativeErr *C.char
	result := C.tempora_write_alias(targetCString, aliasCString, &nativeErr)
	if nativeErr != nil {
		defer C.free(unsafe.Pointer(nativeErr))
	}
	if result != 0 {
...
func resolveMacAlias(aliasPath string) (string, error) {
	...
	result := C.tempora_resolve_alias(aliasCString, &target, &nativeErr)
	if target != nil {
...
func macURLReferencesTemporaBundle(rawURL string) (bool, error) {
	urlCString := C.CString(rawURL)
	defer C.free(unsafe.Pointer(urlCString))
	var nativeErr *C.char
	result := C.tempora_is_tempora_bundle_url_string(urlCString, &nativeErr)
	if nativeErr != nil {
		defer C.free(unsafe.Pointer(nativeErr))
	}
	if result < 0 {
