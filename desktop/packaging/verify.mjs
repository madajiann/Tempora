...
if (kind === "windows-portable-zip" && missing.length === 0 && forbidden.length === 0) {
  ...
  const gui = readMember("Tempora.exe");
  if (gui.equals(readMember("tempora-cli.exe"))) layoutErrors.push("GUI entry contains CLI bytes");
  if (portableLayout === "legacy-dual" && !gui.equals(readMember("tempora-launcher.exe"))) {
    layoutErrors.push("legacy GUI entry differs from Tempora.exe");
  }
}
