...
with tempfile.TemporaryDirectory(prefix="tempora-registry-previous-") as temporary:
    ...
    manifest = (root / "desktop/go.mod").read_text().replace("replace tempora => ../", f"replace tempora => {root}")
...
''')
    ...
    (Path(tempfile.gettempdir()) / "tempora-independent-registry-compatibility.json").write_text(json.dumps(evidence, indent=2))
