...
function category(name) {
  ...
  if (/(^|\/)(tempora-desktop)(\.exe)?$/.test(path)) return "service";
  if (/electron framework|\/macos\/tempora$|(^|\/)tempora\.exe$/.test(path)) return "electron";
  if (/(^|\/)(tempora|tempora-cli)(\.exe)?$/.test(path)) return "cli";
...
function bundledBuildInfo(root) {
  ...
  for (const path of candidates.sort()) {
    try {
      ...
      // A non-Tempora build.json remains visible in the file report.
    }
  }
  return null;
}
...
function markdown(report) {
  ...
  const lines = [
    `# Tempora package size report: ${report.platform}`,
    "",
...
export function generateReport(options) {
  ...
  const artifacts = distEntries
    .filter((entry) => entry.isFile() && !entry.name.endsWith(".minisig") && entry.name.startsWith(`Tempora-${platformName}`))
    .map((entry) => {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  generateReport({
    ...
    sourceSHA: process.env.TEMPORA_COMMIT,
    buildSeconds: process.env.TEMPORA_BUILD_SECONDS,
    installSeconds: process.env.TEMPORA_INSTALL_SECONDS,
    temporaryPeakBytes: process.env.TEMPORA_TEMPORARY_PEAK_BYTES,
    baseline: args.baseline || process.env.TEMPORA_SIZE_BASELINE_REPORT,
    baselineLabel: args["baseline-label"] || process.env.TEMPORA_SIZE_BASELINE_LABEL,
  });
}
