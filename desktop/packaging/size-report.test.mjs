...
test("size report classifies files, duplicates and links without following links", () => {
  const root = mkdtempSync(join(tmpdir(), "tempora-size-report-"));
  mkdirSync(join(root, "bundle", "service"), { recursive: true });
  writeFileSync(join(root, "bundle", "service", "tempora-desktop"), "service");
  writeFileSync(join(root, "bundle", "tempora"), "same");
  writeFileSync(join(root, "bundle", "copy"), "same");
  symlinkSync("service/tempora-desktop", join(root, "bundle", "service-link"));
  mkdirSync(join(root, "dist"));
  writeFileSync(join(root, "dist", "Tempora-linux-amd64.tar.gz"), "archive");

  const tree = inspectTree(join(root, "bundle"));
  assert.equal(tree.fileCount, 3);
  assert.equal(tree.symlinks[0].target, "service/tempora-desktop");
  ...
  assert.equal(report.artifacts[0].name, "Tempora-linux-amd64.tar.gz");
