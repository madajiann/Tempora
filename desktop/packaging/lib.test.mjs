...
test("NSIS project defines replace the Wails-generated INFO_* values", () => {
  ...
  assert.match(defines, /!define TEMPORA_DISPLAY_VERSION "1\.38\.9-2"\r\n/);
  assert.match(defines, /!define INFO_COPYRIGHT "Copyright © 2026 Tempora Contributors"\r\n/);
  assert.match(defines, /!define TEMPORA_VERSION_TAG "v1\.38\.9-2"\r\n/);
});
...
test("required members cover every artifact and the checks report gaps", () => {
  ...
  const portable = [
    "Tempora.exe", "tempora-cli.exe", "current.json",
  ...
  assert.deepEqual(checkMembers([...portable, "tempora-launcher.exe"], "windows-portable-zip").forbidden, ["tempora-launcher.exe"]);
  assert.deepEqual(checkMembers([...portable, "tempora-launcher.exe"], "windows-portable-zip", "legacy-dual"), { missing: [], forbidden: [] });
  assert.deepEqual(checkMembers(portable, "windows-portable-zip", "legacy-dual").missing, ["tempora-launcher.exe"]);
...
test("Windows ZIP verification checks launcher bytes and explicitly selects historical layout", t => {
  const dir = mkdtempSync(join(tmpdir(), "tempora-entry-zip-"));
  t.after(() => rmSync(dir, { recursive: true, force: true }));
  const archive = join(dir, "Tempora-windows-amd64.zip");
  const version = "versions/v1.38.9";
  const entries = [
    ["Tempora.exe", "gui"], ["tempora-cli.exe", "cli"], ["current.json", "{}"],
    ...["tempora-desktop.exe", "tempora-update-helper.exe", "tempora-cli.exe", "app/Tempora.exe", "app/resources/bin/tempora-cli-launcher.exe", "app/resources/app.asar", "app/resources/app/index.html", "app/resources/build.json"].map(name => [`${version}/${name}`, name]),
  ];
  ...
  for (const compressed of [false, true]) {
    ...
    assert.equal(readZipMember(archive, "Tempora.exe").toString(), "gui");
    ...
    writeFileSync(archive, storedZip([...entries, ["tempora-launcher.exe", "gui"]], compressed));
    assert.notEqual(verify("canonical").status, 0);
    assert.equal(verify("legacy-dual").status, 0);
    writeFileSync(archive, storedZip([...entries, ["tempora-launcher.exe", "different"]], compressed));
    assert.notEqual(verify("legacy-dual").status, 0);
  }
  writeFileSync(archive, storedZip([...entries, ["Tempora.exe", "duplicate"]]));
  assert.throws(() => readZipMember(archive, "Tempora.exe"), /duplicate/);
});
...
test("candidate layout declaration agrees with the portable verifier default", () => {
  ...
  assert.ok(!requiredMembers("windows-portable-zip").includes("tempora-launcher.exe"));
});
...
test("the NSIS script installs the Electron tree with both payload modes and no WebView2", () => {
  ...
  for (const releaseIdentity of [
    /\$INSTDIR\\versions\\\$\{TEMPORA_VERSION_TAG\}/,
    /\.installer-\$\{TEMPORA_VERSION_TAG\}-\$R8/,
    /--version "\$\{TEMPORA_VERSION_TAG\}"/,
