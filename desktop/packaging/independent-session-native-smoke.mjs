...
// Build first: cd desktop && go build -o build/bin/tempora-desktop-service .
...
const service = resolve(process.argv[2] || join(desktop, "build/bin/tempora-desktop-service"));
const evidence = resolve(process.argv[3] || join(tmpdir(), "tempora-independent-native"));
const home = mkdtempSync(join(tmpdir(), "tempora-independent-native-home-"));
...
async function launch() {
  application = await _electron.launch({ args: [join(desktop, "electron")], env: {
    ...packagedSmokeEnv(process.env, home), TEMPORA_DEV: "1", TEMPORA_DESKTOP_SERVICE: service,
    TEMPORA_ELECTRON_DEV_URL: `http://127.0.0.1:${vite.httpServer.address().port}`,
    INDEPENDENT_FIXTURE_KEY: "loopback-only",
  }, timeout: 60_000 });
  page = await application.firstWindow();
  page.setDefaultTimeout(30000);
  page.on("pageerror", error => errors.push(error.message));
  await page.waitForFunction(() => Boolean(window.temporaDesktop));
  await page.evaluate(() => new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error("native service readiness timed out")), 30000);
    const off = window.temporaDesktop.native.onServiceState(state => {
