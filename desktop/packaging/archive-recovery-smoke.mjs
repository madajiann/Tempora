...
const home = mkdtempSync(join(tmpdir(), "tempora-archive-recovery-"));
...
const errors = [];
const launch = async () => {
  application = await _electron.launch({ executablePath: join(process.argv[2], "Contents/MacOS/Tempora"),
  ...
  assert.equal(await page.evaluate(() => window.temporaDesktop.contract.protocolVersion), 10);
};
