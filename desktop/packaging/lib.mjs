...
export function nsisProjectDefines(identity, version) {
  ...
  const lines = [
    ...
    `!define TEMPORA_DISPLAY_VERSION "${versions.display}"`,
    `!define INFO_COPYRIGHT "${identity.copyright}"`,
    `!define TEMPORA_VERSION_TAG "${versions.canonical}"`,
  ];
...
const MEMBERS = {
  ...
  "windows-portable-zip": {
    required: [
      ...
      new RegExp(`^${VERSION_DIR}/app/resources/bin/tempora-cli-launcher\\.exe$`),
...
function memberSpec(kind, portableLayout) {
  ...
  return portableLayout === "legacy-dual"
    ? { required: [...spec.required, "tempora-launcher.exe"], forbidden: spec.forbidden }
    : { required: spec.required, forbidden: [...spec.forbidden, "tempora-launcher.exe"] };
}
...
export function checkMembers(entries, kind, portableLayout = "canonical") {
  ...
  if (kind === "windows-portable-zip") {
    const rootEntries = new Set(["Tempora.exe", "tempora-cli.exe", ...(portableLayout === "legacy-dual" ? ["tempora-launcher.exe"] : [])]);
