...
// Usage: node desktop/packaging/attachment-native-smoke.mjs <Tempora.app> <evidence-dir>
...
const home = mkdtempSync(join(tmpdir(), "tempora-attachment-native-"));
...
const invoke = (method, args = []) => page.evaluate(({ method, args }) => window.temporaDesktop.invoke(method, args), { method, args });
...
try {
  app = await _electron.launch({ executablePath: join(bundle, "Contents/MacOS/Tempora"), env: { ...packagedSmokeEnv(process.env, home), ATTACHMENT_FIXTURE_KEY: "loopback-only" }, timeout: 60_000 });
  ...
  await waitForSmokeCondition(async () => {
    for (const candidate of app.windows()) {
      try {
        if (await candidate.evaluate(() => Boolean(window.temporaDesktop))) {
          const version = await candidate.evaluate(() => window.temporaDesktop.invoke("Version", []));
  ...
  mkdirSync(join(project, ".tempora/attachments"), { recursive: true });
  writeFileSync(join(project, ".tempora/attachments/cli.png"), Buffer.from(png, "base64"));
  const beforeCLI = requests.length;
  const cli = await promisify(execFile)(join(bundle, "Contents/Resources/service/tempora"), ["run", "--dir", project, "--print", "inspect @.tempora/attachments/cli.png"], {
  ...
  const workspaceSources = globSync("**/.tempora/attachments/*", { cwd: home }).filter(path => {
  ...
  if (process.argv[4]) {
    ...
    await app.close();
    app = null;
    const oldHome = mkdtempSync(join(tmpdir(), "tempora-attachment-downgrade-"));
    ...
    app = await _electron.launch({ executablePath: join(previous, "Contents/MacOS/Tempora"), env: { ...packagedSmokeEnv(process.env, oldHome), ATTACHMENT_FIXTURE_KEY: "loopback-only" }, timeout: 60_000 });
    await waitForSmokeCondition(async () => {
      for (const candidate of app.windows()) {
        try {
          if (await candidate.evaluate(() => window.temporaDesktop?.invoke("Version", []))) { page = candidate; return true; }
