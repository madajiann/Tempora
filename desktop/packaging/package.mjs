...
if (process.env.TEMPORA_PACKAGE_REUSE_FRONTEND === "1") {
  const pnpmVersion = (process.env.TEMPORA_FRONTEND_PNPM_VERSION ?? "").trim();
  if (!pnpmVersion) throw new Error("TEMPORA_FRONTEND_PNPM_VERSION is required when reusing a frontend artifact");
  verifyFrontendArtifact({
    root: repo,
    dist: frontendDist,
    manifest: process.env.TEMPORA_FRONTEND_ARTIFACT_MANIFEST || join(desktop, "frontend", ".tempora-frontend-artifact.json"),
    shell: "electron",
    channel,
  ...
  runBuildScript(join(desktop, "frontend"), "build-for-shell.mjs", ["electron"], { TEMPORA_CHANNEL: channel, TEMPORA_COMMIT: commit });
}
