...
func TestWindowsSandboxHelperRouteRecognized(t *testing.T) {
	...
	code, ok := runWindowsSandboxHelperIfRequested([]string{"tempora-desktop", sandbox.WindowsHelperCommand})
	if !ok {
...
func TestWindowsSandboxHelperRouteIgnoresNormalLaunch(t *testing.T) {
	for _, argv := range [][]string{
		{"tempora-desktop"},
		{"tempora-desktop", "--some-flag"},
		{"tempora-desktop", "not-the-helper", sandbox.WindowsHelperCommand},
	} {
