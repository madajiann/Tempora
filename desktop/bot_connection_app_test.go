...
func TestDiagnoseBotConnectionBuildsReportDetailForMissingSecret(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/Users/alice/work/tempora",
...
func TestRememberBotConnectionRemoteStoresStableScope(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/tmp/tempora-project",
	}, nil); err != nil {
	...
	if projectMapping.Scope != "project" || projectMapping.WorkspaceRoot != "/tmp/tempora-project" || projectMapping.RemoteID != "wxid_project" {
