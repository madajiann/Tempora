...
// Plugin themes are .tempora-theme packs contributed by ENABLED installed
...
type pluginTheme struct {
	...
	path       string // absolute path of the .tempora-theme ZIP
...
func discoverPluginThemes() ([]pluginTheme, []string) {
	pkgs, warnings := pluginpkg.LoadInstalled(config.TemporaHomeDir())
	var out []pluginTheme
