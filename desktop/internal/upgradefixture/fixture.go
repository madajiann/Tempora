...
func Run(mode, home, reportPath, phase string) error {
	...
	for key, value := range map[string]string{
		"TEMPORA_HOME": absHome, "TEMPORA_STATE_HOME": absHome, "TEMPORA_CACHE_HOME": filepath.Join(absHome, "cache"),
	} {
...
func createFixture(ctx context.Context, home, reportPath string) error {
	...
	if err := os.WriteFile(filepath.Join(config.TemporaHomeDir(), "desktop-projects.json"), projectsBody, 0o600); err != nil {
		return err
	}
	...
	registry, err := json.Marshal(map[string]any{
		...
		"workspaces": map[string]any{
			"global": map[string]any{
				"id":              "global",
				"root":            filepath.Join(config.TemporaHomeDir(), "global-workspace"),
				"title":           "Global",
				"visible":         true,
				"sessionIds":      []string{},
	...
	if err := os.WriteFile(filepath.Join(config.TemporaHomeDir(), "desktop-tabs.json"), tabsBody, 0o600); err != nil {
		return err
	}
...
func verifyBackups(ctx context.Context, report fixtureReport) error {
	backupRoot := filepath.Join(config.TemporaHomeDir(), "desktop", "upgrade-backups")
