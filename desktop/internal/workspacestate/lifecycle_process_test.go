...
func startLifecycleProcess(t *testing.T, path, action string, expected uint64) *lifecycleProcess {
	...
	cmd.Env = append(os.Environ(),
		"TEMPORA_WORKSPACE_PROCESS_HELPER=1",
		"TEMPORA_WORKSPACE_PROCESS_PATH="+path,
		"TEMPORA_WORKSPACE_PROCESS_ACTION="+action,
		"TEMPORA_WORKSPACE_PROCESS_EXPECTED="+strconv.FormatUint(expected, 10),
	)
...
func TestWorkspaceLifecycleProcessHelper(t *testing.T) {
	if os.Getenv("TEMPORA_WORKSPACE_PROCESS_HELPER") != "1" {
		return
	}
	path := os.Getenv("TEMPORA_WORKSPACE_PROCESS_PATH")
	action := os.Getenv("TEMPORA_WORKSPACE_PROCESS_ACTION")
	expected, err := strconv.ParseUint(os.Getenv("TEMPORA_WORKSPACE_PROCESS_EXPECTED"), 10, 64)
	if err != nil {
		t.Fatal(err)
	}
	store := NewStore(path)
