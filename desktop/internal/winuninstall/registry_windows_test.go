...
func TestDeleteInstallLocationIfOwned(t *testing.T) {
	tests := []struct {
		...
		{name: "same root", location: `D:\Tempora`, wantDelete: true},
		{name: "quoted case variant", location: `"d:\tempora\"`, wantDelete: true},
		{name: "separate legacy install", location: `C:\Legacy\Tempora`, wantDelete: false},
	}

	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			keyPath := fmt.Sprintf(`Software\TemporaTests\winuninstall-%d-%d`, os.Getpid(), i)
			...
			if err := deleteInstallLocationIfOwned(keyPath, `D:\Tempora`); err != nil {
				t.Fatal(err)
			}
...
func TestDeleteInstallLocationIfOwnedIgnoresMissingKey(t *testing.T) {
	keyPath := fmt.Sprintf(`Software\TemporaTests\missing-%d`, os.Getpid())
	_ = registry.DeleteKey(registry.CURRENT_USER, keyPath)
	if err := deleteInstallLocationIfOwned(keyPath, `D:\Tempora`); err != nil {
		t.Fatal(err)
	}
