...
const (
	CurrentKeyName = "TemporaTempora"
	LegacyKeyName  = "Tempora"
)
...
// installed Tempora tree. Portable trees have no owned registration and no
...
func registrationOwnsRoot(reg *Registration, root string) bool {
	if reg == nil || !strings.EqualFold(strings.TrimSpace(reg.DisplayName), "Tempora") {
		return false
	}
