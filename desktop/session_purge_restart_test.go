	...
	for _, phase := range []string{"before-tombstone", "after-tombstone", "after-file-cleanup", "after-content-removed", "after-purge-committed", "before-command-result", "after-command-result"} {
		t.Run(phase, func(t *testing.T) {
			...
			run := func(point string, crash bool) {
				...
				cmd.Env = append(os.Environ(), "TEMPORA_PURGE_APP_ROOT="+root, "TEMPORA_PURGE_APP_REGISTRY="+registry, "TEMPORA_PURGE_APP_REQUEST="+string(body), "TEMPORA_PURGE_APP_POINT="+point)
...
func TestPurgeCommandCrashHelper(t *testing.T) {
	root := os.Getenv("TEMPORA_PURGE_APP_ROOT")
	if root == "" {
		return
	}
	a := NewApp()
	...
	a.desktopSessions.workspaceState = workspacestate.NewStore(os.Getenv("TEMPORA_PURGE_APP_REGISTRY"))
	installNoopRuntimeEvents(a)
	defer a.closeSessionServices()
	var req SessionLifecycleRequest
	if err := json.Unmarshal([]byte(os.Getenv("TEMPORA_PURGE_APP_REQUEST")), &req); err != nil {
		t.Fatal(err)
	}
	point := os.Getenv("TEMPORA_PURGE_APP_POINT")
	if point != "" {
