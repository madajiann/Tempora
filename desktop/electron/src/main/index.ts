  ...
  lifecycle = new QuitSequencer({
    ...
    onShutdownFailed: async (message) => {
      const options = {
        ...
        message: "Tempora could not safely finish saving and closing. / Tempora 未能安全完成保存与收尾。",
        ...
        if (state.phase === "starting" || state.phase === "restarting") {
          ...
          startupTimer = setTimeout(() => {
            ...
            lastFailure = {
              ...
              detail: "Tempora did not become ready within 30 seconds. Open logs or retry. / 30 秒内未完成启动，请打开日志或重试。",
            };
        ...
        invoke: async (method, args) => {
          ...
          if (method === "ReportDesktopWebViewReady") {
            ...
            if (status.rendererVersion === "")
              void mainWindow.browserWindow?.webContents.executeJavaScript('window.temporaDesktop.invoke("Version", [])').catch(() => undefined);
          }
          return result;
        },
