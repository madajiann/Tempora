...
export const SHELL_ACTION_PREFIX = "tempora://app/__shell/";
...
<title>${failure.name === "starting" ? "Tempora is starting" : "Tempora cannot start"}</title>
<style>
...
<main>
  ...
  <div class="actions">
    ${failure.name === "starting" ? "" : failure.name === "build_mismatch" || failure.name === "contract_mismatch" ? '<p>Install the full Tempora package to repair this installation. / 请安装完整包恢复启动，无需清理会话。</p>' : `<a class="primary" href="${SHELL_ACTION_PREFIX}restart">Retry / 重试</a>`}
