...
async function fixture() {
  ...
  const window = new module.exports.MainWindow({ platform: "win32", appURL: "tempora://app/index.html", preloadPath: "unused", zoomStore: { current: { appZoomFactor: 1 } }, log: { warn() {}, error() {} }, onShellAction() {}, onAppDomReady() {} });
...
test("app window flushes the renderer draft through a fixed internal script", async () => {
  ...
  assert.deepEqual(f.scripts, ["Promise.resolve(globalThis.__temporaFlushSessionDraft?.())"]);
  f.loads.shift()!();
  await app;
});
...
test("app window can release the renderer draft exit barrier after a veto", async () => {
  ...
  assert.deepEqual(f.scripts, ["Promise.resolve(globalThis.__temporaResumeSessionDraftEditing?.())"]);
  f.loads.shift()!();
  await app;
});
