...
const DEB_SHELL_DIR = "/usr/lib/tempora";
...
// The service normally hands its own path over in TEMPORA_DESKTOP_SERVICE; a
...
export function resolveServiceBinary({ env, platform, execPath, resourcesPath, isFile = isRegularFile }: ServiceBinaryLookup): ServiceBinaryResolution {
  const configured = (env.TEMPORA_DESKTOP_SERVICE ?? "").trim();
  if (configured !== "") return { binary: configured, probed: [] };
  const path = platform === "win32" ? win32 : posix;
  const name = platform === "win32" ? "tempora-desktop.exe" : "tempora-desktop";
