...
async function mainRenderer(app) {
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const page = app.windows().find((candidate) => candidate.url().startsWith("tempora://app/"));
    if (page) return page;
    await new Promise((resolveWait) => setTimeout(resolveWait, 100));
  }
  throw new Error("Tempora main renderer is unavailable");
}
...
try {
  ...
  await page.waitForURL("tempora://app/index.html", { timeout: 60_000 });
  ...
  let tab;
  try {
    tab = await page.evaluate(() => window.temporaDesktop.browser.open("example.com", { temporary: true }));
  } catch (error) {
