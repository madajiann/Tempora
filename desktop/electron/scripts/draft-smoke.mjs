...
const home = mkdtempSync(join(tmpdir(), 'tempora-draft-native-'));
const workspace = join(home, 'workspace');
mkdirSync(workspace);
const report = { home, checks: [] };
const check = name => { report.checks.push(name); console.log(`PASS ${name}`); };
const launch = () => _electron.launch({ args: [root], env: { ...process.env,
  TEMPORA_HOME: home, TEMPORA_STATE_HOME: home, TEMPORA_CACHE_HOME: join(home,'cache'),
  TEMPORA_DEV: '1', TEMPORA_DESKTOP_SERVICE: resolve(root,'../build/bin/tempora-desktop-service') }, timeout:60000 });
async function ready(app) {
  const page = await app.firstWindow();
  await page.waitForFunction(() => Boolean(window.temporaDesktop) && !document.querySelector('.boot-shell'), null, { timeout:60000 });
  await page.waitForFunction(async () => { try { await window.temporaDesktop.invoke('ListTabs', []); return true; } catch { return false; } }, null, { timeout:60000 });
  return page;
}
const invoke = (page, name, args=[]) => page.evaluate(({name,args}) => window.temporaDesktop.invoke(name,args),{name,args});
...
try {
  ...
  await page.evaluate(() => window.__temporaFlushSessionDraft?.());
  await shell.close();
  shell = await launch();
  ...
  await page.waitForFunction(async id => {
    const state = await window.temporaDesktop.invoke('GetSessionDraftState',[id]);
