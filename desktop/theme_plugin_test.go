...
// contributes.themes globbing themes/*.tempora-theme, plus the
...
func installPluginThemeFixture(t *testing.T, home, pluginName string, enabled bool, themes []pluginThemeFixture) string {
	...
	manifestJSON := fmt.Sprintf(
		`{"apiVersion":%q,"name":%q,"version":"1.0.0","contributes":{"themes":["themes/*.tempora-theme"]}}`,
...
func TestPluginThemeDiscoveryAndList(t *testing.T) {
	...
	pluginRoot := installPluginThemeFixture(t, home, "themery", true, []pluginThemeFixture{
		{fileName: "neon.tempora-theme", manifest: testPluginThemeManifest("neon-dusk", "Neon Dusk"), withImage: true},
	})
	app := NewApp()
	...
	if _, err := os.Stat(filepath.Join(pluginRoot, "themes", "neon.tempora-theme")); err != nil {
		t.Fatal(err)
	}
}
...
func TestPluginThemeInvalidSkippedWithWarning(t *testing.T) {
	...
	installPluginThemeFixture(t, home, "themery", true, []pluginThemeFixture{
		{fileName: "neon.tempora-theme", manifest: testPluginThemeManifest("neon-dusk", "Neon Dusk")},
		{fileName: "broken.tempora-theme", manifest: nil}, // invalid ZIP
	})
	app := NewApp()
	...
	if len(view.Warnings) == 0 || !strings.Contains(view.Warnings[0], "broken.tempora-theme") {
	...
	for _, w := range exp.Warnings {
		if strings.Contains(w, "broken.tempora-theme") {
			found = true
		}
	}
	if !found {
