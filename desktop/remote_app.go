...
func (a *App) requireRemotePresentedFiles(tabID, hostID string) (remotePresentedFilesFence, error) {
	...
	if !fence.tab.capabilities["present-files-v1"] {
		return remotePresentedFilesFence{}, fmt.Errorf("this remote Tempora Serve does not support present-files-v1")
	}
	return fence, nil
}
...
func (a *App) saveRemoteFileAs(hostID, remotePath string) (string, error) {
	...
	tmp, err := os.CreateTemp(filepath.Dir(target), ".tempora-remote-download-*")
	if err != nil {
		return "", err
	}
	tmpPath := tmp.Name()
