...
func (a *App) ResolveRemoteTabPromptExact(target RemotePromptTarget, answer PromptAnswerView) error {
	...
	if !supported {
		return fmt.Errorf("this remote Tempora Serve does not support %s; upgrade it to answer this card safely", serveCapabilityInteractionTargetV1)
	}
	if !current {
...
func (a *App) SubmitRemoteTabExtensionFormExact(target ExtensionFormTarget, values map[string]any) error {
	...
	a.remoteTabMu.Unlock()
	if !supported {
		return fmt.Errorf("this remote Tempora Serve does not support %s; upgrade it to submit this extension form safely", serveCapabilityExtensionFormInstanceV1)
	}
	if !current {
