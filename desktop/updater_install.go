...
const (
	...
	linuxUpdateHelperPath = "/usr/lib/tempora/tempora-update-helper"
	linuxPkexecPath       = "/usr/bin/pkexec"
	linuxDebPackageName   = "tempora-desktop"
)
...
func profileForManifest(base installProfile, m *update.Manifest) installProfile {
	...
	case installModeDeb:
		...
		if !linuxDebHelperReady() {
			...
			base.RequiresElev = false
			base.ArtifactKind = ""
			base.ManualReason = "system update helper is unavailable; install with: sudo apt install ./Tempora-linux-amd64.deb"
			return base
		}
...
func manualDebInstallHint() string {
	return "Install manually with: sudo apt install ./Tempora-linux-amd64.deb"
}
