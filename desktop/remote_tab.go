...
func (a *App) requireRemotePermissionPresets(tabID string) error {
	...
	if !supported {
		return fmt.Errorf("this remote Tempora Serve is read-only because it does not support permission-presets-v1; upgrade the remote service to run tools or change permissions")
	}
	return nil
}
...
func (a *App) requireRemoteExecutionProtocol(tabID string) error {
	...
	if !supported {
		return fmt.Errorf("this remote Tempora Serve is read-only because it does not support %s, %s, %s, and %s; upgrade the remote service to execute or control a session", serveCapabilityExecutionV2, serveCapabilitySessions, serveCapabilitySessionIdentityV1, serveCapabilitySessionOwnershipV1)
	}
	return nil
}
...
func (a *App) requireRemoteGoalLifecycle(tabID string) error {
	...
	if !supported {
		return fmt.Errorf("this remote Tempora Serve does not support %s; upgrade it before creating or controlling goals", serveCapabilityGoalLifecycleV2)
	}
	return nil
}
