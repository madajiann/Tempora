...
static void tempora_set_error(char **output, NSError *error, NSString *fallback) {
    if (output == NULL) {
        return;
    }
...
static void tempora_set_exception(char **output, NSException *exception, NSString *fallback) {
    if (output == NULL) {
        return;
    }
...
static BOOL tempora_is_alias(NSURL *url, NSError **error) {
    NSNumber *value = nil;
...
static int tempora_create_alias(NSURL *targetURL, NSURL *aliasURL, char **error_message) {
    NSError *error = nil;
    NSData *bookmark = [targetURL bookmarkDataWithOptions:NSURLBookmarkCreationSuitableForBookmarkFile
                           includingResourceValuesForKeys:nil
                                            relativeToURL:nil
                                                    error:&error];
    if (bookmark == nil) {
        tempora_set_error(error_message, error, @"create Finder alias bookmark");
        return -1;
    }
    if (![NSURL writeBookmarkData:bookmark toURL:aliasURL options:0 error:&error]) {
        tempora_set_error(error_message, error, @"write Finder alias bookmark");
        return -1;
    }
    return 0;
}
...
static int tempora_is_tempora_bundle_url(NSURL *url, char **error_message) {
    if (url == nil || !url.isFileURL) {
        return 0;
    }
    @try {
        NSBundle *bundle = [NSBundle bundleWithURL:url];
        return [bundle.bundleIdentifier isEqualToString:@"com.wails.tempora-desktop"] ? 1 : 0;
    } @catch (NSException *exception) {
        tempora_set_exception(error_message, exception, @"inspect Finder alias target");
        return -1;
    }
}
...
int tempora_is_tempora_bundle_url_string(const char *url_string,
                                           char **error_message) {
    @autoreleasepool {
        @try {
            if (url_string == NULL) {
            ...
            return tempora_is_tempora_bundle_url([NSURL URLWithString:value], error_message);
        } @catch (NSException *exception) {
            tempora_set_exception(error_message, exception, @"parse Finder alias target");
            return -1;
        }
    }
}

static int tempora_write_alias_impl(const char *target_path, const char *alias_path,
                                     char **error_message) {
    ...
    return tempora_create_alias(targetURL, aliasURL, error_message);
}

int tempora_write_alias(const char *target_path, const char *alias_path,
                         char **error_message) {
    @autoreleasepool {
        @try {
            return tempora_write_alias_impl(target_path, alias_path, error_message);
        } @catch (NSException *exception) {
            tempora_set_exception(error_message, exception, @"write Finder alias");
            return -1;
        }
    }
}

static int tempora_resolve_alias_impl(const char *alias_path, char **target_path,
                                       char **error_message) {
    NSError *error = nil;
    NSURL *aliasURL = [NSURL fileURLWithPath:[NSString stringWithUTF8String:alias_path]];
    ...
    if (resolved == nil || !resolved.isFileURL) {
        tempora_set_error(error_message, error, @"resolve Finder alias to file URL");
        return -1;
    }
    if (target_path != NULL) {
...
int tempora_resolve_alias(const char *alias_path, char **target_path,
                           char **error_message) {
    @autoreleasepool {
        @try {
            return tempora_resolve_alias_impl(alias_path, target_path, error_message);
        } @catch (NSException *exception) {
            tempora_set_exception(error_message, exception, @"resolve Finder alias");
            return -1;
        }
    }
}

static int tempora_repair_alias_impl(const char *alias_path, const char *current_app_path,
                                      int allow_broken, char **error_message) {
    NSError *error = nil;
    NSURL *aliasURL = [NSURL fileURLWithPath:[NSString stringWithUTF8String:alias_path]];
    if (!tempora_is_alias(aliasURL, &error)) {
    ...
    if (resolved != nil) {
        ...
        int ownership = tempora_is_tempora_bundle_url(resolved, error_message);
        if (ownership < 0) {
            return -1;
        }
        owned = ownership == 1;
    ...
    NSString *temporaryName = [NSString stringWithFormat:@".tempora-alias-%@", NSUUID.UUID.UUIDString];
    NSURL *temporaryURL = [directory URLByAppendingPathComponent:temporaryName];
    if (tempora_create_alias(currentURL, temporaryURL, error_message) != 0) {
        return -1;
    }
    ...
    if (![[NSFileManager defaultManager] replaceItemAtURL:aliasURL
        ...
        tempora_set_error(error_message, error, @"replace Finder alias");
        return -1;
    }
    return 1;
}

int tempora_repair_alias(const char *alias_path, const char *current_app_path,
                          int allow_broken, char **error_message) {
    @autoreleasepool {
        @try {
            return tempora_repair_alias_impl(alias_path, current_app_path, allow_broken, error_message);
        } @catch (NSException *exception) {
            tempora_set_exception(error_message, exception, @"repair Finder alias");
            return -1;
        }
    }
}
