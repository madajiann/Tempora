...
import (
	...
	"tempora/desktop/internal/draftstate"
...
func TestDraftExternalFolderWithSpacesUsesStableRuntimeToken(t *testing.T) {
	...
	got := rewriteDraftExternalFolderRef(input, path, "__tempora_external_folder/abc/Folder-With-Spaces")
	want := "inspect @__tempora_external_folder/abc/Folder-With-Spaces/ and keep this visible"
	if got != want {
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(rootA, "tempora.toml"), []byte(`default_model = "local/model-a"`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(rootB, "tempora.toml"), []byte(`default_model = "local/model-b"`), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestMissingDraftAttachmentFailsBeforeSessionReservation(t *testing.T) {
	...
	saved, err := a.SaveSessionDraft(SessionDraftSaveRequest{
		...
		ContentJSON: `{"text":"inspect","attachments":[{"path":".tempora/attachments/missing.txt"}]}`,
		Settings:    draft.Settings,
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := a.BeginDraftSubmission(SessionDraftSubmissionRequest{DraftID: draft.ID, Revision: saved.Draft.Revision, Display: "inspect", Input: "inspect @.tempora/attachments/missing.txt"}); err == nil {
...
func TestMissingDraftImageReturnsStableErrorWithoutHostPath(t *testing.T) {
	...
	saved, err := a.SaveSessionDraft(SessionDraftSaveRequest{
		DraftID: draft.ID, Revision: draft.Revision,
		ContentJSON: `{"text":"inspect","attachments":[{"path":".tempora/attachments/missing.png"}]}`,
		Settings:    draft.Settings,
	})
	if err != nil {
		t.Fatal(err)
	...
	_, err = a.BeginDraftSubmission(SessionDraftSubmissionRequest{
		...
		Display: "inspect", Input: "inspect @.tempora/attachments/missing.png",
	})
	if err == nil || err.Error() != "tempora_error:image_attachment_unreadable" {
