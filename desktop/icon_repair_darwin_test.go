...
func writeTemporaTestBundle(t *testing.T, path string) {
	t.Helper()
...
<plist version="1.0"><dict><key>CFBundleIdentifier</key><string>com.wails.tempora-desktop</string></dict></plist>`
	if err := os.WriteFile(filepath.Join(path, "Contents", "Info.plist"), []byte(plist), 0o644); err != nil {
		t.Fatal(err)
	}
}

func TestRepairMacDesktopAliasesRetargetsBrokenTemporaAlias(t *testing.T) {
	root := t.TempDir()
	desktop := filepath.Join(root, "Desktop")
	if err := os.Mkdir(desktop, 0o755); err != nil {
		t.Fatal(err)
	}
	oldApp := filepath.Join(root, "Tempora.app.tempora-update-backup")
	currentApp := filepath.Join(root, "Tempora.app")
	writeTemporaTestBundle(t, oldApp)
	writeTemporaTestBundle(t, currentApp)
	alias := filepath.Join(desktop, "Tempora")
...
func TestRepairMacDesktopAliasesPreservesUnrelatedBrokenAlias(t *testing.T) {
	...
	currentApp := filepath.Join(root, "Tempora.app")
	writeTemporaTestBundle(t, oldTarget)
	writeTemporaTestBundle(t, currentApp)
...
func TestRepairMacDesktopAliasesPreservesOrdinaryTemporaFile(t *testing.T) {
	root := t.TempDir()
	desktop := filepath.Join(root, "Desktop")
	if err := os.Mkdir(desktop, 0o755); err != nil {
		t.Fatal(err)
	...
	currentApp := filepath.Join(root, "Tempora.app")
	writeTemporaTestBundle(t, currentApp)
	ordinary := filepath.Join(desktop, "Tempora")
	...
	if string(got) != content {
		t.Fatalf("ordinary Tempora file changed to %q", got)
	}
}

func TestRepairMacDesktopAliasesPreservesHealthyTemporaAlias(t *testing.T) {
	root := t.TempDir()
	desktop := filepath.Join(root, "Desktop")
	if err := os.Mkdir(desktop, 0o755); err != nil {
		t.Fatal(err)
...
func TestRepairMacDesktopAliasesRetargetsCustomNamedInstalledAlias(t *testing.T) {
	...
	oldApp := filepath.Join(root, "Tempora-old.app")
	currentApp := filepath.Join(root, "Tempora.app")
	writeTemporaTestBundle(t, oldApp)
	writeTemporaTestBundle(t, currentApp)
	alias := filepath.Join(desktop, "My Tempora")
	if err := writeMacAlias(oldApp, alias); err != nil {
		t.Fatal(err)
	}
...
func TestMacBundleOwnershipRejectsNonFileURL(t *testing.T) {
	owned, err := macURLReferencesTemporaBundle("https://example.com/Tempora.app")
	if err != nil {
		t.Fatal(err)
	}
	if owned {
		t.Fatal("non-file URL was classified as a Tempora application bundle")
	}
}

func TestMacBundleOwnershipRecognizesInstalledTemporaBundle(t *testing.T) {
	app := filepath.Join(t.TempDir(), "Tempora.app")
	writeTemporaTestBundle(t, app)
	owned, err := macURLReferencesTemporaBundle((&url.URL{Scheme: "file", Path: app}).String())
	if err != nil {
		t.Fatal(err)
	}
	if !owned {
		t.Fatal("installed Tempora application bundle was not recognized")
	}
}
