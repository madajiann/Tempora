...
func TestResolveTerminalCommandTrustsOnlyUserConfigPath(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "tempora.toml"), []byte(projectConfig), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if command.path == projectShell {
		t.Fatal("project tempora.toml selected the integrated terminal executable")
	}
...
func TestTerminalEnvironmentOverridesInheritedTerminalCapabilities(t *testing.T) {
	...
	if !strings.Contains(joined, "PATH=/bin") || !strings.Contains(joined, "TEMPORA_TEST=value") {
