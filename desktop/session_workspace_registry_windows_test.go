...
func TestGlobalWorkspaceSurvivesWindowsUpdateEnvironment(t *testing.T) {
	...
	normalHome := config.TemporaHomeDir()
	updateHome := ""
	for _, entry := range instanceidentity.UpdateEnvironment(os.Environ(), normalHome) {
		if value, ok := strings.CutPrefix(entry, "TEMPORA_HOME="); ok {
			updateHome = value
		}
	}
	...
	// v1.38.10 wrote the lowercase comparison key into TEMPORA_HOME. Seed that
	...
	t.Setenv("TEMPORA_HOME", legacyUpdateHome)
	first := NewApp()
