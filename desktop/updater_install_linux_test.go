...
func TestIsDpkgOwnedTemporaRejectsEmpty(t *testing.T) {
	if isDpkgOwnedTempora("") {
		t.Fatal("empty path must not be dpkg-owned")
	}
	if isDpkgOwnedTempora("relative/path") {
		t.Fatal("relative path must not be dpkg-owned")
	}
	// A random absolute path that is not packaged.
	if isDpkgOwnedTempora(filepath.Join(t.TempDir(), "tempora-desktop")) {
