...
func TestWorkspaceGitBranchForMetaDoesNotBlockOnColdProbe(t *testing.T) {
	...
	start := time.Now()
	if got := workspaceGitBranchForMeta("/tmp/tempora-cold-probe"); got != "" {
	...
	releaseProbe()
	eventuallyBranchForMeta(t, "/tmp/tempora-cold-probe", "feature/async")
}

func TestWorkspaceGitBranchForMetaReturnsStaleDuringRefresh(t *testing.T) {
	resetWorkspaceGitBranchMetaCacheForTest(t)
	workspaceGitBranchCache.Lock()
	workspaceGitBranchCache.entries[filepath.Clean("/tmp/tempora-stale-probe")] = workspaceGitBranchCacheEntry{
	...
	start := time.Now()
	if got := workspaceGitBranchForMeta("/tmp/tempora-stale-probe"); got != "feature/stale" {
	...
	releaseProbe()
	eventuallyBranchForMeta(t, "/tmp/tempora-stale-probe", "feature/fresh")
}
