...
func TestLegacyAttachmentRPCRejectsMissingTarget(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(".tempora", "attachments")); !os.IsNotExist(err) {
...
func TestAttachmentDataURLForTabIsolatesSameRelativePath(t *testing.T) {
	projectA := t.TempDir()
	projectB := t.TempDir()
	rel := ".tempora/attachments/shared.png"
...
func TestSavePastedImageForTabRejectsClosedTargetAndRemovesWrite(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(projectRoot, ".tempora", "attachments"))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 0 {
...
func TestAttachDroppedOutsideWorkspaceCopiesToAttachments(t *testing.T) {
	...
	if got.Kind != "attachment" || !strings.HasPrefix(got.Path, ".tempora/attachments/") || !strings.HasSuffix(got.Path, ".pdf") {
...
func TestAttachDroppedOutsideWorkspaceDirRegistersWorkspaceRef(t *testing.T) {
	...
	if !strings.HasPrefix(got.Path, "__tempora_external_folder/") || strings.ContainsAny(got.Path, " \t\r\n") {
