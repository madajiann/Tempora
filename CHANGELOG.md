...
All notable changes to the Go line (Tempora 1.0+) are recorded here. The legacy
...
- **serve Host-header allowlist:** `tempora serve` now rejects requests whose
...
- **Project MCP session reliability:** The MCP client now uses the official Go
  SDK for stdio, legacy SSE, and Streamable HTTP while retaining Tempora's
...
- Added a **Remote SSH** module (VS Code Remote-SSH style): a user-global
  `[remote]` host config, `tempora remote` CLI (add/list/remove/import/test/
  ...
  bootstraps a persistent `tempora serve` on the remote host and tunnels its
...
- Added `tempora serve --port-file/--token-file/--pid-file` so a supervised
  headless serve can bind an ephemeral port and read its auth token from a file
  (keeping it out of `ps`).
- Added an authenticated, loopback-only Provider setup page for `tempora
  serve`. A Serve whose selected Provider is missing its API key now remains
  reachable, stores the submitted key in that host's Tempora credential file,
...
- Remote SSH workspaces now open as a standalone remote web window again.
  ...
  reuses the remote `tempora serve`, tunnels its loopback port, and opens the
  ...
  `tempora remote connect <host> --open`.
...
- Fixed Desktop sessions incorrectly locking themselves during Goal + Delivery
  ...
  `TEMPORA_HOME` instead of the executable path. Switching saved sessions is
...
- Isolated the Windows desktop WebView2 shell from stale system proxies, so an
  ...
  WebView2 still does not reach DOM-ready within 15 seconds, Tempora now shows
  the native window with a recovery prompt instead of appearing not to launch.
  Remote Markdown images are fetched by the backend with Tempora's proxy
...
- Full bilingual release notes:
  <https://tempora.io/changelog/v1.20.0/> ·
...
[bilingual release notes](https://tempora.io/changelog/) and on the
...
- **Go kernel**: a single static binary (CGO-free), cross-compiled for
  ...
  the native binary), Homebrew (`esengine/tempora` tap), and release archives;
...
- **Code intelligence via CodeGraph**: a tree-sitter symbol/call graph
  ...
  or API cost. Fetched into a local cache on first use (or `tempora codegraph
...
- **Memory**: `TEMPORA.md` hierarchy + auto-memory, folded into the cache-stable
  prefix.
- **ACP** (`tempora acp`) and an HTTP/SSE server frontend; desktop app (Wails).
