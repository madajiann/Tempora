  ...
  return {
    ...
    assets: [{
      ...
      browser_download_url:
        `https://github.com/esengine/DeepSeek-Tempora/releases/download/${tag}/latest.json`,
      size: 42,
    }],
    ...overrides,
  };
...
const cliRelease = (tag: string, prerelease: boolean) => ({
  tag_name: tag,
  prerelease,
  html_url: `https://github.com/esengine/DeepSeek-Tempora/releases/tag/${tag}`,
  assets: cliAssets.map((name) => ({
    name,
    browser_download_url: `https://github.com/esengine/DeepSeek-Tempora/releases/download/${tag}/${name}`,
    size: 42,
  })),
});
...
describe("desktop Stable GitHub fallback", () => {
  ...
  it("uses /releases/latest and requires the release tag to match the manifest version", async () => {
    const githubBase =
      "https://github.com/esengine/DeepSeek-Tempora/releases/download/desktop-v1.18.0/";
    ...
    expect(fetchMock.mock.calls.map((call) => call[0])).toEqual([
      ...
      "https://api.github.com/repos/esengine/DeepSeek-Tempora/releases/latest",
  ...
  it("rejects a GitHub manifest whose version disagrees with the latest release tag", async () => {
    const manifestBase =
      "https://github.com/esengine/DeepSeek-Tempora/releases/download/desktop-v1.17.9/";
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(new Response("missing", { status: 404 }))
      .mockResolvedValueOnce(new Response(JSON.stringify(githubDesktopRelease("v1.18.0")), { status: 200 }))
    ...
    for (const asset of [
      {
        name: "latest.json",
        browser_download_url:
          "https://evil.invalid/esengine/DeepSeek-Tempora/releases/download/desktop-v1.18.0/latest.json",
        size: 42,
      },
      {
        name: "latest.json",
        browser_download_url:
          "https://github.com/esengine/DeepSeek-Tempora/releases/download/desktop-v1.18.0/latest.json",
        size: 0,
      },
    ]) {
...
describe("CLI public release gateway", () => {
  ...
  it("rewrites release notes to the canonical repository tag URL", async () => {
    ...
    expect(body.html_url).toBe(
      "https://github.com/esengine/DeepSeek-Tempora/releases/tag/v1.18.0",
    );
  });
  ...
  it("requires every CLI asset URL to be canonical", async () => {
    const invalidURLs = [
      "https://evil.invalid/esengine/DeepSeek-Tempora/releases/download/v1.20.0/tempora-darwin-amd64.tar.gz",
      "https://github.com@evil.invalid/esengine/DeepSeek-Tempora/releases/download/v1.20.0/tempora-darwin-amd64.tar.gz",
      "http://github.com/esengine/DeepSeek-Tempora/releases/download/v1.20.0/tempora-darwin-amd64.tar.gz",
      "https://github.com/esengine/DeepSeek-Tempora/releases/download/v1.19.0/tempora-darwin-amd64.tar.gz",
      "https://github.com/esengine/DeepSeek-Tempora/releases/download/v1.20.0/tempora-darwin-arm64.tar.gz",
    ];
