const R2_BASE = "https://dl.tempora.io";
const GITHUB_RELEASES_API = "https://api.github.com/repos/esengine/DeepSeek-Tempora/releases?per_page=100";
const GITHUB_LATEST_RELEASE_API = "https://api.github.com/repos/esengine/DeepSeek-Tempora/releases/latest";
...
function gatewayHeaders(source: string): Record<string, string> {
  return {
    ...
    "x-tempora-release-source": source,
  };
}
...
function expectedCLIAssetURL(value: unknown, tag: string, name: string): string {
  ...
  const path = `/esengine/DeepSeek-Tempora/releases/download/${tag}/${name}`;
...
function normalizeCLIRelease(
  ...
  const releaseURL = `https://github.com/esengine/DeepSeek-Tempora/releases/tag/${tag}`;
  return {
    order,
    release: {
      tag_name: tag,
...
async function fetchCLIRelease(url: string, channel: PublicReleaseChannel, source: string): Promise<Response | null> {
  try {
    const response = await fetch(url, {
      headers: { accept: "application/json", "user-agent": "tempora-release-gateway" },
    });
...
async function fetchLatestCLIReleaseFromGitHub(channel: PublicReleaseChannel): Promise<Response | null> {
  try {
    const response = await fetch(GITHUB_RELEASES_API, {
      headers: { accept: "application/vnd.github+json", "user-agent": "tempora-release-gateway" },
    });
    if (!response.ok) return null;
...
function desktopAssetBases(
  ...
  return [r2, `https://github.com/esengine/DeepSeek-Tempora/releases/download/desktop-${version}/`];
}
  ...
  try {
    ...
    const res = await fetch(safeURL, {
      headers: {
        ...
        "user-agent": "tempora-release-gateway",
      },
    });
...
export async function handleCLIRelease(channel: PublicReleaseChannel): Promise<Response> {
  ...
  const cacheKey = new Request(`https://crash.tempora.io/v1/cli/releases/${channel}/latest.json`);
