...
Tempora no longer ships a product `tempora-guard` recovery shell. Crash
...
```text
tempora doctor
tempora doctor repair
tempora crash report   # when available in your build
```
...
InstallRoot/
  ...
  Tempora.exe                 # Windows portable / Start Menu alias
  tempora[-cli.exe]
  current.json
  versions/<version>/
    tempora-desktop[.exe]
...
2. Install it directly over the current copy (Windows: double-click; macOS:
   replace `Tempora.app`). Do not uninstall first: keeping the existing install
   root lets the compatibility migrator prove which stale transaction it owns.
3. Start Tempora once and confirm **Settings > Updates** shows the installed
   version before trying another in-app update.
4. Compatibility payloads may still include a one-shot binary named
   `tempora-guard` that only migrates the flat layout into `current.json` and
...
2. If that button is missing or fails, quit Tempora fully and start it once so
...
3. If in-app update still fails, download the latest signed installer from the
   ...
   uninstalling first.
4. On macOS, also allow Tempora under System Settings → Privacy & Security →
   App Management when the dialog appears; a leftover
   `Tempora.app.tempora-update-backup` that TCC will not let the app remove
   may still require the official installer path.

If the Windows installer reports `Tempora layout activation failed`, expand
the installer details and copy the lines under `Tempora layout activator
...
After the replacement window becomes visible, Tempora commits only the exact
