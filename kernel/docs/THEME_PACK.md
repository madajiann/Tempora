# Tempora Theme Pack V2

**Status:** this describes the retired Wails desktop and its `.tempora-theme`
archives. What Tempora Studio reads instead:
...
Native theme packs for the Tempora desktop app. Packs are controlled skins:
...
| **User themes** | Created in the editor, duplicated, or imported as `.tempora-theme` | yes | yes | yes |
| **Plugin themes** | `.tempora-theme` packs contributed by enabled plugins (Manifest v2 `contributes.themes`), read straight from the plugin root — never copied into the user library | no | no (disable/uninstall the plugin) | no |
...
real Tempora builds. **Screenshots of the app must not be imported as theme
...
Distribute as a `.tempora-theme` ZIP. The archive root may contain **only**:
...
| Path under Tempora home | Purpose |
| --- | --- |
...
The UI only receives temporary asset URLs (`/__tempora_theme_asset/...`) or data URLs — never absolute host paths.
...
Zip as `paper-dawn.tempora-theme` with only `theme.json` at the root.
