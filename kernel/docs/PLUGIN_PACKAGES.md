# Tempora Plugin Packages

Tempora plugin packages bundle skills, hooks, MCP servers, prompts, themes,
and code extensions behind one installable unit.

## CLI Mode

Use `tempora plugin` when installing or managing plugin packages from a
terminal. Plugin packages are installed globally under the Tempora home
directory.
...
- A local directory that contains `tempora-plugin.json`,
  `.codex-plugin/plugin.json`, or `.claude-plugin/plugin.json`.

Preview the install plan without writing files:

```bash
tempora plugin install git:github.com/obra/superpowers --dry-run
```

Install a plugin after reviewing the plan:

```bash
tempora plugin install git:github.com/obra/superpowers --yes
```

Install with an explicit name or replace an installed plugin with the same name:

```bash
tempora plugin install git:github.com/obra/superpowers --name superpowers --replace --yes
```

Use a local directory in developer mode:

```bash
tempora plugin install /path/to/plugin --link --replace --yes
```
...
- `--link` links a local plugin directory instead of copying it into Tempora's
  plugin storage. Moving or deleting that directory breaks the linked plugin.

Running `tempora plugin install <source>` without `--dry-run` or `--yes`
...
```text
~/.tempora/plugin-packages.json
~/.tempora/plugins/<name>/
```
...
```bash
tempora plugin show superpowers
```
...
```bash
tempora plugin doctor superpowers
```
...
```bash
tempora plugin disable superpowers
tempora plugin enable superpowers
```

Remove a plugin:

```bash
tempora plugin remove superpowers --yes
```
...
Tempora loads its capabilities into normal interactive sessions:
...
- **MCP servers** join the normal MCP/tool flow. Ask for the task you want done;
  Tempora can call the plugin's tools when they are relevant.

After installing, enabling, disabling, or updating a plugin from a separate
terminal while a session is already running, start a new `tempora` session or
...
(`tempora plugin install ./pkg.zip`) and the `install_source` tool.
...
- **Developer mode: link source folder** appears for **Local folder** installs.
  It links the selected directory instead of copying it into Tempora's plugin
...
Tempora plugins can declare `tempora-plugin.json` at the plugin root:
...
Relative paths are resolved inside the plugin root. Tempora does not run
...
- Existing native hooks that declare neither field keep the historical
  Tempora shell-command behavior. `shellCommand: true` remains supported as
  the legacy spelling of shell form.

## Manifest v2 (Extensions)

Native Tempora extensions use the exact v2 `apiVersion`:
...
```json
{
  ...
  "runtime": {
    "command": "${TEMPORA_PLUGIN_ROOT}/bin/example",
    "args": [],
    "env": {},
    "required": true,
    "priority": 0,
...
- v2 resource discovery is explicit. Tempora loads only the skills, agents,
...
The `runtime` block declares a code extension — a sidecar process Tempora
...
- `command`/`args`/`env` are **exec form only** — the command is the
  executable (never shell-interpreted); `${TEMPORA_PLUGIN_ROOT}` expands to
...
`tempora plugin show`, capability diagnostics, and the Desktop installer
...
Tempora also reads Codex plugin manifests at `.codex-plugin/plugin.json` and
...
Tempora construct; it does not by itself guarantee every runtime decision an
...
Superpowers and Claude-style skill packs, Tempora maps the following
compatibility conventions. Native v2 manifests use only their explicit
declarations and do not apply these fallbacks:

- `skills` to Tempora skill roots. A Claude manifest that declares no
...
- `commands/` (and `.claude/commands/`) to Tempora custom slash commands: each
  ...
  details report that conflict. Native `tempora-plugin.json` manifests can
...
- `agents/*.md` to manually invoked, plugin-owned subagent profiles. Claude
  model aliases inherit the active Tempora model; inline `tools` lists map to
  Tempora tool names, including wildcard MCP names such as `mcp__*__search`.
  Agents use `/<plugin>:agent:<name>`, so an upstream agent and skill may share
  the same name without shadowing one another.
- `hooks/session-start-codex` to the Tempora `SessionStart` hook when present.
- For Codex compatibility packages, a plugin-root `CLAUDE.md` file to a built-in
  `SessionStart` context hook. The file is read directly by Tempora, without
  spawning a shell command. Claude plugin manifests ignore this file, matching
  Claude Code's plugin contract.
- `.claude/settings.json` and `hooks/hooks.json` command hooks to Tempora hook
  ...
  translated between Tempora's own tool names and Claude's (`bash` ↔
  `Bash`, `write_file` ↔ `Write`, ...), so a matcher like `"Bash"` fires
  correctly; every Tempora subagent-spawning tool (`task`, `read_only_task`,
  ...
  includes Claude's required `prompt` and `description`; Tempora supplies a
  stable operation label when its tool call omitted the optional description.
  `tool_input` keys that
  Tempora names differently from Claude are renamed too — `path` becomes
  ...
  interrupted}` shape (Tempora combines both streams into `stdout`; the
  ...
  expands `${CLAUDE_PLUGIN_ROOT}` and `${TEMPORA_PLUGIN_ROOT}` (plus their
  ...
  PowerShell-fallback selection as Tempora's shell tool; when a hook points to
  ...
  explicit Bash hooks. `tempora plugin doctor <name>` and
  `tempora doctor capabilities` report a missing required shell before the
  ...
  `Stop`/`SubagentStop` hook (which cannot block the turn in Tempora), or a
  matcher that covers one of three inputs Tempora cannot losslessly express:
  `WebFetch.prompt`, `NotebookEdit.cell_id` for a Tempora `cell_number` call,
  or `TaskOutput.task_id` when Tempora `wait` covers multiple/all jobs. Each
...
Unsupported Claude hook item types are skipped with a warning. Tempora does not
...
- `TEMPORA_PLUGIN_ROOT`
- `TEMPORA_PLUGIN_NAME`
- `TEMPORA_PLUGIN_VERSION`
- `TEMPORA_HOME`
- `TEMPORA_WORKSPACE_ROOT`
- `CLAUDE_PROJECT_DIR`
- `CLAUDE_PLUGIN_ROOT`
...
`tempora plugin doctor` has no route; run it from the CLI.
