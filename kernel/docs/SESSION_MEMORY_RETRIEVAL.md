...
Context Engine v2 gives Tempora two durable context layers with different
authority:
...
| --- | --- | --- |
| `AGENTS.md`, `TEMPORA.md`, or `CLAUDE.md` | Rules that must be present on every relevant turn | required test commands, repository boundaries, review conventions |
...
Tempora recognizes `TEMPORA.md`, `AGENTS.md`, and `CLAUDE.md`, plus matching
`.local.md` variants. It first loads user-global instruction files from the
Tempora home directory. It then walks from the workspace root to the target
...
pinning, with overflow directed to TEMPORA.md/AGENTS.md instructions, where
...
Before each real user turn, Tempora searches active facts using the raw user
...
The ordinary path is zero-configuration. Tempora may automatically create a
...
Restoring an old revision does not rewind storage in place. Tempora copies the
...
Tempora does not fall back to local sessions or local memory when the remote
