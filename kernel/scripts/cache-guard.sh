...
set +e
TEMPORA_RELEASE_CACHE_GUARD=1 go test ./internal/runtime/agent -run '^TestReleaseCacheHitGuard$' -v -count=1 2>&1 | tee "$tmp"
status=${PIPESTATUS[0]}
set -e
...
while IFS= read -r line; do
  ...
  echo "::warning title=Tempora cache guard::$msg"
