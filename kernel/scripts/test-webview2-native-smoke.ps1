...
$smokeRoot = Join-Path $tempRoot ("tempora-webview2-native-" + [guid]::NewGuid().ToString("N"))
...
$oldHome = $env:TEMPORA_HOME
$oldStateHome = $env:TEMPORA_STATE_HOME
$oldCacheHome = $env:TEMPORA_CACHE_HOME
$process = $null
try {
    $env:TEMPORA_HOME = $smokeHome
    $env:TEMPORA_STATE_HOME = $smokeState
    $env:TEMPORA_CACHE_HOME = $smokeCache
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        $lastState = $state
        if ($state.Exited) {
            throw "Tempora exited before the native window became healthy (exit code $($process.ExitCode))"
        }
    ...
    if ($null -eq $readyState) {
        throw "Tempora did not keep its main window, WebView2 renderer, document, and composer healthy for $HealthySeconds consecutive seconds within $TimeoutSeconds seconds; last_state=$(Format-NativeSmokeState -State $lastState)"
    }

    if (-not $process.CloseMainWindow()) {
        throw "Tempora main window rejected the graceful close request"
    }
    if (-not $process.WaitForExit(10000)) {
        throw "Tempora did not exit within 10 seconds after the graceful close request"
    }
    if ($process.ExitCode -ne 0) {
        throw "Tempora exited with code $($process.ExitCode) after the graceful close request"
    }
    ...
    $env:TEMPORA_HOME = $oldHome
    $env:TEMPORA_STATE_HOME = $oldStateHome
    $env:TEMPORA_CACHE_HOME = $oldCacheHome
