...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/tempora-docs-verify.XXXXXX")"
cleanup() {
	case "$tmp_dir" in
	*/tempora-docs-verify.*) rm -rf -- "$tmp_dir" ;;
	*) echo "refusing to clean unexpected docs verification directory: $tmp_dir" >&2 ;;
	esac
}
trap cleanup EXIT

binary="$tmp_dir/tempora"
build_time_utc="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
git_commit="$(printf '%s' "$revision" | cut -c1-12)"
ldflags="-s -w -X main.version=$version -X main.gitCommit=$git_commit -X main.buildTimeUTC=$build_time_utc -X tempora/internal/tools/productdocs.linkedVersion=$version -X tempora/internal/tools/productdocs.linkedRevision=$revision"
(
	cd "$repo_root"
	CGO_ENABLED=0 go build -trimpath -ldflags="$ldflags" -o "$binary" ./cmd/tempora
)

mkdir -p "$tmp_dir/tempora-home"
manifest="$(
	TEMPORA_HOME="$tmp_dir/tempora-home" "$binary" docs-manifest \
