// Package extension is the Go SDK for Tempora extension sidecars speaking
// Extension Protocol v2 over stdio. An extension is a separate process: the
// Tempora host launches it, sends extension/initialize first, drives
...
// internal/ext/extension/protocol/schema.generated.json in the Tempora
// repository.
package extension
...
func Serve(ctx context.Context, h Handler, opts Options) error {
	...
	logger := opts.Logger
	if logger == nil {
		logger = log.New(os.Stderr, "tempora-extension: ", log.LstdFlags)
	}
	s := &server{
		handler: h,
		opts:    opts,
