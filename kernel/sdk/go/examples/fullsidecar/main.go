// Command fullsidecar is the reference Tempora extension sidecar: one small
...
// Environment:
//
//	TEMPORA_PLUGIN_NAME   plugin ID, set by the host at launch (provider
...
import (
	...
	extension "github.com/esengine/DeepSeek-Tempora/sdk/go"
)
...
func main() {
	...
	id := strings.TrimSpace(os.Getenv("TEMPORA_PLUGIN_NAME"))
	if id == "" {
		id = defaultPluginID
	}
...
func (p *plugin) interceptSystemPrompt(_ context.Context, payload json.RawMessage) (*extension.InterceptResult, error) {
	...
	owned := "You are Tempora running under the fullsidecar demo strategy.\n\n" +
	...
	go func() {
		...
		script := []extension.StreamChunk{
			...
			{Type: extension.ChunkToolCall, ToolCall: &extension.ProviderToolCall{
				ID: "call-1", Name: "lookup", Arguments: `{"query":"tempora"}`,
			}},
