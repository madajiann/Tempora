...
// TestLoadDotEnvReadsGlobalCredentials proves `tempora setup`'s target — the
// tempora-owned credentials file under Tempora home — is loaded from any
// working directory and wins over a project ./.env on a shared key.
func TestLoadDotEnvReadsGlobalCredentials(t *testing.T) {
	cwd := testenv.TempDir(t)
	cfgHome := testenv.TempDir(t)
...
func TestDotEnvFileWarningsReportDuplicateKeysWithoutValues(t *testing.T) {
	path := filepath.Join(testenv.TempDir(t), ".env")
...
func TestDotEnvFileFilteredPreservesProjectScopeRules(t *testing.T) {
	...
	if _, ok := got["TEMPORA_HOME"]; ok {
		t.Fatalf("TEMPORA_HOME should be filtered: %+v", got)
	}
	if _, ok := got["HOME"]; ok {
...
func TestLoadForRootScopesProjectDotEnvPerWorkspace(t *testing.T) {
	home := testenv.TempDir(t)
	projectA := testenv.TempDir(t)
	projectB := testenv.TempDir(t)
...
func TestLoadForRootFiltersProjectDotEnvControlVars(t *testing.T) {
	...
	os.Unsetenv("TEMPORA_STATE_HOME")

	wantCred := UserCredentialsPath()
	if wantCred == "" {
		t.Skip("user credentials path unavailable")
	}
	if err := os.WriteFile(filepath.Join(project, ".env"), []byte("TEMPORA_STATE_HOME="+redirect+"\nPLUGIN_TOKEN=project-token\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(project, ".mcp.json"), []byte(`{
  "mcpServers": {
    "svc": {
      ...
      "headers": {
        ...
        "State": "${TEMPORA_STATE_HOME:-default-state}"
      }
    }
  }
}`), 0o644); err != nil {
	...
	if got := os.Getenv("TEMPORA_STATE_HOME"); got != "" {
...
func TestCredentialResolverCachesGlobalFirstLookups(t *testing.T) {
	...
	stubStoredCredentialValueForTest(t, func(got string) (string, CredentialSource, bool) {
		...
		return "from_credentials", CredentialSource{Kind: CredentialSourceCredentials, Label: "Tempora credentials"}, true
	})
...
func TestResolveCredentialSourceShowsCredentialsShadowingProjectEnv(t *testing.T) {
	...
	if !got.Set || got.Source.Kind != CredentialSourceCredentials {
		t.Fatalf("source = %+v set=%v, want Tempora credentials", got.Source, got.Set)
	}
...
func TestUserCredentialsPathIgnoresTemporaStateHome(t *testing.T) {
	home := testenv.TempDir(t)
	...
	want := filepath.Join(home, "tempora-home", ".env")
	...
	if strings.HasPrefix(UserCredentialsPath(), state) {
		t.Fatalf("credentials path must not live under TEMPORA_STATE_HOME: %q", UserCredentialsPath())
	}
}
...
func TestProjectConfigCannotOverrideCredentialStoreMode(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "tempora.toml"), []byte(`credentials_store = "keyring"`), 0o644); err != nil {
		t.Fatal(err)
	}
...
// TestLoadDotEnvGlobalCredentialsOverrideEnv confirms Tempora-owned global
