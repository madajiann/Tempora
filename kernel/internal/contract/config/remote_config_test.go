...
func TestProjectConfigCannotOverrideRemote(t *testing.T) {
	...
	if _, ok := cfg.RemoteHost("evil"); ok {
		t.Error("project tempora.toml injected a remote host; [remote] must stay user-global")
	}
}
...
passphrase_env = "TEMPORA_REMOTE_GPUBOX_PASSPHRASE"
...
func TestUpsertRemoteHostRoundTripsThroughSave(t *testing.T) {
	...
	host := RemoteHostEntry{
		...
		Port:          22,
		User:          "dev",
		PassphraseEnv: "TEMPORA_REMOTE_BOX_PASSPHRASE",
...
// head of it is the default a bare `tempora remote connect` uses.
...
func TestRemoteProviderModeDefaultsToLocal(t *testing.T) {
	cases := []struct {
		...
		// An entry written by a newer Tempora reads as the default here rather
