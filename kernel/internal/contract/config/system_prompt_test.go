...
func TestDefaultSystemPromptStaysLean(t *testing.T) {
	...
	for _, want := range []string{"Tempora", "available tools", "focused", "concise"} {
...
func TestResolveSystemPromptForRootAbsolutePath(t *testing.T) {
	...
	got, err := cfg.ResolveSystemPromptForRoot(testenv.TempDir(t))
	if err != nil {
		t.Fatalf("ResolveSystemPromptForRoot: %v", err)
	}
...
func TestResolveSystemPromptProjectCannotReadTemporaHome(t *testing.T) {
	home := testenv.TempDir(t)
	root := testenv.TempDir(t)
	t.Setenv("TEMPORA_HOME", home)
	...
	if strings.Contains(err.Error(), filepath.Join(home, ".env")) || strings.Contains(got, secret) {
		t.Fatalf("project prompt resolution probed Tempora credentials: got=%q err=%v", got, err)
	}
}
...
func TestResolveSystemPromptUserConfigCanFallBackToTemporaHome(t *testing.T) {
	home := testenv.TempDir(t)
	root := testenv.TempDir(t)
	t.Setenv("TEMPORA_HOME", home)
...
func TestResolveSystemPromptRejectsProjectPathEscapes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err := os.WriteFile(filepath.Join(root, "tempora.toml"), fmt.Appendf(nil, "[agent]\nsystem_prompt_file = %q\n", outside), 0o600); err != nil {
				t.Fatal(err)
			}
			cfg, err := LoadForRootReadOnly(root)
			if err != nil {
...
func TestResolveSystemPromptForRootFallsBackToTemporaHome(t *testing.T) {
	home := testenv.TempDir(t)
	t.Setenv("TEMPORA_HOME", home)
	if err := os.MkdirAll(filepath.Join(home, "prompts"), 0o755); err != nil {
		t.Fatal(err)
	...
	// Workspace root has no such file; the Tempora-home copy must win the probe.
	got, err := cfg.ResolveSystemPromptForRoot(testenv.TempDir(t))
	if err != nil {
		t.Fatalf("ResolveSystemPromptForRoot: %v", err)
	}
