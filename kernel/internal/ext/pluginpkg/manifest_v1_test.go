...
func writeV2ExampleAssets(t *testing.T, root string) {
	...
	writeTestFile(t, filepath.Join(root, "themes", "neon.tempora-theme"), "theme pack bytes")
...
func TestManifestV2FullParse(t *testing.T) {
	...
	if pkg.ManifestKind != "tempora" {
		t.Fatalf("ManifestKind = %q, want tempora", pkg.ManifestKind)
	}
	m := pkg.Manifest
	...
	if !reflect.DeepEqual(m.Themes, []string{"themes/*.tempora-theme"}) {
	...
	if rt.Command != "${TEMPORA_PLUGIN_ROOT}/bin/example" {
		t.Fatalf("Runtime.Command = %q, want the unexpanded ${TEMPORA_PLUGIN_ROOT} form", rt.Command)
	}
...
func TestManifestAPIVersionV2Gating(t *testing.T) {
	cases := []struct {
		...
		{"tempora.io/plugin/v2", ""},
		{"tempora.io/plugin/v1", `unsupported apiVersion`},
		{"tempora.io/plugin/v0", `unsupported apiVersion`},
		{"tempora.io/plugin/v10.2", `unsupported apiVersion`},
		{"v1", `unsupported apiVersion`},
		{"tempora.io/plugin/vX", `unsupported apiVersion`},
		{"foo", `unsupported apiVersion`},
		{42, `apiVersion must be a string`},
		// Exact v2 only — v2.x minor aliases are rejected by design.
		{"tempora.io/plugin/v2.1", `unsupported apiVersion`},
		{"tempora.io/plugin/v2.0", `unsupported apiVersion`},
	}
	for _, tc := range cases {
...
func TestManifestV2RuntimeValidation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			root := testenv.TempDir(t)
			writeV2Plugin(t, root, `{"apiVersion": "tempora.io/plugin/v2", "name": "rt-demo", "runtime": `+tc.runtime+`}`)
			_, _, err := ParseDir(root)
			if err == nil || !strings.Contains(err.Error(), tc.wantErr) {
				t.Fatalf("error = %v, want it to contain %q", err, tc.wantErr)
			}
...
func TestManifestV2PathSafety(t *testing.T) {
	t.Run("dot-dot escape rejected", func(t *testing.T) {
		root := testenv.TempDir(t)
		writeV2Plugin(t, root, `{"apiVersion": "tempora.io/plugin/v2", "name": "esc", "contributes": {"skills": ["../outside"]}}`)
	...
	t.Run("symlink escape rejected", func(t *testing.T) {
		...
		root := testenv.TempDir(t)
		writeV2Plugin(t, root, `{"apiVersion": "tempora.io/plugin/v2", "name": "linkesc", "contributes": {"skills": ["skills-link"]}}`)
	...
	t.Run("in-root symlink accepted", func(t *testing.T) {
		root := testenv.TempDir(t)
		writeV2Plugin(t, root, `{"apiVersion": "tempora.io/plugin/v2", "name": "linkok", "contributes": {"skills": ["skills-link"]}}`)
	...
	t.Run("theme symlink escape rejected", func(t *testing.T) {
		outside := testenv.TempDir(t)
		writeTestFile(t, filepath.Join(outside, "evil.tempora-theme"), "evil")
		root := testenv.TempDir(t)
		writeV2Plugin(t, root, `{"apiVersion": "tempora.io/plugin/v2", "name": "themesc", "contributes": {"themes": ["themes/*.tempora-theme"]}}`)
		if err := os.MkdirAll(filepath.Join(root, "themes"), 0o755); err != nil {
			t.Fatal(err)
		}
		if err := os.Symlink(filepath.Join(outside, "evil.tempora-theme"), filepath.Join(root, "themes", "evil.tempora-theme")); err != nil {
			t.Skipf("symlinks unavailable: %v", err)
		}
		if _, _, err := ParseDir(root); err == nil || !strings.Contains(err.Error(), "escapes the plugin root through a symlink") {
	...
	t.Run("non-regular theme rejected", func(t *testing.T) {
		root := testenv.TempDir(t)
		writeV2Plugin(t, root, `{"apiVersion": "tempora.io/plugin/v2", "name": "themedir", "contributes": {"themes": ["themes"]}}`)
		if err := os.MkdirAll(filepath.Join(root, "themes"), 0o755); err != nil {
			t.Fatal(err)
		}
  ...
  "contributes": {
    ...
    "themes": ["themes/*.tempora-theme"]
		...
		for _, want := range []string{`skills path "no-such-skills" does not exist`, `prompts path "no-such-prompts" does not exist`, `theme glob "themes/*.tempora-theme" matched no files`} {
...
func TestManifestV2DescribeRendersPromptsThemesRuntime(t *testing.T) {
	...
	writeV2Plugin(t, root, v2ExampleManifest)
	writeV2ExampleAssets(t, root)
	if err := Upsert(home, InstalledPlugin{Name: "example", Root: "plugins/example", Version: "1.0.0", ManifestKind: "tempora", Enabled: true}); err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{
		...
		"runtime: FULL TRUST",
		"command: ${TEMPORA_PLUGIN_ROOT}/bin/example --serve",
		"intercepts: input.receive, tool.before",
		"replaces: system_prompt",
		"capabilities: interceptors, strategies, providers, ui",
		"bypass permissions",
...
func TestExpandRuntimeCommand(t *testing.T) {
	...
	if got := ExpandRuntimeCommand("${TEMPORA_PLUGIN_ROOT}/bin/example", root); got != filepath.Join(root, "bin", "example") {
...
func TestManifestV2RuntimeDeclaresTools(t *testing.T) {
	root := testenv.TempDir(t)
	writeV2Plugin(t, root, `{"apiVersion": "tempora.io/plugin/v2", "name": "rt-tools", "runtime": {"command": "./run", "capabilities": ["tools"], "tools": [
