...
// through a shell.
const PluginRootEnvVar = "${TEMPORA_PLUGIN_ROOT}"

// RuntimeSpec declares a plugin-owned runtime process (Manifest v2). The
// command is exec form only: Tempora never runs it through a shell, so
// pipes, && and ; carry no special meaning. Command may start with
// ${TEMPORA_PLUGIN_ROOT} to address a binary inside the installed package;
...
// duplicate internal/ext/extension (intercept.go, replace.go). They are NOT
...
// ExpandRuntimeCommand substitutes the ${TEMPORA_PLUGIN_ROOT} prefix with
