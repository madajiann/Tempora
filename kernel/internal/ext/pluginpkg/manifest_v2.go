...
// ManifestAPIVersionV2 is the only native plugin manifest apiVersion Tempora
...
// upgraded explicitly, or automatically when they live in Tempora's managed
// plugin directory where an atomic backup is safe.
const ManifestAPIVersionV2 = "tempora.io/plugin/v2"
...
// tempora.io/plugin/v2 is accepted (no v2.0 / v2.1 aliases).
...
func MigrateManifestToV2(pkg Package) ([]byte, error) {
	if pkg.ManifestKind != "tempora" && pkg.ManifestKind != "" {
		return nil, fmt.Errorf("migrate: only native tempora manifests can be migrated (got %s)", pkg.ManifestKind)
	}
...
// previous tempora-plugin.json when present.
