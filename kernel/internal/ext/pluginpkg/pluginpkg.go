// Package pluginpkg handles installed Tempora plugin packages.
//
...
const (
	NativeManifest = "tempora-plugin.json"
...
// ThemeRef is one theme file (*.tempora-theme) a plugin contributes,
...
// Manifest is the normalized manifest shape used by Tempora.
type Manifest struct {
	// APIVersion is tempora.io/plugin/v2 for native packages; empty for Claude/Codex.
	APIVersion  string
	Name        string
	Version     string
	Description string
	...
	// loaded as plugin-owned, manually invoked Tempora subagent profiles.
	Agents []string
	...
	// once.
	Prompts []string
	// Themes are *.tempora-theme file paths or per-segment glob patterns
	// (e.g. "themes/*.tempora-theme"), all lexically inside the plugin root.
	Themes []string
...
// State is persisted at <Tempora home>/plugin-packages.json.
type State struct {
...
func StatePath(temporaHome string) string {
	return filepath.Join(temporaHome, StateFilename)
}

func PluginsDir(temporaHome string) string {
	return filepath.Join(temporaHome, "plugins")
}

func InstallRoot(temporaHome, name string) string {
	return filepath.Join(PluginsDir(temporaHome), name)
}

func LoadState(temporaHome string) (State, error) {
	var st State
	b, err := fileencoding.ReadFileUTF8(StatePath(temporaHome))
	if err != nil {
...
func SaveState(temporaHome string, st State) error {
	if st.Version == 0 {
		st.Version = 1
	}
	slices.SortStableFunc(st.Plugins, func(a, b InstalledPlugin) int { return cmp.Compare(a.Name, b.Name) })
	...
	b = append(b, '\n')
	return fileutil.AtomicWriteFile(StatePath(temporaHome), b, 0o644)
}
...
// not a cross-process lock — concurrent Tempora processes can still race.
var stateMu sync.Mutex

func Upsert(temporaHome string, p InstalledPlugin) error {
	if !IsValidName(p.Name) {
		return fmt.Errorf("invalid plugin name %q", p.Name)
	}
	stateMu.Lock()
	defer stateMu.Unlock()
	st, err := LoadState(temporaHome)
	if err != nil {
		return err
	}
	for i := range st.Plugins {
		if st.Plugins[i].Name == p.Name {
			st.Plugins[i] = p
			return SaveState(temporaHome, st)
		}
	}
...
func Remove(temporaHome, name string) (InstalledPlugin, bool, error) {
	stateMu.Lock()
	defer stateMu.Unlock()
	st, err := LoadState(temporaHome)
	if err != nil {
	...
	for i, p := range st.Plugins {
		...
		return p, true, SaveState(temporaHome, st)
	}
	return InstalledPlugin{}, false, nil
}

func SetEnabled(temporaHome, name string, enabled bool) error {
	stateMu.Lock()
	defer stateMu.Unlock()
	st, err := LoadState(temporaHome)
	if err != nil {
...
func ResolveRoot(temporaHome, root string) string {
	if filepath.IsAbs(root) {
		return filepath.Clean(root)
	}
	return filepath.Join(temporaHome, filepath.Clean(root))
}

func RelativeRoot(temporaHome, root string) string {
	if rel, err := filepath.Rel(temporaHome, root); err == nil && rel != "." && !strings.HasPrefix(rel, ".."+string(filepath.Separator)) && rel != ".." {
...
func parseNativeLegacy(b []byte, root string) (Package, []string, error) {
	...
	pkg := Package{Root: root, ManifestKind: "tempora", Manifest: manifest}
...
// the user invokes as /<name> — exactly Tempora's custom-command shape
// (internal/ext/command) — so these directories map onto Manifest.Commands and
...
// reports the conventional capabilities Tempora cannot map.
...
// definition. It delegates to the runtime loader (internal/ext/command), so the
...
// runtime uses (internal/ext/command), so names, namespacing, and frontmatter
...
// command inventory uses (internal/ext/command): prompt templates share the
