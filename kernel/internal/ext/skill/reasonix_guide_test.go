...
func TestTemporaGuideBuiltinRegistered(t *testing.T) {
	store := skill.New(skill.Options{HomeDir: testenv.TempDir(t), DisableBuiltins: false})
	sk, ok := store.Read("tempora-guide")
	if !ok {
		t.Fatal("tempora-guide must be registered as a builtin")
	}
...
func TestTemporaGuideIndexLineOnly(t *testing.T) {
	...
	if !found {
		t.Fatal("tempora-guide missing from List")
	}
	idx := skill.IndexBlock(list)
	if !strings.Contains(idx, "tempora-guide") {
		t.Fatal("index missing tempora-guide line")
	}
	...
	if c := strings.Count(idx, "- tempora-guide"); c != 1 {
		t.Fatalf("index lines for tempora-guide = %d, want 1", c)
	}
}
...
func TestTemporaGuideOverriddenByProject(t *testing.T) {
	home := testenv.TempDir(t)
	root := testenv.TempDir(t)
	store := skill.New(skill.Options{HomeDir: home, ProjectRoot: root})
	// Create project override.
	path, err := store.CreateWithContent("tempora-guide", skill.ScopeProject, "---\ndescription: override\nrunAs: inline\n---\nproject body\n")
	if err != nil {
		t.Fatal(err)
	}
	_ = path
	store2 := skill.New(skill.Options{HomeDir: home, ProjectRoot: root})
	sk, ok := store2.Read("tempora-guide")
	if !ok {
...
func TestTemporaGuideDisabled(t *testing.T) {
	store := skill.New(skill.Options{
		HomeDir:       testenv.TempDir(t),
		DisabledNames: skill.StaticDisabled("tempora-guide"),
	})
	if _, ok := store.Read("tempora-guide"); ok {
...
func TestTemporaGuideIndexStableAcrossCalls(t *testing.T) {
	store := skill.New(skill.Options{HomeDir: testenv.TempDir(t)})
