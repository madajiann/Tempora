...
func TestRunSkillInline(t *testing.T) {
	...
	writeSkill(t, home, ".tempora/skills/note.md", "---\ndescription: take a note\n---\nDo the thing.")
...
func TestRunSkillDoesNotBlockOnDiagnosticProfiles(t *testing.T) {
	home := testenv.TempDir(t)
	writeSkill(t, home, ".tempora/skills/delivery-only.md", "---\ndescription: ship it\nprofiles: delivery\n---\nDeliver it.")
...
func TestRunSkillSubagentNeedsRunner(t *testing.T) {
	home := testenv.TempDir(t)
	writeSkill(t, home, ".tempora/skills/dig.md", "---\ndescription: dig\nrunAs: subagent\n---\nbody")
...
func TestReadOnlySkillSubagentResolvesProfile(t *testing.T) {
	home := testenv.TempDir(t)
	writeSkill(t, home, ".tempora/skills/deep.md", "---\ndescription: deep\nrunAs: subagent\nmodel: deepseek-pro\neffort: max\n---\nbody")
	tl := NewReadOnlySkillTool(New(Options{HomeDir: home, DisableBuiltins: true}), nil)
...
func TestInstallSkill(t *testing.T) {
	...
	wantPath := filepath.Join(home, ".tempora", "skills", "deploy", SkillFile)
	...
	if _, err := os.Stat(filepath.Join(home, ".tempora", "skills", "deploy.md")); !errors.Is(err, os.ErrNotExist) {
