...
// Bounded-close budgets, mirrored from internal/ext/plugin's stdio transport: a
...
const (
	...
	// diagnostics, mirrored from internal/ext/plugin's tailBuffer.
	stderrTailBytes = 16 << 10
)

// pluginEnvVarPrefix is the well-known environment block every sidecar sees.
const (
	envPluginRoot    = "TEMPORA_PLUGIN_ROOT"
	envPluginName    = "TEMPORA_PLUGIN_NAME"
	envPluginVersion = "TEMPORA_PLUGIN_VERSION"
)
...
// The shape mirrors internal/ext/plugin's startupFailure.
...
// resolveRuntimeCommand expands ${TEMPORA_PLUGIN_ROOT} and enforces the exec
