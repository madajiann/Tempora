...
import (
	...
	"tempora/internal/ext/extension/rpcwire"
	"tempora/internal/ext/pluginpkg"
)

// Re-exec fake sidecar (the standard Go helper-process pattern): the test
// binary re-executes itself with TEMPORA_FAKE_SIDECAR=1 and speaks the real
// Extension Protocol v2 over stdin/stdout. Behavior is steered through env:
//
//		TEMPORA_FAKE_SIDECAR=1            enable the helper
//		TEMPORA_FAKE_INIT_RESULT          raw JSON InitializeResult to answer with
//		TEMPORA_FAKE_MODE                 comma-separated behavior flags:
...
//	                                  wedge_after_init
const (
	fakeEnvEnable     = "TEMPORA_FAKE_SIDECAR"
	fakeEnvInitResult = "TEMPORA_FAKE_INIT_RESULT"
	fakeEnvMode       = "TEMPORA_FAKE_MODE"
)
