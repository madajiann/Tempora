...
func TestResolveRuntimeCommandExpandsPluginRoot(t *testing.T) {
	root := testenv.TempDir(t)
	got, err := resolveRuntimeCommand(&pluginpkg.RuntimeSpec{Command: "${TEMPORA_PLUGIN_ROOT}/bin/sidecar"}, root)
	if err != nil {
...
func TestRuntimeEnvFullTrustContract(t *testing.T) {
	t.Setenv("TEMPORA_TEST_INHERITED_MARKER", "present")
	root := testenv.TempDir(t)
	...
	if values["TEMPORA_TEST_INHERITED_MARKER"] != "present" {
