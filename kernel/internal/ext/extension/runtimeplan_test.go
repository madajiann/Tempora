...
func TestRuntimePlanNoOp(t *testing.T) {
	g, err := BuildDependencyGraph([]ComponentDescriptor{
		{ID: "host", Provides: []extensioncontract.Capability{cap("tempora", "provider", "p", "1.0.0", "sha256:p")}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRuntimePlanProviderOnlyChange(t *testing.T) {
	from, err := BuildDependencyGraph([]ComponentDescriptor{
		...
		{ID: "consumer", Requires: []extensioncontract.Requirement{req("tempora", "provider", "p", ">=1.0.0", false)}},
	})
	if err != nil {
		t.Fatal(err)
	}
	to, err := BuildDependencyGraph([]ComponentDescriptor{
		{ID: "host", Provides: []extensioncontract.Capability{cap("tempora", "provider", "p", "1.1.0", "sha256:b")}},
		{ID: "consumer", Requires: []extensioncontract.Requirement{req("tempora", "provider", "p", ">=1.0.0", false)}},
	})
	if err != nil {
		t.Fatal(err)
