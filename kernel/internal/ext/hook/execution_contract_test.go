...
func TestHookExecHelperProcess(t *testing.T) {
	if os.Getenv("TEMPORA_HOOK_EXEC_HELPER") != "1" {
		return
	}
...
func TestExecFormPreservesLiteralArgumentsEndToEnd(t *testing.T) {
	...
	want := []string{
		...
		`double"quote`,
		"single'quote",
		`C:\Program Files\Tempora\hook.cmd`,
		"第一行\n第二行",
		"emoji-🧪",
	}
	...
	result := DefaultSpawner(context.Background(), SpawnInput{
		...
		Args:    args,
		Mode:    ExecutionExec,
		Env:     map[string]string{"TEMPORA_HOOK_EXEC_HELPER": "1"},
...
func TestResolvedHookShellPathAcceptsExecutableAndRejectsMissing(t *testing.T) {
	...
	if _, err := resolvedHookShellPath(sandbox.Shell{Kind: sandbox.ShellBash, Path: "/definitely/missing/tempora-hook-shell"}); err == nil {
...
func FuzzPowerShellCommandEncodingRoundTrip(f *testing.F) {
	for _, seed := range []string{
		"",
		`Write-Output "a && 'b'"`,
		`$value = "C:\Program Files\Tempora"; $value`,
		"第一行\n第二行",
		"Write-Output '🧪'",
