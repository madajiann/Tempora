...
func TestNormalizeCommandRepairsOnlyPowerShellFileEscapedQuotes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			command: `powershell -File \"C:\Users\Example\.tempora\hooks\archive-attachments.ps1\"`,
			want:    `powershell -File "C:\Users\Example\.tempora\hooks\archive-attachments.ps1"`,
		},
		{
			name:    "pwsh file path with spaces",
			command: `pwsh.exe -NoProfile -NonInteractive -File \"C:\Program Files\Tempora Hooks\archive attachments.ps1\"`,
			want:    `pwsh.exe -NoProfile -NonInteractive -File "C:\Program Files\Tempora Hooks\archive attachments.ps1"`,
		},
		{
			name:    "doubly escaped copied quotes",
			command: `pwsh -File \\\"C:\Program Files\Tempora Hooks\archive attachments.ps1\\\" \"arg with spaces\"`,
			want:    `pwsh -File "C:\Program Files\Tempora Hooks\archive attachments.ps1" "arg with spaces"`,
		},
		{
		...
		},
		{
			...
			command: `powershell -NoProfile -File "C:\Program Files\Tempora Hooks\archive attachments.ps1"`,
			want:    `powershell -NoProfile -File "C:\Program Files\Tempora Hooks\archive attachments.ps1"`,
		},
		{
...
func TestLoadNormalizesPowerShellFileEscapedQuotes(t *testing.T) {
	home := testenv.TempDir(t)
	bad := `powershell -File \"C:\Program Files\Tempora Hooks\archive attachments.ps1\"`
	want := `powershell -File "C:\Program Files\Tempora Hooks\archive attachments.ps1"`
...
func TestRepairablePowerShellFileArgs(t *testing.T) {
	command := `powershell -NoProfile -NonInteractive -File \"C:\Program Files\Tempora Hooks\archive attachments.ps1\" -Mode \"startup\"`
	...
	wantArgs := []string{"-NoProfile", "-NonInteractive", "-File", `C:\Program Files\Tempora Hooks\archive attachments.ps1`, "-Mode", "startup"}
...
func installSuperpowersV611HookFixture(t *testing.T, home string) string {
	...
	root := filepath.Join(temporaHome, "plugins", "superpowers fixture")
...
func TestLoadSuperpowersV620PreservesExplicitBashRequirement(t *testing.T) {
	home := testenv.TempDir(t)
	root := filepath.Join(home, ".tempora", "plugins", "superpowers")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.CodexManifest), `{
  "name": "superpowers",
  "version": "6.2.0",
  "skills": "./skills/"
  ...
  "hooks": {
	...
	if err := pluginpkg.Upsert(filepath.Join(home, ".tempora"), pluginpkg.InstalledPlugin{
		Name:         "superpowers",
...
func TestPluginExplicitBashCommandUsesPOSIXCompatibleRoot(t *testing.T) {
	root := `C:\Users\Test User\AppData\Roaming\tempora\plugins\superpowers`
	config := pluginHookExecutionConfigForPlatform(pluginpkg.Hook{
		Command:      `"${CLAUDE_PLUGIN_ROOT}/hooks/run-hook.cmd" session-start`,
		ShellCommand: true,
		Shell:        "bash",
	}, root, "windows")
	want := `"C:/Users/Test User/AppData/Roaming/tempora/plugins/superpowers/hooks/run-hook.cmd" session-start`
...
func TestLoadIncludesPluginSessionStartHook(t *testing.T) {
  ...
  "skills": "./skills/"
	...
	if got[0].Env["TEMPORA_PLUGIN_NAME"] != "superpowers" || got[0].Env["TEMPORA_WORKSPACE_ROOT"] != "/workspace" {
...
// (the hook-machine default after #7420) and an isolated TEMPORA_HOME, the
// plugin probe and global settings resolve from the platform Tempora home —
// <home>/plugins and <home>/settings.json — not a doubled .tempora segment.
func TestInspectNoHomeDirResolvesPluginRootFromPlatformHome(t *testing.T) {
	...
	// Global settings live directly under the platform Tempora home
	// (writeSettings would add .tempora, the OS-home convention #7420 fixes).
  ...
  "skills": "./skills/"
	...
	// platform Tempora home, not a doubled .tempora segment.
...
func TestLoadIncludesPluginClaudeCompatibilityHooks(t *testing.T) {
	home := testenv.TempDir(t)
	temporaHome := filepath.Join(home, ".tempora")
	root := filepath.Join(temporaHome, "plugins", "claude-pack")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.CodexManifest), `{
  "name": "claude-pack",
  "version": "1.0.0",
  "skills": "skills"
  ...
  "hooks": {
	...
	if h := byEvent[PostToolUse]; h.Env["CLAUDE_PROJECT_DIR"] != "/workspace" || h.Env["TEMPORA_PLUGIN_NAME"] != "claude-pack" {
...
func TestLoadPluginHooksPreservesExecutionContract(t *testing.T) {
	home := testenv.TempDir(t)
	temporaHome := filepath.Join(home, ".tempora")
	root := filepath.Join(temporaHome, "plugins", "hook-contract")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.NativeManifest), `{
  "apiVersion": "tempora.io/plugin/v2", "name": "hook-contract",
  "hooks": {
    "SessionStart": [
...
func TestLoadExpandsTemporaPluginRootBeforeShellLaunch(t *testing.T) {
	home := testenv.TempDir(t)
	temporaHome := filepath.Join(home, ".tempora")
	root := filepath.Join(temporaHome, "plugins", "impeccable")
	projectRoot := filepath.Join(home, "$CLAUDE_PLUGIN_ROOT-project")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.NativeManifest), `{
  "apiVersion": "tempora.io/plugin/v2", "name": "impeccable",
  "version": "3.9.1",
  "hooks": {
    "PostToolUse": [{
      "match": "edit",
      "command": "node \"${TEMPORA_PLUGIN_ROOT}/skills/impeccable/scripts/hook.mjs\"",
      "shellCommand": true,
      "cwd": "${TEMPORA_PLUGIN_ROOT}/work",
      "env": {"IMPECCABLE_CACHE": "%TEMPORA_PLUGIN_ROOT%/cache"}
    }]
  }
}`)
	if err := pluginpkg.Upsert(temporaHome, pluginpkg.InstalledPlugin{
	...
	if got[0].Env["CLAUDE_PROJECT_DIR"] != projectRoot || got[0].Env["TEMPORA_WORKSPACE_ROOT"] != projectRoot {
		t.Fatalf("host-provided workspace paths were expanded: %#v", got[0].Env)
	}
}

func TestExpandPluginRootSupportsClaudeTemporaAndCmdAliases(t *testing.T) {
	root := `C:\Program Files\Tempora\plugins\impeccable`
	for _, token := range []string{
		"${CLAUDE_PLUGIN_ROOT}", "$CLAUDE_PLUGIN_ROOT", "%CLAUDE_PLUGIN_ROOT%",
		"${TEMPORA_PLUGIN_ROOT}", "$TEMPORA_PLUGIN_ROOT", "%TEMPORA_PLUGIN_ROOT%",
	} {
	...
	if got, want := expandPluginRoot(`$CLAUDE_PLUGIN_ROOT/$TEMPORA_PLUGIN_ROOT`, root), root+"/"+root; got != want {
		t.Fatalf("both root aliases = %q, want %q", got, want)
	}
}

func TestExpandPluginRootDoesNotReprocessResolvedRoot(t *testing.T) {
	root := `/tmp/$TEMPORA_PLUGIN_ROOT/%CLAUDE_PLUGIN_ROOT%/${CLAUDE_PLUGIN_ROOT}`
	value := `${CLAUDE_PLUGIN_ROOT}|$TEMPORA_PLUGIN_ROOT|%CLAUDE_PLUGIN_ROOT%`
	want := root + "|" + root + "|" + root
...
func TestWindowsBatchCommandLinePreservesQuotedPluginPath(t *testing.T) {
	command := `"C:\Users\Test User\AppData\Roaming\tempora\plugins\superpowers/hooks/run-hook.cmd" session-start`
	got, ok := windowsBatchCommandLine(command)
	if !ok {
		t.Fatal("quoted plugin batch command was not recognized")
	}
	want := `cmd.exe /d /s /c ""C:\Users\Test User\AppData\Roaming\tempora\plugins\superpowers\hooks\run-hook.cmd" session-start"`
	if got != want {
...
func TestWindowsBatchArgvCommandLineSupportsNativePluginHooks(t *testing.T) {
	got, ok := windowsBatchArgvCommandLine(
		`C:\Program Files\Tempora\plugins\example/hooks/run-hook.cmd`,
		[]string{"session-start", "argument with spaces"},
	)
	if !ok {
		t.Fatal("native plugin batch command was not recognized")
	}
	want := `cmd.exe /d /s /c ""C:\Program Files\Tempora\plugins\example\hooks\run-hook.cmd" session-start "argument with spaces""`
	if got != want {
...
func TestTemporaHomeOverridesGlobalHookPaths(t *testing.T) {
	home := testenv.TempDir(t)
	temporaHome := filepath.Join(testenv.TempDir(t), "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("TEMPORA_HOME", temporaHome)
	if err := os.MkdirAll(temporaHome, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(temporaHome, SettingsFilename), []byte(`{"hooks":{"PostToolUse":[{"command":"echo rx"}]}}`), 0o644); err != nil {
		t.Fatal(err)
	}
	writeSettings(t, home, `{"hooks":{"PostToolUse":[{"command":"echo old"}]}}`)

	if got := GlobalSettingsPath(""); got != filepath.Join(temporaHome, SettingsFilename) {
		t.Fatalf("GlobalSettingsPath = %q, want Tempora home", got)
	}
	hooks := Load(LoadOptions{})
	if len(hooks) != 1 || hooks[0].Command != "echo rx" {
		t.Fatalf("Load hooks = %+v, want Tempora home hook only", hooks)
	}
}

func TestLoadOptionsTemporaHomeDirUsesExactGlobalHookPath(t *testing.T) {
	home := testenv.TempDir(t)
	temporaHome := filepath.Join(home, "AppData", "Roaming", "tempora")
	settingsPath := filepath.Join(temporaHome, SettingsFilename)
	if err := os.MkdirAll(temporaHome, 0o755); err != nil {
		t.Fatal(err)
	}
	...
	hooks := Load(LoadOptions{HomeDir: home, TemporaHomeDir: temporaHome})
	if len(hooks) != 1 || hooks[0].Command != "echo exact" || hooks[0].Source != settingsPath {
		t.Fatalf("Load hooks = %+v, want exact Tempora home hook", hooks)
	}
}

func TestTemporaHomeDoesNotFallBackToLegacyWhenIsolated(t *testing.T) {
	home := testenv.TempDir(t)
	temporaHome := filepath.Join(testenv.TempDir(t), "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	hooks := Load(LoadOptions{})
	if len(hooks) != 0 {
		t.Fatalf("Load hooks = %+v, want empty (isolated TEMPORA_HOME must not load legacy hooks)", hooks)
	}
...
func TestMatchesToolTranslatesClaudeToolNames(t *testing.T) {
	...
	if !MatchesTool(claude("Bash"), "bash") {
		t.Error(`Claude matcher "Bash" should match Tempora tool "bash"`)
	}
	if !MatchesTool(claude("Write|Edit"), "write_file") {
		t.Error(`Claude matcher "Write|Edit" should match Tempora tool "write_file"`)
	}
	if !MatchesTool(claude("Write|Edit"), "edit_file") {
		t.Error(`Claude matcher "Write|Edit" should match Tempora tool "edit_file"`)
	}
	if MatchesTool(claude("Bash"), "write_file") {
		t.Error(`Claude matcher "Bash" must not match Tempora tool "write_file"`)
	}
	// A native (non-Claude) hook's matcher stays in Tempora's own vocabulary.
	...
	// using either name must still fire against Tempora's "task" tool.
	if !MatchesTool(claude("Agent"), "task") {
		t.Error(`Claude matcher "Agent" (current name) should match Tempora tool "task"`)
	}
	if !MatchesTool(claude("Task"), "task") {
		t.Error(`Claude matcher "Task" (legacy alias) should still match Tempora tool "task"`)
	}
	if !MatchesTool(claude("AskUserQuestion"), "ask") {
		t.Error(`Claude matcher "AskUserQuestion" should match Tempora tool "ask"`)
	}
	for _, name := range []string{"bash_output", "wait"} {
		if !MatchesTool(claude("TaskOutput"), name) || !MatchesTool(claude("BashOutput"), name) {
			t.Errorf(`current "TaskOutput" and legacy "BashOutput" matchers should match Tempora tool %q`, name)
		}
	}
	if !MatchesTool(claude("TaskStop"), "kill_shell") || !MatchesTool(claude("KillShell"), "kill_shell") {
		t.Error(`current "TaskStop" and legacy "KillShell" matchers should match Tempora tool "kill_shell"`)
	}
}
...
func TestClaudeFacingToolNameUsesCurrentNames(t *testing.T) {
	...
	for _, name := range []string{"task", "read_only_task", "parallel_tasks", "explore", "research", "review", "security_review"} {
		...
		if !MatchesTool(claude, name) {
			t.Errorf(`Claude matcher "Agent" should match Tempora tool %q`, name)
		}
		legacy := ResolvedHook{HookConfig: HookConfig{Match: "Task", PayloadFormat: "claude"}, Event: PreToolUse}
		if !MatchesTool(legacy, name) {
			t.Errorf(`legacy Claude matcher "Task" should still match Tempora tool %q`, name)
		}
	}
}
...
// contract ("file_path must be absolute"): a relative Tempora path resolves
...
func TestDecideOutcome(t *testing.T) {
	cases := []struct {
		...
		// same way PreToolUse does; native Tempora PermissionRequest hooks
...
func TestRunClaudePayloadAndDirectArgs(t *testing.T) {
	...
	if payload["tool_name"] != "Bash" {
		t.Fatalf("Claude payload tool_name = %v, want the Claude vocabulary name Bash for Tempora tool bash", payload["tool_name"])
	}
...
// Tempora write_file call and sees the absolute target path Claude's
...
func TestRunClaudeWriteFileGuardFiresAndSeesFilePath(t *testing.T) {
	...
	if input.Command == "" {
		t.Fatal(`matcher "Write" did not fire for Tempora tool "write_file"`)
	}
	var payload map[string]any
	if err := json.Unmarshal([]byte(input.Stdin), &payload); err != nil {
		t.Fatal(err)
	...
	if _, hasPath := toolInput["path"]; hasPath {
		t.Fatalf("tool_input still has Tempora's \"path\" key: %#v", toolInput)
	}
}

// TestRunClaudeAgentGuardFiresAndSeesRequiredFields covers the full matcher to
// stdin path for a dedicated Tempora subagent wrapper. Claude Agent requires
...
func TestRunClaudeAgentGuardFiresAndSeesRequiredFields(t *testing.T) {
	...
	if input.Command == "" {
		t.Fatal(`matcher "Agent" did not fire for Tempora tool "security_review"`)
	}
	var payload map[string]any
	if err := json.Unmarshal([]byte(input.Stdin), &payload); err != nil {
		t.Fatal(err)
