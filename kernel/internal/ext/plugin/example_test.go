...
// buildExamplePlugin compiles cmd/tempora-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(testenv.TempDir(t), "tempora-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "tempora/cmd/tempora-plugin-example").CombinedOutput()
	if err != nil {
