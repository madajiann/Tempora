...
func MCPStateDir(temporaHome, workspace, server string) string {
	...
	return filepath.Join(temporaHome, "mcp-state", workspaceID, slug(server))
}
...
func ReconcileCachedToolSafety(server, rawName string, cached CachedToolSafety, target tool.Tool) (CachedToolSafety, error) {
	...
	if cached.ReadOnly && !live.ReadOnly {
		return live, fmt.Errorf("MCP server %q no longer marks tool %q as read-only; the current call was blocked before execution — retry so Tempora can apply the current Plan/read-only safety boundary", server, rawName)
	}
	if !cached.Destructive && live.Destructive {
		return live, fmt.Errorf("MCP server %q now marks tool %q as destructive; retry so Tempora can apply the current Plan/read-only safety boundary before execution", server, rawName)
	}
	return live, nil
}
