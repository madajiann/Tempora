...
func TestApplyLocalSkillRootRegistersPath(t *testing.T) {
	...
	cfg := config.LoadForEdit(filepath.Join(project, "tempora.toml"))
...
func TestApplyLocalCodexPluginPackage(t *testing.T) {
  ...
  "skills": "./skills/"
	...
	statePath := filepath.Join(home, ".tempora", "plugin-packages.json")
	...
	if _, err := os.Stat(filepath.Join(home, ".tempora", "plugins", "superpowers", ".codex-plugin", "plugin.json")); err != nil {
...
func TestApplyLocalClaudePluginPackage(t *testing.T) {
  ...
  "skills": "./.claude/skills/"
	...
	if _, err := os.Stat(filepath.Join(home, ".tempora", "plugins", "ui-ux-pro-max", ".claude-plugin", "plugin.json")); err != nil {
		t.Fatalf("installed plugin missing: %v", err)
	}
}
...
func TestApplyCopiedPluginPreservesExecutableHookCommand(t *testing.T) {
  ...
  "hooks":{"UserPromptSubmit":[{"hooks":[{"type":"command","command":"${CLAUDE_PLUGIN_ROOT}/bin/hook","args":["--hook"]}]}]}
	...
	installed := filepath.Join(home, ".tempora", "plugins", "executable-plugin", "bin", "hook")
...
func TestPlanClaudeCompatibilityReportsAgentsHooksAndMCP(t *testing.T) {
	...
	a := planned.Actions[0]
	// A Stop hook is imported best-effort, but Tempora's Stop hook is
...
func TestApplyLocalSkillFileCopiesToProject(t *testing.T) {
	...
	target := filepath.Join(project, ".tempora", "skills", "beta", "SKILL.md")
...
func TestApplyLocalSkillFileDoesNotShadowFlatCompatInstall(t *testing.T) {
	project := testenv.TempDir(t)
	home := testenv.TempDir(t)
	existing := filepath.Join(project, ".tempora", "skills", "beta.md")
...
func TestApplyLocalSKILLFileCopiesSiblingResources(t *testing.T) {
	...
	target := filepath.Join(project, ".tempora", "skills", "frontend-design", "SKILL.md")
	if resp.Actions[0].CanonicalPath != target {
		t.Fatalf("canonicalPath = %q, want %q", resp.Actions[0].CanonicalPath, target)
	}
	if _, err := os.Stat(filepath.Join(project, ".tempora", "skills", "frontend-design", "references", "style.md")); err != nil {
		t.Fatalf("reference file should be copied with SKILL.md source: %v", err)
	}
	if _, err := os.Stat(filepath.Join(project, ".tempora", "skills", "frontend-design", "scripts", "lint.sh")); err != nil {
...
func TestApplyLocalSkillLinkMode(t *testing.T) {
	...
	target := filepath.Join(project, ".tempora", "skills", "gamma", "SKILL.md")
...
func TestPlanProjectMCPJSONDefaultsProject(t *testing.T) {
  ...
  "mcpServers": {
	...
	wantPath := filepath.Join(project, "tempora.toml")
...
func TestApplyRemoteMCPURLDefaultsGlobal(t *testing.T) {
	...
	projectCfg := config.LoadForEdit(filepath.Join(project, "tempora.toml"))
...
func TestApplyMCPRejectsDuplicateByDefault(t *testing.T) {
	...
	if err := cfg.SaveTo(filepath.Join(project, "tempora.toml")); err != nil {
		t.Fatal(err)
	}
...
func TestApplyMCPReplaceOverwritesExisting(t *testing.T) {
	...
	reloaded := config.LoadForEdit(filepath.Join(project, "tempora.toml"))
...
func TestParseGitHubRepoSourceRejectsPagesAndUnsafePaths(t *testing.T) {
	for _, source := range []string{
		...
		"https://github.com/o/r/blob/main/tempora-plugin.json",
...
func TestGlobalSkillInstallRootUsesTemporaHome(t *testing.T) {
	home := testenv.TempDir(t)
	...
	want := filepath.Join(temporaHome, skill.SkillsDirname)
	if root != want {
...
func TestUninstallRemovesSkillByName(t *testing.T) {
	project := testenv.TempDir(t)
	home := testenv.TempDir(t)
	target := filepath.Join(project, ".tempora", "skills", "doomed.md")
	writeFile(t, target, "---\nname: doomed\ndescription: Doomed\n---\nbody")
...
func TestUninstallWithoutScopePrefersProjectSkill(t *testing.T) {
	project := testenv.TempDir(t)
	home := testenv.TempDir(t)
	projectTarget := filepath.Join(project, ".tempora", "skills", "dupe.md")
	globalTarget := filepath.Join(home, ".tempora", "skills", "dupe.md")
...
func TestPlanIDIncludesActionDetails(t *testing.T) {
	...
	a := action{Kind: "mcp", Action: "install_mcp_server", Name: "same", URL: "https://mcp.one.example/mcp", Transport: "http", ConfigPath: "/repo/tempora.toml"}
	b := a
...
func TestGitHubClaudeMarketplacePlansAndAppliesRelativePlugins(t *testing.T) {
  ...
  "plugins": [
	...
	for _, name := range []string{"alpha-legal", "beta-legal"} {
		if _, ok, err := pluginpkg.FindInstalled(filepath.Join(home, ".tempora"), name); err != nil || !ok {
...
func TestGitHubPluginApplyRefusesUnpinnableDrift(t *testing.T) {
	...
	if _, ok, _ := pluginpkg.FindInstalled(filepath.Join(home, ".tempora"), "pwf"); ok {
...
func TestCopyMaterializesInRootSymlinkedCommands(t *testing.T) {
	...
	installedRoot := filepath.Join(home, ".tempora", "plugins", "aliases")
...
func TestCopyRefusesUnmaterializableSymlinkCommands(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(home, ".tempora", "plugins", "escapes")); !os.IsNotExist(err) {
		t.Fatal("failed install must not leave the copied tree behind")
	}
	if _, ok, _ := pluginpkg.FindInstalled(filepath.Join(home, ".tempora"), "escapes"); ok {
...
func TestFailedReplaceKeepsExistingPluginInstall(t *testing.T) {
	...
	installedRoot := filepath.Join(home, ".tempora", "plugins", "pwf")
	...
	if p, ok, _ := pluginpkg.FindInstalled(filepath.Join(home, ".tempora"), "pwf"); !ok || !p.Enabled {
...
func TestBackupPathCannotCollideWithSiblingPlugin(t *testing.T) {
	...
	siblingRoot := filepath.Join(home, ".tempora", "plugins", "foo.pre-replace")
	...
	if _, ok, _ := pluginpkg.FindInstalled(filepath.Join(home, ".tempora"), "foo.pre-replace"); !ok {
		t.Fatal("sibling plugin must stay registered")
	}
	pkg, _, err := pluginpkg.ParseDir(filepath.Join(home, ".tempora", "plugins", "foo"))
	if err != nil {
