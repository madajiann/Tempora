...
func writeRuntimePlugin(t *testing.T, root string) {
  ...
  "runtime": {
    "command": "${TEMPORA_PLUGIN_ROOT}/bin/rtsync",
    "args": ["--serve"],
    "required": true,
	...
	writeFile(t, filepath.Join(root, "themes", "neon.tempora-theme"), "theme bytes")
...
func TestPluginRuntimePlanCarriesFullTrust(t *testing.T) {
	...
	for _, reason := range act.RiskReasons {
		if strings.HasPrefix(reason, "FULL TRUST:") {
			fullTrust = true
			if !strings.Contains(reason, "${TEMPORA_PLUGIN_ROOT}/bin/rtsync --serve") {
	...
	if rt.Command != "${TEMPORA_PLUGIN_ROOT}/bin/rtsync" || !rt.FullTrust {
	...
	if rtJSON["fullTrust"] != true || rtJSON["command"] != "${TEMPORA_PLUGIN_ROOT}/bin/rtsync" {
...
func TestPluginLegacyPlanOmitsRuntimeFields(t *testing.T) {
	...
	writeFile(t, filepath.Join(src, "tempora-plugin.json"), `{"apiVersion":"tempora.io/plugin/v2","name":"legacy","skills":["skills"]}`)
