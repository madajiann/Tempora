...
func (t *installSourceTool) pluginPackageAction(req request, pkg pluginpkg.Package, source string) (action, error) {
	...
	root := ""
	if t.temporaHome != "" {
		root = pluginpkg.InstallRoot(t.temporaHome, name)
	}
	skills, commands, hooks, mcp := pkg.CapabilityCounts()
	agents := pkg.Inventory().Agents
	if pkg.ManifestKind != "tempora" && skills+commands+hooks+mcp+len(agents) == 0 {
		return action{}, newErr(ErrNoCompatibleCapabilities, "plugin %q has no Tempora-compatible capabilities; skipped: %v", name, pkg.Compatibility.Skipped)
	}
	...
	a := action{
		...
		ConfigPath:          pluginpkg.StatePath(t.temporaHome),
	...
	if hooks > 0 {
		...
		a.RiskReasons = append(a.RiskReasons, "registers shell hooks that execute during Tempora sessions")
	}
	if mcp > 0 {
		a.RiskLevel = RiskHigh
	...
	if a.Runtime != nil {
		a.RiskLevel = RiskHigh
		a.RiskReasons = append(a.RiskReasons, "FULL TRUST: declares a runtime process ("+pluginpkg.RuntimeCommandLine(pkg.Manifest.Runtime)+") that runs inside Tempora — it can read the full session and environment, bypass permissions, and operate this machine directly")
	}
...
func (t *installSourceTool) applyInstallPluginPackage(ctx context.Context, req request, act *action) error {
	if t.temporaHome == "" {
		return newErr(ErrSourceUnreadable, "plugin install requires a Tempora home directory")
	}
	if !pluginpkg.IsValidName(act.Name) {
		return newErr(ErrInvalidManifest, "invalid plugin name %q", act.Name)
	}
	target := pluginpkg.InstallRoot(t.temporaHome, act.Name)
	...
	if pkg.ManifestKind != "tempora" {
		...
		if skills+commands+hooks+mcp+pkg.AgentCount() == 0 {
			return newErr(ErrInvalidManifest, "plugin %q no longer has any Tempora-compatible capabilities", act.Name)
		}
	}
	...
	installed := pluginpkg.InstalledPlugin{
		Name:         act.Name,
		Source:       act.Source,
		Root:         pluginpkg.RelativeRoot(t.temporaHome, target),
		Version:      pkg.Manifest.Version,
	...
	if err := pluginpkg.Upsert(t.temporaHome, installed); err != nil {
		return err
	}
	act.Target = target
...
func (t *installSourceTool) preparePluginSource(ctx context.Context, source, mode string) (string, string, func(), error) {
	...
	if isURL(source) {
		...
		tmp, err := os.MkdirTemp("", "tempora-plugin-*")
		if err != nil {
			return "", "", func() {}, err
		}
...
func (t *installSourceTool) applyRemovePluginPackage(_ request, act *action) error {
	installed, ok, err := pluginpkg.Remove(t.temporaHome, act.Name)
	if err != nil || !ok {
		return err
	}
	root := pluginpkg.ResolveRoot(t.temporaHome, installed.Root)
	...
	pluginsDir := pluginpkg.PluginsDir(t.temporaHome)
