...
func TestResolvedBuiltinDocsSlashNameMatchesRuntimeOwner(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !IsBuiltinDocsSlash(TemporaDocsSlashName, tt.commands, tt.skills) {
				t.Fatal("preferred qualified Tempora docs name should resolve to the built-in when unoccupied")
			}
		})
	}
}
...
func TestQualifiedBuiltinDocsSlashPreservesExistingQualifiedCommands(t *testing.T) {
	commands := []command.Command{
		{Name: DocsSlashName},
		{Name: TemporaDocsSlashName},
		{Name: "tempora:builtin:docs"},
	}
	want := "tempora:builtin:docs:2"
	if got := ResolvedBuiltinSlashName(DocsSlashName, commands, nil); got != want {
		t.Fatalf("resolved built-in name = %q, want %q", got, want)
	}
	for _, existing := range []string{DocsSlashName, TemporaDocsSlashName, "tempora:builtin:docs"} {
