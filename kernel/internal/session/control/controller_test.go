...
func TestApprovalSessionGrantCanScopeBashToCommandPrefix(t *testing.T) {
	...
	for i, subject := range []string{"go test ./...", "go test ./internal/session/control", "go build ./..."} {
		allow, _, err := gateApprover{c}.Approve(context.Background(), "bash", subject, nil)
		if err != nil || !allow {
			t.Fatalf("call %d = (%v,%v), want allow", i, allow, err)
		}
...
func TestRememberResultNoticesCarryCodeAndSubject(t *testing.T) {
	...
	c.emitRememberResult(RememberResult{Rule: "Bash=uname -a; id", Path: "/root/tempora.toml", Saved: true})
	c.emitRememberResult(RememberResult{Rule: "Bash(go test ./x)", Path: "tempora.toml", CoveredBy: "Bash(go test:*)"})
	c.emitRememberResult(RememberResult{Rule: "WebFetch", Path: "tempora.toml", Err: errors.New("disk unavailable")})
	...
	if !strings.Contains(got[0].Text, "/root/tempora.toml") {
