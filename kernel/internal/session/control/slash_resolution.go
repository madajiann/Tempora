...
// or skill owns that short name, the built-in stays reachable through Tempora's
// explicit namespace.
...
// QualifiedBuiltinSlashName returns a deterministic Tempora-owned fallback
// that does not displace any existing custom command or skill. The preferred
// form is /tempora:<name>; progressively more explicit names are used only if
...
func QualifiedBuiltinSlashName(name string, commands []command.Command, skills []skill.Skill) string {
	name = strings.TrimPrefix(strings.TrimSpace(name), "/")
	candidates := []string{"tempora:" + name, "tempora:builtin:" + name}
	...
	for suffix := 2; ; suffix++ {
		candidate := fmt.Sprintf("tempora:builtin:%s:%d", name, suffix)
		if ResolveSlashCommandOwner(candidate, commands, skills) == SlashOwnerBuiltin {
			return candidate
		}
	}
...
// IsBuiltinDocsSlash reports whether name should execute the embedded Tempora
