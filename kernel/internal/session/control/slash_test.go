...
func TestSlashArgItems(t *testing.T) {
	data := ArgData{
		...
		ModelRefs:       []string{"deepseek-flash/deepseek-flash", "deepseek-pro/deepseek-v4-pro"},
		CurrentModel:    "deepseek-flash/deepseek-flash",
...
func TestManagementMemoryArchiveRecoveryAcceptsQuotedPathWithSpaces(t *testing.T) {
	userDir := filepath.Join(testenv.TempDir(t), "tempora home with spaces")
	cwd := filepath.Join(testenv.TempDir(t), "project")
	store := memory.StoreFor(userDir, cwd)
