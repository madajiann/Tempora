...
func TestRunProbesReportsMissingCommand(t *testing.T) {
	results := runProbesWithOverrides(context.Background(), []string{"__tempora_missing_probe__ --version"}, nil)
	if len(results) != 1 {
...
func TestRunProbesAllowsStaticEnvAssignment(t *testing.T) {
	...
	results := runProbesWithOverrides(context.Background(), []string{`TEMPORA_PROBE_ENV=ok envtool --version`}, map[string]string{"envtool": toolPath})
	if len(results) != 1 {
		t.Fatalf("results len = %d, want 1", len(results))
	}
...
func TestPrepareProbeCommandSetsCancellationBudget(t *testing.T) {
	cmd := exec.Command("tempora-test-probe")
...
func TestRunProbesFilterSubprocessEnv(t *testing.T) {
	...
	dir := testenv.TempDir(t)
	toolPath := filepath.Join(dir, "envtool")
	body := "#!/bin/sh\nprintf 'tok=%s' \"${TEMPORA_TEST_SECRET_TOKEN:-none}\"\n"
	if err := os.WriteFile(toolPath, []byte(body), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("TEMPORA_TEST_SECRET_TOKEN", "ghp_abcdefghijklmnopqrstuvwxyz")
