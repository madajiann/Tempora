...
func TestCapturePanicWritesBoundedSanitizedReport(t *testing.T) {
	...
	stack := "goroutine 7 [running]:\n" +
		"tempora/internal/runtime/agent.run(" + secret + ")\n" +
		"\t/Users/alice/private-project/internal/runtime/agent/run.go:42 +0x123\n" +
	...
	if !strings.Contains(report.Stack, "tempora/internal/runtime/agent.run(...)") || !strings.Contains(report.Stack, "<path>/run.go:42") {
		t.Fatalf("sanitized stack = %q", report.Stack)
	}
	if report.TopFrame != "tempora/internal/runtime/agent.run <path>/run.go:42" {
