...
func TestMacUpdateHandoffReadinessSurfacesChildStartupFailure(t *testing.T) {
	...
	macHandoffLogPath = func() string { return filepath.Join(tempdir.New(t), "update-helper.log") }
	t.Cleanup(func() {
		readMacUpdateHandoff = originalRead
		macHandoffLogPath = originalLogPath
	})
...
func TestRetainMacHandoffNodeRetriesNameCollision(t *testing.T) {
	path := filepath.Join(tempdir.New(t), "Tempora.app")
...
func TestApplyMacRefusesAnApplicationNobodyStated(t *testing.T) {
	line := MacLine{SelfUpdate: true, BundleID: "io.tempora.studio"}
	for _, tc := range []struct {
		name string
		app  Application
	}{
		{"nothing stated", Application{}},
		{"bundle without a process", Application{Bundle: "/Applications/Tempora Studio.app"}},
...
func TestMacAppBundleFollowsTheExecutableItIsGiven(t *testing.T) {
	...
	bundle := filepath.Join(root, "Tempora Studio.app")
	entry := filepath.Join(bundle, "Contents", "MacOS", "Tempora Studio")
	host := filepath.Join(bundle, "Contents", "Resources", "bin", "tempora-studio-host")
