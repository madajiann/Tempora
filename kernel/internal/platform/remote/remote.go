// Package remote is the SSH transport for Tempora's remote module: host
// resolution ([remote] config + ~/.ssh/config), authentication, host-key
// verification (system known_hosts read-only + a Tempora-managed TOFU file),
...
// through this package — remote workspaces are driven by a `tempora serve`
// process bootstrapped on the remote host (internal/platform/remote/bootstrap) and
...
// TUI, and Studio consume the same surface.
package remote
