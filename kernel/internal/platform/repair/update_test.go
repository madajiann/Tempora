...
func TestFileUpdateRollbackRestoresPreviousBinary(t *testing.T) {
	...
	target := filepath.Join(testenv.TempDir(t), "tempora-desktop")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return filepath.Join(filepath.Dir(target), "tempora-guard"), nil }
...
func TestReconcilePendingFileUpdateQuarantinesForeignTarget(t *testing.T) {
	...
	repairExecutable = func() (string, error) { return filepath.Join(testenv.TempDir(t), "tempora-guard"), nil }
...
func TestPrepareFileUpdateQuarantinesForeignTarget(t *testing.T) {
	...
	repairExecutable = func() (string, error) { return filepath.Join(newLauncherDir, "tempora-guard"), nil }
	newTarget := filepath.Join(newLauncherDir, "tempora-desktop")
...
func TestAppBundleRollbackRestoresWhenLiveBundleIsMissing(t *testing.T) {
	...
	app := filepath.Join(dir, "Tempora.app")
	exe := filepath.Join(app, "Contents", "MacOS", "Tempora")
...
func TestMarkUpdateHealthyRejectsAppBundleDrift(t *testing.T) {
	...
	stagedExe := filepath.Join(stagedApp, "Contents", "MacOS", "Tempora")
...
func TestAppBundleRollbackRejectsChangedBackupTree(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(backup, "Contents", "MacOS", "Tempora"), []byte("tampered"), 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestAppBundleRollbackCompensatesBackupChangedDuringPublish(t *testing.T) {
	...
	rollbackSwapRename = func(oldPath, newPath string) error {
		...
		if oldPath == backup && newPath == app {
			return os.WriteFile(filepath.Join(app, "Contents", "MacOS", "Tempora"), []byte("tampered-during-publish"), 0o700)
		}
		return nil
	}
	...
	rejected, err := filepath.Glob(app + ".tempora-cleanup-*")
	if err != nil || len(rejected) != 1 {
		t.Fatalf("rejected backup bundle = %v, %v", rejected, err)
	}
	rejectedExe := filepath.Join(rejected[0], "Contents", "MacOS", "Tempora")
...
func TestLegacyAppBundleRollbackWithoutTreeDigest(t *testing.T) {
	...
	backupExe := filepath.Join(backup, "Contents", "MacOS", "Tempora")
...
func TestRepairPlanRejectsLegacyAppBundleSymlinkBackupBeforeMutation(t *testing.T) {
	...
	if got, err := os.ReadFile(filepath.Join(external, "Contents", "MacOS", "Tempora")); err != nil || string(got) != "old" {
...
func TestRollbackPendingUpdateRejectsUnexpectedVersion(t *testing.T) {
	...
	target := filepath.Join(dir, "tempora-desktop")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return filepath.Join(dir, "tempora-guard"), nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
...
func TestRollbackPendingUpdateFailsClosedWhenStateLockIsUnavailable(t *testing.T) {
	...
	target := filepath.Join(dir, "tempora-desktop")
	guard := filepath.Join(dir, "tempora-guard")
	originalExecutable := repairExecutable
...
func TestFileUpdateRollbackRestoresReleaseUnit(t *testing.T) {
	...
	missingSibling := filepath.Join(dir, "tempora-update-helper.exe")
...
func TestClearUpdateApplyFailureExactPreservesDifferentTransactionMarker(t *testing.T) {
	...
	first := &UpdateTransaction{
		...
		TargetKind:    "file",
		TargetPath:    filepath.Join(testenv.TempDir(t), "tempora-desktop"),
...
func TestRecordClaimedFileUpdateInstalledBindsOptionalMissingSibling(t *testing.T) {
	...
	target := filepath.Join(dir, "tempora-desktop")
	guard := filepath.Join(dir, "tempora-guard")
	optionalCLI := filepath.Join(dir, "tempora")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return guard, nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
	for path, body := range map[string]string{target: "old-desktop", guard: "old-guard"} {
...
func TestHealthyAppUpdateRejectsBackupDrift(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(backup, "Contents", "MacOS", "Tempora"), []byte("unrelated"), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if got, err := os.ReadFile(filepath.Join(backup, "Contents", "MacOS", "Tempora")); err != nil || string(got) != "unrelated" {
...
func TestFileUpdateRollbackCompensatesOnPartialFailure(t *testing.T) {
	...
	target := filepath.Join(dir, "tempora-desktop")
	guard := filepath.Join(dir, "tempora-guard")
	added := filepath.Join(dir, "tempora-update-helper.exe")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return guard, nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
	...
	rollbackPublishStage = func(oldpath, newpath string) error {
		if strings.HasPrefix(oldpath, guard+".tempora-rollback-stage-") && newpath == guard {
	...
	leftovers, err := filepath.Glob(filepath.Join(dir, "*.tempora-rollback-*"))
...
func TestFileUpdateRollbackPreservesAsideReplacedBeforeCleanup(t *testing.T) {
	...
	rollbackPublishStage = func(oldpath, newpath string) error {
		...
		return os.WriteFile(newpath+".tempora-rollback-aside", []byte("concurrent-aside"), 0o700)
	}
	t.Cleanup(func() { rollbackPublishStage = originalPublish })
	...
	aside := target + ".tempora-rollback-aside"
...
func TestFileUpdateRollbackRetryPreservesRetainedNewBinary(t *testing.T) {
	...
	targetAside := target + ".tempora-rollback-aside"
...
func TestFileUpdateRollbackRechecksWholeReleaseUnitBeforeCommit(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldPath, newPath string) error {
		if !injected && oldPath == guard && newPath == guard+".tempora-rollback-aside" {
			injected = true
	...
	if got, err := os.ReadFile(target + ".tempora-rollback-aside"); err != nil || string(got) != "new-desktop" {
...
func TestRollbackPendingUpdateStateRejectsDriftInsideRetainRename(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldpath, newpath string) error {
		if !injected && oldpath == target && newpath == target+".tempora-rollback-aside" {
			injected = true
...
func TestRollbackPendingAppBundleStateRejectsDriftInsideRetainRename(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldpath, newpath string) error {
		if !injected && oldpath == app && strings.Contains(newpath, ".tempora-failed-") {
			injected = true
...
func TestFileUpdateRollbackStageFailureLeavesInstallUntouched(t *testing.T) {
	...
	originalCopy := rollbackStageCopy
	rollbackStageCopy = func(src, dst string, mode os.FileMode) (string, error) {
		if strings.HasPrefix(dst, guard+".tempora-rollback-stage-") {
...
func TestPrepareFileUpdateDoesNotOverwriteLegacyBackupName(t *testing.T) {
	...
	legacyBackup := filepath.Join(backupDir, "tempora-desktop.previous")
...
func TestFileUpdateRollbackBypassesAndPreservesCrashedStage(t *testing.T) {
	...
	stage := target + ".tempora-rollback-stage-crashed"
...
func TestRetainUpdateRollbackNodeRetriesNameCollision(t *testing.T) {
	target := filepath.Join(testenv.TempDir(t), "Tempora.app")
	...
	retained, err := retainUpdateRollbackNode(target, "tempora-failed")
	if err != nil {
		t.Fatal(err)
	}
	if calls != 2 || !strings.Contains(retained, ".tempora-failed-") {
...
func TestPublishClaimedFileUpdateMemberUsesCompareAndPublish(t *testing.T) {
	...
	aside := target + ".tempora-update-aside-" + UpdateTransactionID(claimed)[:16]
...
func TestPublishClaimedFileUpdateMemberRestoresPreparedFileWhenStageSwapped(t *testing.T) {
	...
	fileUpdateAfterRetain = func(_, _ string) {
		stages, err := filepath.Glob(filepath.Join(dir, ".tempora-desktop.tempora-update-stage-*"))
		if err != nil {
			hookErr = err
			return
		}
	...
	if err == nil || !strings.Contains(err.Error(), "installed tempora-desktop changed") {
	...
	rejected, err := filepath.Glob(target + ".tempora-cleanup-*")
	if err != nil {
		t.Fatal(err)
	}
	if len(rejected) != 1 {
...
func TestFileUpdateRollbackCleansInstalledNodeBoundToTransaction(t *testing.T) {
	...
	if _, err := os.Stat(target + ".tempora-rollback-aside"); !os.IsNotExist(err) {
...
func TestAppBundleRollbackPreservesLiveBundleWithoutReplacementOwnership(t *testing.T) {
	...
	failed, err := filepath.Glob(app + ".tempora-failed-*")
	if err != nil || len(failed) != 1 {
		t.Fatalf("retained unbound bundle = %v, %v", failed, err)
	}
	if got, err := os.ReadFile(filepath.Join(failed[0], "Contents", "MacOS", "Tempora")); err != nil || string(got) != "unbound-live" {
