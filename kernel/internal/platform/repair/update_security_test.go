...
func TestPendingUpdateRejectsTargetOutsideGuardInstall(t *testing.T) {
	...
	backup := filepath.Join(home, "repair", "updates", "tempora-desktop.previous")
	...
	repairExecutable = func() (string, error) { return filepath.Join(guardDir, "tempora-guard"), nil }
...
func TestPendingUpdateRejectsUnexpectedReleaseFile(t *testing.T) {
	...
	bad := []UpdateTransactionFile{
		...
		{TargetPath: filepath.Join(testenv.TempDir(t), "tempora-guard"), BackupPath: backup, SHA256: hash},
		{TargetPath: filepath.Join(dir, "tempora-guard"), BackupPath: filepath.Join(testenv.TempDir(t), "loose.previous"), SHA256: hash},
		{TargetPath: filepath.Join(dir, "tempora-guard"), BackupPath: backup}, // missing hash
		{TargetPath: filepath.Join(dir, "tempora-guard"), BackupPath: backup, SHA256: hash, MissingBefore: true},
...
func TestPendingUpdateRejectsBackupSymlinkEscape(t *testing.T) {
	...
	backup := filepath.Join(repairDir, "updates", "tempora-desktop.previous")
	if err := os.WriteFile(backup, []byte("old"), 0o700); err != nil {
		t.Fatal(err)
	}
	tx := &UpdateTransaction{
...
func TestPendingUpdateAcceptsMissingReleaseSibling(t *testing.T) {
	...
	tx := &UpdateTransaction{
		...
		Files: []UpdateTransactionFile{
			...
			{TargetPath: filepath.Join(dir, "Tempora.exe"), MissingBefore: true},
		},
		CreatedAt: time.Now().UTC().Format(time.RFC3339Nano),
	}
	if err := overwritePendingUpdateForTest(tx); err != nil {
...
func TestPendingUpdateAcceptsWindowsReleaseUnit(t *testing.T) {
	...
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return filepath.Join(dir, "tempora-launcher.exe"), nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
...
func TestPendingUpdateAcceptsLinuxReleaseUnit(t *testing.T) {
	...
	names := []string{"tempora-desktop", "tempora-guard", "tempora"}
	paths := make([]string, 0, len(names))
	for _, name := range names {
		path := filepath.Join(dir, name)
		if err := os.WriteFile(path, []byte(name), 0o700); err != nil {
...
func TestPendingUpdateRejectsHashlessOrPrimaryLessTransactions(t *testing.T) {
	...
	repairExecutable = func() (string, error) { return filepath.Join(dir, "tempora-guard"), nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
	guardBackup := filepath.Join(home, "repair", "updates", "tempora-guard.previous")
	txs := map[string]*UpdateTransaction{
		...
		"release unit omits primary executable": {
			...
			Files:    []UpdateTransactionFile{{TargetPath: filepath.Join(dir, "tempora-guard"), BackupPath: guardBackup, SHA256: "deadbeef"}},
			Platform: runtime.GOOS + "/" + runtime.GOARCH, CreatedAt: time.Now().UTC().Format(time.RFC3339Nano),
		},
	}
...
func TestPendingUpdateRejectsPortableAliasAsPrimaryTarget(t *testing.T) {
	...
	t.Setenv("TEMPORA_HOME", home)
	dir := testenv.TempDir(t)
	target := filepath.Join(dir, "Tempora.exe")
	guard := filepath.Join(dir, "tempora-guard.exe")
	backup := filepath.Join(home, "repair", "updates", "Tempora.exe.previous")
	if err := os.MkdirAll(filepath.Dir(backup), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := ReadPendingUpdate(); err == nil ||
		!strings.Contains(err.Error(), "not a Tempora executable") {
