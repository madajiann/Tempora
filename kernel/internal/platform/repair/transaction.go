...
func validateRepairChange(change RepairChange) error {
	...
	case change.Scope == "project":
		if filepath.Base(target) != "tempora.toml" {
	...
	if filepath.Dir(previous) == filepath.Dir(target) && strings.HasPrefix(filepath.Base(previous), filepath.Base(target)+".tempora-") {
		return nil
	}
...
func UndoLastRepair() (*RepairTransaction, error) {
	...
	for i, v := range slices.Backward(tx.Changes) {
		...
		if _, err := os.Lstat(change.TargetPath); err == nil {
			...
			redo = fmt.Sprintf("%s.tempora-redo-%s-%d", change.TargetPath, now.Format("20060102T150405.000000000Z"), i)
