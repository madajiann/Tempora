...
func Diagnose(ctx context.Context, opts DiagnoseOptions) (DiagnosticReport, error) {
	...
	for _, check := range configReport.Checks {
		if check.Exists && !check.Valid {
			valid = false
			report.add("error", "config.invalid_toml", check.Scope, "Configuration cannot be parsed: "+check.Error, "Run tempora doctor repair; add --project only for a project config.")
		}
	}
	...
	checkDirectoryMode(&report, config.TemporaHomeDir(), "Tempora home")
	if config.MemoryUserDir() != config.TemporaHomeDir() {
		checkDirectoryMode(&report, config.MemoryUserDir(), "Tempora state directory")
	}
	...
	if err != nil {
		report.add("error", "config.load_failed", "runtime", err.Error(), "Run tempora doctor repair, then inspect the reported global or project configuration.")
		return report, nil
	}
...
func validateProviders(report *DiagnosticReport, cfg *config.Config) {
	...
	for i := range cfg.Providers {
		...
		if entry.Kind != "openai" && entry.Kind != "anthropic" {
			report.add("error", "provider.unsupported_kind", scope, fmt.Sprintf("Provider kind %q is not registered by packaged Tempora builds.", entry.Kind), "Use openai or anthropic compatibility.")
		}
		...
		if entry.APIKeyEnv != "" && !config.IsValidCredentialKey(entry.APIKeyEnv) {
			...
			report.add("warning", "provider.missing_key", scope, "The configured API key is missing from the global Tempora credential file.", "Add the key in Settings or <Tempora home>/.env.")
		}
	}
...
func checkDirectoryMode(report *DiagnosticReport, path, label string) {
	if path == "" {
		report.add("warning", "directory.unavailable", label, label+" path is unavailable.", "Restore the user home or Tempora path environment configuration.")
		return
	}
	st, err := os.Stat(path)
	if err != nil {
...
func checkDerivedJSON(report *DiagnosticReport) {
	...
	for name, path := range paths {
		...
		if !json.Valid(b) {
			report.add("warning", "derived.invalid_json", "derived:"+name, fmt.Sprintf("Derived desktop state %s is malformed.", filepath.Base(path)), "Run tempora doctor repair, or delete the broken derived file after backing it up.")
		}
	}
}
		...
		if err == nil {
			...
			if callErr != nil {
				...
				case resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden:
					report.add("error", "network.authentication_failed", "provider:"+entry.Name, fmt.Sprintf("Provider rejected credentials with HTTP %d.", resp.StatusCode), "Update the provider credential in Tempora Settings.")
