...
func TestApplyRepairPlanPreservesUncooperativeWriteAfterRename(t *testing.T) {
	...
	quarantines, err := filepath.Glob(tabs + ".tempora-rebuild-*")
	...
	redos, err := filepath.Glob(tabs + ".tempora-redo-*")
...
func TestProjectRepairPlanRequiresExplicitPermission(t *testing.T) {
	...
	if _, err := PreviewRepairPlan(plan, ApplyPlanOptions{Root: testenv.TempDir(t)}); err == nil || !strings.Contains(err.Error(), "--allow-project") {
...
func TestApplyRepairPlanMultiActionUndoRevertsWholePlan(t *testing.T) {
	...
	if _, err := ApplyRepairPlan(plan, ApplyPlanOptions{Root: testenv.TempDir(t)}); err != nil {
		t.Fatal(err)
	}
...
func TestReconcilePreparedRepairRechecksSourceAfterTargetLock(t *testing.T) {
	...
	tx.Changes = append(tx.Changes, preparedRepairChangeForPrevious(
		"derived:tabs",
		tabs,
		tabs+".tempora-rebuild-20260729T000000Z",
	))
...
func TestProjectRepairPlanDoesNotRepairGlobalConfig(t *testing.T) {
	...
	root := testenv.TempDir(t)
	global := filepath.Join(home, "config.toml")
	project := filepath.Join(root, "tempora.toml")
...
func TestRepairPlanPreviewIDRejectsSameContentDifferentTargets(t *testing.T) {
	...
	for _, root := range []string{rootA, rootB} {
		if err := os.WriteFile(filepath.Join(root, "tempora.toml"), content, 0o600); err != nil {
			t.Fatal(err)
		}
	}
	plan := RepairPlan{SchemaVersion: 1, Summary: "project", Actions: []RepairPlanAction{{Type: "repair_config", Scope: "project", Reason: "bad toml"}}}
	...
	if got, err := os.ReadFile(filepath.Join(rootB, "tempora.toml")); err != nil || string(got) != string(content) {
		t.Fatalf("project B was modified without confirmation: %q, %v", got, err)
	}
	if _, err := os.Stat(filepath.Join(rootA, "tempora.toml")); err != nil {
...
func TestRepairMutationLockConvergesSymlinkAliases(t *testing.T) {
	...
	realFile := filepath.Join(realDir, "tempora.toml")
	aliasFile := filepath.Join(linkDir, "tempora.toml")
...
func TestRepairPlanPreviewIDDistinguishesLeafSymlinkTargets(t *testing.T) {
	...
	for _, root := range []string{rootA, rootB} {
		...
		if err := os.Symlink(shared, filepath.Join(root, "tempora.toml")); err != nil {
			t.Fatal(err)
		}
	}
	plan := RepairPlan{SchemaVersion: 1, Summary: "project", Actions: []RepairPlanAction{{Type: "repair_config", Scope: "project", Reason: "bad toml"}}}
	...
	if _, err := os.Lstat(filepath.Join(rootA, "tempora.toml")); !os.IsNotExist(err) {
		t.Fatalf("project A symlink was not quarantined: %v", err)
	}
	if _, err := os.Lstat(filepath.Join(rootB, "tempora.toml")); err != nil {
...
func TestApplyRepairPlanRejectsAppBundleInteriorDrift(t *testing.T) {
	...
	app := filepath.Join(base, "Tempora.app")
	...
	if err := os.WriteFile(filepath.Join(backup, "Contents", "MacOS", "Tempora"), []byte("backup-old"), 0o700); err != nil {
		t.Fatal(err)
	}
