...
import (
	...
	textdiff "tempora/internal/base/diff"
...
func PreviewRepairPlan(plan RepairPlan, opts ApplyPlanOptions) ([]RepairPlanPreview, error) {
	...
	for i, action := range plan.Actions {
		...
		case "rollback_update":
			...
			preview.Description = fmt.Sprintf("Restore Tempora %s over probationary %s", tx.FromVersion, tx.ToVersion)
...
func projectConfigPath(root string) string {
	...
	return filepath.Join(root, "tempora.toml")
}
