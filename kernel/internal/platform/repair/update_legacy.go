...
// the original transaction is archived under Tempora repair state. Neither is
...
func archiveSupersededAppBundleBackup(tx *UpdateTransaction, transactionID string) (string, string, error) {
	...
	base := fmt.Sprintf("%s.tempora-retired-%s-%s", tx.BackupPath, shortID, time.Now().UTC().Format("20060102T150405.000000000Z"))
