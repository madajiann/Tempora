	...
	t.mutate(func(t *statusTelemetry) {
		...
		default:
			...
			case errors.As(runErr, &readinessErr):
				...
				t.turnOutcome = TemporaTurnOutcome{Kind: "paused", Reason: clipStatusText(readinessErr.Error(), 2_048)}
				t.finalReadiness.Risks = redactStatusTexts(readinessErr.Missing, 2_048)
				eventName = "pause"
			case runErr != nil:
				t.phase = "error"
				t.turnOutcome = TemporaTurnOutcome{Kind: "error", Reason: clipStatusText(runErr.Error(), 2_048)}
