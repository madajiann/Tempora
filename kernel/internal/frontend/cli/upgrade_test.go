...
func TestVerifyChecksum(t *testing.T) {
	...
	t.Run("match", func(t *testing.T) {
		checksumFile := fmt.Appendf(nil, "%s  tempora-linux-amd64.tar.gz\n", hash)
		if err := verifyChecksum(content, "tempora-linux-amd64.tar.gz", checksumFile); err != nil {
	...
	t.Run("not found", func(t *testing.T) {
		checksumFile := fmt.Appendf(nil, "%s  tempora-darwin-arm64.tar.gz\n", hash)
		if err := verifyChecksum(content, "tempora-linux-amd64.tar.gz", checksumFile); err == nil {
...
func TestExtractFromTarGz(t *testing.T) {
	// Build a .tar.gz in memory containing a "tempora" entry.
	...
	got, err := extractFromTarGz(buf.Bytes(), "tempora")
	if err != nil {
...
func TestExtractFromTarGz_Nested(t *testing.T) {
	...
	if err := tw.WriteHeader(&tar.Header{
		Name: "tempora-linux-amd64/tempora",
		Mode: 0o755,
		Size: int64(len(body)),
	}); err != nil {
		t.Fatal(err)
...
func TestExtractFromTarGz_NotFound(t *testing.T) {
	...
	_, err := extractFromTarGz(buf.Bytes(), "tempora")
	if err == nil {
...
func completeCLIRelease(tag string, prerelease bool) ghRelease {
	...
	for _, name := range requiredCLIAssets {
		assets = append(assets, ghAsset{
			Name:               name,
			BrowserDownloadURL: fmt.Sprintf("https://github.com/esengine/DeepSeek-Tempora/releases/download/%s/%s", tag, name),
			Size:               42,
		})
	}
...
func TestPickCLIRelease(t *testing.T) {
	...
	insecure.Assets[0].BrowserDownloadURL = "http://example.invalid/tempora.tar.gz"
	...
	spoofed.Assets[0].BrowserDownloadURL = "https://github.com@evil.invalid/esengine/DeepSeek-Tempora/releases/download/v1.7.0/tempora-darwin-amd64.tar.gz"
	...
	wrongTag.Assets[0].BrowserDownloadURL = "https://github.com/esengine/DeepSeek-Tempora/releases/download/v1.6.0/tempora-darwin-amd64.tar.gz"
...
func TestFindCLIPlatformAssetRequiresExactArchiveName(t *testing.T) {
	...
	release.Assets = append([]ghAsset{{
		Name:               "tempora-linux-amd64.signature",
		BrowserDownloadURL: "https://example.invalid/signature",
	}}, release.Assets...)

	asset := findCLIPlatformAsset(&release, "linux", "amd64")
	if asset == nil || asset.Name != "tempora-linux-amd64.tar.gz" {
		t.Fatalf("Linux asset = %+v, want exact tar.gz archive", asset)
	}
	if got := cliPlatformAssetName("windows", "arm64"); got != "tempora-windows-arm64.zip" {
		t.Fatalf("Windows asset name = %q, want tempora-windows-arm64.zip", got)
	}

	expectedURL := asset.BrowserDownloadURL
	release.Assets = append([]ghAsset{{
		Name:               "tempora-linux-amd64.tar.gz",
		BrowserDownloadURL: "https://evil.invalid/tempora-linux-amd64.tar.gz",
	}}, release.Assets...)
	asset = findCLIPlatformAsset(&release, "linux", "amd64")
	...
	for i := range release.Assets {
		if release.Assets[i].Name == "tempora-linux-amd64.tar.gz" &&
...
func TestFetchCLIReleasePointer(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Accept") != "application/json" || r.Header.Get("User-Agent") != "tempora-cli" {
