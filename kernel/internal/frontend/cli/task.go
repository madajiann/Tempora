...
// CLI defaults to a FileStore backed by .tempora/tasks under the
// project directory.
...
func taskCommand(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: tempora task <list|show|monitor|status|events|stop|cancel|requeue|open-session|tmux> [flags]")
		return 2
	}
	store := taskStore
	if store == nil {
...
func taskMonitorCommand(store taskmonitor.Store, args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: tempora task monitor <list|status|events|stop|cancel|requeue|open-session> [flags]")
		return 2
	}
	switch args[0] {
	case "list":
...
func taskTmuxCmd(store taskmonitor.Store, args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: tempora task tmux <attach|status|open|detach>")
		return 2
	}
