...
func TestParseCommandFlagsSuccessProceedsSilently(t *testing.T) {
	...
	var proceed bool
	stderr := captureStderr(t, func() {
		code, proceed = parseCommandFlags(fs, []string{"--name", "tempora"})
	})
	if code != 0 || !proceed || *name != "tempora" {
		t.Fatalf("parseCommandFlags success = (%d, %v, %q), want (0, true, tempora)", code, proceed, *name)
	}
	if stderr != "" {
