...
func parseCLIReleaseChannel(value string) (cliReleaseChannel, error) {
	...
	default:
		return "", fmt.Errorf("release channel %q is unsupported; Tempora now uses the official release", value)
	}
}
...
// upgradeCommand handles `tempora upgrade` (and `tempora update`).
func upgradeCommand(args []string, version string) int {
	...
	base := fmt.Sprintf("tempora-%s-%s", runtime.GOOS, runtime.GOARCH)
...
func cliPlatformAssetName(goos, goarch string) string {
	...
	return fmt.Sprintf("tempora-%s-%s%s", goos, goarch, suffix)
}
...
func fetchLatestRelease(c *http.Client, channel cliReleaseChannel) (*ghRelease, error) {
	...
	req.Header.Set("User-Agent", "tempora-cli")
...
// extractBinary pulls the "tempora" binary from a .tar.gz or .zip archive.
...
// rename it aside to .tempora.old first, then place the new binary.
