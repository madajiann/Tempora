...
func reviewCommand(args []string) int {
	...
	// every `tempora review` invocation.
	cfg, err := config.Load()
	if err != nil {
...
func buildReviewSubagentRegistry(reviewSk skill.Skill, cfg *config.Config, root string) *tool.Registry {
	...
	// [sandbox] config, so `tempora review` previously read forbid_read
	...
	if reviewSk.ReadOnly {
		...
		// permission-classified read-only policy) so `tempora review` is not a
		// writable backdoor.
