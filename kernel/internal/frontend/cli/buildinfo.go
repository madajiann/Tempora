...
// `tempora version` / `tempora --version`. Release builds may only fill
...
//   - `tempora --version` / `-v` stay single-line: "tempora <version>"
//   - `tempora version --verbose` prints structured fields
//   - `tempora version --json` prints a JSON object
...
// versionCommand handles `tempora version [flags]`. When allowFlags is false
...
func versionUsage(w *os.File) {
  ...
  tempora version
  tempora version --verbose
  tempora version --json
  tempora --version
  tempora -v

--version / -v always print a single line (tempora <version>).
version --verbose prints build metadata; version --json prints the same as JSON.`)
}

func (b BuildInfo) singleLine() string {
	return "tempora " + strings.TrimSpace(b.withDefaults().Version)
}
