...
const serveMissingKeyHelperEnv = "TEMPORA_TEST_SERVE_MISSING_KEY_HELPER"
...
func TestServeStartsWithMissingProviderKey(t *testing.T) {
	if os.Getenv(serveMissingKeyHelperEnv) == "1" {
		...
		if err := os.Setenv("TEMPORA_HOME", os.Getenv("TEMPORA_TEST_SERVE_HOME")); err != nil {
			os.Exit(2)
		}
		code := runServe([]string{
			"--model", "remote-demo/model-a",
			"--addr", "127.0.0.1:0",
			"--port-file", os.Getenv("TEMPORA_TEST_SERVE_PORT_FILE"),
			"--auth", "token",
...
models = ["model-a"]
default = "model-a"
api_key_env = "TEMPORA_TEST_REMOTE_MISSING_KEY"
`
	...
	cmd.Env = replaceServeTestEnv(os.Environ(),
		...
		"TEMPORA_HOME="+home,
		"TEMPORA_TEST_SERVE_HOME="+home,
		"TEMPORA_CREDENTIALS_STORE=file",
		"TEMPORA_TEST_REMOTE_MISSING_KEY=",
		"TEMPORA_TEST_SERVE_PORT_FILE="+portFile,
	)
	...
	if !bytes.Contains(body, []byte(`"required":true`)) || !bytes.Contains(body, []byte("TEMPORA_TEST_REMOTE_MISSING_KEY")) {
