...
import (
	...
	"tempora/internal/ext/mcpregistry"
	"tempora/internal/ext/mcpsetup"
)

// mcp.go is the `tempora mcp` subcommand (config-only; takes effect next
// session) plus the in-chat /mcp verbs (which hot-connect via the controller).
// The grammar itself lives in internal/ext/mcpsetup, shared with the frontends that
...
func mcpEnableCLI(args []string, enabled bool) int {
	if len(args) == 0 {
		...
		fmt.Fprintf(os.Stderr, "usage: tempora mcp %s <name> [--global]\n", action)
		return 2
	}
