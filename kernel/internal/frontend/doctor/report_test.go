...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("TEMPORA_TEST_SECRET", "sk-live-secret")
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	t.Setenv("TEMPORA_HOME", filepath.Join(testenv.TempDir(t), "tempora"))
	...
	if !strings.Contains(text, "tempora 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config tempora.toml: parse boom"}})
...
func TestHomeIsolationWarningDetectsMismatch(t *testing.T) {
	...
	if !strings.Contains(got, "TEMPORA_HOME") {
		t.Fatalf("warning = %q, want TEMPORA_HOME guidance", got)
	}
	...
	if got := homeIsolationWarning(); got != "" {
		t.Fatalf("TEMPORA_HOME set should silence warning, got %q", got)
	}
}
