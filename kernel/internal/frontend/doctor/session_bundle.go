...
func resolveSessionBundlePath(ref string) (string, error) {
	...
	if len(matches) == 0 {
		return "", fmt.Errorf("session %q not found under Tempora session directories", branch)
	}
	slices.Sort(matches)
	if len(matches) > 1 {
	...
	if root != "" {
		...
		if rootErr == nil && pathErr == nil {
			if rel, err := filepath.Rel(absRoot, absPath); err == nil && rel != ".." && !strings.HasPrefix(rel, ".."+string(os.PathSeparator)) {
				...
				return filepath.ToSlash(filepath.Join("<TEMPORA_HOME>", rel))
			}
		}
	}
...
func redactSessionBundleText(text string) string {
	...
	if root := strings.TrimSpace(config.MemoryUserDir()); root != "" {
		text = replacePathPrefix(text, root, "<TEMPORA_HOME>")
		if absRoot, err := filepath.Abs(root); err == nil {
			text = replacePathPrefix(text, absRoot, "<TEMPORA_HOME>")
		}
	}
...
func defaultSessionBundlePath(branch string) string {
	...
	return filepath.Join(os.TempDir(), "tempora-session-"+branch+"-diag.zip")
}
