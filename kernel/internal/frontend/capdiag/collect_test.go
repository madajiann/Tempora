...
import (
	...
	"tempora/internal/frontend/capdiag"
)
...
func TestCollectStaticNoNetworkSideEffects(t *testing.T) {
	...
	write(t, filepath.Join(root, ".tempora", "skills", "demo", "SKILL.md"),
		"---\nname: demo\ndescription: project demo\n---\nbody\n")
	write(t, filepath.Join(home, ".tempora", "skills", "demo", "SKILL.md"),
		"---\nname: demo\ndescription: global demo\n---\nbody\n")
	write(t, filepath.Join(root, ".tempora", "skills", "nodesc", "SKILL.md"),
		"---\nname: nodesc\n---\nbody\n")
	write(t, filepath.Join(root, ".tempora", "commands", "hi.md"),
		"---\ndescription: project hi\n---\nP $ARGUMENTS\n")
	write(t, filepath.Join(home, ".tempora", "commands", "hi.md"),
		"---\ndescription: home hi\n---\nH $ARGUMENTS\n")

	// Project hooks load automatically.
	write(t, filepath.Join(root, ".tempora", "settings.json"), `{
  "hooks": {
	...
	write(t, filepath.Join(root, "tempora.toml"), `
[[plugins]]
name = "broken"
type = "stdio"
command = "definitely-not-a-real-binary-xyzzy-tempora"
auto_start = false
`)
	...
	r := capdiag.Collect(capdiag.Options{
		Root:            root,
		HomeDir:         home,
		TemporaHomeDir: filepath.Join(home, ".tempora"),
		Live:            false,
	})
	...
	mcp := r.MCP.Servers[0]
	if !mcp.Effective || mcp.Source != "project_config" || mcp.SourcePath != "<workspace>/tempora.toml" {
	...
	r2 := capdiag.Collect(capdiag.Options{
		Root: root, HomeDir: home, TemporaHomeDir: filepath.Join(home, ".tempora"),
	})
...
func TestCollectUsesExactTemporaHomeForGlobalHooks(t *testing.T) {
	root := testenv.TempDir(t)
	home := testenv.TempDir(t)
	temporaHome := filepath.Join(home, "AppData", "Roaming", "tempora")
	write(t, filepath.Join(temporaHome, "settings.json"), `{
  "hooks": {
	...
	for _, source := range report.Hooks.Sources {
		if source.Scope == "global" {
			if source.Status != "ok" || source.HookCount != 1 {
				t.Fatalf("global hook source = %+v, want exact Tempora home settings", source)
			}
			if source.Path != "<tempora-home>/settings.json" && source.Path != "<tempora-home>\\settings.json" {
				// displayPath uses ToSlash
				if !strings.Contains(source.Path, "<tempora-home>") {
					t.Fatalf("global path = %q, want <tempora-home> prefix", source.Path)
				}
			}
			return
		}
...
func TestLoadForRootReadOnlyDoesNotRewriteTier(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("TEMPORA_HOME", filepath.Join(home, ".tempora"))
	userCfg := filepath.Join(home, ".tempora", "config.toml")
...
func TestPluginPackageCommandsAreReported(t *testing.T) {
	...
	pluginRoot := filepath.Join(temporaHome, "plugins", "demo")
	write(t, filepath.Join(pluginRoot, pluginpkg.NativeManifest), `{"apiVersion":"tempora.io/plugin/v2","name":"demo","commands":["commands"]}`)
