...
const wellFormedPlanArgs = `{
  ...
  "assumptions":[{"text":"warm caches are disposable","confirm":"rg cacheKey internal/contract/provider"}],
  "steps":[
    {"id":"p1","title":"thread the model ref through","verified_files":["internal/contract/provider/cache.go"],"candidate_files":["internal/assembly/boot/boot.go"]},
    {"id":"s1","parent_id":"p1","title":"extend cacheKey",
     ...
     "verification":[{"command":"go test ./internal/contract/provider/","expect":"all green"}]},
...
func TestSubmitPlanRecordsAStructuredPlan(t *testing.T) {
	...
	if plan.Steps[0].VerifiedFiles[0] != "internal/contract/provider/cache.go" || plan.Steps[0].CandidateFiles[0] != "internal/assembly/boot/boot.go" {
