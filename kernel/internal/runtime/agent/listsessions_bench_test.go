...
// directory. It is gated behind TEMPORA_BENCH=1 so it never runs (or writes
// hundreds of MB) during a normal `go test`.
//
//	TEMPORA_BENCH=1 go test ./internal/runtime/agent -run TestListSessionsScalingManual -v
func TestListSessionsScalingManual(t *testing.T) {
	if os.Getenv("TEMPORA_BENCH") != "1" {
		t.Skip("set TEMPORA_BENCH=1 to run the scaling benchmark")
	}
...
//
//	TEMPORA_BENCH=1 go test ./internal/runtime/agent -run TestColdStartProjectTreeManual -v
func TestColdStartProjectTreeManual(t *testing.T) {
	if os.Getenv("TEMPORA_BENCH") != "1" {
		t.Skip("set TEMPORA_BENCH=1 to run the cold-start benchmark")
	}
