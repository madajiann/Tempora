...
func TestOutcomeVersusLegacyOnTheRunawayShape(t *testing.T) {
	...
	realWork := []scriptedCall{
		{"read_file", `{"path":"internal/runtime/agent/agent.go"}`},
		{"read_file", `{"path":"internal/runtime/agent/run_loop.go"}`},
		{"edit_file", `{"path":"internal/runtime/agent/run_loop.go"}`},
		{"bash", `{"command":"go test ./internal/runtime/agent/"}`},
		{"edit_file", `{"path":"internal/runtime/agent/agent.go"}`},
		{"bash", `{"command":"go test ./internal/runtime/agent/"}`},
	}
