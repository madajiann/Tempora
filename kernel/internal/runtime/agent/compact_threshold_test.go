...
func TestCompactTriggerIndependentOfOutputBudget(t *testing.T) {
	...
	trigger := a.window().compactTrigger()
	...
	if got := a.window().compactTrigger(); got != want {
		t.Fatalf("trigger after larger output budget = %d, want %d", got, want)
	}
	hard := a.window().hardInputCeiling()
...
func TestRecentTailBudgetClamp(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if got := a.window().recentTailBudget(); got != tc.want {
...
func TestRecentTailUsesWindowClampUnderCapacityDefault(t *testing.T) {
	...
	if got, want := a.window().recentTailBudget(), maxRecentTailTokens; got != want {
...
func TestCheckpointCeilingAndExceptionalSavings(t *testing.T) {
	a := &Agent{agentConfig: agentConfig{contextWindow: 1_000_000, compactRatio: 0.85,
		budgets: CompactionBudgets{ContextSoftLimitTokens: -1}}}
	if got := a.window().checkpointCeiling(); got != 500_000 {
		t.Fatalf("checkpointCeiling = %d, want 500000", got)
	}
	if got := a.window().exceptionalMinimumSavings(); got != 250_000 {
		t.Fatalf("exceptionalMinimumSavings = %d, want 250000", got)
	}
	if got := a.window().compactTrigger(); got != 850_000 {
...
func TestAcceptCheckpointCandidateRules(t *testing.T) {
	...
	if err := a.window().acceptCheckpointCandidate(CompactionTriggerPressure, compactionScope{}, 850_000, 180_000, 50_000); err != nil {
		t.Fatalf("20%% candidate: %v", err)
	}
	// 51% candidate: reject.
	if err := a.window().acceptCheckpointCandidate(CompactionTriggerPressure, compactionScope{}, 850_000, 510_000, 50_000); err == nil {
		t.Fatal("51% candidate should be rejected")
	}
	// Fixed prefix > 50% with enough savings: accept.
	if err := a.window().acceptCheckpointCandidate(CompactionTriggerPressure, compactionScope{}, 900_000, 600_000, 520_000); err != nil {
		t.Fatalf("fixed-prefix exception: %v", err)
	}
	// Fixed prefix > 50% without 25% savings: reject.
	if err := a.window().acceptCheckpointCandidate(CompactionTriggerPressure, compactionScope{}, 600_000, 550_000, 520_000); err == nil {
		t.Fatal("fixed-prefix without savings should reject")
	}
	// Manual below trigger: accept any reduction.
	if err := a.window().acceptCheckpointCandidate(CompactionTriggerManual, compactionScope{}, 100_000, 80_000, 10_000); err != nil {
