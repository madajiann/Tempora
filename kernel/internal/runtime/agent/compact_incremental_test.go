...
func TestIncrementalFoldSummarizesPriorDigestPlusNewWork(t *testing.T) {
	...
	if err := a.window().compact(context.Background(), CompactionTriggerManual, "", compactionScope{ignoreThreshold: true, ignoreEconomics: true}); err != nil {
		t.Fatalf("first compact: %v", err)
	}
	if !hasCompactionSummary(a.window().modelVisibleMessages()) {
