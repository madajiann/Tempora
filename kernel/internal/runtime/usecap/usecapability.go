...
func (o *onDemandMCPTool) executeWithImages(ctx context.Context, args json.RawMessage) (string, []string, error) {
	...
	if !plugin.MCPRuntimeSpecMatches(spec, o.spec) {
		return "", nil, fmt.Errorf("MCP server %q runtime identity changed after resolution; retry so Tempora can bind the current configuration", o.server)
	}
	...
	if tool.HasNonDestructiveMCPExecutionIntent(ctx) {
		if !tool.IsMCPServerAuthorized(target) || tool.HasMCPDestructiveHint(target) {
			return "", nil, fmt.Errorf("MCP server %q changed the authorization or destructive classification for tool %q; the call was blocked before dispatch — retry so Tempora can re-apply the current Planner MCP safety boundary", o.server, o.raw)
		}
	}
