...
func richPlan() Plan {
	return Plan{
		...
		Assumptions: []Assumption{{Text: "- warm caches are disposable", Confirm: "rg cacheKey internal/contract/provider"}},
		NonGoals:    []string{"1. rewriting the retry loop"},
		Steps: []Step{
			{
				ID: "p1", Title: "thread the model ref through",
				VerifiedFiles:  []string{"internal/contract/provider/cache.go"},
				CandidateFiles: []string{"internal/assembly/boot/boot.go"},
			...
			},
			{
				...
				Verification: []Verification{{Command: "go test ./internal/contract/provider/", Expect: "all green"}},
			},
...
func TestRenderKeepsStepDetailOffTheList(t *testing.T) {
	...
	for _, want := range []string{
		"   verified: internal/contract/provider/cache.go",
		"   candidate: internal/assembly/boot/boot.go",
		...
		"     verify: go test ./internal/contract/provider/ — all green",
	} {
