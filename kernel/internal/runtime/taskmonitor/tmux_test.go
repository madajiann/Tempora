...
func TestTmuxAdapterAttachIdempotent(t *testing.T) {
	...
	mock := &tmuxMock{}
	a := NewTmuxAdapterWithRunner(s, ".tempora/tasks", mock)
...
func TestTmuxAdapterBoundsGeneratedSessionName(t *testing.T) {
	...
	a := NewTmuxAdapterWithRunner(s, ".tempora/tasks", &tmuxMock{})
...
func TestTmuxAdapterUnavailableDoesNotChangeTask(t *testing.T) {
	...
	root := testenv.TempDir(t)
	seedTmuxTask(t, s, root)
	a := NewTmuxAdapterWithRunner(s, ".tempora/tasks", nil)
...
func TestTmuxAdapterStaleAndDetach(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, ".tempora/tasks/.tmux/t1.json")); !os.IsNotExist(err) {
		t.Fatalf("mapping was not removed: %v", err)
	}
}

func TestTmuxAdapterRejectsTaskPathTraversal(t *testing.T) {
	a := NewTmuxAdapterWithRunner(NewInMemoryStore(), ".tempora/tasks", &tmuxMock{})
...
func TestTmuxAdapterRejectsSymlinkMappingDirectory(t *testing.T) {
	...
	a := NewTmuxAdapterWithRunner(s, ".tempora/tasks", attachRunner)
	...
	readRunner := &tmuxMock{}
	a = NewTmuxAdapterWithRunner(s, ".tempora/tasks", readRunner)
...
func TestTmuxAdapterRejectsForgedMappingIdentityWithoutTmuxCalls(t *testing.T) {
	...
	path := filepath.Join(project, ".tempora", "tasks", ".tmux", "t1.json")
...
func TestTmuxAdapterWritesPrivateMappingFiles(t *testing.T) {
	...
	for path, want := range map[string]os.FileMode{
		...
		filepath.Join(project, ".tempora", "tasks", ".tmux"):            0o700,
		filepath.Join(project, ".tempora", "tasks", ".tmux", "t1.json"): 0o600,
	} {
