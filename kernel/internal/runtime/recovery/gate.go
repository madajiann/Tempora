...
// clearNoProgressLocked clears the observing task's local counters after real
// mutation/verification success. Caller holds g.mu.
...
func (g *Gate) reviewOrEscalate(ctx context.Context, taskID, fp string, gen uint64, proposal Proposal, failure *FailureEvent, diagNotes []string) (Decision, error) {
	...
	if g.opts.Reviewer != nil {
		...
		g.recordReviewBlock(taskID, verdict)
		return Decision{
			Allow:      false,
			Blocked:    true,
			Message:    reviewerBlockerMessage(verdict),
			Generation: gen,
		}, nil
	}
	if proposal.PlanTransition {
...
func (g *Gate) recordReviewBlock(taskID string, verdict ReviewVerdict) {
	g.mu.Lock()
	st := g.ensureTaskLocked(taskID)
	if st.lastFailure != nil {
...
func reviewerBlockerMessage(verdict ReviewVerdict) string {
	reason := firstNonEmpty(verdict.Rationale, "the proposal could not be classified as a bounded plan continuation")
	return fmt.Sprintf(
		"blocked: Auto plan reviewer could not accept this transition: %s. Continue the current plan, propose a task-aligned plan, or ask the user about a genuine product choice.",
		reason,
	)
}
