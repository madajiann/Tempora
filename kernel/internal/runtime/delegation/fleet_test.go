...
func TestBackgroundFleetProgressLifecycleUsesStableIDs(t *testing.T) {
	...
	manager := jobs.NewManager(event.Discard)
	defer manager.Close()
	ctx := agent.WithCallContext(context.Background(), "fleet-call", rec, nil, false)
	ctx = jobs.WithManager(ctx, manager)
...
func TestBackgroundFleetRegistersReservationWhileItemsAreQueued(t *testing.T) {
	...
	manager := jobs.NewManager(event.Discard)
	defer manager.Close()
	ctx := agent.WithCallContext(context.Background(), "queued-fleet", event.Discard, nil, false)
	ctx = jobs.WithManager(ctx, manager)
...
func TestFleetRejectsSingleTaskAndPathConflict(t *testing.T) {
	...
	_, err = f.Execute(agent.WithCallContext(context.Background(), "fleet-call", event.Discard, nil, false), args)
...
func TestFleetAggregatePreservesEveryReferenceUnderToolLimit(t *testing.T) {
	...
	if len(out) > agent.MaxToolOutputBytes {
		t.Fatalf("bounded fleet aggregate still hit generic truncation: %d bytes over the %d cap", len(out), agent.MaxToolOutputBytes)
	}
	for i := range results {
