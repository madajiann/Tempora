...
func TestSubagentToolRegistryFiltersUnavailableToolsAndWrapsBash(t *testing.T) {
	...
	sub := agent.SubagentToolRegistry(parent, nil)
...
func TestSubagentToolRegistryRestrictsCapabilityProxyToAllowedMCPIDs(t *testing.T) {
	...
	for _, sub := range []*tool.Registry{
		agent.SubagentToolRegistry(parent, []string{allowedID}),
		agent.ReadOnlySubagentToolRegistry(parent, []string{allowedID}),
	} {
	...
	converted := agent.SubagentToolRegistry(parent, []string{"mcp__figma__search"})
...
func TestReadOnlySubagentToolRegistryAllowsOnlyReadOnlyDelegationBeforeDepthLimit(t *testing.T) {
	...
	firstLayer := agent.ReadOnlySubagentToolRegistryForDepth(parent, nil, 1, 2)
	...
	secondLayer := agent.ReadOnlySubagentToolRegistryForDepth(parent, nil, 2, 2)
...
func TestCustomProfileAllowlistRestrictsMCPTools(t *testing.T) {
	...
	general := agent.SubagentToolRegistry(parent, []string{"read_file"})
	...
	explicit := agent.SubagentToolRegistry(parent, []string{"mcp__chrome__*"})
	...
	ro := agent.ReadOnlySubagentToolRegistry(parent, []string{"read_file"})
	if _, ok := ro.Get("use_capability"); ok {
		t.Fatalf("read-only built-in-only profile should not install MCP proxy; got %v", ro.Names())
	}

	explicitRO := agent.ReadOnlySubagentToolRegistry(parent, []string{"mcp__chrome__*"})
...
func TestMCPToolAvailabilityAcrossGeneralAndReadOnlySubagents(t *testing.T) {
	...
	general := agent.SubagentToolRegistry(parent, nil)
	...
	ro := agent.ReadOnlySubagentToolRegistry(parent, nil)
	...
	if _, ok := agent.FilterReadOnlyRegistry(parent).Get("mcp__srv__tool"); !ok {
		t.Fatalf("FilterReadOnlyRegistry should keep authorized read-only MCP for non-planner surfaces; got %v", agent.FilterReadOnlyRegistry(parent).Names())
	}
	if _, ok := agent.PlannerToolRegistry(parent).Get("mcp__srv__tool"); ok {
		t.Fatalf("PlannerToolRegistry must strip direct MCP: %v", agent.PlannerToolRegistry(parent).Names())
	}
	if _, ok := agent.PlannerToolRegistry(parent).Get("use_capability"); !ok {
		t.Fatalf("PlannerToolRegistry must keep use_capability: %v", agent.PlannerToolRegistry(parent).Names())
	}
}
...
func TestRestrictedCapabilityProxyDescriptionIsStable(t *testing.T) {
	...
	before := agent.SubagentToolRegistry(parent, []string{"mcp__alpha__*"})
	...
	after := agent.SubagentToolRegistry(parent, []string{"mcp__alpha__*"})
...
func TestRestrictedCapabilityProxyListFiltersServers(t *testing.T) {
	...
	sub := agent.SubagentToolRegistry(parent, []string{"mcp__alpha__search"})
...
func TestMalformedCapabilityAllowlistDoesNotInstallProxy(t *testing.T) {
	...
	for _, allow := range [][]string{
		...
		sub := agent.SubagentToolRegistry(parent, allow)
		if _, ok := sub.Get("use_capability"); ok {
