...
func testTaskContext() context.Context {
	return agent.WithParentSession(context.Background(), "parent-session")
}
...
func TestTaskToolReturnsSubAgentFinalAnswer(t *testing.T) {
	...
	if got := lastUser(sub.lastReq); !strings.Contains(got, `<subagent-context event="SubagentStart">`) || !strings.Contains(got, "find callers of Foo") || !strings.Contains(got, agent.CompleteSubtaskContract) {
...
func TestTaskToolInjectsWorkspaceContextIntoSubagentPrompt(t *testing.T) {
	...
	if !strings.Contains(got, `<workspace-context event="SubagentWorkspace">`) ||
		...
		!strings.Contains(got, "inspect project") || !strings.Contains(got, agent.CompleteSubtaskContract) {
...
func TestTaskToolPropagatesSubagentImageCandidates(t *testing.T) {
	...
	task := newTestTaskTool(t, sub, tool.NewRegistry(), "sys", "", "", nil)
	ctx := agent.WithSubagentImageCandidates(testTaskContext(), []string{"data:image/png;base64,AAAA"})
...
func TestTaskToolAllowsSecondLayerAndStopsThere(t *testing.T) {
	...
	depthOneCtx := agent.WithSubagentDepth(testTaskContext(), 1)
	...
	depthTwoCtx := agent.WithSubagentDepth(testTaskContext(), 2)
...
// TestTaskToolRunsEphemerallyWithoutParentSession mirrors headless `tempora run`:
...
func TestTaskToolContinueFromAncestorReturnsCopiedReferenceGuidance(t *testing.T) {
	...
	rootCtx := agent.WithParentSession(context.Background(), "root")
	...
	childCtx := agent.WithParentSession(context.Background(), "child")
...
func TestBackgroundEvidenceNotCommittedWhenTurnFails(t *testing.T) {
	...
	a := agent.New(prov, reg, sessionstore.NewSession(""), agent.Options{DeliveryProfile: true, Jobs: jm}, event.Discard)
	ctx := jobs.WithManager(agent.WithParentSession(context.Background(), "parent-session"), jm)
	ctx = jobs.WithSession(ctx, "parent-session")
...
func TestBackgroundEvidenceCommittedWhenTurnDelivers(t *testing.T) {
	...
	a := agent.New(prov, reg, sessionstore.NewSession(""), agent.Options{Jobs: jm}, event.Discard)
	ctx := jobs.WithManager(agent.WithParentSession(context.Background(), "parent-session"), jm)
	ctx = jobs.WithSession(ctx, "parent-session")
...
func TestRestartRecoversPendingBackgroundMutationForcesReadinessWithoutWait(t *testing.T) {
	...
	a := agent.New(prov, reg, sessionstore.NewSession(""), agent.Options{DeliveryProfile: true, Jobs: second}, event.Discard)
	ctx := jobs.WithManager(agent.WithParentSession(context.Background(), "parent-session"), second)
	ctx = jobs.WithSession(ctx, "parent-session")
