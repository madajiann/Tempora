...
func newTestTracker(t *testing.T, clock progressClock, sink event.Sink, childID string) *subagentProgressTracker {
	...
	ctx := withSubagentProgressMerger(agent.WithCallContext(context.Background(), childID, sink, nil, false), merger)
...
func TestSubagentProgressWrapConvertsAndForwards(t *testing.T) {
	...
	t.Cleanup(merger.Close)
	parent := &recordSink{}
	ctx := withSubagentProgressMerger(agent.WithCallContext(context.Background(), "task-1", parent, nil, false), merger)
...
func TestRunProfileSpecEmitsSubagentProgress(t *testing.T) {
	rec := &recordSink{}
	ctx := agent.WithCallContext(context.Background(), "task-1", rec, nil, false)
...
func TestRunProfileSpecProgressCancelledTerminal(t *testing.T) {
	rec := &recordSink{}
	ctx, cancel := context.WithCancel(agent.WithCallContext(context.Background(), "task-1", rec, nil, false))
	cancel()
	task := newTestTaskTool(t, reasoningTextProvider{}, tool.NewRegistry(), "sys", "", "", nil)
...
func TestBackgroundTaskEmitsQueuedRunningCompleted(t *testing.T) {
	...
	defer jm.Close()
	ctx := jobs.WithManager(agent.WithCallContext(context.Background(), "bg-task", rec, nil, false), jm)
	ctx = jobs.WithSession(ctx, "sess-bg")
	ctx = agent.WithParentSession(ctx, "sess-bg")
...
func TestParallelTasksGroupLifecycleCancelled(t *testing.T) {
	...
	parallel := NewParallelTasksTool(task, tool.NewRegistry())
	ctx, cancel := context.WithCancel(agent.WithCallContext(context.Background(), "parallel-call", rec, nil, false))
	defer cancel()
	go func() {
		<-started
		cancel()
