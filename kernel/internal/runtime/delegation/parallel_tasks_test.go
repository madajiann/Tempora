...
func TestParallelTasksCancelReturnsPartialAggregate(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(agent.WithCallContext(context.Background(), "parallel-call", event.Discard, nil, false))
	defer cancel()
...
func TestSubagentRecoveryTaskIDIsStableAndIsolated(t *testing.T) {
	ctx := agent.WithToolCallContext(context.Background(), "call-17", event.Discard, nil, false)
...
func TestParallelTasksLongResultsStayIndependentlyRetrievable(t *testing.T) {
	...
	parallel := NewParallelTasksTool(task, tool.NewRegistry())
	ctx := agent.WithParentSession(agent.WithCallContext(context.Background(), "parallel-call", event.Discard, nil, false), "parent-session")
	...
	if len(out) > agent.MaxToolOutputBytes {
		t.Fatalf("bounded aggregate still hit the generic truncator: %d bytes over the %d cap", len(out), agent.MaxToolOutputBytes)
	}
	...
	for i, ref := range refs {
		...
		if !strings.Contains(page, wantBegin) || !strings.Contains(page, wantEnd) || !strings.Contains(page, "End of subagent result") {
			t.Fatalf("read result %d was not complete: head=%q tail=%q", i+1, page[:min(120, len(page))], page[max(0, len(page)-120):])
		}
	}

	otherCtx := agent.WithParentSession(context.Background(), "other-session")
