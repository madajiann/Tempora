...
type sessionEventLogProbe struct {
	...
	dag           bool // a schema 2 log: 1.x owns it, this build reads it
	schemaVersion int
}
...
func probeSessionEventLogWithLimits(sessionPath string, limits sessionReplayLimits) (sessionEventLogProbe, error) {
	...
	if !ok && info.Size() <= limits.maxBytes {
		// Native writers put both fields in the bounded prefix; other in-budget
		// JSON gets a minimal decode, so field order is not part of the format.
		// Unknown fields are not materialized into messages.
