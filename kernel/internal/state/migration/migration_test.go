...
func migrationRescueHome(t *testing.T) string {
	...
	t.Setenv("TEMPORA_STATE_HOME", filepath.Join(home, "new-state"))
...
func isolateMigrationHome(t *testing.T) string {
	...
	t.Setenv("TEMPORA_HOME", filepath.Join(home, "new-tempora"))
...
func TestRunLegacyRescueImportsSessionsAndEmitsProgress(t *testing.T) {
	home := migrationRescueHome(t)
	legacyDir := filepath.Join(home, ".tempora", "sessions")
...
func TestRunLegacyRescueImportsMemory(t *testing.T) {
	home := migrationRescueHome(t)
	legacyRoot := filepath.Join(home, ".tempora")
	if err := os.MkdirAll(filepath.Join(legacyRoot, "memory", "global"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(legacyRoot, "TEMPORA.md"), []byte("legacy user memory\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for _, path := range []string{
		filepath.Join(config.MemoryUserDir(), "TEMPORA.md"),
...
func TestRunLegacyRescueSkipsImplicitSourcesWhenIsolated(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(config.MemoryUserDir(), "TEMPORA.md")); !os.IsNotExist(err) {
		t.Fatalf("isolated rescue imported legacy memory, stat err=%v", err)
	}
	joined := strings.Join(notices, "\n")
	if !strings.Contains(joined, "TEMPORA_HOME is set; implicit legacy migration is skipped") {
...
func TestRunLegacyRescueCommandImportsFromExplicitInstallDir(t *testing.T) {
	home := isolateMigrationHome(t)
	installRoot := filepath.Join(home, "Custom Tempora")
...
func TestMigrateLegacySessionSourcesSkipsCurrentProjectTree(t *testing.T) {
	...
	t.Setenv("TEMPORA_HOME", "")
	t.Setenv("TEMPORA_STATE_HOME", "")
	if !samePath(config.MemoryUserDir(), filepath.Join(home, ".tempora")) {
		t.Skip("current Tempora home is not ~/.tempora on this platform")
	}
...
// TEMPORA_STATE_HOME at a scratch directory copied every project's history
...
func TestAutomaticImportSkipsProductionWhenStateRootRedirected(t *testing.T) {
	...
	production := filepath.Join(home, ".tempora", "projects", "some-project", "sessions")
	...
	t.Setenv("TEMPORA_STATE_HOME", isolated)
