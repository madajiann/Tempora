...
func TestResolveDuplicateReplacementPreservesPrecedenceOrder(t *testing.T) {
	root := testenv.TempDir(t)
	user := testenv.TempDir(t)
	mustWriteInstruction(t, filepath.Join(user, "TEMPORA.md"), "duplicate")
...
func TestResolveKeepsDistinctConventionFilesInDeterministicOrder(t *testing.T) {
	root := testenv.TempDir(t)
	mustWriteInstruction(t, filepath.Join(root, "TEMPORA.md"), "Tempora rule")
	...
	for i, name := range []string{"TEMPORA.md", "AGENTS.md", "CLAUDE.md"} {
...
// The convention trio is the shipped layout: TEMPORA.md holds the rules and
...
func TestResolveExpandsAFileSharedBySeveralDocumentsOnce(t *testing.T) {
	...
	mustWriteInstruction(t, filepath.Join(root, "TEMPORA.md"), rule)
...
func TestResolveUserInstructionsImportTrustedConventionRoots(t *testing.T) {
	...
	userDir := filepath.Join(home, ".tempora")
	agentsDir := filepath.Join(home, ".agents")
	root := filepath.Join(home, "repo")
	mustWriteInstruction(t, filepath.Join(agentsDir, "AGENTS.md"), "SHARED USER RULE")
	mustWriteInstruction(t, filepath.Join(userDir, "TEMPORA.md"), "@~/.agents/AGENTS.md")
...
func TestResolveUserInstructionsRejectArbitraryHomeAndConventionSymlinkEscape(t *testing.T) {
	...
	mustWriteInstruction(t, filepath.Join(userDir, "TEMPORA.md"), "@~/secret.md\n@~/.agents/linked.md")
...
func TestInstructionBlockIsStableAcrossWorkspaceRoots(t *testing.T) {
	resolve := func(base string) string {
		...
		target := filepath.Join(root, "services", "api")
		user := filepath.Join(base, "tempora-home")
