...
// dropStalePlansLocked forgets prepares nobody committed. Both prepares call it:
// a cancelled confirm leaves its entry, and the single-file one is reachable
// from every diff card in a transcript.
func (s *Store) dropStalePlansLocked() {
	for id, prepared := range s.plans {
		if time.Since(prepared.created) > 10*time.Minute {
			delete(s.plans, id)
		}
	}
}
