...
// ~/.tempora/remote/. Only name derivation lives here (this package is the
// path authority and does no I/O); reads and writes happen over SFTP in
// internal/platform/remote. Local-side absolute paths (managed known_hosts) are
// derived in internal/contract/config/paths.go, which owns TEMPORA_HOME resolution.

// RemoteDirName is the directory under the remote ~/.tempora that holds all
// remote-module state, and under the local Tempora home that holds the
// managed known_hosts file.
const RemoteDirName = "remote"

// RemoteBinDirName holds an uploaded tempora binary on the remote host:
// ~/.tempora/remote/bin/tempora.
