...
func (s Store) validatePinnedBudget(m Memory) error {
	...
	return fmt.Errorf("pinning this fact would put pinned guidance at %d chars, over the %d budget you configured (memory.pinned_budget_chars): unpin or consolidate existing pinned facts, raise the budget, or move rules that must always hold into TEMPORA.md/AGENTS.md instructions", total, s.PinnedBudgetChars)
}
