...
//  1. does DeepSeek's auto cache actually serve tempora's request shape, and how
...
//
// Run with:  set -a; source .env; set +a; go test -tags live ./internal/model/openai/ -run TestRealDeepSeekCacheProbe -v -count=1
func TestRealDeepSeekCacheProbe(t *testing.T) {
	...
	// noR exercises Tempora's empty-key recovery fallback. Warm each prefix once,
