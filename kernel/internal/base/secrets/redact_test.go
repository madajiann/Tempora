...
func TestProcessEnvUnfilteredByDefault(t *testing.T) {
	...
	if !strings.Contains(joined, "TEMPORA_TEST_SECRET_TOKEN=ghp_abcdefghijklmnopqrstuvwxyz") {
	...
	joined = strings.Join(ProcessEnv(), "\n")
	if strings.Contains(joined, "TEMPORA_TEST_SECRET_TOKEN") {
...
func TestProcessEnvAlwaysFiltersRegisteredCredentialKeys(t *testing.T) {
	const key = "TEMPORA_TEST_CUSTOM_PROVIDER_CREDENTIAL"
	t.Setenv(key, "opaque-provider-value")
	t.Setenv("TEMPORA_TEST_BENIGN_ENV", "visible")
	...
	if !strings.Contains(joined, "TEMPORA_TEST_BENIGN_ENV=visible") {
