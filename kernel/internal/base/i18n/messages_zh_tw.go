...
var ChineseTraditional = Messages{
  ...
  tempora report delete [ID]     不傳送，直接刪除本機報告`,
	...
	StandaloneConsoleHint: "這是 Tempora 命令列工具，需要在 PowerShell 等終端機裡執行。\n要使用桌面應用程式，請安裝 Tempora Studio（TemporaStudio-windows-amd64-installer.exe）：\n  https://github.com/esengine/DeepSeek-Tempora/releases?q=studio-v&expanded=true\n\n按 Enter 關閉此視窗。\n",
	UsageBody: `tempora — 由設定和插件驅動的 coding agent（多模型）

用法：
  tempora [--model NAME] [-c|--continue] [-r|--resume [QUERY]] [--permission-mode MODE] [--effort LEVEL] [--add-dir PATH]   互動式會話（終端介面，同 tempora tui）
  ...
  tempora config compact-ratio [--local] [65..85]       設定自動壓縮閾值
  ...
  tempora login | whoami | logout                      登入 tempora.io（僅用於社群和當機追蹤）
