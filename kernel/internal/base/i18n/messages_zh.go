...
var Chinese = Messages{
  ...
  tempora report delete [ID]     不发送，直接删除本地报告`,
	...
	StandaloneConsoleHint: "这是 Tempora 命令行工具，需要在 PowerShell 等终端里运行。\n要用桌面应用，请安装 Tempora Studio（TemporaStudio-windows-amd64-installer.exe）：\n  https://github.com/esengine/DeepSeek-Tempora/releases?q=studio-v&expanded=true\n\n按回车关闭此窗口。\n",
	UsageBody: `tempora — 由配置和插件驱动的 coding agent（多模型）

用法：
  tempora [--model NAME] [-c|--continue] [-r|--resume [QUERY]] [--permission-mode MODE] [--effort LEVEL] [--add-dir PATH]   交互式会话（终端界面，同 tempora tui）
  ...
  tempora config compact-ratio [--local] [65..85]       配置自动压缩阈值
  ...
  tempora login | whoami | logout                      登录 tempora.io（仅用于社区和崩溃跟进）
