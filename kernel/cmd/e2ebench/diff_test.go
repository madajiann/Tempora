...
func TestEnv(t *testing.T) {
	if got := os.Getenv("TEMPORA_E2E_ENV"); got != "ok" {
		t.Fatalf("TEMPORA_E2E_ENV = %q", got)
	}
}
`)

	ok, out := runTests(repo, "GOWORK=off TEMPORA_E2E_ENV=ok go test", []string{"./..."})
	if !ok {
...
func TestRunTestsRejectsDynamicEnvAssignment(t *testing.T) {
	ok, out := runTests(testenv.TempDir(t), "TEMPORA_E2E_ENV=$(echo ok) go test", []string{"./..."})
	if ok {
