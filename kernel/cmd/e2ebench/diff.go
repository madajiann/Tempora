...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "tempora.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 Tempora e2e — diff test-gen (%s)\n\n", profile)
...
// repoRelFromModulePath turns "tempora/internal/runtime/agent/foo.go" into
