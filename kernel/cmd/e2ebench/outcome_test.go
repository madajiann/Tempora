...
func TestSummarizeOutcomeSumsDebtExposure(t *testing.T) {
	path := writeTrajectory(t, "debt-area.trajectory.jsonl", []string{
		`{"seq":1,"ts":1000,"event":{"kind":"turn_started"}}`,
		`{"seq":2,"ts":2000,"outcome_progress":{"round":1,"churn":2,"debt_age":1,"blind_mutations":2}}`,
		`{"seq":3,"ts":3000,"outcome_progress":{"round":2,"churn":1,"debt_age":2,"blind_mutations":3}}`,
	...
	o := s.Outcome
	if o == nil || o.DebtArea != 6 || o.BlindPeak != 3 {
		t.Fatalf("outcome = %+v, want debt area 6 and blind peak 3", o)
	}
}
