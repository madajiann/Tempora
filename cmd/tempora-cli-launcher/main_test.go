...
func TestCLIHelperProcess(t *testing.T) {
	if os.Getenv("TEMPORA_CLI_LAUNCHER_HELPER") != "1" {
		return
	}
...
func TestRunCLIPreservesArgumentsStreamsAndExitCode(t *testing.T) {
	...
	target := filepath.Join(dir, "tempora-cli.exe")
	script := "#!/bin/sh\nTEMPORA_CLI_LAUNCHER_HELPER=1 exec \"" + os.Args[0] + "\" -test.run=TestCLIHelperProcess -- \"$@\"\n"
	...
	launcher := filepath.Join(root, "tempora-cli.exe")
