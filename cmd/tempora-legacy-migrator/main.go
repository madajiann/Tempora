// Command tempora-legacy-migrator is the one-shot flat→versioned install
// migrator used by v1.20+ packaging. In compatibility payloads it is still
// named tempora-guard(.exe) so 1.18–1.19.1 updaters can hand off; the source
...
import (
	...
	"tempora/internal/desktoplauncher"
...
const migrationLockName = ".tempora-layout-migrate.lock"
...
func run(args []string) int {
	if len(args) == 1 {
		...
		case "version", "--version", "-v":
			fmt.Println("tempora-legacy-migrator", version)
			return 0
		case "help", "--help", "-h":
			fmt.Println("usage: tempora-legacy-migrator [--install-root PATH] [--version VERSION] [--activate-staging PATH]")
			return 0
		}
	}
...
func ensureLauncherEntry(installRoot string) error {
	...
	// On Windows also accept Tempora.exe as the alias.
	if runtime.GOOS == "windows" {
		if _, err := os.Lstat(filepath.Join(installRoot, "Tempora.exe")); err == nil {
			return nil
		}
...
func cleanupLegacyFlatFiles(installRoot string) error {
	...
	if runtime.GOOS == "windows" {
		names = append(names, "tempora-guard.exe")
	} else {
		names = append(names, "tempora-guard")
	}
...
func archiveInstallRootLegacyMarkers(installRoot string) error {
	...
	// state under Tempora home is finalized through repair transaction APIs.
	markers := []string{
		"pending-update.json",
		"startup-state.json",
	}
...
func startLauncher(installRoot string) error {
	...
	if _, err := os.Lstat(path); err != nil {
		if runtime.GOOS == "windows" {
			path = filepath.Join(installRoot, "Tempora.exe")
		}
	}
	if _, err := os.Lstat(path); err != nil {
...
func selfDelete() {
	...
	if base != "tempora-guard" && base != "tempora-guard.exe" {
		return
	}
	_ = os.Remove(exe)
}
