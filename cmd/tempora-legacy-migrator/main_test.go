...
func TestMigrateFlatInstallToVersioned(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(root, "tempora-launcher"), []byte("launcher"), 0o644)
	_ = os.WriteFile(filepath.Join(root, "tempora-launcher.exe"), []byte("launcher"), 0o644)
	_ = os.WriteFile(filepath.Join(root, "Tempora.exe"), []byte("launcher"), 0o644)
...
func TestActivateInstallerStagingPrefersThinCLIEntryAndRejectsInvalidEntry(t *testing.T) {
	root := t.TempDir()
	staging := writeInstallerStaging(t, root, "new", true)
	entry := filepath.Join(staging, "app", "resources", "bin", "tempora-cli-launcher.exe")
	...
	badEntry := filepath.Join(badStaging, "app", "resources", "bin", "tempora-cli-launcher.exe")
