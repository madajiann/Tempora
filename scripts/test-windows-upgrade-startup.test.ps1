...
$testRoot = Join-Path ([IO.Path]::GetTempPath()) ('tempora-upgrade-contract-' + [guid]::NewGuid().ToString('N'))
$savedHome = $HOME
try {
  $release = Join-Path $testRoot 'install/releases/test/app/resources'
  New-Item -ItemType Directory -Path $release -Force | Out-Null
  $application = Join-Path $testRoot 'install/Tempora.exe'
  ...
  $pending = New-UIElement 'Import and open' 'tempora-prepare-restored-session'
  ...
  $transcript = New-UIElement '' 'tempora-chat-transcript-upgrade-tab' $false @($body)
  ...
  $recovery = New-UIElement 'Loading history' 'tempora-session-recovery-test'
