...
const packages = ["tempora/cmd/tempora", "tempora/internal/agent", "tempora/internal/cli",
  "tempora/internal/cli/theme", "tempora/internal/clipboard", "tempora/internal/sandbox",
  "tempora/internal/tool/builtin", "tempora/internal/toolbox", "tempora/tools/repolint"];

test("the pull-request group keeps darwin packages and drops the rest", () => {
  assert.deepEqual(selectPackages(packages, "darwin"), ["tempora/internal/cli",
    "tempora/internal/cli/theme", "tempora/internal/sandbox", "tempora/internal/tool/builtin"]);
  // Prefix matching must not swallow a sibling that merely shares a name stem.
  assert.ok(!selectPackages(packages, "darwin").includes("tempora/internal/clipboard"));
  assert.ok(!selectPackages(packages, "darwin").includes("tempora/internal/toolbox"));
...
test("every darwin-specific package is inside the pull-request group", () => {
  ...
  const walk = dir => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (darwin) owners.add(`tempora/${path.relative(root, dir).replaceAll(path.sep, "/")}`);
    }
  };
