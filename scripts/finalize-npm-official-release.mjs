...
const packages = [
  "tempora",
  "@tempora/cli-darwin-arm64", "@tempora/cli-darwin-x64",
  "@tempora/cli-linux-arm64", "@tempora/cli-linux-x64",
  "@tempora/cli-win32-arm64", "@tempora/cli-win32-x64",
];
...
function metadata(name) {
  const output = execFileSync(
    "npm",
    ["view", `${name}@${version}`, "name", "version", "gitHead", "temporaCandidateSha", "--json"],
    { encoding: "utf8" },
  );
...
for (const name of packages) {
  ...
  if (expectedSha && actual.temporaCandidateSha !== expectedSha) {
    throw new Error(`${name}@${version} temporaCandidateSha ${actual.temporaCandidateSha || "<missing>"} does not match ${expectedSha}`);
  }
}
