...
test("packaging changes run native installer acceptance before merge", () => {
  assert.match(job(ci, "desktop-prepare"), /TEMPORA_COMMIT: \$\{\{ github.sha \}\}/,
...
test("notes pushes preserve required ancestor CI while code pushes cancel obsolete runs", () => {
  const dir = mkdtempSync(path.join(os.tmpdir(), "tempora-ci-cancel-"));
  const git = (...args) => {
...
test("App memory workflow tiers pull requests and keeps full scheduled coverage", t => {
  ...
  assert.match(appMemory, /TEMPORA_APP_MEMORY_PROFILE: \$\{\{ needs\.changes\.outputs\.memory_profile \}\}/);
  const script = shellStep(job(appMemory, "changes"), "Select memory profile");
  const root = mkdtempSync(path.join(os.tmpdir(), "tempora-memory-workflow-"));
...
test("browser matrix preserves five entry points and fails closed through desktop-browser", () => {
  ...
  assert.match(groups, /TEMPORA_TRANSCRIPT_MODE=native-scrollbar TEMPORA_LAYOUT_ARTIFACTS="\$evidence\/native-scrollbar"/);
  assert.match(groups, /TEMPORA_TRANSCRIPT_MODE=headless-reader TEMPORA_LAYOUT_ARTIFACTS="\$evidence\/headless-reader"/);
...
test("installer evidence excludes running payloads and cache files on every publisher", () => {
  for (const body of [job(ci, "desktop-windows-package"), job(release, "windows-runtime-acceptance")]) {
    ...
    for (const extension of ["json", "png", "log"])
      assert.ok(upload.includes(`tempora-installer-acceptance/**/*.${extension}`));
    for (const excluded of ["installed/**", "**/cache/**"])
      assert.ok(upload.includes(`!\${{ runner.temp }}/tempora-installer-acceptance/${excluded}`));
  }
});
