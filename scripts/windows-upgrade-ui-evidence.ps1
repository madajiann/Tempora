...
function Invoke-PendingHistoricalSession($root) {
  foreach ($element in (Get-UpgradeUIDescendants $root)) {
    ...
    if ($current.AutomationId -eq 'tempora-prepare-restored-session' -and -not $current.IsOffscreen -and
...
function Test-VisibleUpgradeHistory($root, [string]$text) {
  ...
  foreach ($element in $descendants) {
    ...
    if (([string]$current.AutomationId).StartsWith('tempora-session-recovery-') -or
        $current.AutomationId -eq 'tempora-prepare-restored-session' -or
  ...
  foreach ($transcript in $descendants) {
    if (-not ([string]$transcript.Current.AutomationId).StartsWith('tempora-chat-transcript-') -or $transcript.Current.IsOffscreen) { continue }
