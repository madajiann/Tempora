...
$scoopVersion = "v0.5.3"
$appName = "tempora-desktop"
$appVersion = "0.0.0-ci"
$activeVersion = "v$appVersion"
$shortcutName = "Tempora Desktop"
$repoRoot = Split-Path -Parent $PSScriptRoot
$workRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("tempora-scoop-launch-" + [guid]::NewGuid().ToString("N"))
...
$archivePath = Join-Path $fixtureRoot "tempora-windows-portable.zip"
...
try {
    ...
    $desktopPath = Join-Path $packageRoot "versions\$activeVersion\tempora-desktop.exe"
...
func main() {
	marker := os.Getenv("TEMPORA_SCOOP_MARKER")
...
{
  "version": "$appVersion",
  "description": "Tempora Scoop integration fixture",
  "homepage": "https://github.com/esengine/DeepSeek-Reasonix",
  "license": "Apache-2.0",
  "url": "$archiveUrl",
  "hash": "$archiveHash",
  "shortcuts": [
    ["tempora-launcher.exe", "$shortcutName", "launch --detach"]
  ]
}
"@
    ...
    $env:TEMPORA_SCOOP_MARKER = $markerPath
    ...
    $expectedTarget = Join-Path $currentPath "tempora-launcher.exe"
    ...
    $expectedDesktop = Join-Path $versionPath "versions\$activeVersion\tempora-desktop.exe"
