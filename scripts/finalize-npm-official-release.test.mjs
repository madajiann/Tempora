...
function run(
  ...
  const directory = mkdtempSync(join(tmpdir(), "tempora-npm-alias-test-"));
...
if (args[0] === "view" && args[1].includes("@1.19.2") && args.at(-1) === "--json") {
  ...
  console.log(JSON.stringify({ name, version: "1.19.2", gitHead: ${JSON.stringify(publishedSha)}, temporaCandidateSha: ${JSON.stringify(publishedSha)} }));
