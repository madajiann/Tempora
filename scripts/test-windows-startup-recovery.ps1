...
if ($PSCmdlet.ParameterSetName -eq 'Portable') {
  ...
  throw "Installed Tempora directory is missing: $install"
}
...
if (-not (Test-Path -LiteralPath $launcher -PathType Leaf)) {
  $launcher = Join-Path $install 'tempora-launcher.exe'
}
if (-not (Test-Path -LiteralPath $launcher -PathType Leaf)) {
  throw 'Stable Tempora launcher is missing from the package root.'
}
