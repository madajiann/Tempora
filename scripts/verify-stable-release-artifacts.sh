...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/tempora-release-postflight.XXXXXX")"
cleanup() {
	case "$tmp_dir" in
	*/tempora-release-postflight.*) rm -rf -- "$tmp_dir" ;;
...
verify_site() {
	...
	local cask="$tmp_dir/tempora.rb"
	curl -fsSL https://dl.tempora.io/latest/latest.json >"$manifest"
	jq -e --arg version "v$version" '
		.version == $version and
		([.platforms[], (.native_packages // {})[], (.downloads // {})[]] |
		 all(.url | type == "string" and startswith("https://dl.tempora.io/desktop-" + $version + "/")))
	' "$manifest" >/dev/null
	curl -fsSL "https://tempora.io/?download=desktop&release-postflight=v$version" >"$homepage"
	! grep -Eq 'href="[^"]*(tag|download)/desktop-v[0-9]+\.[0-9]+\.[0-9]+' "$homepage"
	curl -fsSL "https://tempora.io/changelog/v$version/" >"$changelog"
	grep -Fq "v$version" "$changelog"
	curl -fsSL https://raw.githubusercontent.com/esengine/homebrew-tempora/main/Casks/tempora.rb >"$cask"
	...
	"$browser" --headless=new --disable-gpu --no-sandbox --virtual-time-budget=10000 \
		--dump-dom "https://tempora.io/?download=desktop&release-postflight=v$version" \
...
jq -e '
  ...
  ([.assets[].name] as $names |
    ["SHA256SUMS", "tempora-darwin-amd64.tar.gz", "tempora-darwin-arm64.tar.gz",
     "tempora-linux-amd64.tar.gz", "tempora-linux-arm64.tar.gz",
     "tempora-windows-amd64.zip", "tempora-windows-arm64.zip"] |
...
if [ "${DESKTOP_MANUAL_ONLY:-false}" = "true" ]; then
	...
	curl -fsSL https://dl.tempora.io/latest/latest.json > "$tmp_dir/desktop-pointer.json"
...
npm_names=(
	"tempora"
	"@tempora/cli-darwin-arm64"
	"@tempora/cli-darwin-x64"
	"@tempora/cli-linux-arm64"
	"@tempora/cli-linux-x64"
	"@tempora/cli-win32-arm64"
	"@tempora/cli-win32-x64"
)
for attempt in $(seq 1 "$attempts"); do
	...
	for index in "${!npm_names[@]}"; do
		...
		if ! npm view "$package@$version" name version temporaCandidateSha gitHead dist.integrity dist-tags.latest --json >"$raw" 2>/dev/null; then
			visible=false
			continue
		fi
		jq --arg name "$package" '
		...
		jq -e --arg name "$package" --arg version "$version" --arg sha "$cli_sha" '
			...
			((.temporaCandidateSha == null or .temporaCandidateSha == $sha) and
			 (.gitHead == null or .gitHead == $sha) and
			 ((.temporaCandidateSha // .gitHead) == $sha)) and
	...
	if [ "$visible" = "true" ]; then
		...
		if node "$script_dir/release-publication-ledger.mjs" core "$version" "$cli_sha" "$operation" \
			...
			if [ "$verify_homepage" = "true" ]; then
				pointer_version="$(curl -fsSL https://dl.tempora.io/latest/latest.json | jq -r .version)"
