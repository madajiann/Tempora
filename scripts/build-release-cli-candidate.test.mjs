...
test("platform package embeds the immutable full candidate identity", () => {
  ...
  assert.equal(pkg.name, "@tempora/cli-darwin-arm64");
  assert.equal(pkg.temporaCandidateSha, "a".repeat(40));
  assert.equal(pkg.gitHead, "a".repeat(40));
});

test("homebrew cask is checksum-bound to prepared CLI archives", () => {
  const sums = Object.fromEntries(targets.filter(target => target.goos !== "windows").map(target => [`tempora-${target.key}.tar.gz`, target.key.includes("arm64") ? "a".repeat(64) : "b".repeat(64)]));
