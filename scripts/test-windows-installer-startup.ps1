...
param(
  ...
  [string]$EvidenceDirectory = (Join-Path $env:TEMP ('tempora-installer-' + [guid]::NewGuid().ToString('N'))),
...
function Get-DefaultTemporaDataHome {
  return (Join-Path $env:APPDATA 'tempora')
}
...
function Assert-InstallerTestAccount {
  ...
  $protectedPaths = @(Get-InstallerIntegrationPaths) + @(
    ...
    (Get-DefaultTemporaDataHome)
  )
  foreach ($path in $protectedPaths) {
    if (Test-Path -LiteralPath $path) {
      throw 'Existing Tempora installation, shortcut or legacy data detected; use a clean disposable Windows account.'
    }
  }
  if (@(Get-Process -Name Tempora,tempora-desktop,tempora-launcher -ErrorAction SilentlyContinue).Count -ne 0) {
    throw 'Tempora is running; use a clean disposable Windows account.'
  }
}

function Install-Tempora {
...
function Assert-InstalledIdentity {
  ...
  $registration = Get-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\TemporaTempora'
...
function Assert-UninstalledState {
  ...
  foreach ($name in @('versions', 'app', 'current.json', 'Tempora.exe', 'tempora-launcher.exe',
      'tempora-desktop.exe', 'tempora-cli.exe', 'tempora-update-helper.exe', 'tempora-guard.exe', 'uninstall.exe')) {
  ...
  if (@(Get-Process -Name Tempora,tempora-desktop,tempora-launcher -ErrorAction SilentlyContinue).Count -ne 0) {
    throw 'Uninstall left Tempora processes running.'
  }
...
function Invoke-WindowsInstallerAcceptance {
  ...
  if ($ExpectedVersion -notmatch '^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)(?:-[0-9A-Za-z.-]+)?$') {
    throw "ExpectedVersion is not a canonical Tempora version: $ExpectedVersion"
  }
  ...
  try {
    ...
    $defaultHome = Get-DefaultTemporaDataHome
    ...
    Install-Tempora -Installer $installer -InstallRoot $install
    ...
    if ($FixtureBuilderPath) {
      & (Join-Path $PSScriptRoot 'test-windows-upgrade-startup.ps1') `
        -ApplicationPath (Join-Path $install 'Tempora.exe') -FixtureBuilderPath $FixtureBuilderPath `
    ...
    $homes = @($env:TEMPORA_HOME, (Join-Path $EvidenceDirectory 'fresh-install\home'))
