...
function fixture(t) {
  const root = mkdtempSync(path.join(tmpdir(), "tempora-candidate-"));
  ...
  mkdirSync(cli);
  for (const name of ["tempora-darwin-amd64.tar.gz", "tempora-darwin-arm64.tar.gz", "tempora-linux-amd64.tar.gz", "tempora-linux-arm64.tar.gz", "tempora-windows-amd64.zip", "tempora-windows-arm64.zip", "SHA256SUMS", "tempora.rb"]) {
...
for (const mutation of ["payload", "expired", "revoked", "wrong-source", "missing-acceptance"]) {
  test(`rejects ${mutation} candidate reuse`, t => {
    ...
    if (mutation === "wrong-source") record.source.repository = "fork/Tempora";
