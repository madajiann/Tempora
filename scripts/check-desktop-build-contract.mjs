...
const fixture = fs.mkdtempSync(path.join(os.tmpdir(), "tempora-contract-guard-"));
try {
...
assert.match(
  windowsJob,
  /go build -trimpath -ldflags "-s -w -H windowsgui -X main\.version=v0\.0\.0-ci -X main\.channel=canary" -o build\/bin\/tempora-desktop\.exe \./,
  "Windows native startup CI must build the service as a GUI-subsystem image",
);
assert.match(
  windowsJob,
  /node \.\.\/scripts\/verify-windows-gui-subsystem\.mjs build\/bin\/tempora-desktop\.exe/,
