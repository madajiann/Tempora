...
cp "$signing_work/desktop/build/windows/installer/tempora_project.nsh" \
...
(
	...
	go run ./cmd/sign sign "$signed_payload/tempora-payload.json"
	go run ./cmd/sign verify "$signed_payload/tempora-payload.json"
)
...
installer="$dist/Tempora-windows-$arch-installer.exe"
portable="$dist/Tempora-windows-$arch.zip"
