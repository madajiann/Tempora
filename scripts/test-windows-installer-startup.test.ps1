...
$script:testRoot = [IO.Path]::GetFullPath((Join-Path $env:TEMP ('tempora-installer-tests-' + [guid]::NewGuid().ToString('N'))))
...
function Get-DefaultTemporaDataHome {
...
function Assert-IsolatedEnvironment([string]$HomePath) {
  ...
  Assert-Equal $env:TEMPORA_HOME $HomePath
  Assert-Equal $env:TEMPORA_STATE_HOME $HomePath
  Assert-Equal $env:TEMPORA_CACHE_HOME (Join-Path $HomePath 'cache')
  Assert-Equal $env:TEMPORA_NONINTERACTIVE '1'
  foreach ($key in @('TEMPORA_DEV','TEMPORA_DESKTOP_SERVICE','TEMPORA_ELECTRON_DEV_URL',
      'TEMPORA_UNKNOWN_TEST_OVERRIDE','NODE_OPTIONS','ELECTRON_RUN_AS_NODE')) {
    Assert-Equal ([string][Environment]::GetEnvironmentVariable($key, 'Process')) ''
  }
}
function Assert-ParentEnvironment {
  Assert-Equal $env:TEMPORA_HOME 'untouched-user-home'
  Assert-Equal $env:TEMPORA_STATE_HOME 'untouched-user-state'
  Assert-Equal $env:TEMPORA_CACHE_HOME 'untouched-user-cache'
  Assert-Equal $env:TEMPORA_DEV 'untouched-dev-mode'
  Assert-Equal $env:TEMPORA_UNKNOWN_TEST_OVERRIDE 'untouched-override'
...
function Start-Process {
  ...
  if ((Split-Path $FilePath -Leaf) -eq 'uninstall.exe') {
    ...
    if ($script:uninstallFault -eq 'program') {
      ...
      [IO.File]::WriteAllText((Join-Path $script:installRoot 'Tempora.exe'), 'leftover')
    } elseif ($script:uninstallFault -eq 'shortcut') {
      $script:occupiedPath = Join-Path ([Environment]::GetFolderPath('Programs', 'DoNotVerify')) 'Tempora.lnk'
    } elseif ($script:uninstallFault -eq 'registration') {
      $script:occupiedPath = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\TemporaTempora'
    } elseif ($script:uninstallFault -eq 'process') {
      $script:running = $true
    } elseif ($script:uninstallFault -eq 'default-data') {
      [IO.Directory]::Delete((Get-DefaultTemporaDataHome), $true)
...
try {
  ...
  $env:TEMPORA_HOME = 'untouched-user-home'
  $env:TEMPORA_DEV = 'untouched-dev-mode'
  $env:TEMPORA_STATE_HOME = 'untouched-user-state'
  $env:TEMPORA_CACHE_HOME = 'untouched-user-cache'
  $env:TEMPORA_UNKNOWN_TEST_OVERRIDE = 'untouched-override'
  ...
  $protected = @(
    (Join-Path ([Environment]::GetFolderPath('DesktopDirectory', 'DoNotVerify')) 'Tempora.lnk'),
    (Join-Path ([Environment]::GetFolderPath('Programs', 'DoNotVerify')) 'Tempora.lnk'),
    (Join-Path $env:LOCALAPPDATA 'Programs\Tempora'),
    (Join-Path $env:APPDATA 'tempora-desktop.exe')
  )
  foreach ($hive in @('HKCU:', 'HKLM:')) {
    foreach ($software in @('Software', 'Software\WOW6432Node')) {
      foreach ($product in @('TemporaTempora', 'Tempora')) {
  ...
  foreach ($path in $protected) {
    ...
    Assert-Throws { Invoke-TestCase } 'Existing Tempora'
    Assert-Equal $script:events.Count 0
    Assert-Equal (Test-Path -LiteralPath $script:evidence) $false
  }
  New-TestCase
  $script:occupiedPath = Get-DefaultTemporaDataHome
  Assert-Throws { Invoke-TestCase } 'Existing Tempora'
  Assert-Equal $script:events.Count 0
  Assert-Equal (Test-Path -LiteralPath $script:evidence) $false
  New-TestCase
  $script:running = $true
  Assert-Throws { Invoke-TestCase } 'Tempora is running'
  Assert-Equal $script:events.Count 0
  ...
  foreach ($mode in @('Installed', 'Portable')) {
    ...
    [IO.File]::WriteAllText((Join-Path $fixture 'Tempora.exe'), 'never executed')
  ...
  try {
    ...
    $env:TEMPORA_CREATED_INSIDE_TEST = 'remove on exit'
  } finally { Restore-WindowsAcceptanceEnvironment -Snapshot $outerEnvironment }
  Assert-ParentEnvironment
  Assert-Equal ([string]$env:TEMPORA_CREATED_INSIDE_TEST) ''
  ...
  Assert-Throws { Assert-InstallerTestAccount } 'Existing Tempora'
