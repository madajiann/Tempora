...
plugin_root="$(pwd -P)"
tempora plugin install "$plugin_root" --dry-run
tempora plugin install "$plugin_root" --link --replace --yes
```
...
tempora plugin install $pluginRoot --dry-run
tempora plugin install $pluginRoot --link --replace --yes
```

安装前请检查 dry-run 输出中的 `FULL TRUST` 区块。链接安装会持续信任本目录
后续的内容变化，而且代码型 Runtime 运行在 Tempora Sandbox 之外。
...
`tempora plugin doctor starter-extension`。
