...
function fixture(
  ...
  const root = mkdtempSync(join(tmpdir(), "tempora-npm-publish-test-"));
  t.after(() => rmSync(root, { recursive: true, force: true }));

  const packages = ["@tempora/cli-linux-x64", "tempora"].map((name, index) => {
    const dir = join(root, `package-${index}`);
    mkdirSync(dir);
    writeFileSync(
      join(dir, "package.json"),
      `${JSON.stringify({ name, version, temporaCandidateSha: candidateSha })}\n`,
    );
    return { name, dir };
  });
  ...
  function runner(args, { cwd, missingOk = false } = {}) {
    ...
    if (args[0] === "publish") {
      ...
      state.versions.set(pkg.version, {
        name: pkg.name,
        version: pkg.version,
        temporaCandidateSha: pkg.temporaCandidateSha,
        gitHead: pkg.temporaCandidateSha,
      });
...
test("publishes the exact prepared tarball instead of rebuilding a package directory", (t) => {
  const fx = fixture(t);
  const tarball = join(fx.packages[0].dir, "tempora-cli-linux-x64-1.5.0-canary.42.tgz");
