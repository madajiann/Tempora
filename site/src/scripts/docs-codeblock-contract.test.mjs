...
test("the macOS quarantine comment and command remain separate source lines", async () => {
  ...
  assert.match(
    page,
    /# Quit Tempora first, then run in Terminal\.<\/span>\nsudo xattr -rd com\.apple\.quarantine \/Applications\/Tempora\.app/,
  );
});
...
test("website documents the version-matched built-in docs command", async () => {
  ...
  assert.match(page, /normally <code>\/tempora:docs<\/code>/);
  assert.match(page, /Tempora never overwrites the existing command/);
...
test("website exposes the Extension Protocol developer path", async () => {
  ...
  assert.match(page, /tempora plugin install "\$plugin_root" --dry-run/);
  assert.match(page, /tempora plugin install "\$plugin_root" --link --replace --yes/);
