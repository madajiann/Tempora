...
// allowed tempora.io host. Asserting this (rather than a fixed string) catches
...
function assertLandsSomewhereSafe(returned) {
  ...
  const ok = landed.origin === ORIGIN || landed.host === "tempora.io" || landed.host.endsWith(".tempora.io");
...
test("same-origin path passes through as an absolute same-origin URL", () => {
  ...
  assert.equal(got, "https://tempora.io/account/settings?x=1#y");
  assertLandsSomewhereSafe(got);
});

test("allowed tempora.io subdomain passes through", () => {
  const got = safeNext("https://crash.tempora.io/x", ORIGIN);
  assert.equal(got, "https://crash.tempora.io/x");
  assertLandsSomewhereSafe(got);
});
...
test("dot-segment inputs that normalize to a //host pathname do not escape", () => {
  ...
  // returning the absolute href keeps them on tempora.io. Assert end to end.
...
test("rejects a tempora.io-lookalike host", () => {
  assert.equal(safeNext("https://tempora.io.evil.example/", ORIGIN), null);
});
