...
test("homepage presents one VS Code extension through both registries", async () => {
  ...
  assert.match(page, /More ways to use Tempora:[\s\S]*更多使用方式：/);
  assert.match(page, /Local Web UI:[\s\S]*tempora serve[\s\S]*本地 Web UI：/);
  assert.match(page, /ACP editor integration[\s\S]*ACP 编辑器接入/);
  assert.match(page, /SivanLiu\.tempora-agent/);
  assert.match(page, /marketplace\.visualstudio\.com\/items\?itemName=SivanLiu\.tempora-agent/);
  assert.match(page, /open-vsx\.org\/extension\/SivanLiu\/tempora-agent/);
