...
The account service for `tempora.io`: email/password sign-up, email verification,
sessions, password reset, and public profiles. A Cloudflare Worker (Hono) backed
by D1. Runs at `id.tempora.io`, separate from the internal crash dashboard.
...
```sh
wrangler d1 create tempora-accounts        # paste database_id into wrangler.toml
wrangler d1 migrations apply tempora-accounts --remote
wrangler secret put SESSION_PEPPER
wrangler secret put RESEND_API_KEY          # if EMAIL_PROVIDER=resend
wrangler deploy
```

The `id.tempora.io` custom domain route is declared in `wrangler.toml`; point the
