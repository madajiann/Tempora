...
function bindings(db: D1Database): Bindings {
  return {
    DB: db,
    APP_ORIGIN: "https://tempora.io",
    ALLOWED_ORIGINS: "https://tempora.io",
    ID_ORIGIN: "https://id.tempora.io",
  };
}
...
describe("forum public API", () => {
  it("maps invalid query input to a client error", async () => {
    ...
    const response = await app.request("https://forum.tempora.io/topics?sort=invalid", {}, bindings(db));
  ...
  it("selects public handles instead of stored author emails", async () => {
    ...
    const response = await app.request("https://forum.tempora.io/topics", {}, bindings(db));
  ...
  it("paginates topic posts with a stable created-at and id cursor", async () => {
    ...
    const response = await app.request(
      "https://forum.tempora.io/topics/7?limit=1&after=2026-08-05T00%3A00%3A00.000Z&afterId=1",
      {},
      bindings(db),
    );
    expect(response.status).toBe(200);
