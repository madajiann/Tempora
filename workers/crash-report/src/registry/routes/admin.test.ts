...
function bindings(db: D1Database): Bindings {
  return {
    DB: db,
    ACCOUNTS_ORIGIN: "https://id.tempora.test",
    APP_ORIGIN: "https://tempora.test",
    ALLOWED_ORIGINS: "https://tempora.test",
  };
}

function approvalRequest(body: object): Request {
  return new Request("https://registry.tempora.test/v1/admin/packages/publisher/devkit/approve", {
    method: "POST",
