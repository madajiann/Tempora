  ...
  .superRefine((val, ctx) => {
    ...
    if (val.kind === "plugin" && !isGitHubRepoSource(val.source)) {
      ctx.addIssue({
        ...
        path: ["source"],
        message:
          "a plugin source must point at a GitHub repository or path containing tempora-plugin.json, .codex-plugin/plugin.json, .claude-plugin/plugin.json, or a supported .claude-plugin/marketplace.json.",
      });
    }
