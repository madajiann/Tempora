// Dashboard authorization. Identity comes from id.tempora.io (the shared
...
// The id.tempora.io session cookie is scoped to `.tempora.io`, so the browser
...
function idOrigin(env: Env): string {
  return (env.ID_ORIGIN ?? "https://id.tempora.io").replace(/\/$/, "");
}

function appOrigin(env: Env): string {
  return (env.APP_ORIGIN ?? "https://tempora.io").replace(/\/$/, "");
}
...
async function identityCacheKey(token: string): Promise<Request> {
  ...
  return new Request(`https://id-cache.tempora.io/${hex}`);
}
...
// Ends the shared id.tempora.io session (best-effort) and returns a Set-Cookie
...
export async function sharedLogout(request: Request, env: Env): Promise<string> {
  ...
  return `${SHARED_COOKIE}=; HttpOnly; Secure; SameSite=Lax; Path=/; Domain=.tempora.io; Max-Age=0`;
}
