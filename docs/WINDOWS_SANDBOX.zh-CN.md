...
Tempora 在 Windows 的 Shell 和写入进程中采用了 DeepSeek Harness
...
Tempora 按规范化目录、用途和 Tempora 专用域确定性派生能力 SID。工作区
...
修改工作区 DACL 之前，Helper 会在宿主控制的 Tempora 状态目录内原子写入
...
依赖正常控制台继承。Tempora 的 Helper 本身仍由桌面进程启动器隐藏。
...
获准工作区不能包含 Tempora 受保护状态目录，任意状态子目录也不能成为写入
...
CI 的 Windows smoke 组已经包含 `tempora/internal/winsandbox`。非 Windows
