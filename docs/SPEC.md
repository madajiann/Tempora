...
- **User decisions are separate from tool approvals.** Runtime permission has
  ...
  and Tempora filesystem/network confinement. Explicit host deny rules still
  run before launch, but Tempora does not constrain the launched process.
...
- **Collaboration mode is separate from tool approval.** The desktop composer
  ...
  runtime. Legacy `.tempora/autoresearch/...`
...
| Full access / `danger-full-access` | Ordinary prompts and Tempora process sandboxing are skipped; explicit host deny rules still run before launch | Waits for user | Waits for user |
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  Tempora never offers an unconfined retry as a fallback.
