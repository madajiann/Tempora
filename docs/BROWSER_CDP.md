...
the user shares, served by the Electron shell. CLI, `tempora serve`, and
...
That is the whole ordinary path. On the first browser tool call Tempora
...
| `chrome_path` | Browser binary. Empty searches the usual Chrome, Chromium, and Edge locations, then `TEMPORA_CHROME` and `CHROME_PATH`. |
...
```bash
TEMPORA_LIVE_CHROME=1 go test ./internal/browser/cdp -run '^TestLiveChrome$' -v -count=1
```
