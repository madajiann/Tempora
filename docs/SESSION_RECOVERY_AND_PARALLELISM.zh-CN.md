...
Tempora 将会话记录持久化和工作区文件修改分成两条安全边界。只读任务
...
`tempora sessions cleanup` 移入可恢复回收站。格式 2 日志不会形成恢复分组，
因此 cleanup 对它的候选数恒为零；`tempora sessions diagnose` 会在恢复副本
...
格式 2 需要 Tempora 1.39.0 及更新版本打开。混用新旧版本时应先升级旧的一方
