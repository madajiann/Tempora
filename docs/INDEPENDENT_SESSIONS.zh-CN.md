...
| 层次 | 本地证据 |
| --- | --- |
| 根 Go | 全量通过，`tempora-independent-root-final.log` |
| Desktop Go | 最终全量通过，主包耗时 301.282 秒，`tempora-independent-desktop-final.log` |
| Race | registry、导航/AI标题/分叉生命周期及空白会话创建复用测试通过，`tempora-independent-registry-race.log`、`tempora-independent-lifecycle-race.log`、`tempora-independent-blank-race.log` |
| 前端 | 50 项会话 focused tests、通知/音效回归、生产/测试 typecheck 及 build 通过。`pnpm test` 的 pretest 通过，随后 discovery runner 发现旧组织 CAS 夹具；将其迁至新语义接口后，394 个 discovery suites 全部通过（`tempora-frontend-discovery-final.log`）。build 包含 hooks、界面层级、CSS 及当前工作区预算检查 |
| 契约/inventory | 按当前源码重新生成；inventory 新鲜度检查通过，`tempora-independent-inventory-check.log`；`git diff --check` 通过 |
| Chromium | 真实 ProjectTree、顶部、HistoryPanel、审批卡和 command owners；仅 RPC 边界使用 stub。覆盖键盘、拖拽/分组/排序、审批/停止、归档竞态、分页及未读。`tempora-independent-browser/result.json`、workbench/creation 明暗四种截图与 `pagination-recovered.png` |
| Electron | 真实开发壳、生产构建 App 前端、真实 Go service、临时 loopback provider；七项场景通过，含启动身份复用、后台完成及两次重启。`tempora-independent-native/result.json`；`phase-*.json` 记录 canonical 身份和 pending-create 检查点 |
| 旧版写入 | 真实基线拒绝 v3 且保留文件字节，通过。`tempora-independent-registry-compatibility.json` |
| 远程/工作区 | 协议/owner 隔离夹具及前端 `pnpm test:remote`、`pnpm test:workspace` 通过，`tempora-independent-remote-front.log`、`tempora-independent-workspace-front.log`；未进行真实 SSH 主机断线重连验收 |
