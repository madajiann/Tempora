...
The production chat path adopts the content layering, compact process rows, call inspection, and file-artifact interactions of DeepSeek Harness `c291e7961a515f6d7af9304e7fd1d257929aef26`, while retaining Tempora session protocols, permissions, the built-in `present` tool, resource APIs, and Electron workspace.
...
Folding unmounts Markdown, diagrams, terminal output, and detail subtrees. Tempora intentionally does not retain Harness `hidden="until-found"` DOM, so browser find covers mounted content only.
