...
八款 Tempora 官方主题的全部图片均为**原创**，由 `scripts/official-theme-art/` 中的脚本
从零程序化生成（numpy + Pillow，固定随机种子，可完全复现）。视觉*方向*参考了 MIT 许可的
[Codex-Dream-Skin](https://github.com/Fei-Away/Codex-Dream-Skin) 概念图库，但是：

- **未复制参考项目或任何第三方素材的任何像素**、布局、界面元素、文字、标志或水印；
  所有背景均为代码重新生成的独立素材。
- 图中人物均为生成器绘制的**原创虚构成年人**：插画女性（玫瑰晨光）、吉祥程序员（鸿运工坊）、
  读者（鼠尾草清风）、动漫人物（灵感手账）、剪影女性（紫曜星夜）、数字表演者（青岚舞台）、
  绅士（黑金序曲）。赤曜新城不含人物。
- 背景中不含窗口、侧栏、卡片、按钮、输入框或可读文字，并已去除 EXIF 等元数据。

素材随 Tempora 仓库以 MIT 许可发布，© Tempora Contributors。人工审核：Tempora Contributors（发布 PR 审核）。

生成日期：2026-07-17

| 主题 | 生成器（最终提示词等价物） | background.webp SHA-256 | preview.webp SHA-256 |
| --- | --- | --- | --- |
