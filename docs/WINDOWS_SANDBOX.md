...
Tempora uses the same restricted-token pattern as DeepSeek Harness commit
...
For each canonical writable directory, Tempora derives a deterministic
capability SID from a Tempora-specific domain, the capability purpose, and the
...
record below the host-controlled Tempora state root. After the exact ACE and
...
Tempora helper process remains hidden by the desktop process launcher.
...
Protected Tempora state cannot be inside a granted workspace, and arbitrary
...
sandbox failure; Tempora does not silently rerun the command without a sandbox.
...
- NTFS hard links and filesystem objects with unusual ACL inheritance require
  native regression coverage. Tempora revalidates directory identity but does
...
CI includes `tempora/internal/winsandbox` in the Windows smoke group. Non-Windows
