...
| 完全权限 | 以当前系统账户运行，不使用 Tempora 文件和网络沙箱；宿主仍在启动前执行显式禁止规则。 |
...
Tempora 始终会从工具子进程环境中移除已保存的 provider 与 bot 凭据变量。在 macOS
...
工具链。权限模式仍作为
Tempora 工具层边界生效：仅可查看拒绝文件写入并在每条 Shell 命令前询问；工作区内修改
...
可读文件。`[sandbox] bash = "enforce"` 在 Windows 上解析为 `off`，`tempora doctor`
会报告被忽略的值。
...
`.tempora/autoresearch/<task-id>/` 目录保持只读。旧预算参数仍可解析，但不显示在帮助或补全中。
