...
with `runAs: subagent`: Tempora starts an isolated child agent, gives it the
...
```bash
tempora subagent create reviewer \
...
`.tempora/skills/<name>/SKILL.md`; global profiles are stored under the
Tempora home Skill directory described in
...
printf '%s\n' 'Review the task and report only actionable findings.' | \
  tempora subagent create reviewer --description "Code reviewer"
```

Names may contain letters, digits, `_`, `-`, and `.`. Tempora refuses a name
...
tempora subagent try reviewer "review the current diff"
...
git diff | tempora subagent run reviewer --max-steps 20
```
...
apply. Ordinary `tempora run` remains a plain one-shot task entry point and
...
tempora subagent create <name> --description TEXT (--prompt TEXT | --prompt-file PATH)
  [--scope project|global] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
tempora subagent edit <name> [--description TEXT]
...
```bash
tempora subagent edit reviewer --model= --effort= --tools= --color=
```
...
Profiles created in desktop settings and with `tempora subagent create` share
...
`tempora subagent list`, the current `--dir`, and `skills.disabled_skills`. If
...
invalid effort levels are rejected when Tempora resolves the effective model.
