...
> 如何在 IM 里使用 Tempora，以及权限、授权、问答和常用命令的交互方式。
...
连接机器人后，你可以在飞书、Lark、微信或 QQ 里给 Tempora 发消息，让桌面端
Tempora 或 `tempora bot start` 进程在本机执行同一套模型、工具、权限与
沙盒逻辑。

典型场景：

- 让 Tempora 查代码、读文档、解释错误、整理结论。
...
- **桌面端 runtime**：在 **设置 -> 机器人** 中配置。桌面端会启动 gateway，
  ...
  本地 IM 会话。
- **CLI runtime**：执行 `tempora bot start` 启动无界面长期进程。它复用
  与桌面端相同的配置、白名单、路由、队列设置、配对存储、适配器和
  项目/会话索引。

普通 `tempora run` 不会自动启动 IM 网关。只有桌面端 bot runtime 正在运行，
或存在一个存活的 `tempora bot start` 进程时，远端 IM bot 能力才会生效。

## 连接四个渠道

打开桌面端 Tempora，进入 **设置 -> 机器人**。在 **添加 IM Bot** 区域选择
渠道并扫码。
...
格式；如果卡片超过平台限制，Tempora 会自动降级为纯文本。
...
一次性配对码。配对码需要在本机执行 `tempora bot pairing approve <code>`
...
命令能力。远端用户进入的是同一个 Tempora controller、权限策略、工具
...
sequenceDiagram
  ...
  participant R as "Tempora 桌面端或 bot start"
...
| `/projects [关键词]` | 查看已索引项目工作区 | `/projects tempora` |
...
`/queue resume`。`tempora bot doctor --deep` 会显示队列、配对和角色诊断信息。
...
`.tempora/attachments`，并以 `@.tempora/attachments/...` 形式传给
Tempora。保存失败的附件会在 IM 中提示，文本内容仍会继续处理。内置
Feishu、Weixin、QQ 适配器当前仍以文本事件为主，普通 IM 附件抽取可以继续在
适配器层补齐。

## 权限与授权

Tempora 的机器人沿用桌面端权限系统，默认使用工作区内修改。工作区和会话私有临时目录内的普通操作直接执行，越界操作才请求范围明确的授权。
...
正常升级或覆盖安装 Tempora app 后，不需要重新绑定。

绑定信息保存在用户配置目录，而不是 app 包内：

- Bot 连接、远端 ID、白名单、模型和审批模式保存在用户配置文件。
- 飞书和 Lark 的密钥保存在 CLI 与桌面端共用的 Tempora 全局
  `<Tempora home>/.env`。
- 微信扫码后的账号 token 保存在 Tempora 的用户数据目录。
- QQ 的 App ID 保存在用户配置文件；App Secret 通过配置的环境变量
  （默认 `QQ_BOT_APP_SECRET`）保存在 Tempora 全局凭据文件中。
...
| 已连接但没有回复 | 确认桌面端 bot runtime 或 `tempora bot start` 进程正在运行，Bot 连接已开启，用户 ID 在白名单内、已配对或允许所有人。 |
