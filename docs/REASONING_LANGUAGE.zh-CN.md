...
`agent.reasoning_language` 控制模型服务暴露“可见思考过程”时，Tempora 希望它优先使用哪种语言。
...
桌面端设置会写入用户级默认值。项目仍然可以通过 `./tempora.toml` 覆盖。
...
```bash
tempora config reasoning-language auto
tempora config reasoning-language zh
tempora config reasoning-language en
```

默认写入用户配置。要写入当前项目的覆盖配置：

```bash
tempora config reasoning-language --local zh
```
...
斜杠命令会写入用户级设置，并立即更新当前 chat controller，后续 turn 生效。它不会改写当前项目的 `tempora.toml`；如果要写项目级覆盖，请使用带 `--local` 的 shell 命令。
...
```text
./tempora.toml > 用户 config.toml > 内置默认值
```
...
`auto` 仍然对缓存友好。用户原始提问明显是中文时，Tempora 会为这一轮加入同样很小的 `<reasoning-language>` 临时 block；英文或信号不明确时不注入，只复用已有的稳定语言策略。`@file` 等注入的引用内容不会参与这个自动判断。

当设置为 `zh` 或 `en` 时，Tempora 总是会把一个很小的 `<reasoning-language>` 临时 block 放进本次 user turn。所有模式下，它都不会改变工具 schema 的字节或顺序。官方 DeepSeek-V4-Pro 还会在 system prompt 开头稳定写入一句 agent 人设；其他模型的 system prompt 不变。
