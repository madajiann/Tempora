...
When LANG, LC_ALL and LC_CTYPE are all empty or absent, Tempora selects an
...
当 LANG、LC_ALL、LC_CTYPE 均为空或未设置时，Tempora 为子进程选择已安装的 UTF-8
...
Tempora does not change the parent process environment or force LC_ALL. Thus an
