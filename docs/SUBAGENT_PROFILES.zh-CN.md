...
聚焦任务。每个 Profile 都是带有 `runAs: subagent` 的手动 Skill：Tempora 会启动隔离的
...
`.tempora/skills/<name>/SKILL.md`，全局 Profile 存放在 Tempora home 的 Skill 目录中，
...
```bash
printf '%s\n' '检查任务，只报告可执行的问题。' | \
  tempora subagent create reviewer --description "代码评审"
```

名称可以包含字母、数字、`_`、`-` 和 `.`。如果名称已经被项目级、全局、自定义或内置 Skill
占用，Tempora 会拒绝创建，避免覆盖已有内容。
...
```bash
# 使用只读工具预览。
tempora subagent try reviewer "评审当前 diff"

# 按正常权限和沙盒策略运行。
tempora subagent run reviewer "评审并修复当前 diff"
...
仍然有效。普通 `tempora run` 仍是单次任务入口，不会隐式解释 `/<profile>` 语法。
...
桌面设置页和 `tempora subagent create` 创建的 Profile 共用同一批文件。修改 Profile 后，
请刷新或新建会话，让已经运行的会话重新加载 Skill registry。

如果调用时报 Profile 未知或已禁用，请检查 `tempora subagent list`、当前 `--dir` 和
`skills.disabled_skills`。如果编辑时报 custom 或 rich Profile，应直接编辑其 `SKILL.md`，
不要强行经过 Profile 编辑器。Tempora 在解析有效模型时会拒绝未知模型引用和无效 effort。
