...
扩展让插件包在运行时改变 Tempora 的行为——改写输入、拦截工具调用、
...
```bash
tempora plugin install git:github.com/owner/extension --dry-run   # 预览
tempora plugin install git:github.com/owner/extension --yes       # 安装
tempora plugin show <name>                                        # 详情
tempora plugin doctor <name>                                      # 校验
```
...
Sidecar，也不会发生 JSON 编码、RPC 或事件排队。安装 Runtime 后，Tempora
...
1. 在 `tempora-plugin.json` 中加入
   `apiVersion: "tempora.io/plugin/v2"`，声明 `contributes` 与
   （可选的）`runtime`——见
...
3. 构建 Runtime 二进制，先用
   `tempora plugin install /path/to/plugin --dry-run` 检查信任与能力，再用
   `--link --yes` 安装。
4. 用 `tempora plugin doctor <name>` 校验，在空闲时运行 `/reload`，然后验证
...
- 原生 `tempora-plugin.json` 必须声明精确版本
  `tempora.io/plugin/v2`。扩展 Manifest v1 从未公开发布，因此不提供
  v1 双读或自动迁移路径。
- 旧版本 Tempora 会忽略扩展专有状态：会话级
  `<session>.extensions.json` sidecar 文件、`plugin/...` 模型 ref
  （仅报告模型不可用），以及 `extension_surface`/`extension_status`
  事件类型（旧前端丢弃未知类型；未声明 `tempora.extensionSurface`
...
代码型扩展运行在 Tempora Sandbox 之外，继承未过滤的完整环境：可以
...
- Sidecar 的诊断输出、结构化 UI、拦截器原因和 Provider 错误在进入 UI、
  ...
  数据保持原样；
- Sidecar 崩溃只令其自身操作明确失败——Tempora 绝不静默回退到
  其他模型或策略。
