...
Inside `tempora`, use the slash command:
...
`tempora.toml`; use the shell command with `--local` for that.

Headless runs also use the same setting:

```bash
tempora run "explain this module"
```
...
```text
./tempora.toml > user config.toml > built-in defaults
```
...
Tempora adds the same small transient `<reasoning-language>` block for that
...
When set to `zh` or `en`, Tempora always adds a small transient
