...
Tempora 插件包把 skills、hooks、MCP servers、prompts、主题和代码型扩展组织成一个可安装单元。

## CLI 模式

在终端里使用 `tempora plugin` 安装和管理插件包。插件包当前按全局范围安装，
写入 Tempora home 目录。
...
- 本地目录，目录内需要包含 `tempora-plugin.json`、
...
- `--name <name>` 或 `--name=<name>` 覆盖插件 manifest 里的名称，
  作为本次安装名称。
- `--link` 链接本地插件目录，而不是复制到 Tempora 的插件存储目录。
  移动或删除该目录会导致这个链接插件失效。

如果运行 `tempora plugin install <source>` 时既没有 `--dry-run`，
...
已安装插件不会打开一个独立聊天界面。插件启用后，Tempora 会把它的能力加载到普通交互会话里：
...
- **MCP servers** 会进入正常 MCP/工具流程。用户只需要描述任务，
  Tempora 会在相关时调用插件提供的工具。

如果是在另一个终端里安装、启用、禁用或更新插件，而当前已有 `tempora` 会话正在运行，
...
Tempora 原生插件在根目录声明 `tempora-plugin.json`：
...
相对路径都按插件根目录解析。Tempora 安装插件时不会执行第三方安装脚本。
...
- 既未声明 `args` 也未声明 `shell` 的已有原生 Hook 继续使用 Tempora
  历史 Shell 命令行为；`shellCommand: true` 仍作为 shell form 的旧写法兼容。

## Manifest v2（扩展）

Tempora 原生扩展使用精确的 v2 `apiVersion`：
...
- v2 的资源发现是显式的。Tempora 只加载原生 manifest 中声明的 skills、
...
- `themes` 是 `.tempora-theme` 文件，在 Desktop 设置中以只读插件主题
  展示（ID 为 `plugin:<plugin>:<theme>`），不会复制进用户主题库。插件
  被禁用或卸载时，若当前使用的是它的主题，界面回退到基础样式但保留该
  ID，重新安装同一插件后自动恢复。

`runtime` 块声明的是代码型扩展——由 Tempora 启动并通过 Extension
...
- `command`/`args`/`env` 仅支持 **exec form**：command 即可执行文件，
  绝不经过 Shell 解释；`${TEMPORA_PLUGIN_ROOT}` 展开为插件安装根目录。
...
`tempora plugin show`、能力诊断和 Desktop 安装界面都会显著展示
...
跳过，按名称选中时则直接报错。
对于 Superpowers 和 Claude 风格 skill 包，Tempora 会映射以下兼容约定。
原生 v2 manifest 只使用显式声明，不应用这些回退规则：

- `skills` 到 Tempora skill root。Claude 清单若未声明 `skills` 字段，会回退到
...
- `commands/`（以及 `.claude/commands/`）映射为 Tempora 自定义斜杠命令：每个
  ...
  `tempora-plugin.json` 清单也可以通过 `"commands"` 路径列表显式声明。
- `agents/*.md` 映射为插件所属、需要手动调用的子代理配置。Claude 模型别名会继承
  当前 Tempora 模型；内联 `tools` 列表会转换为 Tempora 工具名，并支持
  `mcp__*__search` 这类 MCP 通配符。Agent 使用独立的
  `/<插件>:agent:<名称>` 命名空间，因此上游 Agent 与 Skill 同名时不会互相遮蔽。
- 如果存在 `hooks/session-start-codex`，映射为 Tempora `SessionStart` hook。
- 对 Codex 兼容包，插件根目录的 `CLAUDE.md` 会映射为内置的 `SessionStart`
  上下文 hook，Tempora 会直接读取该文件，不通过 shell 命令。Claude 插件
...
- `.claude/settings.json` 和 `hooks/hooks.json` 里的 command hooks 会按同名事件映射。
  ...
  `tool_name` 会在 Tempora 与 Claude 的工具名之间互译（`bash` ↔ `Bash`、
  `write_file` ↔ `Write` 等），因此 `"Bash"` 这类 matcher 能正确触发；Tempora 里所有会
  ...
  `tempora plugin doctor <名称>` 和 `tempora doctor capabilities` 会在 Hook 首次触发前
  ...
  字段也不会被求值。声明其中之一、声明 `Stop`/`SubagentStop` hook（Tempora 中
  不能阻止本轮结束），或 matcher 覆盖三种无法无损表达的输入时，插件都会报告
  部分兼容并附具体警告：`WebFetch.prompt`、Tempora 以 `cell_number` 调用时的
  `NotebookEdit.cell_id`，以及 Tempora `wait` 同时覆盖多个/全部任务时的
...
不支持的 Claude hook item type 会跳过并产生 warning。Tempora 不会执行第三方安装脚本。
