...
| Full access | Runs as the current OS user without Tempora filesystem or network sandboxing. Explicit host deny rules still apply before launch. |
...
toolchains. Permission presets still apply as Tempora tool-layer
...
`tempora doctor` reports the ignored value.
...
`.tempora/autoresearch/<task-id>/` archives remain read-only. Deprecated
