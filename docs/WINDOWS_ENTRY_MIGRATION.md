...
`Tempora.exe` is the canonical GUI entry; `tempora-cli.exe` remains the CLI
forwarder. Fresh installers and newly extracted ZIPs omit the root
`tempora-launcher.exe`. A managed upgrade preserves and refreshes that entry
only when it already exists. There is no automatic retirement deadline.

`Tempora.exe` 是标准 GUI 入口，`tempora-cli.exe` 仍为 CLI 转发入口。
新安装、全新解压 ZIP 不包含根目录 `tempora-launcher.exe`；受控升级仅在旧入口
...
| `io.tempora.desktop` | Same window and shortcut identity / 相同窗口及链接身份 | None / 无 |
| User configuration and sessions / 用户配置与会话 | No migration or new writes / 无迁移或新增写入 | None / 无 |

Signed payloads and NSIS staging continue to use `tempora-launcher.exe`.
...
verified bytes to `Tempora.exe`, and to the old name when preservation applies.
...
签名载荷和 NSIS staging 继续使用 `tempora-launcher.exe`。安装新版 helper 不会
...
node desktop/packaging/verify.mjs dist/Tempora-windows-amd64.zip
...
scripts/verify-windows-portable.sh STAGING canonical PAYLOAD/tempora-launcher.exe
```
