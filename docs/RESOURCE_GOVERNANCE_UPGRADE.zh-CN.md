...
生产前端构建的全部 bundle 预算通过；`desktop/ go test ./...` 全绿（主包 335.895 s）。根 `go test ./...` 仅有 3 个 `cmd/tempora-legacy-migrator` 既存失败，错误均为 `pending update belongs to a different installation: pending update bundle is not the current Guard installation`；资源治理涉及的其余根包通过。
