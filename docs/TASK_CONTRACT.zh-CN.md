...
有些 prompt 模板把最后一段叫 “Checkpoint”；Tempora 文档里叫 “Pause policy”，避免和 Checkpoints/Rewind 快照混淆。
...
- **Goal 模式**会把目标当作任务合约持续推进，由模型显式报告完成或阻塞；用户控制和资源限制始终有效。
- **计划模式**适合“先产出并确认方案，再进入实施”的场景；批准前由宿主强制禁止写入。
...
模型将用户要求的检查和验收说明作为任务上下文执行。宿主不再从文件数量、路径、计划或“完整验证”等措辞推导质量义务。权限、Plan 审批前写入限制、取消、显式预算和可靠执行仍然有效。结果展示真实命令、结果及模型声明，不再生成总体质量判定。详见[执行语义与迁移说明](EXECUTION_MODEL_SIMPLIFICATION.md)。
