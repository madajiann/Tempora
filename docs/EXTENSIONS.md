...
Extensions let a plugin package change what Tempora does at runtime —
...
```bash
tempora plugin install git:github.com/owner/extension --dry-run   # preview
tempora plugin install git:github.com/owner/extension --yes       # install
tempora plugin show <name>                                        # details
tempora plugin doctor <name>                                      # validate
```
...
vendor method `_tempora.io/session/reloadExtensions`:
...
2. When idle, Tempora starts new sidecars and builds a new runtime
   snapshot.
...
When runtimes are installed, Tempora initializes at most four sidecars at once
...
1. Add `apiVersion: "tempora.io/plugin/v2"` to `tempora-plugin.json` and
...
3. Build the runtime binary, preview its trust and capabilities with
   `tempora plugin install /path/to/plugin --dry-run`, then install it with
   `--link --yes`.
4. Validate with `tempora plugin doctor <name>`, run `/reload` while idle,
...
- Native `tempora-plugin.json` manifests must declare the exact
  `tempora.io/plugin/v2` API version. There is no v1 dual-read or automatic
  migration path because extension manifests were not publicly released on v1.
- Older Tempora versions ignore extension-only state: the per-session
  ...
  clients without `tempora.extensionSurface` get text fallbacks).
- `plugin-packages.json` keeps its existing schema; an enabled installed
  runtime *is* the trust record.

## Security model

A code extension runs outside the Tempora sandbox with the unfiltered
...
- a crashed sidecar fails its own operations explicitly — Tempora never
