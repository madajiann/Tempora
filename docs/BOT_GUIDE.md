...
> WeChat, and QQ bots, how to use Tempora from IM, and how permissions, model
...
After a bot is connected, you can send Tempora messages from Feishu, Lark,
WeChat, or QQ. The desktop app or `tempora bot start` process handles the
model, tools, permissions, sandboxing, and local context, then sends progress
and results back to the IM channel.

Common uses:

- Ask Tempora to inspect code, read docs, explain errors, or summarize findings.
...
- **CLI runtime**: run `tempora bot start` for a headless long-lived process.
...
The normal `tempora run` command does not automatically start the IM gateway.
Remote bot behavior is active only while the desktop bot runtime is running or
while a `tempora bot start` process is alive.

## Connect the four channels

Open the Tempora desktop app and go to **Settings -> Bots**. In **Add IM Bot**,
...
CommonMark formatting. If a card is too large for the platform limit, Tempora
...
```sh
tempora bot doctor
tempora bot doctor --deep
tempora bot start --channels qq,feishu,lark,weixin --dir /path/to/project
```
...
- `credential.app_id`, `credential.app_secret_env`, `credential.account_id`,
  ...
  tokens. Secrets stay in environment variables or the Tempora user credentials
  store.
...
`tempora bot pairing approve <code>` before the sender can drive the bot; when
...
```sh
export TEMPORA_BOT_CONTROL_TOKEN="change-me"

curl -H "Authorization: Bearer $TEMPORA_BOT_CONTROL_TOKEN" \
  http://127.0.0.1:37913/status

curl -X POST http://127.0.0.1:37913/send \
  -H "Authorization: Bearer $TEMPORA_BOT_CONTROL_TOKEN" \
...
sequenceDiagram
  ...
  participant R as "Tempora desktop or bot start"
...
| `/projects [query]` | List indexed project workspaces | `/projects tempora` |
...
paused until `/queue resume`. `tempora bot doctor --deep` reports queue,
...
- `/search all <query>` searches file contents across indexed project roots.
  Tempora uses `rg` when available and falls back to a bounded Go scanner.
...
current workspace's `.tempora/attachments` directory and passes them to
Tempora as `@.tempora/attachments/...` references. If an attachment cannot be
...
Tempora bots use the same permission runtime as the desktop app. Workspace
...
- Full access runs commands as the current OS user without Tempora filesystem
...
No. A normal Tempora app upgrade or overwrite install does not require
rebinding.

Bindings are stored in the user's Tempora data, not inside the app bundle:

- Bot connections, remote IDs, allowlists, model choices, and approval modes are
  stored in the user config.
- Feishu and Lark secrets are stored in Tempora's global
  `<Tempora home>/.env`, shared by CLI and desktop.
- The WeChat scanned account token is stored in the Tempora user data
  directory.
...
- The Tempora user config directory was deleted.
...
| Connected but no reply | Make sure the desktop bot runtime or `tempora bot start` process is running, the bot connection is enabled, and the sender ID is allowlisted, paired, or access is open. |
