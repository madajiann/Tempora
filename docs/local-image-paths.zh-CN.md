...
粘贴或拖入的文件先保存在发起操作的标签页工作区中的 `.tempora/attachments`，再作为不可变原图写入会话内容存储。保存、预览和原生视觉读取在异步操作期间始终绑定同一个标签页及工作区；切换标签页不会把结果写入新激活的草稿。目标绑定操作会在浏览器读取文件或计算摘要前协商 `attachments-v2`；旧标签页 API 保留 `attachments-v1` 能力名，两条路径都不信任客户端自报摘要。历史 `@.tempora/attachments/...` 引用无需改写即可继续读取。已接纳图片的历史卡片只带 digest；桌面通过 `ReadSessionAttachmentForTab` 在会话内容图授权后再加载原图。远程 Serve 不声明这两项本地暂存能力。
...
| 工作区 `.tempora/attachments/...` 文件 | 写入 | 仍是 Composer/CLI 入口；接纳后的对象在 `.content-v1` | 按文件读取 |
