...
| 名称 | 载荷 |
|---|---|
| `tempora.subagent.status` | 恰好为 `queued`、`running`、`reasoning`、`responding`、`tool`、`retrying`、`completed`、`failed`、`cancelled` 之一 |
| `tempora.subagent.reasoning` | 受限的 UTF-8 文本增量（子任务的思考） |
| `tempora.subagent.text` | 受限的 UTF-8 文本增量（子任务的回答预览） |
| `tempora.subagent.notice` | 受限的 UTF-8 文本增量（子任务的提示） |
...
前端按 `tempora.subagent.` 前缀匹配保留名称，因此较新 agent 新增的频道会被较旧前端忽略（绝不追加进普通工具输出）。
