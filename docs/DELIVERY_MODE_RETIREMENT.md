...
Tempora no longer offers Delivery as a session mode. New and restored local
...
recovery rules. Tempora never clears missing checks or rewrites failed evidence
as passed.
...
Tempora 不再提供“交付”会话模式。新建和恢复的本地会话统一采用标准执行。
...
检查点继续使用原有恢复规则。Tempora 不会清空缺项，也不会把失败证据改写成通过。
