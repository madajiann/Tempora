...
Tempora uses one permission preset for every execution surface: Desktop, CLI,
...
| **Full access** (`danger-full-access`) | Commands use the normal host path as the current OS user; Tempora filesystem and network sandboxing are disabled. | Ordinary `ask` fallbacks are skipped. Explicit host `deny` rules still apply before launch, but Tempora does not constrain the launched process. |
