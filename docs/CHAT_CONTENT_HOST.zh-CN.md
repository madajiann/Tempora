...
当前聊天实现借鉴 DeepSeek Harness `c291e7961a515f6d7af9304e7fd1d257929aef26` 的内容分层、过程摘要、工具行、调用检查和文件产物交互，并适配 Tempora 已有协议和工作台。生产路径只有一套实现：本地会话、远程会话和已有工作台都消费统一投影。
...
投影是可重建的视图数据，不是第二份持久化会话事实。Tempora 没有引入 Cordis、另一套服务端事件日志或 Harness 的整个 Conversation 框架。
