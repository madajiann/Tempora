...
Tempora already has a solid extension kernel: `Contribution → RuntimeSnapshot`,
...
- Upgrade native plugins to `tempora.io/plugin/v2`
...
Native `tempora-plugin.json` requires:
...
- `tempora plugin migrate <name> --to-v2` rewrites only pre-extension native
...
- `tempora plugin doctor` reports dependency and protocol errors
...
| Manifest `tempora.io/plugin/v2` + reject v1 | Done |
