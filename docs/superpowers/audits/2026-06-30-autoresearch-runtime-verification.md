...
`.tempora/autoresearch/<task-id>/`, injects a runtime summary into active goal
...
| State files are host-owned JSON/JSONL under `.tempora/autoresearch/<task-id>/` | `task_spec.json`, `progress.json`, `directions_tried.json`, `findings.jsonl`, `iteration_log.jsonl`, `heartbeat.jsonl`; store tests and `./dev` smoke task directory | Verified |
...
| Explicit `.tempora/autoresearch/<task-id>/` resumes existing task | `ResumeFromGoalText`; `TestResumeFromGoalTextLoadsExplicitTaskPath` | Verified |
...
```bash
GOCACHE=/private/tmp/tempora-go-build-cache go test ./internal/autoresearch ./internal/control -count=1
GOCACHE=/private/tmp/tempora-go-build-cache go test . -run 'TestAutoResearch|TestMetaReportsGoalStatus' -count=1
GOCACHE=/private/tmp/tempora-go-build-cache go test . -count=1
...
- External browser loaded the page with title `Tempora`.
...
- Host state files were created under `.tempora/autoresearch/...`.
