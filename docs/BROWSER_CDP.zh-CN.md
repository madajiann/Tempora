...
Chromium 界面。CLI、`tempora serve` 和 headless 会话背后没有外壳，于是同一批
...
普通路径到此为止。第一次调用浏览器工具时，Tempora 才会用一次性 profile 启动一个
...
| `chrome_path` | 浏览器可执行文件。留空则按常见位置查找 Chrome、Chromium 和 Edge，再读 `TEMPORA_CHROME` 与 `CHROME_PATH`。 |
