...
Tempora 的桌面端、CLI、Serve、ACP、机器人、自动任务和子代理共用同一套权限模式。协作方式（普通、计划、目标）决定任务如何推进；权限模式决定文件系统和进程的强制边界。
...
| **完全权限**（`danger-full-access`） | 命令以当前系统账户走宿主正常执行路径，不使用 Tempora 文件和网络沙箱。 | 跳过普通询问规则；宿主会在启动前执行显式 `deny`，但不再限制已启动进程。 |
