...
- System and permission receipts are compact disclosures. Uninformative empty
  ...
  the adapted Harness styles; Tempora's parsing and security remain in place.
...
The port retains Tempora theme/font settings, composer, approvals, host-safe
...
or every plugin-specific renderer is embedded in Tempora.
...
The test build is installed at `~/Applications/Tempora-Canary/Tempora.app`
with the isolated `Tempora-Test-ChatRefactor` profile. Native WebView hosts on
