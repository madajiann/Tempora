...
| In-progress model credential commits | `<Tempora home>/transactions/model-credentials/` |
| Completed model settings receipts | `<Tempora home>/transactions/model-settings-receipts/` |
...
legacy `desktop-topic-*.json` files from a project's `.tempora/` directory (or
the global Tempora directory). A scope with legacy files continues mirroring
...
and `tempora.toml` are unaffected.
...
skill, sandbox, bot, and agent settings that Tempora renders into user config.
...
credential `.env` is also hidden from Tempora's file readers, sandboxed shell
...
v1.38.10) blocks the credential store, Tempora removes it automatically when
a marker from that sandbox run proves the entry came from Tempora. Saving a
...
desktop settings, TUI `/setup`, or `tempora setup`, Tempora allocates a fresh
`TEMPORA_CONNECTION_*_KEY` slot. It writes that slot first and atomically
