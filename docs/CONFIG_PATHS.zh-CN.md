...
从 **Tempora v1.8.1** 开始，Tempora 使用一个用户可见的全局目录存放配置和用户状态。CLI 与桌面端共用这个目录。
...
可以设置 `TEMPORA_HOME` 覆盖 Tempora home，主要用于测试、CI 或便携安装。普通用户通常不需要设置。

设置 `TEMPORA_HOME` 后，运行时会变成完整自包含模式：配置、状态、缓存和数据都会位于该目录树下。
Legacy 迁移、OS home 约定目录扫描以及其他 fallback 路径都会跳过，避免从系统级正式安装带入或写回数据。

高级测试或便携安装可以设置 `TEMPORA_STATE_HOME` 来移动 sessions、archive、memory 等运行状态。
它不会移动全局配置或 provider 凭据；这些仍然位于 `TEMPORA_HOME` 下。如果旧版本曾把 provider key
写到 `TEMPORA_STATE_HOME/.env`，Tempora 会在 `<Tempora home>/.env` 缺少对应 key 时非破坏性导入。

## 目录内容

| 数据 | 路径 |
| --- | --- |
| 全局配置 | `<Tempora home>/config.toml` |
| 全局 provider 凭据 | `<Tempora home>/.env` |
| 进行中的模型凭据提交 | `<Tempora home>/transactions/model-credentials/` |
| 已完成的模型设置回执 | `<Tempora home>/transactions/model-settings-receipts/` |
| 旧 credentials 导入来源 | `<Tempora home>/credentials` |
| 全局斜杠命令 | `<Tempora home>/commands/` |
| 全局 skills | `<Tempora home>/skills/` |
| 全局 hooks | `<Tempora home>/settings.json` |
| 远程 SSH 托管 known_hosts | `<Tempora home>/remote/known_hosts` |
...
全局用户配置文件名是 `config.toml`。项目本地配置文件仍叫 `tempora.toml`。
如果有人说“全局 tempora.toml”，通常指的是 `<Tempora home>/config.toml`。

## 全局 `config.toml`

`<Tempora home>/config.toml` 存放 CLI 与桌面端共用的非密钥配置。它可以包含
Tempora 写入用户配置的 provider、plugin、UI、desktop、tool、skill、sandbox、
bot 和 agent 设置。Provider 条目只保存 `api_key_env` 里的凭据变量名，不保存真实密钥值。

已保存的 provider 与 bot 凭据变量不会进入任何由模型控制的子进程环境。在 macOS
和 Linux 上，Tempora 的文件读取工具、受沙盒保护的 shell 命令和 MCP server 也
...
Tempora 会在该沙箱运行留下的标记能证明拒绝项来自 Tempora 时自动移除它。即使没有
...
通过桌面端设置、TUI `/setup` 或 `tempora setup` 新增、替换或明确清空 provider
凭据时，Tempora 会分配新的 `TEMPORA_CONNECTION_*_KEY` 独立槽位，先写入槽位，再
...
Tempora 不会追加或改写路径。已有 TOML 配置不会被重新解释：旧 `chat_url` 继续
...
模型发现需要单独地址时可设置 `models_url`；否则 Tempora 会继续从 `base_url`
推测模型发现地址。

## 全局 `.env`

`<Tempora home>/.env` 是 Tempora 保存的 provider API key 的唯一运行时来源。
...
- `# tempora-cleared KEY` 是删除 key 后写入的非密钥标记，用来防止旧存储把它静默迁回；
- 在操作系统支持的情况下，Tempora 会用受限权限写入该文件。
...
项目 `.env` 仍会作为当前 workspace 范围内的非 provider 变量展开来源，例如 MCP/plugin 的 env、headers、URL、command 和 args 中的 `${VAR}`；这些值不会写入进程环境，`TEMPORA_HOME`、`TEMPORA_STATE_HOME`、`XDG_CONFIG_HOME` 等 Tempora 控制变量也会被忽略。

缓存仍放在系统缓存目录，例如 macOS 的 `~/Library/Caches/tempora`、
Linux 的 `$XDG_CACHE_HOME/tempora` 或 `~/.cache/tempora`、Windows 的
`%LOCALAPPDATA%\tempora\cache`。可以设置 `TEMPORA_CACHE_HOME` 覆盖缓存根目录。
设置 `TEMPORA_HOME` 后，缓存会放在 `$TEMPORA_HOME/cache`；如果同时设置
`TEMPORA_CACHE_HOME`，后者优先。
...
从 **v1.8.1** 开始，Tempora 启动时会在第一次加载配置前自动检查旧路径。迁移是同步、一次性、非破坏性的：旧文件会被复制或转换到 Tempora home，原文件保留。
...
旧 credentials、memory 文件和 sessions 也会在新目标不存在时导入到 Tempora home。
旧 provider key 只会在 `<Tempora home>/.env` 尚未包含同名 key 时复制进去。若新的全局配置已经存在，则新配置优先；旧配置只作为兼容 fallback 保留。

从 **v1.9.1** 开始，Tempora 还会在升级时把已知旧路径、legacy `config.json`、
桌面端已登记项目和恢复 tabs 对应项目里的 MCP 配置汇总补齐到全局
`<Tempora home>/config.toml`。已有的全局 `[[plugins]]` 按名称优先，不会被旧
配置或项目配置覆盖；源文件会保留不变。该补齐会写入一次性 marker，避免用户之后
主动删除某个全局 MCP 时又被旧项目配置反复恢复。

## 手动补救迁移

如果 Tempora 已经创建了新的 home 目录，但当时旧数据还不在可扫描路径里；或者先打开了桌面端，导致自动迁移没有把旧路径数据补齐，可以在任一前端运行补救命令：
...
显式形式只导入 sessions。这个路径可以是旧安装目录、`.tempora`/数据目录，或者
`sessions` 目录本身；Tempora 会在该根目录下检查常见布局，并使用按来源目录区分的
marker，因此之前已经运行过普通 `/migrate` 也不会挡住这次后补导入。

该补救命令仍然是非破坏性的。它不会覆盖已有的
`<Tempora home>/config.toml`；如果新配置已经存在，需要手动把旧配置里缺失的设置复制过去。旧 memory 文件只会在目标文件不存在时复制。它也会尊重 session 导入 marker，因此已经迁移过、之后又被用户删除的会话，不会在后续 `/migrate` 中被重新恢复。

版本限制：

- 自动迁移从 **v1.8.1** 开始。
- `/migrate` 只存在于包含该命令的 Go 版 Tempora 构建中。如果 Tempora 提示 `unknown command`，请先升级后再运行。
