...
已保存的 provider 与 bot 凭据变量不会进入任何由模型控制的子进程环境。Tempora 的
...
通过桌面端设置或 `tempora setup` 添加自定义 provider 时，Tempora 会把生成的
`api_key_env` 保存到 `config.toml`，并把真实密钥值写入全局 `.env` 中同名的 key。
生成结果是稳定的，因此同一个 provider 重启后仍会读取同一个凭据槽位。

Tempora 会根据 provider 名称生成默认值。能规范化成 ASCII 的名称会得到可读的
