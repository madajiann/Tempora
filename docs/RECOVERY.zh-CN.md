...
Tempora 不再提供产品化的 `tempora-guard` 恢复壳。崩溃记录、pending 更新状态
...
tempora crash report   # 视构建是否包含而定
```
...
1. 从官方下载页获取最新签名安装包。
2. 直接覆盖当前安装（Windows：双击安装；macOS：替换 `Tempora.app`）。不要先卸载：
   保留原安装根目录，兼容迁移程序才能证明遗留事务属于哪个安装。
3. 启动 Tempora 一次，确认“设置 > 更新”显示的版本正确，再尝试下一次应用内更新。
4. 兼容载荷中可能仍带有名为 `tempora-guard` 的**一次性迁移程序**，它只把旧平铺
...
2. 若没有该按钮或操作失败：完全退出 Tempora 后重新启动一次（让启动路径提交或清理试用事务），再试应用内更新。
3. 若仍失败：从官方下载页获取最新签名安装包，**直接覆盖安装**当前版本，不要先卸载。
4. macOS 还需在「系统设置 → 隐私与安全性 → App 管理」中允许 Tempora；若
   `Tempora.app.tempora-update-backup` 因 TCC 无法删除，请改走官方安装包覆盖路径。

如果 Windows 安装器显示 `Tempora layout activation failed`，请展开安装详情并复制
`Tempora layout activator output:` 下方的内容。当前安装器会保留 activator 的具体错误，
不再只显示 exit code 1。
...
替换后的窗口真正显示后，Tempora 只会提交启动前捕获的精确 pending 事务。对于缺少
