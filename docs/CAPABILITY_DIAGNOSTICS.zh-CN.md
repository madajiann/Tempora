...
```json
{"action":"call","capability_id":"tool:read_skill","arguments":{"name":"tempora-guide","reference":"references/hooks.md"}}
```
...
内置 `tempora-guide` 在下次变化的 `session-context` Skills 目录中增加一行；
