...
Tempora exposes a single `/effort` knob (and the per-provider `effort` /
...
|----------------|----------|-------------------|------------------|-------|
| Kimi CN/Global `kimi-k3` | `api.moonshot.cn/v1`, `api.moonshot.ai/v1` | `reasoning_effort` | `low`, `high`, `max` | Always thinks; defaults to `max`. Tempora replays the complete assistant message, uses `max_completion_tokens`, and omits K3's fixed sampling fields. |
...
and custom model IDs. While GLM thinking is enabled, Tempora retains and
...
While this protocol is selected, Tempora always exposes K3's fixed
...
available. When selected for the main conversation, Tempora emits
...
400, Tempora rebuilds only the provider-visible projection of the old history,
...
Tempora preserves those declared values instead of applying the generic
...
Distinguishing "provider ignores the field" from a Tempora bug starts here:
the request shape Tempora emits is fixed per the table, so a mismatch between
the table and observed behaviour is the provider's, not Tempora's.
