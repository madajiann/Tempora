# 迁移到 Tempora 1.0（Go 重写版）

<a href="./MIGRATING.md">English</a>

Tempora 1.0 是一次从零开始的 **Go 重写**。它使用全新的代码库，并不是 `0.x` TypeScript 版本的增量升级。本文说明两个版本的差异以及迁移方法。

## 摘要

| | 旧版（v1） | Tempora 1.0+（v2） |
| --- | --- | --- |
...
| 安装 | `npm i -g tempora@0.53.2`（固定到某个 `0.x` 版本） | `npm i -g tempora`；也可使用 release 归档或源码构建 |
...
**`npm i -g tempora` 会安装当前正式的 `1.x` 版本。** npm 的 `latest` 标签已从 `1.17.5` 起切换到 Go 版本。以后不再公开发布候选版本；旧 `next` 与 `canary` 标签仅作为兼容别名，始终指向同一个正式版本。旧版 `0.x` 仍可通过固定版本安装：

```sh
npm i -g tempora          # 当前正式的 1.x
npm i -g tempora@0.53.2   # 固定到旧版 TypeScript 构建
```

每个 GitHub release 都附带预编译归档（`tempora-<os>-<arch>.tar.gz` / `.zip`）和桌面安装包。它们与 npm 是不同的安装渠道：桌面安装包不会改动通过 `npm i -g` 安装的 CLI，因此 shell 中的 npm `0.53` 与 `1.x` 桌面应用可以共存，并不冲突。
...
| 旧版 | Tempora 1.0 |
| --- | --- |
| TypeScript 配置文件 | 项目使用 `tempora.toml`；从 v1.8.1 起，全局配置为 Tempora home 下的 `config.toml`（macOS/Linux：`~/.tempora/`；Windows：`%AppData%\tempora\`）。参见 `tempora.example.toml` 和[配置路径](./CONFIG_PATHS.zh-CN.md) |
| 环境变量 / API key | provider 配置保留 `api_key_env`；保存的 key 位于 Tempora home 的 `.env`（`DEEPSEEK_API_KEY`、`MIMO_API_KEY` 等） |
| 项目记忆 | `TEMPORA.md`（含自动记忆），兼容 Claude Code |
| MCP server | 在 `tempora.toml` 中使用 `[[plugins]]`，或直接读取 Claude Code 的 `.mcp.json` |
...
- v0.x 的 `~/.tempora/config.json`

导入内容包括 API key、base URL、语言和 MCP server；缺失的旧凭据会迁移到 `<Tempora home>/.env`，旧会话也会从历史目录导入。原文件不会被修改，Tempora 会在导入后显示启动提示。
...
Desktop 会自动把四个旧 `desktop-topic-*.json` 索引迁移到 Tempora 状态根目录下按
...
迁移不会删除旧文件，也不会修改项目拥有的 `.tempora` 资产。不支持新旧 Desktop 同时写
同一个工作区。
...
| 现有数据 | 升级行为 |
| --- | --- |
| `TEMPORA.md`、`AGENTS.md`、`CLAUDE.md` | 作为常驻指令加载，并附带来源、目录、precedence、imports 和 diagnostics；原文件名继续有效。 |
...
- **MCP 添加后即可使用**：用户通过桌面端、CLI、全局配置、旧配置导入或主动安装插件包添加的 server 默认可信，全局安装统一写入 `config.toml`。仓库内 `tempora.toml` / `.mcp.json` 声明保留在项目中，同样无需额外启动确认。同名时项目覆盖全局，项目内部 `tempora.toml` 高于 `.mcp.json`。打开陌生仓库等同于接受其中可执行的项目配置；启动 Tempora 前应检查 `.tempora/settings.json`、`tempora.toml` 和 `.mcp.json`。如果仓库引发异常的 MCP 或 Hooks 行为，可用安全模式重新启动，在恢复期间禁用这些外部集成。
...
- **Web Dashboard 仍然可用，桌面端更推荐**：需要浏览器访问时，可运行
  `tempora serve` 启动本地 Web UI；日常可视化使用优先选择 Electron 桌面端，
  终端工作流继续使用 CLI/TUI。
- 一些细粒度 v1 工具被合并，例如文件管理操作改由 `bash` 完成；少数工具尚未移植，进度在 Discussions 中跟踪。

## 文件编码

Tempora 1.0 支持读取和编辑 UTF-8、UTF-8 BOM、UTF-16 LE/BE 与 GB18030（GBK 的超集），与 v1 行为一致。
