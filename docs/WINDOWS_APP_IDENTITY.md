...
Tempora Desktop uses the version-independent AppUserModelID
`io.tempora.desktop` for its Electron windows, launcher, Go desktop process,
shortcuts, and notifications. Its visible notification name remains `Tempora`.
This identity is separate from Tempora Studio (`io.tempora.studio`) and the
older Tauri desktop (`dev.tempora.desktop`). Wails Studio v2.10.0 and older
Tempora Desktop builds used the shared ID `Tempora`; new Desktop builds no
longer claim it.

## Installation and upgrade

Use `Tempora.exe` at the installation root to open Desktop; `tempora-cli.exe`
...
`tempora-launcher.exe` when present, with no scheduled removal, so old pins,
...
the Electron `app/Tempora.exe`, remain internal components.

Before showing any Electron window, Desktop sets its relaunch command and icon
to `Tempora.exe`, falling back to the legacy launcher when necessary. New taskbar pins therefore
...
Normal launcher and desktop startup also repair existing Tempora-named links
in the installation directory, private/public desktop and Start Menu Programs
directories (including their Tempora subdirectory), and the current user's
...
An owned shortcut with an empty ID or the old `Tempora` ID adopts the new ID.
...
is named Tempora. Separate installations are not modified.

Links into `versions/<version>/tempora-desktop.exe` or
`versions/<version>/app/Tempora.exe`, as well as a flat `app/Tempora.exe`, move to the permanent
`Tempora.exe` when it exists; owned links to `tempora-launcher.exe` also move
...
Signed update payloads and `/TEMPORASTAGE=1` still contain
`tempora-launcher.exe`: already installed update helpers require this name.
...
The old `Tempora` notification registration and notification history are not
