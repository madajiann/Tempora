# Migrating to Tempora 1.0 (the Go rewrite)

Tempora 1.0 is a **ground-up rewrite in Go**. It is a new codebase, not an
...
| Install | `npm i -g tempora@0.53.2` (pin a `0.x` version) | `npm i -g tempora` — `latest` points at the current official `1.x` release; or a release archive / `go build` |
...
**`npm i -g tempora` installs the current official `1.x` release.** npm's `latest` tag
...
```sh
npm i -g tempora          # current official 1.x release
npm i -g tempora@0.53.2   # pin the legacy TS build
```

Prebuilt archives (`tempora-<os>-<arch>.tar.gz` / `.zip`) and the desktop
...
cd DeepSeek-Reasonix && make build                        # -> bin/tempora(.exe)
```
...
| Legacy | Tempora 1.0 |
|---|---|
| TS config files | `tempora.toml` (project) / `config.toml` in Tempora home (`~/.tempora/` on macOS/Linux; `%AppData%\tempora\` on Windows) from v1.8.1 — see `tempora.example.toml` and [Configuration paths](./CONFIG_PATHS.md) |
| env / API keys | Provider config keeps `api_key_env`; saved key values live in Tempora home `.env` (`DEEPSEEK_API_KEY`, `MIMO_API_KEY`, …) |
| project memory | `TEMPORA.md` (+ auto-memory), Claude-Code-compatible |
| MCP servers | `[[plugins]]` in `tempora.toml`, or a Claude-Code `.mcp.json` (read as-is) |

On first launch, v1.8.1+ runs a one-time, **non-destructive** import: it reads
legacy config from `~/Library/Application Support/tempora/config.toml`,
`~/.config/tempora/config.toml`, `~/.tempora/tempora.toml`, or v0.x
`~/.tempora/config.json` (API key, base URL, language, MCP servers), migrates
legacy credentials into `<Tempora home>/.env` when a key is missing there, and
imports past sessions from legacy session directories. Old files are left
untouched, and Tempora prints a boot notice when it imports data. Each session lands in the
...
The command is available only in Go-based Tempora builds that include it; if you
...
`/migrate --from "D:\OldTempora"` to import sessions from that explicit source.
See
...
to an authoritative per-scope SQLite database under the Tempora state root.
...
changes project-owned `.tempora` assets. Running old and new Desktop versions
...
| --- | --- |
| `TEMPORA.md`, `AGENTS.md`, `CLAUDE.md` | Loaded as standing instructions with source, directory, precedence, imports, and diagnostics. Existing file names remain valid. |
...
changing the fact body. Compatibility routing fields keep older Tempora
...
- **MCP setup is now add-and-use.** Servers added by the user (Desktop, CLI,
  ...
  `tempora.toml` / `.mcp.json` servers stay project-scoped and are trusted
  without a separate launch confirmation. Project entries override same-name
  global entries; `tempora.toml` overrides `.mcp.json` inside the project.
  Treat opening an unfamiliar repository as opting into executable project
  configuration: review `.tempora/settings.json`, `tempora.toml`, and
  `.mcp.json` before starting Tempora. If a repository causes unexpected MCP
...
- **Web dashboard remains available; desktop is recommended**: run
  `tempora serve` when a local browser UI is useful. For the primary visual
...
Tempora 1.0 supports reading and editing files in UTF-8, UTF-8 BOM, UTF-16
