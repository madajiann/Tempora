...
The production frontend build passed every bundle budget, and `desktop/ go test ./...` passed (the main package took 335.895 s). Root `go test ./...` had only three pre-existing `cmd/tempora-legacy-migrator` failures, all reporting `pending update belongs to a different installation: pending update bundle is not the current Guard installation`; the remaining root packages touched by this change passed.
