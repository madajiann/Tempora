...
This document fixes the executable contracts for the Tempora runtime
...
`tempora.session.linear/v3.1`. The retired `tempora.session.events/v3`
prototype and `tempora.session.linear/v3` preview cannot be opened for
