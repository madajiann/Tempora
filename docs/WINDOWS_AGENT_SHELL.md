...
Tempora no longer runs a nested shell/child-process preflight before each Windows command, and it no longer launches commands through a restricted-token runner: the command starts directly as the current OS user. A process that fails to start is still reported as not run rather than as command output, so stdout/stderr cannot claim that execution never started; runtime failures remain `execution` with possible partial effects.

Tempora 不再在每条 Windows 命令前运行嵌套 shell／子进程预检，也不再通过
