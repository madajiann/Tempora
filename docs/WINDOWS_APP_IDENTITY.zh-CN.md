...
使用安装根目录的 `Tempora.exe` 打开桌面端；`tempora-cli.exe` 是独立的命令行入口。
新安装和解压到空目录的 ZIP 只提供这个 GUI 入口。已有安装原本存在的
`tempora-launcher.exe` 会保留，且不设置强制删除期限，以兼容旧固定项、自定义快捷方式
和脚本。受控升级会用同一份已验证启动器字节更新两个名字。`versions/` 中的程序，
包括 Electron 的 `app/Tempora.exe`，仍是内部组件。

在任何 Electron 窗口显示前，Desktop 会在永久 launcher 存在时，将窗口的重新启动
命令和图标优先设为 `Tempora.exe`，必要时回退到旧 launcher。新建任务栏固定项因此不会依赖版本目录内的 Electron 程序，
...
标准入口 `Tempora.exe` 存在时，指向旧 `tempora-launcher.exe`、
...
签名更新载荷和 `/TEMPORASTAGE=1` 仍包含 `tempora-launcher.exe`，因为已经安装的
