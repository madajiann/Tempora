...
var (
	...
	draggableRe    = regexp.MustCompile(`--tempora-draggable`)
...
func scanFrontend(root string, inv *inventory) error {
	...
	for _, path := range files {
		...
		if draggableRe.MatchString(text) {
			inv.add(entry{Kind: kindCSSMarker, Name: "--tempora-draggable", Location: rel, Class: classKeepBusiness, Owner: "rewritten to -webkit-app-region by scripts/shell-css.mjs for the Electron bundle"})
		}
